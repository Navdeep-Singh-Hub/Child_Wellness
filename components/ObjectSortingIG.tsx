/**
 * Sort the Objects — Animals (pig, cow) vs Tools/Actions (dig, shovel). Game 4 for Grouper Session 8.
 */
...
type Category = 'animals' | 'tools';
const BASKETS: { id: Category; label: string; emoji: string }[] = [
  { id: 'animals', label: 'Animals', emoji: '🐾' },
  { id: 'tools', label: 'Tools / Actions', emoji: '🔧' },
];
const ITEMS: { id: string; label: string; category: Category; image: string }[] = [
  { id: 'pig', label: 'pig', category: 'animals', image: 'https://placehold.co/100x100/FBCFE8/9D174D?text=pig' },
  { id: 'cow', label: 'cow', category: 'animals', image: 'https://placehold.co/100x100/FDE68A/92400E?text=cow' },
  { id: 'dig', label: 'dig', category: 'tools', image: 'https://placehold.co/100x100/FEF3C7/B45309?text=dig' },
  { id: 'shovel', label: 'shovel', category: 'tools', image: 'https://placehold.co/100x100/E5E7EB/374151?text=shovel' },
];

export function ObjectSortingIG({ onComplete }: { onComplete: () => void }) {
  ...
  const [currentItem, setCurrentItem] = useState<string | null>('pig');
