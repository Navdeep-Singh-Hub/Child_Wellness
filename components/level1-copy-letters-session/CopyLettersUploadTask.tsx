/**
 * Game 4: Copy Portfolio — write 5 letters on paper and upload.
 * POST /api/verify-copy-letters
 */
...
export function CopyLettersUploadTask({
  currentStep,
  totalSteps,
  onBack,
  onComplete,
  ...
  useEffect(() => {
    ...
    if (!spokeIntro.current) {
      ...
      speak('Write any five letters from A to Z on paper, then photograph your page for the portfolio!', 0.72);
    }
    return () => stopTTS();
  }, []);
  ...
  const mascotHint =
    phase === 1
      ? 'Write any 5 letters from A to Z on paper!'
      : phase === 2
        ...
        : result?.success
          ...
          : result
            ? result.feedback
            : 'The curator is reviewing your page…';
    ...
    async (uri: string) => {
      ...
      try {
        ...
        if (Platform.OS === 'web' && (uri.startsWith('blob:') || uri.startsWith('data:'))) {
          ...
          fd.append('file', blob, 'copy-letters.jpg');
        } else {
          fd.append('file', { uri, name: 'copy-letters.jpg', type: 'image/jpeg' } as any);
        }
        const h = await authHeaders({ multipart: true });
        delete (h as Record<string, string>)['Content-Type'];
        const res = await fetch(`${API_BASE_URL}/api/verify-copy-letters`, { method: 'POST', headers: h, body: fd });
        if (!res.ok) {
        ...
        setResult({
          ...
          feedback: data.feedback || (ok ? 'Great letters!' : 'Try again with clearer letters.'),
        });
        if (ok) {
          setShowCelebrate(true);
          speak('Your copy portfolio is complete!', 0.72);
  ...
  const handleReady = () => {
    setPhase(2);
    speak('Now photograph your five letters!', 0.72);
  ...
  const handleRetry = () => {
    ...
    setImageUri(null);
    setPhase(1);
    speak('Let us try again! Write five letters on paper first.', 0.72);
  };

  const stepDots = Array.from({ length: totalSteps }, (_, i) => i + 1);
  const phases = [
    { n: 1, label: 'Write 5', icon: '✏️' },
      ...
      <View style={styles.root}>
        ...
        <View style={styles.celebrateOverlay}>
          ...
          <View style={styles.celebrateCard}>
            ...
            <Text style={styles.celebrateTitle}>Portfolio Verified!</Text>
            <Text style={styles.celebrateSub}>Your copy work is official</Text>
          </View>
        </View>
      </View>
    );
      ...
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            <Text style={styles.gameLabel}>COPY PORTFOLIO</Text>
            <Text style={styles.gameTitle}>5-Letter Upload</Text>
          </View>
        </View>
        ...
        {phase === 1 ? (
          <View style={styles.paperCard}>
            <View style={styles.paperFrame}>
              <Text style={styles.alphaRow}>A B C D E … Z</Text>
              <Text style={styles.paperTitle}>5 Letters Page</Text>
              <Text style={styles.paperDesc}>
                Write any 5 letters from A to Z on paper by copying from a reference, then upload a photo!
              </Text>
            </View>
            <Pressable onPress={handleReady} style={({ pressed }) => [styles.primaryBtn, pressed && styles.pressed]}>
              ...
              <Text style={styles.primaryBtnText}>I wrote 5 letters!</Text>
            </Pressable>
          </View>
        ) : (
          <View style={styles.uploadArea}>
            {imageUri ? (
              <View style={styles.frameOuter}>
                ...
                <Text style={styles.frameCaption}>Portfolio Page</Text>
              </View>
            ) : (
              <View style={styles.placeholderFrame}>
                <Text style={styles.placeholderIcon}>📷</Text>
                <Text style={styles.placeholderText}>Photograph your letter page</Text>
              </View>
            )}
            {result && (
...
const styles = StyleSheet.create({
  ...
  primaryBtnText: { fontSize: 17, fontWeight: '800', color: '#1E3A5F' },
  secondaryBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
  ...
  celebrateTitle: { fontSize: 26, fontWeight: '900', color: '#1E3A5F', marginBottom: 6 },
  celebrateSub: { fontSize: 15, fontWeight: '600', color: '#475569' },
});
