/**
 * AI Task: Write 5 letters from A–Z on paper and upload photo.
...
const INSTRUCTION = 'Write any 5 letters from A–Z on paper, then upload a photo.';
...
export function CopyLettersUploadTask({
  ...
  const upload = async (uri: string) => {
    ...
    try {
      ...
      setResult({ success: ok, message: data.message || (ok ? 'SUCCESS' : 'TRY AGAIN'), feedback: data.feedback || (ok ? 'Great letters!' : 'Try again.') });
  ...
  return (
    <GameContainerGrip title="Letter Task" currentStep={currentStep} totalSteps={totalSteps} mascot="📷" mascotHint={INSTRUCTION} onBack={onBack}>
...
const styles = StyleSheet.create({
  content: { flex: 1 },
  instruction: { fontSize: 18, fontWeight: '700', color: '#1E40AF', textAlign: 'center', marginBottom: 20, lineHeight: 26 },
  ...
  btnPri: { backgroundColor: '#1E40AF' },
  btnSec: { backgroundColor: '#EFF6FF' },
  btnText: { fontSize: 18, fontWeight: '800', color: '#FFF' },
  btnTextSec: { color: '#1E40AF' },
  pressed: { opacity: 0.9 },
  tryAgain: { marginTop: 12, alignSelf: 'center' },
  tryAgainText: { fontSize: 16, fontWeight: '700', color: '#1E40AF' },
});
