/**
 * Sort the Objects — Cleaning Tools (mop, broom) vs Toys (top, ball). Game 4 for Grouper Session 5.
 */
...
type Category = 'cleaning' | 'toys';
const BASKETS: { id: Category; label: string; emoji: string }[] = [
  { id: 'cleaning', label: 'Cleaning Tools', emoji: '🧹' },
  { id: 'toys', label: 'Toys', emoji: '🧸' },
];
const ITEMS: { id: string; label: string; category: Category; image: string }[] = [
  { id: 'mop', label: 'mop', category: 'cleaning', image: 'https://placehold.co/100x100/E0E7FF/4338CA?text=mop' },
  { id: 'broom', label: 'broom', category: 'cleaning', image: 'https://placehold.co/100x100/A78BFA/5B21B6?text=broom' },
  { id: 'top', label: 'top', category: 'toys', image: 'https://placehold.co/100x100/DBEAFE/1E40AF?text=top' },
  { id: 'ball', label: 'ball', category: 'toys', image: 'https://placehold.co/100x100/FEE2E2/991B1B?text=ball' },
];

export function ObjectSortingOP({ onComplete }: { onComplete: () => void }) {
  ...
  const [currentItem, setCurrentItem] = useState<string | null>('mop');
  const [showSuccess, setShowSuccess] = useState(false);
  const [snapAnim] = useState(() => new Animated.Value(0));
  const [wrongShake] = useState(() => new Animated.Value(0));

  useEffect(() => {
    speak('Sort the objects. Tap a basket for each word.', 0.75);
  }, []);
