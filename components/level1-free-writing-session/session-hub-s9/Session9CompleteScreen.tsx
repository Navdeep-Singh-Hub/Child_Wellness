...
interface Session9CompleteProps {
  completed: number;
  totalSteps: number;
...
export function Session9CompleteScreen({
  completed,
  totalSteps,
  taskSuccess,
  onExit,
}: Session9CompleteProps) {
  ...
  return (
    <View style={styles.root}>
      <View style={StyleSheet.absoluteFill} pointerEvents="none">
        <View style={{ flex: 1, backgroundColor: HUB_S9.bgMid }} />
      </View>
      <ConfettiEffect />
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.eyebrow}>SESSION 9 COMPLETE</Text>
          <Text style={styles.title}>Free Writer!</Text>
          <Text style={styles.subtitle}>You can write letters all on your own</Text>
          ...
          <View style={styles.stats}>
            ...
            <Text style={styles.stat}>
              Manuscript: {taskSuccess === null ? '—' : taskSuccess ? '✓ Verified' : 'Try again'}
            </Text>
          </View>
          {onExit ? (
            <Pressable style={({ pressed }) => [styles.exitBtn, pressed && styles.pressed]} onPress={onExit}>
              <Ionicons name="home-outline" size={20} color={HUB_S9.bgTop} />
...
const styles = StyleSheet.create({
  ...
  eyebrow: { fontSize: 11, fontWeight: '800', color: HUB_S9.accent, letterSpacing: 2, marginBottom: 8 },
  title: { fontSize: 30, fontWeight: '900', color: HUB_S9.textLight, marginBottom: 6 },
  subtitle: { fontSize: 16, color: HUB_S9.textMuted, marginBottom: 28, textAlign: 'center' },
  trophyWrap: { alignItems: 'center', marginBottom: 28 },
  trophy: { fontSize: 72, marginBottom: 12 },
  badge: {
    backgroundColor: HUB_S9.accent,
    paddingHorizontal: 24,
    paddingVertical: 10,
    borderRadius: 20,
    borderWidth: 3,
    borderColor: HUB_S9.accentLight,
  },
  badgeText: { fontSize: 18, fontWeight: '900', color: HUB_S9.bgTop },
  ...
  chipCheck: { position: 'absolute', top: 2, right: 6, fontSize: 12, color: HUB_S9.accentLight, fontWeight: '800' },
  stats: { marginBottom: 28, alignItems: 'center', gap: 6 },
  stat: { fontSize: 16, fontWeight: '600', color: HUB_S9.textLight },
  exitBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
  ...
  exitText: { fontSize: 17, fontWeight: '800', color: HUB_S9.bgTop },
  pressed: { opacity: 0.9 },
});
