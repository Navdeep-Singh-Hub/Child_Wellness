/**
 * Level 7 Reader — Session 9, Game 3: Word Dock
...
const TARGET = ['T', 'A', 'B', 'L', 'E'] as const;
const LETTERS = ['E', 'T', 'A', 'B', 'L'] as const;
const WORD = 'TABLE';

const VOICE = 'Build the word TABLE. Tap the letters in order: T, A, B, L, E.';
const DOCK = { accent: '#7C3AED', glow: '#C4B5FD', teal: '#2DD4BF', wood: '#D4A574' } as const;
...
function CargoLetter({
  ...
  return (
    <Animated.View style={anim}>
      ...
      >
        <LinearGradient
          colors={[`${DOCK.accent}88`, `${DOCK.teal}44`]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
...
export function WordBuilderTableReaderSession9Game({ onComplete }: WordBuilderTableReaderSession9GameProps) {
  ...
  const coachLine =
    nextIndex === 0
      ? 'Spell TABLE: start with T, then A, B, L, and E!'
  ...
  return (
    <ReaderGameShell
      ...
      title="Build TABLE"
      instruction="Tap the letter crates in order to spell TABLE."
      mascot="🪑"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.dockFrame}>
        <LinearGradient
          colors={[`${DOCK.accent}33`, 'transparent', `${DOCK.teal}22`]}
      ...
      <View style={styles.legend}>
        <Text style={styles.legendTxt}>
          Order: T → A → B → L → E{attempts > 0 ? ` · Taps: ${attempts}` : ''}
        </Text>
      </View>
    </ReaderGameShell>
  );
