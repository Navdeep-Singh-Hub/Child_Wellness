/**
 * Level 7 Reader — Session 7: Emotion Recognition (surprised), Memory 12, Word Builder CHAIR, Object Matching (tool/use)
 * 4 games + 1 real-world task (three objects of different sizes). Indigo theme.
 */
...
export function ReaderSession7({ onExit }: ReaderSession7Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '😲', title: 'Emotion Recognition', desc: 'Select the surprised face' },
      { icon: '🎴', title: 'Memory Grid', desc: 'Match 12 cards (6 pairs)' },
      { icon: '🪑', title: 'Word Builder', desc: 'Build CHAIR' },
      { icon: '✂️', title: 'Object Matching', desc: 'Match tool with its use' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Find the surprised face, match 12 cards, build CHAIR, and match tools to their use! Then show three objects of different sizes.</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Reader Session 7</Text>
            <Text style={styles.introSub}>Level 7 · Reader · Session 7</Text>
            <Text style={styles.introDesc}>
              Emotion recognition, memory grid, word CHAIR, object matching, then a real-world photo task!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Reader Session 7</Text>
        </View>
        <Text style={styles.subtitle}>Level 7 · Reader · Session 7</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <ReaderNotebookUploadThreeDifferentSizes onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
