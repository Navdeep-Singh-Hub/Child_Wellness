 ...
 * 4 games + 1 real-world task (build tower with 4 objects). Indigo theme.
...
function FinalCelebrationWithTransition({ onDone }: { onDone: () => void }) {
  ...
  return (
    <SuccessCelebration
      ...
      subtitle="Reader Master — Next level unlocked! 🎉"
      badgeEmoji="🏆"
    />
  );
}
...
export function ReaderSession10({ onExit }: ReaderSession10Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🎯', title: 'Mixed Quiz', desc: 'Shapes, colors, and numbers' },
      { icon: '🎴', title: 'Advanced Memory', desc: 'Match 16 cards (8 pairs)' },
      { icon: '🏫', title: 'Word Builder', desc: 'Build SCHOOL' },
      { icon: '⬜', title: 'Logic Pattern', desc: 'Square, triangle...' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Final session! Mixed quiz, memory 16, SCHOOL, and pattern. Then build a tower with four objects. Complete it to unlock the next level!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Reader Session 10 (Final)</Text>
            <Text style={styles.introSub}>Level 7 · Reader · Session 10</Text>
            <Text style={styles.introDesc}>
              Mixed quiz, advanced memory, word SCHOOL, logic pattern, then build a tower!
            </Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={[styles.resultTitle, { color: DESIGN.primary }]}>
            {taskCorrect ? 'Reader Master (Final)!' : 'Great job!'}
          </Text>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Reader Session 10 (Final)</Text>
        </View>
        <Text style={styles.subtitle}>Level 7 · Reader · Session 10</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <ReaderNotebookUploadTowerFour onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
