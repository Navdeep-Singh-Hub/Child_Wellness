/**
 * Level 7 Reader — Session 3, Real World Task: Arrange FOUR objects in a square shape.
 * Upload photo → AI checks square layout. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Arrange FOUR objects in a square shape.\n\nTake a photo (or upload a picture) showing four objects placed in a square layout.';
...
export function ReaderNotebookUploadSquareLayout({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see four objects in a square shape!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Arrange FOUR objects in a square shape. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
