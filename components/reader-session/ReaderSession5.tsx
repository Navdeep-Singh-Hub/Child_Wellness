/**
 * Level 7 Reader — Session 5: Memory Pattern, Number Logic (5,10,15), Word Builder HOUSE, Bridge Construction
 * 4 games + 1 real-world task (stack three objects vertically). Indigo theme.
 */
...
export function ReaderSession5({ onExit }: ReaderSession5Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧠', title: 'Memory Pattern', desc: 'Watch colors, then repeat' },
      { icon: '🔢', title: 'Number Logic', desc: '5, 10, 15, ?' },
      { icon: '🏠', title: 'Word Builder', desc: 'Build HOUSE' },
      { icon: '🌉', title: 'Build the Bridge', desc: 'Place blocks' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Memory pattern, number logic, build HOUSE, and build a bridge! Then stack three objects vertically.</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Reader Session 5</Text>
            <Text style={styles.introSub}>Level 7 · Reader · Session 5</Text>
            <Text style={styles.introDesc}>
              Memory pattern, number logic, word HOUSE, bridge construction, then a real-world photo task!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Reader Session 5</Text>
        </View>
        <Text style={styles.subtitle}>Level 7 · Reader · Session 5</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <ReaderNotebookUploadStackThreeVertical onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
