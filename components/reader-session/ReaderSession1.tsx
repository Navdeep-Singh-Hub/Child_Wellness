 ...
 * 4 games + 1 real-world task (three different objects in a straight line). Indigo/space theme.
 */
...
export function ReaderSession1({ onExit }: ReaderSession1Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧩', title: 'Pattern Logic', desc: 'Choose the next shape' },
      { icon: '🎴', title: 'Memory Grid', desc: 'Match 8 cards' },
      { icon: '🌳', title: 'Word Builder', desc: 'Build TREE' },
      { icon: '📂', title: 'Category Sorting', desc: 'Food / Animals / Vehicles' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Welcome to Reader! Let’s solve patterns, play memory, build words, and sort!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Reader Session 1</Text>
            <Text style={styles.introSub}>Level 7 · Reader · Session 1</Text>
            <Text style={styles.introDesc}>
              Pattern logic, memory grid, build TREE, category sorting, then a real-world photo task!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Reader Session 1</Text>
        </View>
        <Text style={styles.subtitle}>Level 7 · Reader · Session 1</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <ReaderNotebookUploadThreeDifferentLine onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
