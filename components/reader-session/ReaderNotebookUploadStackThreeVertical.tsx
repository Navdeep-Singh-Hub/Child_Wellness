/**
 * Level 7 Reader — Session 5, Real World Task: Stack Tower
...
const TOWER = { accent: '#6366F1', glow: '#A5B4FC', cyan: '#38BDF8' } as const;
...
function CheckRow({ label, ok, pending }: { label: string; ok?: boolean; pending?: boolean }) {
  return (
    <View style={check.row}>
      ...
      >
        {pending ? (
          <ActivityIndicator size="small" color={TOWER.glow} />
        ) : (
...
export function ReaderNotebookUploadStackThreeVertical({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const filename = 'reader-s5-stack-3-vertical.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-reader-s5-stack-3-vertical-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
  ...
  return (
    ...
    >
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[TOWER.accent, '#4F46E5']} style={styles.actionGrad}>
            {uploading ? (
      ...
      {(uploading || result) && (
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              <ActivityIndicator color={TOWER.glow} size="large" />
...
const sketch = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3, color: TOWER.glow, marginBottom: 8 },
  glow: {
    position: 'absolute',
    top: 36,
    width: 80,
  ...
  block: {
    ...
    borderRadius: 10,
    borderWidth: 2,
    borderColor: TOWER.glow,
    alignItems: 'center',
    justifyContent: 'center',
  },
  blockTop: { backgroundColor: TOWER.cyan, width: 56 },
  blockMid: { backgroundColor: `${TOWER.accent}CC`, width: 64 },
  blockBase: { backgroundColor: TOWER.accent, width: 72 },
  ...
  hint: { fontSize: 11, fontWeight: '700', color: TOWER.glow },
});
...
const styles = StyleSheet.create({
  ...
  badgeTxt: { fontSize: 9, fontWeight: '900', letterSpacing: 1, color: TOWER.glow },
  ...
  stepTitle: { fontSize: 11, fontWeight: '800', color: TOWER.glow },
  ...
  retryBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 12, backgroundColor: TOWER.accent },
