/**
 * Level 7 Reader — Session 5, Game 2: Number Logic
...
export function NumberLogicReaderSession5Game({ onComplete }: NumberLogicReaderSession5GameProps) {
  ...
  useEffect(() => {
    speak('What number comes next? 5, 10, 15, what?', 0.75);
  }, []);
  ...
  const triggerWrong = useCallback(() => {
    ...
    speak('Try again. Count by fives: 5, 10, 15...', 0.7);
  }, [wrongShake]);
  ...
  return (
    <GameLayout
      title="Number Logic"
      instruction="What number comes next? 5, 10, 15, ?"
      icon="🔢"
      backgroundVariant="indigo"
    >
      <View style={styles.container}>
        <Text style={styles.prompt}>5, 10, 15, ?</Text>
