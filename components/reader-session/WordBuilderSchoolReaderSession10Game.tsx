/**
 * Level 7 Reader — Session 10, Game 3: Word Dock
 * Letters S, C, H, O, O, L — user taps letters in order to build "SCHOOL".
 */
...
const TARGET = ['S', 'C', 'H', 'O', 'O', 'L'] as const;
const LETTERS = ['O', 'L', 'S', 'C', 'H', 'O'] as const;
const WORD = 'SCHOOL';

const VOICE = 'Build the word SCHOOL. Tap the letters in order: S, C, H, O, O, L.';
const DOCK = { accent: '#EAB308', glow: '#FDE047', violet: '#A855F7', wood: '#D4A574' } as const;
...
function CargoLetter({
  ...
  return (
    <Animated.View style={anim}>
      ...
      >
        <LinearGradient
          colors={[`${DOCK.accent}88`, `${DOCK.violet}44`]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
...
export interface WordBuilderSchoolReaderSession10GameProps {
  onComplete: () => void;
}

export function WordBuilderSchoolReaderSession10Game({ onComplete }: WordBuilderSchoolReaderSession10GameProps) {
    ...
    (letter: string, tapIndex: number) => {
      ...
      if (letter === expected) {
        ...
        if (newIndex >= TARGET.length) {
          speak('School! You built SCHOOL!', 0.75);
          setShowSuccess(true);
        ...
        speak('Try again. Tap S, then C, then H, then O, O, then L.', 0.7);
  ...
  const coachLine =
    nextIndex === 0
      ? 'Spell SCHOOL: start with S, then C, H, O, O, and L!'
      : nextIndex === 4
        ? 'Next letter: O — remember SCHOOL has two O letters!'
  ...
  return (
    <ReaderGameShell
      ...
      title="Build SCHOOL"
      instruction="Tap the letter crates in order to spell SCHOOL."
      mascot="🏫"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.progressWrap}>
        ...
        <View style={styles.progressBg}>
          <LinearGradient
            colors={[DOCK.accent, DOCK.violet]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
      ...
      <View style={styles.dockFrame}>
        <LinearGradient
          colors={[`${DOCK.accent}33`, 'transparent', `${DOCK.violet}22`]}
      ...
      <View style={styles.legend}>
        <Text style={styles.legendTxt}>
          Order: S → C → H → O → O → L{attempts > 0 ? ` · Taps: ${attempts}` : ''}
        </Text>
      </View>
    </ReaderGameShell>
  );
...
const styles = StyleSheet.create({
  ...
  targetWord: { fontSize: 24, fontWeight: '900', color: RD.textLight, letterSpacing: 2 },
  ...
  slotLetter: { fontSize: 24, fontWeight: '900', color: RD.textLight },
  slotIdx: {
    position: 'absolute',
    top: 3,
    left: 5,
  ...
  slotQ: { fontSize: 18, fontWeight: '900', color: DOCK.glow },
  prompt: {
    fontSize: 16,
    fontWeight: '800',
    color: RD.textLight,
  ...
  crateLetter: { fontSize: 26, fontWeight: '900', color: RD.textLight },
  crateMark: {
    position: 'absolute',
    bottom: 4,
    right: 6,
