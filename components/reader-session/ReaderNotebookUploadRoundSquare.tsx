/**
 * Level 7 Reader — Session 4, Real World Task: Show something ROUND and something SQUARE together.
 * Upload photo → AI detects both shapes. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Show something ROUND and something SQUARE together.\n\nTake a photo (or upload a picture) showing one round object and one square object in the same image.';
...
export function ReaderNotebookUploadRoundSquare({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see something round and something square!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show something ROUND and something SQUARE together. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
