/**
 * Level 7 Reader — Session 6, Real World Task: Place TWO objects far apart and ONE object in the middle.
 * Upload photo → AI verifies object arrangement. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Place TWO objects far apart and ONE object in the middle.\n\nTake a photo (or upload a picture) showing two objects at the sides and one object in the middle.';
...
export function ReaderNotebookUploadTwoFarOneMiddle({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of two objects far apart and one in the middle.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see two objects far apart and one in the middle!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Place TWO objects far apart and ONE in the middle. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
