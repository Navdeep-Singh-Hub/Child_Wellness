/**
 * Level 7 Reader — Session 9, Game 4: Sequence Forge
 * triangle, circle, square, triangle, circle, ? → square (repeating: triangle, circle, square).
 */
...
const PATTERN = ['triangle', 'circle', 'square', 'triangle', 'circle'] as const;
...
const VOICE =
  'Complete the pattern. Triangle, circle, square, triangle, circle. What comes next?';
...
export interface PatternGameReaderSession9GameProps {
  onComplete: () => void;
}

export function PatternGameReaderSession9Game({ onComplete }: PatternGameReaderSession9GameProps) {
  ...
  const handleTap = useCallback(
    (id: ShapeId) => {
      ...
      if (id === CORRECT_ID) {
        ...
        speak(
          id === 'triangle'
            ? 'Triangle starts the cycle — look at what comes after circle!'
            : 'Circle was just before. The pattern repeats every three shapes!',
          0.7,
        );
        setTimeout(() => {
          setFeedback('idle');
  ...
  const coachLine =
    attempts === 0
      ? 'Triangle, circle, square repeat — what follows circle?'
      : 'Say it aloud: 🔺 ⭕ ⬜ 🔺 ⭕ — ?';
  ...
  if (showSuccess) {
    return (
      <SuccessCelebration
        ...
        subtitle="You completed the shape sequence!"
        badgeEmoji="🔺"
      />
    );
  }
  ...
  return (
    <ReaderGameShell
      ...
      instruction="Triangle, circle, square, triangle, circle — what comes next?"
      mascot="🔺"
      coachLine={coachLine}
