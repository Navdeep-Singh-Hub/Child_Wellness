/**
 * Level 7 Reader — Session 9, Game 4: Pattern Game
...
const PATTERN = ['triangle', 'circle', 'square', 'triangle', 'circle'];
const OPTIONS = [
...
const SHAPE_EMOJI: Record<string, string> = { triangle: '🔺', circle: '⭕', square: '⬜' };
...
export function PatternGameReaderSession9Game({ onComplete }: PatternGameReaderSession9GameProps) {
  ...
  useEffect(() => {
    speak('Complete the pattern. Triangle, circle, square, triangle, circle. What comes next?', 0.75);
  }, []);
  ...
  const triggerWrong = useCallback(() => {
    ...
    speak('Try again. Triangle, circle, square, then triangle, circle. What comes next?', 0.7);
  }, [wrongShake]);
