 ...
 * 4 games + 1 real-world task (two objects same color). Indigo theme.
 */
...
export function ReaderSession2({ onExit }: ReaderSession2Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔢', title: 'Number Sequence', desc: '3, 6, 9, ?' },
      { icon: '🔍', title: 'Spot the Difference', desc: 'Find 4 differences' },
      { icon: '⬠', title: 'Shape Identification', desc: 'Tap the pentagon' },
      { icon: '🚲', title: 'Build the Bicycle', desc: 'Assemble parts' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Numbers, spot the difference, shapes, and build a bicycle! Then show two objects of the same color.</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Reader Session 2</Text>
            <Text style={styles.introSub}>Level 7 · Reader · Session 2</Text>
            <Text style={styles.introDesc}>
              Number sequence, spot 4 differences, find the pentagon, build the bicycle, then a real-world photo task!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Reader Session 2</Text>
        </View>
        <Text style={styles.subtitle}>Level 7 · Reader · Session 2</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <ReaderNotebookUploadSameColor onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
