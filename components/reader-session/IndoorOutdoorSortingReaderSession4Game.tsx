/**
 * Level 7 Reader — Session 4, Game 4: Zone Sort
...
const VOICE = 'Sort into Indoor or Outdoor. Tap an item, then tap the zone.';
const ZONE = { accent: '#F59E0B', glow: '#FCD34D' } as const;
...
function ZonePanel({
  ...
  return (
    <Animated.View style={[styles.zoneWrap, anim]}>
      ...
      >
        <LinearGradient colors={[`${zone.accent}33`, 'transparent']} style={styles.zoneGrad} />
...
export function IndoorOutdoorSortingReaderSession4Game({ onComplete }: IndoorOutdoorSortingReaderSession4GameProps) {
  ...
  const itemsByZone = useMemo(() => {
    const map: Record<Category, string[]> = { indoor: [], outdoor: [] };
  ...
  const coachLine = selectedId
    ? `Tap a zone for ${ITEMS.find((i) => i.id === selectedId)?.label ?? 'this item'}`
    : sortedCount === 0
      ? 'Pick an item — is it usually inside or outside?'
  ...
  return (
    <ReaderGameShell
      studio="ZONE SORT · GAME 4"
      title="Sort the zones"
      instruction="Tap an item, then tap Indoor or Outdoor."
      mascot="🏠"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.beltFrame}>
        <Text style={styles.beltLabel}>CARGO BELT</Text>
      ...
      <Text style={styles.zoneTitle}>ZONE PANELS</Text>
...
const styles = StyleSheet.create({
  ...
  zonesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 12, justifyContent: 'center' },
  zoneWrap: { flex: 1, minWidth: 140 },
  zone: {
    borderRadius: 16,
    borderWidth: 2.5,
    paddingVertical: 14,
  ...
  zoneEmoji: { fontSize: 32, marginBottom: 4 },
  zoneLabel: { fontSize: 13, fontWeight: '900', textAlign: 'center', marginBottom: 8 },
