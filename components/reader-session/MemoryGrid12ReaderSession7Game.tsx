/**
 * Level 7 Reader — Session 7, Game 2: Deep Grid
 * Match 12 cards (6 pairs). Flip two at a time.
 */
...
const VOICE = 'Match 6 pairs. Flip two cards at a time to find matching pictures.';
const DEEP = { accent: '#F43F5E', glow: '#FDA4AF', rose: '#FB7185' } as const;
...
export interface MemoryGrid12ReaderSession7GameProps {
  onComplete: () => void;
}

export function MemoryGrid12ReaderSession7Game({ onComplete }: MemoryGrid12ReaderSession7GameProps) {
  const cards = useMemo(
    () =>
      shuffle(
        PAIRS.flatMap((p) => [
  ...
  return (
    <ReaderGameShell
      ...
      instruction="Tap two cards to find matching pictures. 12 cards, 6 pairs."
      mascot="🎴"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.gridFrame}>
        ...
        />
        <Text style={styles.gridLabel}>FRONTIER GRID · 12 CARDS</Text>
...
const styles = StyleSheet.create({
  ...
  backGlyph: { fontSize: 22, color: DEEP.glow, opacity: 0.9 },
  backNum: {
    position: 'absolute',
    bottom: 3,
    right: 5,
  ...
  legendTxt: { fontSize: 12, fontWeight: '700', color: DEEP.glow, textAlign: 'center' },
});
