/**
 * Level 7 Reader — Session 8, Real World Task: Triangle Beacon
 * Arrange THREE objects forming a triangle. Upload photo → AI verifies triangle layout.
 */
...
const VOICE =
  'Arrange three objects forming a triangle. Take a photo showing one object at each corner of the triangle.';
const BEACON = { accent: '#0EA5E9', glow: '#38BDF8', spark: '#EF4444' } as const;

const STEPS = [
  { icon: '📦', title: 'Gather', body: 'Find 3 objects.' },
  { icon: '🔺', title: 'Arrange', body: 'Triangle layout.' },
    ...
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <View style={sketch.triangleZone}>
          ...
          <View style={sketch.triangleOutline} pointerEvents="none" />
        </View>
        <Text style={sketch.caption}>three objects · one at each corner · triangle shape</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ 3 objects</Text>
          <Text style={sketch.hint}>✓ triangle layout</Text>
        </View>
      </View>
    </View>
  );
...
export function ReaderNotebookUploadTriangleLayout({
  onComplete,
}: {
  ...
  const [result, setResult] = useState<{
    ...
    triangle_layout_detected?: boolean;
  } | null>(null);
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const filename = 'reader-s8-triangle-layout.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-reader-s8-triangle-layout-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      setResult({
        ...
        triangle_layout_detected: data.triangle_layout_detected,
      });

      if (correct) {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
        speak('Success! Great triangle!', 0.75);
        setTimeout(() => onComplete(true), 2600);
      } else {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Arrange three objects in a triangle shape.', 0.75);
      }
    } catch (e: unknown) {
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert(
        'Permission needed',
        'Allow camera to take a photo of three objects in a triangle.',
      );
      return;
    }
  ...
  const coachLine = uploading
    ? 'Scanning for three objects in a triangle layout…'
    : result?.correct
      ? 'Verified! Three objects forming a triangle detected.'
      : result
        ? 'Place one object at each corner — then capture again.'
        : 'Try cups, books, or toys — spread them into a triangle!';

  return (
    <ReaderGameShell
      studio="TRIANGLE BEACON · MISSION"
      title="Real-world proof"
      instruction="Arrange three objects in a triangle, then upload or photograph them together."
      mascot="🔺"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[BEACON.accent, BEACON.spark]} style={styles.actionGrad}>
            {uploading ? (
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <Text style={styles.resultTitle}>Triangle scan in progress…</Text>
              <CheckRow label="Three objects detected" pending />
              <CheckRow label="Triangle layout detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Triangle beacon verified!' : 'Adjust your setup'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Three objects detected" ok={result.objects_detected ?? result.correct} />
              <CheckRow
                label="Triangle layout detected"
                ok={result.triangle_layout_detected ?? result.correct}
              />
              {!result.correct && (
...
const sketch = StyleSheet.create({
  ...
  triangleOutline: {
    ...
    borderBottomColor: `${BEACON.glow}44`,
  },
  topCorner: { position: 'absolute', top: 0, alignItems: 'center' },
  baseRow: {
    position: 'absolute',
    bottom: 0,
    flexDirection: 'row',
  ...
  cornerEmoji: { fontSize: 22 },
  cornerNum: { fontSize: 9, fontWeight: '900', color: BEACON.glow, marginTop: 2 },
