/**
 * Level 7 Reader — Session 10 (final), Real World Task: Build a small tower using FOUR objects.
...
const INSTRUCTIONS =
  'Build a small tower using FOUR objects.\n\nStack four objects on top of each other (e.g. blocks, books, cups) and take a photo (or upload a picture).';
...
export function ReaderNotebookUploadTowerFour({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see a tower of four objects!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Build a small tower using FOUR objects. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
