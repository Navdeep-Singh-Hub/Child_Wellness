 ...
 * 4 games + 1 real-world task (two objects of different colors). Indigo theme.
 */
...
export function ReaderSession9({ onExit }: ReaderSession9Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧠', title: 'Logical Reasoning', desc: 'Which doesn\'t belong? car, bus, apple, train' },
      { icon: '🎴', title: 'Memory Advanced', desc: 'Match 14 cards (7 pairs)' },
      { icon: '🪑', title: 'Word Builder', desc: 'Build TABLE' },
      { icon: '🔺', title: 'Pattern Game', desc: 'Triangle, circle, square...' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Find what doesn't belong, match 14 cards, build TABLE, and complete the pattern! Then show two objects of different colors.</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Reader Session 9</Text>
            <Text style={styles.introSub}>Level 7 · Reader · Session 9</Text>
            <Text style={styles.introDesc}>
              Logical reasoning, memory, word TABLE, pattern, then a real-world photo task!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Reader Session 9</Text>
        </View>
        <Text style={styles.subtitle}>Level 7 · Reader · Session 9</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <ReaderNotebookUploadDifferentColors onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
