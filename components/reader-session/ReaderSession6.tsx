/**
 * Level 7 Reader — Session 6: Visual Puzzle, Category Recognition (animals), Pattern Logic, Counting 15
 * 4 games + 1 real-world task (two far apart, one in middle). Indigo theme.
 */
...
export function ReaderSession6({ onExit }: ReaderSession6Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧩', title: 'Visual Puzzle', desc: 'Complete half of a picture' },
      { icon: '🐾', title: 'Category Recognition', desc: 'Select all animals' },
      { icon: '⬜', title: 'Pattern Logic', desc: 'Circle, circle, square...' },
      { icon: '🔵', title: 'Counting Challenge', desc: 'Count 15 dots' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Complete a picture, find all animals, finish the pattern, and count 15 dots! Then place two objects far apart and one in the middle.</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Reader Session 6</Text>
            <Text style={styles.introSub}>Level 7 · Reader · Session 6</Text>
            <Text style={styles.introDesc}>
              Visual puzzle, category recognition, pattern logic, counting 15, then a real-world photo task!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Reader Session 6</Text>
        </View>
        <Text style={styles.subtitle}>Level 7 · Reader · Session 6</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <ReaderNotebookUploadTwoFarOneMiddle onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
