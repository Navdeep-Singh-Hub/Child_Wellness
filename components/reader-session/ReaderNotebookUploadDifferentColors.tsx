/**
 * Level 7 Reader — Session 9, Real World Task: Show TWO objects of different colors.
 * Upload photo → AI verifies color difference. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Show TWO objects of different colors.\n\nTake a photo (or upload a picture) showing two objects that are clearly different colors (e.g. a red apple and a blue cup).';
...
export function ReaderNotebookUploadDifferentColors({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of two objects of different colors.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see two objects of different colors!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show TWO objects of different colors. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
