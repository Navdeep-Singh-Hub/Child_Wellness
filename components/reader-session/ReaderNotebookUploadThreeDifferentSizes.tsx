/**
 * Level 7 Reader — Session 7, Real World Task: Take a photo showing THREE objects of different sizes.
 * Upload photo → AI checks size variation. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Take a photo showing THREE objects of different sizes.\n\nShow one small, one medium, and one large object (or any three objects that are clearly different sizes).';
...
export function ReaderNotebookUploadThreeDifferentSizes({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of three objects of different sizes.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see three objects of different sizes!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Take a photo showing THREE objects of different sizes. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
