/**
 * Level 7 Reader — Session 7, Real World Task: Size Beacon
 * Take a photo showing THREE objects of different sizes. Upload → AI verifies.
 */
...
const VOICE =
  'Take a photo showing three objects of different sizes. Show one small, one medium, and one large object.';
const BEACON = { accent: '#F43F5E', glow: '#FDA4AF', rose: '#FB7185' } as const;

const STEPS = [
  { icon: '📦', title: 'Gather', body: 'Find 3 objects.' },
  { icon: '📏', title: 'Compare', body: 'Small · med · large.' },
  ...
  return (
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <View style={sketch.sizeRow}>
          <View style={sketch.sizeSlot}>
            <View style={[sketch.block, sketch.blockSm]} />
            <Text style={sketch.tag}>SMALL</Text>
          </View>
          <View style={sketch.sizeSlot}>
            <View style={[sketch.block, sketch.blockMd]} />
            <Text style={sketch.tag}>MEDIUM</Text>
          </View>
          <View style={sketch.sizeSlot}>
            <View style={[sketch.block, sketch.blockLg]} />
            <Text style={sketch.tag}>LARGE</Text>
          </View>
        </View>
        <Text style={sketch.caption}>three objects · clearly different sizes · same photo</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ 3 objects</Text>
          <Text style={sketch.hint}>✓ size variation</Text>
        </View>
      </View>
    </View>
  );
...
export function ReaderNotebookUploadThreeDifferentSizes({
  onComplete,
}: {
  ...
  const [result, setResult] = useState<{
    ...
    size_variation_detected?: boolean;
  } | null>(null);
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const filename = 'reader-s7-three-different-sizes.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-reader-s7-three-different-sizes-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      setResult({
        ...
        size_variation_detected: data.size_variation_detected,
      });

      if (correct) {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
        speak('Success! Great sizes!', 0.75);
        setTimeout(() => onComplete(true), 2600);
      } else {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Show three objects of different sizes.', 0.75);
      }
    } catch (e: unknown) {
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert(
        'Permission needed',
        'Allow camera to take a photo of three objects of different sizes.',
      );
      return;
    }
  ...
  const coachLine = uploading
    ? 'Scanning for three objects with size variation…'
    : result?.correct
      ? 'Verified! Three objects of different sizes detected.'
      : result
        ? 'Pick one small, one medium, and one large — then capture again.'
        : 'Try a cup, a book, and a box — anything with clear size differences!';

  return (
    <ReaderGameShell
      studio="SIZE BEACON · MISSION"
      title="Real-world proof"
      instruction="Show three objects of different sizes, then upload or photograph them together."
      mascot="📏"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[BEACON.accent, '#E11D48']} style={styles.actionGrad}>
            {uploading ? (
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <CheckRow label="Size variation detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Size beacon verified!' : 'Adjust your setup'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Three objects detected" ok={result.objects_detected ?? result.correct} />
              <CheckRow
                label="Size variation detected"
                ok={result.size_variation_detected ?? result.correct}
              />
              {!result.correct && (
...
const sketch = StyleSheet.create({
  ...
  sizeRow: { flexDirection: 'row', alignItems: 'flex-end', gap: 16, marginBottom: 12 },
  sizeSlot: { alignItems: 'center', gap: 8 },
  block: { borderRadius: 10, borderWidth: 2, borderColor: BEACON.glow, backgroundColor: BEACON.rose },
  blockSm: { width: 24, height: 24 },
  blockMd: { width: 36, height: 36, backgroundColor: BEACON.accent },
  blockLg: { width: 48, height: 48, backgroundColor: `${BEACON.accent}CC` },
