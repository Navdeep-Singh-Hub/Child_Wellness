 ...
 * Flow: Intro → Game 1 (Preposition Quiz) → Game 2 (Drag Positions) → Game 3 (Patterns) → Game 4 (Sequence) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Preposition Review</Text>
            <Text style={styles.introSub}>Let's practice the positions we learned!</Text>
          </View>
          <View style={styles.prepositionsRow}>
            <Text style={styles.prepBadge}>IN</Text>
            <Text style={styles.prepBadge}>ON</Text>
            <Text style={styles.prepBadge}>UNDER</Text>
            <Text style={styles.prepBadge}>NEXT TO</Text>
            <Text style={styles.prepBadge}>BEHIND</Text>
            <Text style={styles.prepBadge}>BETWEEN</Text>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job practicing positions!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Preposition Explorer</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        <Text style={styles.title}>Preposition Review</Text>
        <Text style={styles.subtitle}>Logic Lab · Session 7</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <PrepositionQuizReview onComplete={advance} />}
        {step === 2 && <DragPositionReview onComplete={advance} />}
        {step === 3 && <PatternRecognitionReview onComplete={advance} />}
        {step === 4 && <SequencePuzzleReview onComplete={advance} />}
        {step === 5 && <PrepositionReviewNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  prepositionsRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginBottom: 24 },
  prepBadge: {
