/**
 * Game 2 — Place ball IN box, cup ON table, cat UNDER table. Session 7: Mixed Prepositions Review.
 */
...
const ITEMS: { id: string; label: string; emoji: string; correctZone: string }[] = [
  ...
  { id: 'cat', label: 'Cat', emoji: '🐱', correctZone: 'underTable' },
];
const ZONES: { id: string; label: string; prep: string }[] = [
  { id: 'inBox', label: 'IN the box', prep: 'in' },
  { id: 'onTable', label: 'ON the table', prep: 'on' },
  { id: 'underTable', label: 'UNDER the table', prep: 'under' },
];
...
export function DragPositionReview({ onComplete }: { onComplete: () => void }) {
  ...
  const handleZoneTap = (zoneId: string) => {
    ...
    if (correct) {
      speak(`${item.label} ${item.correctZone === 'inBox' ? 'in' : item.correctZone === 'onTable' ? 'on' : 'under'}!`);
      setPlacements((prev) => {
  ...
  return (
    <GameLayout
      ...
      instruction="Tap an object, then tap where it goes: ball IN box, cup ON table, cat UNDER table."
      icon="📦"
    ...
    >
      <View style={styles.content}>
        ...
        <View style={styles.zonesColumn}>
          ...
          <Pressable
            onPress={() => handleZoneTap('underTable')}
            style={[styles.zoneCard, wrongShake && styles.zoneShake]}
          >
            <View style={styles.underVisual}>
              <Text style={styles.zoneLabel}>UNDER the table</Text>
              {Object.entries(placements).find(([, z]) => z === 'underTable') && (
                <Text style={styles.placedEmoji}>{ITEMS.find((i) => placements[i.id] === 'underTable')?.emoji}</Text>
              )}
            </View>
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  tableVisual: { alignItems: 'center' },
  underVisual: { alignItems: 'center' },
