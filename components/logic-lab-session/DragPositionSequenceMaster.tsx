/**
 * Game 2 — Zone Track: place ball IN, cup ON, cat UNDER chair.
...
export function DragPositionSequenceMaster({ onComplete }: { onComplete: () => void }) {
  ...
  const handleZoneTap = useCallback(
    (zoneId: ZoneId) => {
      if (!selectedItem) {
        speak('Pick an object from the depot first.');
      ...
      if (Object.keys(next).length === ITEMS.length) {
        speak('All in the right place! Great job!');
        setShowSuccess(true);
  ...
  const coachLine =
    phase === 'pick'
      ? placedCount === 0
        ...
        : `${ITEMS.length - placedCount} more to place correctly.`
  ...
  if (showSuccess) {
    return (
      <SuccessCelebration
        variant="sunset"
        title="Zone Track!"
        subtitle="IN, ON, UNDER — all placed!"
        badgeEmoji="📦"
      />
    );
  }
...
const bar = StyleSheet.create({
  ...
  label: { fontSize: 12, fontWeight: '800', color: SEQ.glow, textAlign: 'center' },
});
...
const styles = StyleSheet.create({
  ...
  phaseOn: { borderColor: SEQ.glow, backgroundColor: 'rgba(245,158,11,0.12)' },
  phaseNum: { fontSize: 12, fontWeight: '900', color: SEQ.glow },
  ...
  chipSelected: { borderColor: SEQ.glow, backgroundColor: 'rgba(245,158,11,0.1)' },
  chipEmoji: { fontSize: 34 },
  chipLabel: { fontSize: 13, fontWeight: '800', color: LL.textLight, marginTop: 4 },
  chipHint: { fontSize: 9, fontWeight: '700', color: SEQ.amber, marginTop: 2 },
  chipBadge: { marginTop: 4, fontSize: 8, fontWeight: '900', color: SEQ.glow, letterSpacing: 0.8 },
  zonesCol: { gap: 12 },
