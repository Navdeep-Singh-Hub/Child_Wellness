/**
 * Game 4 — Wash Flow: order the hand-washing steps (4 actions).
...
const STEPS = [
  { id: 'water', label: 'Turn on water', emoji: '💧', correctOrder: 1, desc: 'Wet your hands' },
  { id: 'soap', label: 'Use soap', emoji: '🧼', correctOrder: 2, desc: 'Get sudsy' },
  { id: 'rinse', label: 'Rinse hands', emoji: '🙌', correctOrder: 3, desc: 'Wash away soap' },
  { id: 'dry', label: 'Dry hands', emoji: '🧻', correctOrder: 4, desc: 'Pat dry' },
] as const;

type StepId = (typeof STEPS)[number]['id'];

const VOICE = 'Put the steps in the correct order. Turn on water, use soap, rinse hands, dry hands.';

const WEAVE = { teal: '#14B8A6', glow: '#5EEAD4', thread: '#EC4899', ink: '#134E4A' } as const;

function WashPreview({ order }: { order: Partial<Record<StepId, number>> }) {
  ...
  return (
    <View style={preview.wrap}>
      ...
      <LinearGradient colors={[`${WEAVE.teal}33`, 'transparent', `${WEAVE.thread}18`]} style={preview.sky} />
      <View style={preview.track}>
        {placed.length === 0 ? (
          <Text style={preview.empty}>Your wash routine builds here</Text>
        ) : (
...
function WashSlot({
  ...
  const anim = useAnimatedStyle(() => ({
    borderColor: active && !filled ? `rgba(94,234,212,${0.45 + pulse.value * 0.4})` : filled ? WEAVE.glow : LL.glassBorder,
...
function StepCard({
  ...
  return (
    <Animated.View style={anim}>
      ...
      >
        <LinearGradient colors={[`${WEAVE.teal}33`, 'rgba(15,23,42,0.55)']} style={styles.cardGrad} />
...
export function SequencePuzzlePatternBuilder({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <LogicLabGameShell
      studio="WASH FLOW · GAME 4"
      title="Put the actions in order"
      instruction="Tap each hand-washing step in the right order."
      mascot="🪥"
    ...
    >
      ...
      <View style={styles.layout}>
        <WashPreview order={order} />
        <View style={styles.slotsCol}>
          <Text style={styles.slotsTitle}>WASH ORDER</Text>
          {[1, 2, 3, 4].map((n) => {
...
const preview = StyleSheet.create({
  wrap: {
    ...
    borderRadius: 16,
    borderWidth: 2,
    borderColor: `${WEAVE.glow}55`,
    backgroundColor: 'rgba(19,78,74,0.45)',
    padding: 10,
    overflow: 'hidden',
  },
  label: { fontSize: 8, fontWeight: '900', letterSpacing: 1, color: WEAVE.glow, marginBottom: 8 },
  ...
  node: {
    ...
    borderWidth: 2,
    borderColor: WEAVE.glow,
    alignItems: 'center',
    justifyContent: 'center',
  },
  nodeEmoji: { fontSize: 18 },
  arrow: { fontSize: 11, fontWeight: '900', color: WEAVE.teal, marginHorizontal: 2 },
...
const styles = StyleSheet.create({
  ...
  slotsTitle: { fontSize: 9, fontWeight: '900', letterSpacing: 1, color: WEAVE.glow, marginBottom: 4 },
  slot: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 8,
  ...
  slotFilled: { backgroundColor: 'rgba(20,184,166,0.12)' },
  slotNum: { fontSize: 13, fontWeight: '900', color: WEAVE.glow, width: 18 },
  ...
  counterTxt: { fontSize: 15, fontWeight: '800', color: WEAVE.glow },
  ...
  cardDesc: { fontSize: 9, fontWeight: '600', color: WEAVE.teal, marginTop: 3, textAlign: 'center' },
