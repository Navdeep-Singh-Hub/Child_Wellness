/**
 * Game 5 — Sequence Proof: draw 3 actions in order, upload for AI check.
 * Logic Lab · Section 6 · Session 9 (Sequence Master capstone)
 */
...
const VOICE =
  'Draw a sequence of three actions in your notebook. Show them in order, like wake up, brush teeth, then eat. Then take a photo or upload your drawing.';

const SEQ = { amber: '#F59E0B', glow: '#FDE68A', indigo: '#6366F1', paper: '#FFFBEB', ink: '#4338CA' } as const;

const STEPS = [
  { icon: '✏️', title: 'Sketch', body: 'Draw 3 actions in a row.' },
  { icon: '📷', title: 'Capture', body: 'Photograph or upload your page.' },
  { icon: '🔍', title: 'Verify', body: 'Lab checks actions & order!' },
] as const;

const EXAMPLES = [
  { emoji: '🛏️', label: 'Wake up', color: '#818CF8' },
  { emoji: '🪥', label: 'Brush teeth', color: '#38BDF8' },
  { emoji: '🍳', label: 'Eat', color: '#FBBF24' },
] as const;
...
function SequenceSketch() {
  ...
  return (
    <View style={example.wrap}>
      <Text style={example.label}>TARGET SEQUENCE</Text>
      <Animated.View style={[example.glow, glow]} />
      <View style={example.paper}>
        <View style={example.row}>
          {EXAMPLES.map((ex, i) => (
            <View key={ex.label} style={example.nodeWrap}>
              <View style={[example.chip, { borderColor: `${ex.color}66` }]}>
                <Text style={example.chipEmoji}>{ex.emoji}</Text>
                <Text style={[example.chipLbl, { color: ex.color }]}>{ex.label}</Text>
              </View>
              {i < EXAMPLES.length - 1 && <Text style={example.arrow}>→</Text>}
            </View>
          ))}
        </View>
        <Text style={example.caption}>three actions in order</Text>
      </View>
    </View>
  );
}
...
function CheckRow({ label, ok, pending }: { label: string; ok?: boolean; pending?: boolean }) {
  return (
    <View style={check.row}>
      ...
      >
        {pending ? (
          <ActivityIndicator size="small" color={SEQ.glow} />
        ) : (
...
export function SequenceMasterNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-sequence-master-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const threeActions = data.three_actions_detected ?? true;
      const sequenceOk = data.sequence_detected ?? true;
      const correct = data.correct ?? (threeActions && sequenceOk);
      ...
      if (correct) {
        ...
        speak('Great job! Three actions in the right order!');
        setTimeout(() => onComplete(true), 2600);
      } else {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak("Let's try again. Draw three actions with arrows showing the order.");
      }
    } catch (e: unknown) {
  ...
  const coachLine = uploading
    ? 'Scanning your sequence sketch…'
    : result?.correct
      ? 'Verified! Three actions in order.'
      : result
        ? 'Draw 3 actions with arrows — then capture again.'
        : 'Sketch wake up → brush teeth → eat in your notebook.';

  return (
    <LogicLabGameShell
      studio="SEQUENCE PROOF · GAME 5"
      title="Notebook proof"
      instruction="Draw a sequence of 3 actions in order, then upload or photograph your page."
      mascot="📓"
      coachLine={coachLine}
      onReplayVoice={playVoice}
    >
      <View style={styles.badge}>
        <Text style={styles.badgeTxt}>SESSION 9 · SEQUENCE MASTER · CAPSTONE</Text>
      </View>
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[SEQ.amber, SEQ.ink]} style={styles.actionGrad}>
            {uploading ? (
      ...
      {(uploading || result) && (
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              <ActivityIndicator color={SEQ.glow} size="large" />
              <Text style={styles.resultTitle}>Sequence scan in progress…</Text>
              <CheckRow label="Three actions detected" pending />
              <CheckRow label="Sequence order detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Sequence Proof verified!' : 'Adjust your sketch'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Three actions detected" ok={result.three_actions_detected ?? result.correct} />
              <CheckRow label="Sequence order detected" ok={result.sequence_detected ?? result.correct} />
              {!result.correct && (
...
const example = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3, color: SEQ.glow, marginBottom: 8 },
  glow: {
    position: 'absolute',
    top: 20,
    width: 120,
    height: 120,
    borderRadius: 60,
    backgroundColor: SEQ.indigo,
  },
  paper: {
    width: '88%',
    backgroundColor: SEQ.paper,
    borderRadius: 16,
    borderWidth: 2,
    borderColor: `${SEQ.ink}55`,
    paddingVertical: 18,
    alignItems: 'center',
  },
  row: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', gap: 4 },
  ...
  chipLbl: { fontSize: 9, fontWeight: '900', marginTop: 4, textAlign: 'center' },
  arrow: { fontSize: 14, fontWeight: '900', color: SEQ.amber, marginHorizontal: 2 },
  caption: { marginTop: 14, fontSize: 14, fontWeight: '800', color: SEQ.ink },
});
...
const styles = StyleSheet.create({
  ...
  stepTitle: { fontSize: 11, fontWeight: '800', color: SEQ.glow },
  ...
  retryBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 12, backgroundColor: SEQ.indigo },
