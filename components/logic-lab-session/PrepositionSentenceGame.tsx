/**
 * Game 1 — Word Weave: find the position word in sentences.
...
const VOICE_INTRO = 'Read each sentence. Tap the position word that fits.';
...
function ProgressBar({ done, total }: { done: number; total: number }) {
  ...
  return (
    <View style={bar.wrap}>
      ...
      <Text style={bar.label}>Sentence {done + 1} of {total}</Text>
    </View>
  );
}
...
function WordChip({
  ...
  const accent = PREP_COLOR[word] ?? WEAVE.teal;
  ...
  if (showSuccess) {
    return (
      <SuccessCelebration
        variant="ocean"
        title="Word Weave!"
        subtitle="You found the position words!"
        badgeEmoji="📝"
      />
    );
  }
...
const sentence = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.2, color: WEAVE.glow, marginBottom: 10 },
