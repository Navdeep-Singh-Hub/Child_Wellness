/**
 * Game 3 — Complete the pattern: 🍎 🍌 🍎 🍌 __ → 🍎. Session 5: Preposition BEHIND.
 */
...
const CHOICES: { id: Fruit; emoji: string; label: string }[] = [
  { id: 'apple', emoji: '🍎', label: 'Apple' },
  { id: 'banana', emoji: '🍌', label: 'Banana' },
  { id: 'grape', emoji: '🍇', label: 'Grape' },
];
const VOICE = 'Choose the fruit that completes the pattern.';
...
export function PatternRecognitionBehind({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    <GameLayout
      ...
      instruction="Choose the fruit that completes the pattern."
      icon="🍎"
    ...
    >
      <View style={styles.content}>
        <View style={styles.patternRow}>
          {PATTERN.map((c, i) => (
            <Text key={i} style={styles.patternDot}>{c === 'apple' ? '🍎' : '🍌'}</Text>
          ))}
