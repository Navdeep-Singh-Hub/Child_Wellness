...
import { LogicLabBackdrop } from './LogicLabBackdrop';
...
export function LogicLabGameShell({
  studio,
  title,
  instruction,
  mascot = '🏙️',
  ...
  return (
    <View style={styles.root}>
      <LogicLabBackdrop reduceMotion={reduceMotion} />
      <View style={styles.content}>
        <Animated.View entering={reduceMotion ? undefined : FadeInDown.duration(400)} style={styles.header}>
          ...
          {onReplayVoice ? (
            <Pressable onPress={onReplayVoice} style={styles.replayBtn} accessibilityLabel="Replay instruction">
              <Ionicons name="volume-high" size={18} color={LL.cyanGlow} />
        ...
        {coachLine ? (
          <Animated.View entering={reduceMotion ? undefined : FadeInUp.delay(120).duration(380)} style={styles.coach}>
            <Text style={styles.coachLabel}>VECTOR · SPATIAL GUIDE</Text>
...
const styles = StyleSheet.create({
  ...
  mascot: { fontSize: 22 },
  title: { fontSize: 22, fontWeight: '900', color: LL.textLight, flexShrink: 1 },
  instruction: {
    fontSize: 15,
    fontWeight: '600',
    color: LL.textMuted,
  ...
  replayTxt: { fontSize: 13, fontWeight: '700', color: LL.cyanGlow },
  coach: {
    backgroundColor: LL.glass,
    borderRadius: 16,
    borderWidth: 1.5,
  ...
  coachLabel: { fontSize: 9, fontWeight: '900', color: LL.accentGlow, letterSpacing: 1, marginBottom: 4 },
  coachText: { fontSize: 14, fontWeight: '700', color: LL.textLight, lineHeight: 20 },
  stage: {
    flex: 1,
