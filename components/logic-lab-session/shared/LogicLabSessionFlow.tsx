/**
 * Shared intro · play · complete flow for Logic Lab session orchestrators.
 */
import { LogicLabBackdrop } from '@/components/logic-lab-session/shared/LogicLabBackdrop';
import { LL } from '@/components/logic-lab-session/shared/logicLabTheme';
import type { LogicLabSessionConfig } from '@/components/logic-lab-session/shared/logicLabSessionConfigs';
...
export function LogicLabSessionFlow({ config, onExit, renderGame }: Props) {
          ...
          <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.hubBadge}>
              <Text style={[styles.hubBadgeTxt, { color: P.glow }]}>
                LOGIC LAB · SESSION {config.sessionNumber}
              </Text>
            </View>
              ...
              {config.cards.map((card, i) => {
                ...
                return (
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      styles.introCard,
                      { borderColor: unlocked ? `${P.accent}88` : LL.glassBorder },
                  ...
                  >
                    ...
                    {!unlocked ? (
                      <Ionicons name="lock-closed" size={18} color={LL.textMuted} />
                    ) : (
          ...
          <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
            ...
            <View
              style={[
                styles.resultBadge,
                {
                  borderColor: config.finale ? P.accent : LL.good,
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: LL.bg[0] },
  safe: { flex: 1 },
  backRow: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  cardTextCol: { flex: 1 },
  introCardTitle: { fontSize: 16, fontWeight: '800', color: LL.textLight },
  introCardDesc: { fontSize: 13, fontWeight: '600', color: LL.textMuted, marginTop: 2 },
  ...
  headerTitle: { fontSize: 20, fontWeight: '900', color: LL.textLight, textAlign: 'center' },
  ...
  resultBadgeText: { fontSize: 20, fontWeight: '900', color: LL.goodGlow },
  resultStats: { marginBottom: 14 },
  resultStat: { fontSize: 16, fontWeight: '600', color: LL.textMuted, marginBottom: 4, textAlign: 'center' },
  backBtn: {
    marginTop: 20,
    paddingVertical: 14,
