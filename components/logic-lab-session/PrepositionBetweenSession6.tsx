 ...
 * Flow: Intro → Game 1 (Position) → Game 2 (Drag BETWEEN) → Game 3 (Pattern) → Game 4 (Sequence) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Position: BETWEEN</Text>
            <Text style={styles.introSub}>Let's learn what BETWEEN means.</Text>
          </View>
          <View style={styles.exampleBox}>
            <Text style={styles.exampleTitle}>Example:</Text>
            <View style={styles.exampleVisual}>
              <Text style={styles.exampleDog}>🐕</Text>
              <Text style={styles.exampleCat}>🐱</Text>
              <Text style={styles.exampleDog}>🐕</Text>
            </View>
            <Text style={styles.exampleLabel}>cat BETWEEN two dogs</Text>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning the word BETWEEN!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>BETWEEN Position Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        <Text style={styles.title}>Position: BETWEEN</Text>
        <Text style={styles.subtitle}>Logic Lab · Session 6</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <PositionChoiceBetween onComplete={advance} />}
        {step === 2 && <DragBetweenObjects onComplete={advance} />}
        {step === 3 && <PatternRecognitionBetween onComplete={advance} />}
        {step === 4 && <SequencePuzzleBetween onComplete={advance} />}
        {step === 5 && <PrepositionBetweenNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  exampleVisual: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 8 },
  exampleDog: { fontSize: 48 },
  exampleCat: { fontSize: 48 },
