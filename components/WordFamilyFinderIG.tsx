/**
 * Word Family Finder — -IG words (Game 1 for Grouper Session 8). AAC-friendly.
 */
...
const CORRECT = [
  { id: 'pig', label: 'pig', image: 'https://placehold.co/140x140/FBCFE8/9D174D?text=pig' },
  { id: 'dig', label: 'dig', image: 'https://placehold.co/140x140/FEF3C7/B45309?text=dig' },
  { id: 'big', label: 'big', image: 'https://placehold.co/140x140/DBEAFE/1E40AF?text=big' },
  { id: 'wig', label: 'wig', image: 'https://placehold.co/140x140/DDD6FE/5B21B6?text=wig' },
];
const WRONG = [
  { id: 'sun', label: 'sun', image: 'https://placehold.co/140x140/FEF9C3/854D0E?text=sun' },
  { id: 'hat', label: 'hat', image: 'https://placehold.co/140x140/FED7AA/9A3412?text=hat' },
];
const ALL = [...CORRECT, ...WRONG].sort(() => Math.random() - 0.5);

export function WordFamilyFinderIG({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    speak('Tap the words that belong to the -IG family.', 0.75);
  }, []);
  ...
  return (
    <GameLayout
      title="Find the -IG words"
      instruction="Tap the words that belong to the -IG family."
    >
