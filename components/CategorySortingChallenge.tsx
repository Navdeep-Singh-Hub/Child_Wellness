/**
 * Sort objects into Food, Animals, Objects. Game 4 for Grouper Session 9.
 */
...
const ITEMS: { id: string; label: string; category: Category; image: string }[] = [
  ...
  { id: 'chair', label: 'chair', category: 'objects', image: 'https://placehold.co/100x100/E5E7EB/374151?text=chair' },
  { id: 'ball', label: 'ball', category: 'objects', image: 'https://placehold.co/100x100/DBEAFE/1E40AF?text=ball' },
];

export function CategorySortingChallenge({ onComplete }: { onComplete: () => void }) {
  const [assignments, setAssignments] = useState<Record<string, Category | null>>({
    apple: null, bread: null, dog: null, cat: null, chair: null, ball: null,
  });
  const [currentItem, setCurrentItem] = useState<string | null>('apple');
  ...
  if (showSuccess) {
    return <SuccessCelebration variant="indigo" title="Great Job!" subtitle="Everything sorted!" />;
  }
