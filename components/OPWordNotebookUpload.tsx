/**
 * OP Word Notebook — Draw 3 -OP words (top, mop, hop). Grouper Session 5.
 * AI checks: op_objects_present, three_drawings_present.
 */
...
const INSTRUCTIONS = 'Draw these -OP words: top, mop, hop.';
...
export function OPWordNotebookUpload({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const opPresent = data.op_objects_present ?? true;
      const threeDrawings = data.three_drawings_present ?? true;
      const correct = data.correct ?? (opPresent && threeDrawings);
      const feedback = data.feedback || (correct ? 'Great job!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        op_objects_present: data.op_objects_present,
  ...
  return (
    <GameLayout
      ...
      instruction="Draw top, mop, hop. Then upload or take a photo."
    >
      <View style={styles.content}>
        <Text style={styles.instructions}>{INSTRUCTIONS}</Text>
        <View style={styles.exampleRow}>
          <Text style={styles.exampleWords}>top · mop · hop</Text>
        </View>
