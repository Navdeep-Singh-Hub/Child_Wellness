/**
 * PartySyllableGame — Game 3: Clap the Party Words
 * Pinata (3), reward (2), balloon (2), party (2). Bouncing party lights, sparkle on correct.
 */
...
const WORDS = [
  { word: 'Pinata', syllables: 3 },
  { word: 'Reward', syllables: 2 },
  { word: 'Balloon', syllables: 2 },
  { word: 'Party', syllables: 2 },
];

export function PartySyllableGame({ onComplete }: { onComplete: () => void }) {
  ...
  if (showSuccess) return <SuccessCelebration variant="sunset" title="Great Job!" subtitle="Party words complete!" badgeEmoji="🎉" />;

  const starScaleVal = starScale.interpolate({ inputRange: [0, 1], outputRange: [0.5, 1.2] });

  return (
    <GameLayout
      title="Clap the Party Words"
...
const styles = StyleSheet.create({
  ...
  wordText: { fontSize: 28, fontWeight: '800', color: '#9D174D' },
