/**
 * Grouper Session 10 — Master Journal
 * Write one word from -at, -in, -un, -op (e.g. cat, pin, sun, top), upload photo → AI verifies.
 */
...
import { MASTER_NOTEBOOK_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
export interface GrouperMasterNotebookUploadProps {
  onComplete: () => void;
  onBack?: () => void;
  currentStep?: number;
  totalSteps?: number;
...
export function GrouperMasterNotebookUpload({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  sessionTitle,
}: GrouperMasterNotebookUploadProps) {
  ...
  useEffect(() => {
    speakGrouperHint(
      'Master journal! Write one word from each family: an AT word like cat, an IN word like pin, a UN word like sun, and an OP word like top. Then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Journal is scanning for one word from each family…');
    setMsgTone('info');
    setActiveStep(4);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-grouper-master-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const atPresent = data.at_word_present ?? false;
      const inPresent = data.in_word_present ?? false;
      const unPresent = data.un_word_present ?? false;
      const opPresent = data.op_word_present ?? false;
      const correct = data.correct ?? (atPresent && inPresent && unPresent && opPresent);
      const feedback =
        data.feedback ||
        (correct
          ? 'Great job on your master journal!'
          : 'Try again — write one word from each family: -at, -in, -un, and -op.');
      ...
      if (correct) {
        ...
        speakGrouperHint('Success! You wrote words from every family!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(3);
        speakGrouperHint('Try again. Write cat, pin, sun, and top — one from each family.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakGrouperHint(
      'Write one word from each family: cat, pin, sun, and top. Then snap a photo.'
    );
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Master Journal Complete!"
          subtitle="You wrote words from all four families!"
          badgeEmoji="📓"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Write one word from each family:{' '}
              <Text style={styles.highlight}>-at · -in · -un · -op</Text> (like cat, pin, sun, top),
              then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() =>
              speakGrouperHint(
                'Write one word from each family: cat, pin, sun, and top, then upload a photo.'
              )
            }
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Master family examples</Text>
          <View style={styles.exampleRow}>
            {['🐱 cat (-at)', '📌 pin (-in)', '☀️ sun (-un)', '🎯 top (-op)'].map((w) => (
        ...
        <View style={styles.actions}>
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, T.accentDeep]} style={styles.primaryBtn}>
...
const styles = StyleSheet.create({
  ...
  stepLabel: { fontSize: 7, fontWeight: '700', color: T.inkMuted, marginTop: 4, textAlign: 'center' },
  ...
  exampleRow: { flexDirection: 'row', gap: 10, marginTop: 8, flexWrap: 'wrap', justifyContent: 'center' },
  exampleWord: { fontSize: 13, fontWeight: '900', color: T.ink },
