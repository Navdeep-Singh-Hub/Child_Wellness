/**
 * Grouper Session 10 — Game 3: Syllable Clap Summit
 * Clap syllables: elephant (3), butterfly (3), tiger (2)
 */
...
import { MASTER_SYLLABLE_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
const WORDS = [
  { word: 'elephant', emoji: '🐘', syllables: 3 },
  { word: 'butterfly', emoji: '🦋', syllables: 3 },
  ...
  const handleClap = useCallback(() => {
    ...
    if (nextCount === targetClaps) {
      if (round + 1 >= WORDS.length) {
        ...
        speakGrouperHint(`Perfect! ${targetClaps} claps!`);
        setRound((r) => r + 1);
        setClapCount(0);
      }
    } else if (nextCount < targetClaps) {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="You clapped the right number for every word!"
          badgeEmoji="👏"
          variant="sunset"
        />
      </View>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.speechBubble}>
          ...
          <View style={styles.bubbleBody}>
            ...
            <Pressable
              onPress={() =>
                speakGrouperHint(`Clap ${targetClaps} times for ${current.word}.`)
              }
            >
              <Text style={styles.prompt}>
                Clap {targetClaps} times for this word 🔊
              </Text>
            </Pressable>
          </View>
        </View>
