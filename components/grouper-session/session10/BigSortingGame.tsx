/**
 * Grouper Session 10 — Game 4: Big Sort Summit
 * Sort 8 items into animals, clothes, tools, and food baskets
 */
...
import { MASTER_BIG_SORT_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
const ITEMS: { id: string; label: string; emoji: string; category: Category }[] = [
  ...
  { id: 'shirt', label: 'shirt', emoji: '👕', category: 'clothes' },
  { id: 'hat', label: 'hat', emoji: '🎩', category: 'clothes' },
  { id: 'hammer', label: 'hammer', emoji: '🔨', category: 'tools' },
  { id: 'screwdriver', label: 'screwdriver', emoji: '🪛', category: 'tools' },
  { id: 'apple', label: 'apple', emoji: '🍎', category: 'food' },
  { id: 'bread', label: 'bread', emoji: '🍞', category: 'food' },
];

export interface BigSortingGameProps {
  onComplete: () => void;
  onBack?: () => void;
  currentStep?: number;
  totalSteps?: number;
...
export function BigSortingGame({
  onComplete,
  onBack,
  currentStep = 4,
  totalSteps = 5,
  ...
  const [assignments, setAssignments] = useState<Record<string, Category | null>>(() =>
    Object.fromEntries(ITEMS.map((i) => [i.id, null])) as Record<string, Category | null>
  );
  ...
  useEffect(() => {
    speakGrouperHint('Sort each item into the right basket. Animals, clothes, tools, or food.');
  ...
  const handleBasket = useCallback(
    (category: Category) => {
      ...
      if (currentItem.category === category) {
        ...
        setAssignments((prev) => {
          ...
          if (!nextItem) {
            speakGrouperHint('All sorted! Summit champion work!');
            setCelebrating(true);
    ...
    [assignments, currentItem, onComplete, shake, shuffledItems, snap]
  );
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Big Sort Complete!"
          subtitle="You sorted every item into the right basket!"
          badgeEmoji="📦"
          variant="sunset"
        />
      </View>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.speechBubble}>
          ...
          <View style={styles.bubbleBody}>
            ...
            <Pressable
              onPress={() =>
                speakGrouperHint('Sort animals, clothes, tools, and food into the right baskets.')
              }
            >
              <Text style={styles.prompt}>Sort into baskets 🔊</Text>
            </Pressable>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  playArea: { flex: 1, paddingHorizontal: 16, justifyContent: 'center', gap: 16 },
  activeCard: {
    alignSelf: 'center',
    backgroundColor: T.panel,
    borderRadius: GROUPER_SESSION.radius.card,
  ...
  basketsRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8 },
  basket: {
    width: '47%',
    minWidth: 140,
    backgroundColor: T.basket,
  ...
  basketLabel: { fontSize: 12, fontWeight: '900', color: T.accentDeep, marginBottom: 6 },
  assignedList: { alignItems: 'center', gap: 2 },
  assignedWord: { fontSize: 9, fontWeight: '700', color: T.ink },
});
