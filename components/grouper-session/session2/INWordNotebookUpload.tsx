/**
 * Grouper Session 2 — Canyon Scroll Journal
 * Write and draw 3 -IN words, upload photo → AI verifies.
 */
...
export function INWordNotebookUpload({
  ...
  useEffect(() => {
    speakGrouperHint(
      'Canyon scroll journal! Write and draw three IN words like pin, tin, and bin. Then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Journal is scanning for three IN words…');
    setMsgTone('info');
    setActiveStep(3);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-in-word-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const inWords = data.in_words_present ?? true;
      const threeWords = data.three_words_present ?? true;
      const drawings = data.drawings_present ?? true;
      const correct = data.correct ?? (inWords && threeWords && drawings);
      const feedback =
        data.feedback || (correct ? 'Great job on your journal!' : 'Try again — write and draw three IN words.');
      ...
      if (correct) {
        ...
        speakGrouperHint('Success! Beautiful IN words!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakGrouperHint('Try again. Write and draw pin, tin, and bin clearly.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakGrouperHint('Write pin, tin, and bin with drawings. Then snap a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="Your -IN words look amazing!"
          badgeEmoji="📓"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Write and draw <Text style={styles.highlight}>pin · tin · bin</Text> in your journal, then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() => speakGrouperHint('Write and draw three IN words, then upload a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>-IN examples</Text>
        ...
        <View style={styles.actions}>
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#BE123C']} style={styles.primaryBtn}>
...
const styles = StyleSheet.create({
  ...
  stepDotActive: { backgroundColor: 'rgba(251, 113, 133, 0.35)', borderWidth: 2, borderColor: T.accentSoft },
  ...
  exampleGlow: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(251, 113, 133, 0.08)' },
