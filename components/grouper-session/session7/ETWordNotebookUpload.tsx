/**
 * Grouper Session 7 — Sunset Journal
 * Draw pet and jet, upload photo → AI verifies.
 */
...
import { ET_NOTEBOOK_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
import { speakGrouperHint, stopGrouperSpeech } from '../grouperSessionSpeech';
import { SunsetShelfBackground } from '../SunsetShelfBackground';

export function ETWordNotebookUpload({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakGrouperHint(
      'Sunset journal! Draw and write pet and jet in your notebook. Then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Journal is scanning for pet and jet drawings…');
    setMsgTone('info');
    setActiveStep(2);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-et-word-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const petPresent = data.pet_drawing_present ?? true;
      const jetPresent = data.jet_drawing_present ?? true;
      const correct = data.correct ?? (petPresent && jetPresent);
      const feedback =
        data.feedback || (correct ? 'Great job on your journal!' : 'Try again — draw pet and jet.');
      ...
      if (correct) {
        ...
        speakGrouperHint('Success! Beautiful ET words!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(1);
        speakGrouperHint('Try again. Draw pet and jet clearly.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakGrouperHint('Draw and write pet and jet in your notebook. Then snap a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="Your -ET words look amazing!"
          badgeEmoji="📓"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Draw and write <Text style={styles.highlight}>pet · jet</Text> in your journal, then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() => speakGrouperHint('Draw and write pet and jet, then upload a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>-ET examples</Text>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, T.accentDeep]} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>I Drew My Words!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
