/**
 * Grouper Session 5 — Game 4: Shelf Sort Nook
 * Sort mop/broom → Cleaning, top/ball → Toys
 */
...
import { OP_SORT_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
const ITEMS: { id: string; label: string; emoji: string; category: Category }[] = [
  { id: 'mop', label: 'mop', emoji: '🧹', category: 'cleaning' },
  { id: 'broom', label: 'broom', emoji: '🧹', category: 'cleaning' },
  { id: 'top', label: 'top', emoji: '🎯', category: 'toys' },
  { id: 'ball', label: 'ball', emoji: '⚽', category: 'toys' },
];
...
export function ObjectSortingGame({
  ...
  const [currentId, setCurrentId] = useState<string>('mop');
  ...
  useEffect(() => {
    speakGrouperHint('Sort each word into the right basket. Cleaning or toys.');
