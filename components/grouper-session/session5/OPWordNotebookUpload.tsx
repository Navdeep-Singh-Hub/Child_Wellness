/**
 * Grouper Session 5 — Sandstone Journal
 * Draw and write top, mop, hop, upload photo → AI verifies.
 */
...
import { OP_NOTEBOOK_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
import { speakGrouperHint, stopGrouperSpeech } from '../grouperSessionSpeech';
import { SandstoneShelfBackground } from '../SandstoneShelfBackground';

export function OPWordNotebookUpload({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakGrouperHint(
      'Sandstone journal! Write and draw three OP words like top, mop, and hop. Then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Journal is scanning for three OP words…');
    setMsgTone('info');
    setActiveStep(3);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-op-word-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const opObjects = data.op_objects_present ?? true;
      const threeDrawings = data.three_drawings_present ?? true;
      const correct = data.correct ?? (opObjects && threeDrawings);
      const feedback =
        data.feedback || (correct ? 'Great job on your journal!' : 'Try again — write and draw top, mop, and hop.');
      ...
      if (correct) {
        ...
        speakGrouperHint('Success! Beautiful OP words!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakGrouperHint('Try again. Write and draw top, mop, and hop clearly.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakGrouperHint('Write top, mop, and hop with drawings. Then snap a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="Your -OP words look amazing!"
          badgeEmoji="📓"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Write and draw <Text style={styles.highlight}>top · mop · hop</Text> in your journal, then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() => speakGrouperHint('Write and draw three OP words, then upload a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>-OP examples</Text>
