/**
 * Grouper Session 1 — Game 4: Basket Bazaar Sort
 * Sort cat/rat → Animals, hat → Clothes, bat → Sports
 */
...
import { AT_SORT_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
const ITEMS: { id: string; label: string; emoji: string; category: Category }[] = [
  ...
  { id: 'rat', label: 'rat', emoji: '🐀', category: 'animals' },
  { id: 'hat', label: 'hat', emoji: '🎩', category: 'clothes' },
  { id: 'bat', label: 'bat', emoji: '🏏', category: 'sports' },
];
...
export function ObjectSortingGame({
  ...
  const [currentId, setCurrentId] = useState<string>('cat');
  ...
  useEffect(() => {
    speakGrouperHint('Sort each word into the right basket. Tap a basket for each word.');
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          title="Baskets Full!"
          subtitle="You sorted every -AT word!"
          badgeEmoji="🧺"
          variant="sunset"
        />
      </View>
