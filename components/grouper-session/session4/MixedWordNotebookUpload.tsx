/**
 * Grouper Session 4 — Mixed Family Journal
 * Write one word from each family (-at, -in, -un), upload photo → AI verifies.
 */
...
import { MIXED_NOTEBOOK_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
export interface MixedWordNotebookUploadProps {
...
export function MixedWordNotebookUpload({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  sessionTitle,
}: MixedWordNotebookUploadProps) {
  ...
  useEffect(() => {
    speakGrouperHint(
      'Mixed family journal! Write one word from each family: an AT word like cat, an IN word like pin, and a UN word like sun. Then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-mixed-word-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const atWord = data.at_word_present ?? false;
      const inWord = data.in_word_present ?? false;
      const unWord = data.un_word_present ?? false;
      const correct = data.correct ?? (atWord && inWord && unWord);
      const feedback =
        data.feedback ||
        (correct
          ? 'Great job on your mixed family journal!'
          : 'Try again — write one word from each family: -at, -in, and -un.');
      ...
      if (correct) {
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakGrouperHint('Try again. Write cat, pin, and sun — one from each family.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakGrouperHint('Write cat, pin, and sun — one word from each family. Then snap a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="You wrote words from every family!"
          badgeEmoji="📓"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              ...
              <Text style={styles.highlight}>-at · -in · -un</Text> (like cat, pin, sun), then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() =>
              speakGrouperHint('Write one word from each family, then upload a photo.')
            }
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Mixed family examples</Text>
          <View style={styles.exampleRow}>
            {['🐱 cat (-at)', '📌 pin (-in)', '☀️ sun (-un)'].map((w) => (
        ...
        <View style={styles.actions}>
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#14532D']} style={styles.primaryBtn}>
...
const styles = StyleSheet.create({
  ...
  stepDotActive: { backgroundColor: 'rgba(74, 222, 128, 0.35)', borderWidth: 2, borderColor: T.accentSoft },
  ...
  exampleGlow: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(74, 222, 128, 0.08)' },
  ...
  exampleWord: { fontSize: 14, fontWeight: '900', color: T.ink },
