/**
 * Grouper Session 3 — Game 4: Mesa Sort Oasis
 * Sort sun/run → Outdoor, bun/apple → Food
 */
...
import { UN_SORT_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
const ITEMS: { id: string; label: string; emoji: string; category: Category }[] = [
  { id: 'sun', label: 'sun', emoji: '☀️', category: 'outdoor' },
  { id: 'run', label: 'run', emoji: '🏃', category: 'outdoor' },
  { id: 'bun', label: 'bun', emoji: '🍞', category: 'food' },
...
export function ObjectSortingGame({
  ...
  const [currentId, setCurrentId] = useState<string>('sun');
  ...
  useEffect(() => {
    speakGrouperHint('Sort each word into the right basket. Outdoor or food.');
