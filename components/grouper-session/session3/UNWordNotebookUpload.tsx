/**
 * Grouper Session 3 — Mirage Sketch Journal
 * Write and draw 3 -UN words, upload photo → AI verifies.
 */
...
import { UN_NOTEBOOK_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
import { speakGrouperHint, stopGrouperSpeech } from '../grouperSessionSpeech';
import { MirageMesaBackground } from '../MirageMesaBackground';

export function UNWordNotebookUpload({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakGrouperHint(
      'Mirage sketch journal! Write and draw three UN words like sun, bun, and run. Then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Journal is scanning for three UN words…');
    setMsgTone('info');
    setActiveStep(3);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-un-word-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const unWords = data.un_words_present ?? true;
      const threeWords = data.three_words_present ?? true;
      const drawings = data.drawings_present ?? true;
      const correct = data.correct ?? (unWords && threeWords && drawings);
      const feedback =
        data.feedback || (correct ? 'Great job on your journal!' : 'Try again — write and draw three UN words.');
      ...
      if (correct) {
        ...
        speakGrouperHint('Success! Beautiful UN words!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakGrouperHint('Try again. Write and draw sun, bun, and run clearly.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakGrouperHint('Write sun, bun, and run with drawings. Then snap a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="Your -UN words look amazing!"
          badgeEmoji="📓"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Write and draw <Text style={styles.highlight}>sun · bun · run</Text> in your journal, then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() => speakGrouperHint('Write and draw three UN words, then upload a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>-UN examples</Text>
        ...
        <View style={styles.actions}>
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#0E7490']} style={styles.primaryBtn}>
...
const styles = StyleSheet.create({
  ...
  stepDotActive: { backgroundColor: 'rgba(34, 211, 238, 0.35)', borderWidth: 2, borderColor: T.accentSoft },
  ...
  exampleGlow: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(34, 211, 238, 0.08)' },
