/**
 * Grouper Session 6 — Terracotta Journal
 * Write fan, man, pan, upload photo → AI verifies.
 */
...
import { AN_NOTEBOOK_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
export function ANWordNotebookUpload({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakGrouperHint(
      'Terracotta journal! Write fan, man, and pan in your notebook. Then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Journal is scanning for three AN words…');
    setMsgTone('info');
    setActiveStep(3);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-an-word-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const anWords = data.an_words_present ?? true;
      const threeWords = data.three_words_present ?? true;
      const correct = data.correct ?? (anWords && threeWords);
      const feedback =
        data.feedback || (correct ? 'Great job on your journal!' : 'Try again — write fan, man, and pan.');
      ...
      if (correct) {
        ...
        speakGrouperHint('Success! Beautiful AN words!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakGrouperHint('Try again. Write fan, man, and pan clearly.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakGrouperHint('Write fan, man, and pan in your notebook. Then snap a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="Your -AN words look amazing!"
          badgeEmoji="📓"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Write <Text style={styles.highlight}>fan · man · pan</Text> in your journal, then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() => speakGrouperHint('Write fan, man, and pan, then upload a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>-AN examples</Text>
