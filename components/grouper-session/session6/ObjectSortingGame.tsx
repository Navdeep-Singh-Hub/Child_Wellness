/**
 * Grouper Session 6 — Game 4: Trail Sort Nook
 * Sort pan/can → Kitchen, man/chef → People
 */
...
import { AN_SORT_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
const ITEMS: { id: string; label: string; emoji: string; category: Category }[] = [
  { id: 'pan', label: 'pan', emoji: '🍳', category: 'kitchen' },
  { id: 'can', label: 'can', emoji: '🥫', category: 'kitchen' },
  { id: 'man', label: 'man', emoji: '👨', category: 'people' },
  { id: 'chef', label: 'chef', emoji: '👨‍🍳', category: 'people' },
];
...
export function ObjectSortingGame({
  ...
  const [currentId, setCurrentId] = useState<string>('pan');
  ...
  useEffect(() => {
    speakGrouperHint('Sort each word into the right basket. Kitchen or people.');
