/**
 * Grouper Session 9 — Game 1: Family Sort Arena
 * Sort cat→-at, pin→-in, sun→-un, top→-op, fan→-an
 */
...
import { CHALLENGE_FAMILY_SORT_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
...
const BOXES: { id: Family; label: string; suffix: string; emoji: string }[] = [
  ...
  { id: 'op', label: 'OP Family', suffix: '-op', emoji: '🎩' },
  { id: 'an', label: 'AN Family', suffix: '-an', emoji: '🌀' },
];
...
const ITEMS: { id: string; label: string; emoji: string; family: Family }[] = [
  ...
  { id: 'top', label: 'top', emoji: '🎩', family: 'op' },
  { id: 'fan', label: 'fan', emoji: '🌀', family: 'an' },
];
...
export function WordFamilySortGame({
  ...
  useEffect(() => {
    speakGrouperHint('Sort each word into its word family box. AT, IN, UN, OP, or AN.');
    ...
    (family: Family) => {
      ...
      if (currentItem.family === family) {
        ...
        setAssignments((prev) => {
          ...
          if (!nextItem) {
            speakGrouperHint('All sorted! Champion work!');
            setCelebrating(true);
      ...
      <View style={styles.header}>
        ...
        <View style={styles.speechBubble}>
          ...
          <View style={styles.bubbleBody}>
            ...
            <Pressable
              onPress={() =>
                speakGrouperHint('Tap the family box that matches the word ending.')
              }
            >
              <Text style={styles.prompt}>Sort into -at, -in, -un, -op, or -an 🔊</Text>
            </Pressable>
          </View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  playArea: { flex: 1, paddingHorizontal: 12, justifyContent: 'center', gap: 16 },
  activeCard: {
    alignSelf: 'center',
  ...
  activeEmoji: { fontSize: 40, marginVertical: 4 },
  activeWord: { fontSize: 26, fontWeight: '900', color: T.ink },
  doneHint: { fontSize: 18, fontWeight: '800', color: T.accent, textAlign: 'center' },
  boxesRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8 },
  box: {
    width: '30%',
    minWidth: 96,
    backgroundColor: T.box,
  ...
  boxEmoji: { fontSize: 24, marginBottom: 2 },
  boxSuffix: { fontSize: 13, fontWeight: '900', color: T.accentDeep, marginBottom: 4 },
