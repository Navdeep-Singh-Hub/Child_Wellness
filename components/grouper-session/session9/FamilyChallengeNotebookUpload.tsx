/**
 * Grouper Session 9 — Champion Journal
 * Write 5 rhyming words (cat, hat, bat, rat, mat), upload photo → AI verifies.
 */
...
import { CHALLENGE_NOTEBOOK_THEME as T, GROUPER_SESSION } from '../grouperSessionTheme';
import { speakGrouperHint, stopGrouperSpeech } from '../grouperSessionSpeech';
import { ChallengeCampBackground } from '../ChallengeCampBackground';

export function FamilyChallengeNotebookUpload({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakGrouperHint(
      'Champion journal! Write five rhyming words like cat, hat, bat, rat, and mat. Then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Journal is scanning for five rhyming words…');
    setMsgTone('info');
    setActiveStep(1);
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-family-challenge-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const rhymingPresent = data.rhyming_words_present ?? true;
      const fiveWords = data.five_words_present ?? true;
      const correct = data.correct ?? (rhymingPresent && fiveWords);
      const feedback =
        data.feedback ||
        (correct
          ? 'Great job on your champion journal!'
          : 'Try again — write five rhyming words like cat, hat, bat, rat, mat.');
      ...
      if (correct) {
        ...
        speakGrouperHint('Success! Beautiful rhyming words!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(1);
        speakGrouperHint('Try again. Write five rhyming words clearly.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakGrouperHint('Write five rhyming words like cat, hat, bat, rat, and mat. Then snap a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Champion Journal Complete!"
          subtitle="Your five rhyming words look amazing!"
          badgeEmoji="📓"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Write <Text style={styles.highlight}>5 rhyming words</Text> (e.g. cat, hat, bat, rat,
              mat), then photograph your journal.
            </Text>
          </View>
          <Pressable
            onPress={() =>
              speakGrouperHint('Write five rhyming words like cat, hat, bat, rat, and mat, then upload a photo.')
            }
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Rhyming examples</Text>
          <View style={styles.exampleRow}>
            {['🐱 cat', '🎩 hat', '🦇 bat', '🐀 rat', '🧶 mat'].map((w) => (
...
const styles = StyleSheet.create({
  ...
  stepDotActive: { backgroundColor: 'rgba(129, 140, 248, 0.35)', borderWidth: 2, borderColor: T.accentSoft },
  ...
  exampleGlow: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(129, 140, 248, 0.08)' },
  exampleTitle: {
    fontSize: 10,
    fontWeight: '800',
    color: T.accentDeep,
