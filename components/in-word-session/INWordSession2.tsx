/**
 * Level 4 Grouper — Session 2: The -IN Word Family
 * Hub → 4 games + canyon journal task → completion
 */
import { ConfettiEffect } from '@/components/games/Level1/ConfettiEffect';
import { SuccessCelebration } from '@/components/ui/SuccessCelebration';
import { markSessionGameComplete } from '@/components/special-education/shared/useSessionIntroProgress';
import { WordFamilyFinderGame } from '@/components/grouper-session/session2/WordFamilyFinderGame';
import { RhymeMatchGame } from '@/components/grouper-session/session2/RhymeMatchGame';
import { ClapWordGame } from '@/components/grouper-session/session2/ClapWordGame';
import { ObjectSortingGame } from '@/components/grouper-session/session2/ObjectSortingGame';
import { INWordNotebookUpload } from '@/components/grouper-session/session2/INWordNotebookUpload';
import {
  GROUPER_S2_HUB_THEME as H,
  GROUPER_S2_QUESTS,
  GROUPER_SESSION,
} from '@/components/grouper-session/grouperSessionTheme';
import { CanyonCoveBackground } from '@/components/grouper-session/CanyonCoveBackground';
...
const PLAY_STEPS = 5;
const SESSION_TITLE = 'Session 2 · The -IN Family';
const SECTION_NUMBER = 4;
const SESSION_NUMBER = 2;

interface INWordSession2Props {
  onExit?: () => void;
}

export function INWordSession2({ onExit }: INWordSession2Props = {}) {
        ...
        <SafeAreaView style={styles.hubSafe}>
          ...
          <ScrollView contentContainerStyle={styles.hubScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.hubHeader}>
              <Text style={styles.hubEyebrow}>Grouper · Session 2 · Canyon Cove</Text>
              <Text style={styles.hubTitle}>{H.name}</Text>
              <Text style={styles.hubSession}>The -IN Word Family</Text>

              <View style={styles.exampleChip}>
                <Text style={styles.exampleText}>pin · tin · bin · win</Text>
              </View>
              ...
              <View style={styles.speechBubble}>
                ...
                <View style={styles.bubbleBody}>
                  ...
                  <Text style={styles.hubPrompt}>
                    High in the canyon, words ending in -in rhyme together — scout them, echo rhymes, clap, sort, then fill your journal!
                  </Text>
                </View>
              </View>
            </View>

            {GROUPER_S2_QUESTS.map((quest) => {
