/**
 * Explorer Session 1 — Notebook Activity: Write A and B, draw 1 apple.
 * Upload photo → AI check (letters_detected, apple_drawing_detected). AAC-friendly.
 */
...
const INSTRUCTIONS = 'Write A and B and draw 1 apple.\n\nExample:\nA\nB\n🍎';

export function ExplorerNotebookUpload({
  onComplete,
}: {
  ...
  const [result, setResult] = useState<{
    ...
    apple_drawing_detected?: boolean;
  } | null>(null);
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-explorer-ab-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const appleOk = data.apple_drawing_detected === true;
      const correct = data.correct === true || (lettersOk && appleOk);
      const feedback = data.feedback || (correct ? 'Great job! A and B and one apple!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        letters_detected: data.letters_detected,
        apple_drawing_detected: data.apple_drawing_detected,
      });
      if (correct) {
        speak('Great job!');
