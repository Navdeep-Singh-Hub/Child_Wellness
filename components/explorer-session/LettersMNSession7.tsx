/**
 * LettersMNSession7 — Level 1 Explorer, Session 7: Letters M & N
 * Flow: Intro → Game 1 (Find M) → Game 2 (Match M→Mango, N→Nest) → Game 3 (Tap 7) → Game 4 (Count mangoes) → Notebook → Result
...
import { ExplorerNotebookUploadMN } from './ExplorerNotebookUploadMN';
...
const MN_PAIRS = [
  { letter: 'M', object: 'Mango', emoji: '🥭' },
  { letter: 'N', object: 'Nest', emoji: '🪺' },
];

interface LettersMNSession7Props {
  onExit?: () => void;
}

export function LettersMNSession7({ onExit }: LettersMNSession7Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔤', title: 'Find the Letter', desc: 'Tap the letter M' },
      { icon: '🥭', title: 'Match Letter to Picture', desc: 'M → Mango, N → Nest' },
      { icon: '7️⃣', title: 'Tap the Number', desc: 'Tap the number 7' },
      { icon: '🥭', title: 'Count the Mangoes', desc: 'How many mangoes?' },
    ];
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Letters M & N</Text>
            <Text style={styles.introSub}>Let's learn the letters M and N.</Text>
          </View>
          <View style={styles.letterVisuals}>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>M</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🥭</Text>
              <Text style={styles.letterLabel}>Mango</Text>
            </View>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>N</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🪺</Text>
              <Text style={styles.letterLabel}>Nest</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great job learning M and N!</Text>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Letters M & N</Text>
        <Text style={styles.subtitle}>Session 7 · The Explorer</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 2 && (
          <LetterMatchGame onComplete={advance} pairs={MN_PAIRS} />
        )}
        {step === 3 && (
          <NumberTapGame
            onComplete={advance}
        ...
        {step === 5 && <ExplorerNotebookUploadMN onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
