/**
 * LettersABSession1 — Level 1 Explorer, Session 1: Letters A & B
 * Flow: Intro → Game 1 (Find the Letter) → Game 2 (Match Letter to Object) → Game 3 (Number Tap) → Game 4 (Count Objects) → Notebook → Result
 * AAC-friendly: large buttons, primary #4F46E5, success #22C55E, background #F3F4F6.
 */
...
import { ExplorerNotebookUpload } from './ExplorerNotebookUpload';
...
interface LettersABSession1Props {
  onExit?: () => void;
}

export function LettersABSession1({ onExit }: LettersABSession1Props = {}) {
  const [step, setStep] = useState(0); // 0 = intro, 1–4 = games, 5 = notebook, 6 = result
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔤', title: 'Find the Letter', desc: 'Tap the letter A' },
      { icon: '🍎', title: 'Match Letter to Picture', desc: 'A → Apple, B → Ball' },
      { icon: '1️⃣', title: 'Tap the Number', desc: 'Tap the number 1' },
      { icon: '🍎', title: 'Count the Apples', desc: 'How many apples?' },
    ];
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Letters A & B</Text>
            <Text style={styles.introSub}>Let's learn the letters A and B.</Text>
          </View>
          <View style={styles.letterVisuals}>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>A</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🍎</Text>
              <Text style={styles.letterLabel}>Apple</Text>
            </View>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>B</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>⚽</Text>
              <Text style={styles.letterLabel}>Ball</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great job learning A and B!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeStar}>⭐</Text>
            <Text style={styles.resultBadgeText}>Alphabet Starter</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Letters A & B</Text>
        <Text style={styles.subtitle}>Session 1 · The Explorer</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <LetterTapGame onComplete={advance} />}
        {step === 2 && <LetterMatchGame onComplete={advance} />}
        {step === 3 && <NumberTapGame onComplete={advance} />}
        {step === 4 && <ObjectCountGame onComplete={advance} />}
        {step === 5 && <ExplorerNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
