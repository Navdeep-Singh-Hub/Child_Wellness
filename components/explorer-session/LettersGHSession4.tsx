/**
 * LettersGHSession4 — Level 1 Explorer, Session 4: Letters G & H
 * Flow: Intro → Game 1 (Find G) → Game 2 (Match G→Goat, H→Hat) → Game 3 (Tap 4) → Game 4 (Count hats) → Notebook → Result
...
import { ExplorerNotebookUploadGH } from './ExplorerNotebookUploadGH';
...
const GH_PAIRS = [
  { letter: 'G', object: 'Goat', emoji: '🐐' },
  { letter: 'H', object: 'Hat', emoji: '🎩' },
];

interface LettersGHSession4Props {
  onExit?: () => void;
}

export function LettersGHSession4({ onExit }: LettersGHSession4Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔤', title: 'Find the Letter', desc: 'Tap the letter G' },
      { icon: '🐐', title: 'Match Letter to Picture', desc: 'G → Goat, H → Hat' },
      { icon: '4️⃣', title: 'Tap the Number', desc: 'Tap the number 4' },
      { icon: '🎩', title: 'Count the Hats', desc: 'How many hats?' },
    ];
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Letters G & H</Text>
            <Text style={styles.introSub}>Let's learn the letters G and H.</Text>
          </View>
          <View style={styles.letterVisuals}>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>G</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🐐</Text>
              <Text style={styles.letterLabel}>Goat</Text>
            </View>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>H</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🎩</Text>
              <Text style={styles.letterLabel}>Hat</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great job learning G and H!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeStar}>⭐</Text>
            <Text style={styles.resultBadgeText}>Alphabet Adventurer</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Letters G & H</Text>
        <Text style={styles.subtitle}>Session 4 · The Explorer</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 2 && (
          <LetterMatchGame onComplete={advance} pairs={GH_PAIRS} />
        )}
        {step === 3 && (
          <NumberTapGame
            onComplete={advance}
        ...
        {step === 5 && <ExplorerNotebookUploadGH onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
