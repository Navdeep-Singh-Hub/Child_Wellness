/**
 * LettersEFSession3 — Level 1 Explorer, Session 3: Letters E & F
 * Flow: Intro → Game 1 (Find E) → Game 2 (Match E→Egg, F→Fish) → Game 3 (Tap 3) → Game 4 (Count fish) → Notebook → Result
...
import { ExplorerNotebookUploadEF } from './ExplorerNotebookUploadEF';
...
const EF_PAIRS = [
  { letter: 'E', object: 'Egg', emoji: '🥚' },
  { letter: 'F', object: 'Fish', emoji: '🐟' },
];

interface LettersEFSession3Props {
  onExit?: () => void;
}

export function LettersEFSession3({ onExit }: LettersEFSession3Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔤', title: 'Find the Letter', desc: 'Tap the letter E' },
      { icon: '🥚', title: 'Match Letter to Picture', desc: 'E → Egg, F → Fish' },
      { icon: '3️⃣', title: 'Tap the Number', desc: 'Tap the number 3' },
      { icon: '🐟', title: 'Count the Fish', desc: 'How many fish?' },
    ];
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Letters E & F</Text>
            <Text style={styles.introSub}>Let's learn the letters E and F.</Text>
          </View>
          <View style={styles.letterVisuals}>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>E</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🥚</Text>
              <Text style={styles.letterLabel}>Egg</Text>
            </View>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>F</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🐟</Text>
              <Text style={styles.letterLabel}>Fish</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great job learning E and F!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeStar}>⭐</Text>
            <Text style={styles.resultBadgeText}>Letter Learner</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Letters E & F</Text>
        <Text style={styles.subtitle}>Session 3 · The Explorer</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 2 && (
          <LetterMatchGame onComplete={advance} pairs={EF_PAIRS} />
        )}
        {step === 3 && (
          <NumberTapGame
            onComplete={advance}
        ...
        {step === 5 && <ExplorerNotebookUploadEF onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
