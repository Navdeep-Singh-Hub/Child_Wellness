/**
 * LettersKLSession6 — Level 1 Explorer, Session 6: Letters K & L
 * Flow: Intro → Game 1 (Find K) → Game 2 (Match K→Kite, L→Lion) → Game 3 (Tap 6) → Game 4 (Count lions) → Notebook → Result
...
import { ExplorerNotebookUploadKL } from './ExplorerNotebookUploadKL';
...
const KL_PAIRS = [
  { letter: 'K', object: 'Kite', emoji: '🪁' },
  { letter: 'L', object: 'Lion', emoji: '🦁' },
];

interface LettersKLSession6Props {
  onExit?: () => void;
}

export function LettersKLSession6({ onExit }: LettersKLSession6Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔤', title: 'Find the Letter', desc: 'Tap the letter K' },
      { icon: '🪁', title: 'Match Letter to Picture', desc: 'K → Kite, L → Lion' },
      { icon: '6️⃣', title: 'Tap the Number', desc: 'Tap the number 6' },
      { icon: '🦁', title: 'Count the Lions', desc: 'How many lions?' },
    ];
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Letters K & L</Text>
            <Text style={styles.introSub}>Let's learn the letters K and L.</Text>
          </View>
          <View style={styles.letterVisuals}>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>K</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🪁</Text>
              <Text style={styles.letterLabel}>Kite</Text>
            </View>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>L</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🦁</Text>
              <Text style={styles.letterLabel}>Lion</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great job learning K and L!</Text>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Letters K & L</Text>
        <Text style={styles.subtitle}>Session 6 · The Explorer</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 2 && (
          <LetterMatchGame onComplete={advance} pairs={KL_PAIRS} />
        )}
        {step === 3 && (
          <NumberTapGame
            onComplete={advance}
        ...
        {step === 5 && <ExplorerNotebookUploadKL onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
