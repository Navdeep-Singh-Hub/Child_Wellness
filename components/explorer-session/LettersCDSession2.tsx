/**
 * LettersCDSession2 — Level 1 Explorer, Session 2: Letters C & D
 * Flow: Intro → Game 1 (Find C) → Game 2 (Match C→Cat, D→Dog) → Game 3 (Tap 2) → Game 4 (Count dogs) → Notebook → Result
...
import { ExplorerNotebookUploadCD } from './ExplorerNotebookUploadCD';
...
const CD_PAIRS = [
  ...
  { letter: 'D', object: 'Dog', emoji: '🐶' },
];

interface LettersCDSession2Props {
  onExit?: () => void;
}

export function LettersCDSession2({ onExit }: LettersCDSession2Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔤', title: 'Find the Letter', desc: 'Tap the letter C' },
      { icon: '🐱', title: 'Match Letter to Picture', desc: 'C → Cat, D → Dog' },
      { icon: '2️⃣', title: 'Tap the Number', desc: 'Tap the number 2' },
      { icon: '🐶', title: 'Count the Dogs', desc: 'How many dogs?' },
    ];
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Letters C & D</Text>
            <Text style={styles.introSub}>Let's learn the letters C and D.</Text>
          </View>
          <View style={styles.letterVisuals}>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>C</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🐱</Text>
              <Text style={styles.letterLabel}>Cat</Text>
            </View>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>D</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🐶</Text>
              <Text style={styles.letterLabel}>Dog</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great job learning C and D!</Text>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Letters C & D</Text>
        <Text style={styles.subtitle}>Session 2 · The Explorer</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 2 && (
          <LetterMatchGame onComplete={advance} pairs={CD_PAIRS} />
        )}
        {step === 3 && (
          <NumberTapGame
            onComplete={advance}
        ...
        {step === 5 && <ExplorerNotebookUploadCD onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
