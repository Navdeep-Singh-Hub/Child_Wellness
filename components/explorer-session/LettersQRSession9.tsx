/**
 * LettersQRSession9 — Level 1 Explorer, Session 9: Letters Q & R
 * Flow: Intro → Game 1 (Find Q) → Game 2 (Match Q→Queen, R→Rabbit) → Game 3 (Tap 9) → Game 4 (Count rabbits) → Notebook → Result
...
import { ExplorerNotebookUploadQR } from './ExplorerNotebookUploadQR';
...
const QR_PAIRS = [
  { letter: 'Q', object: 'Queen', emoji: '👑' },
  { letter: 'R', object: 'Rabbit', emoji: '🐰' },
];

interface LettersQRSession9Props {
  onExit?: () => void;
}

export function LettersQRSession9({ onExit }: LettersQRSession9Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔤', title: 'Find the Letter', desc: 'Tap the letter Q' },
      { icon: '👑', title: 'Match Letter to Picture', desc: 'Q → Queen, R → Rabbit' },
      { icon: '9️⃣', title: 'Tap the Number', desc: 'Tap the number 9' },
      { icon: '🐰', title: 'Count the Rabbits', desc: 'How many rabbits?' },
    ];
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Letters Q & R</Text>
            <Text style={styles.introSub}>Let's learn the letters Q and R.</Text>
          </View>
          <View style={styles.letterVisuals}>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>Q</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>👑</Text>
              <Text style={styles.letterLabel}>Queen</Text>
            </View>
            <View style={styles.letterRow}>
              <Text style={styles.letterBig}>R</Text>
              <Text style={styles.letterArrow}>→</Text>
              <Text style={styles.letterEmoji}>🐰</Text>
              <Text style={styles.letterLabel}>Rabbit</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great job learning Q and R!</Text>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Letters Q & R</Text>
        <Text style={styles.subtitle}>Session 9 · The Explorer</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 2 && (
          <LetterMatchGame onComplete={advance} pairs={QR_PAIRS} />
        )}
        {step === 3 && (
          <NumberTapGame
            onComplete={advance}
        ...
        {step === 5 && <ExplorerNotebookUploadQR onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
