/**
 * Explorer Session 2 — Notebook Activity: Write C and D, draw 2 cats.
 * Upload photo → AI check (letters_detected, two_cats_detected). AAC-friendly.
 */
...
const INSTRUCTIONS = 'Write C and D and draw 2 cats.\n\nExample:\nC\nD\n🐱🐱';

export function ExplorerNotebookUploadCD({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-explorer-cd-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const catsOk = data.two_cats_detected === true;
      const correct = data.correct === true || (lettersOk && catsOk);
      const feedback = data.feedback || (correct ? 'Great job! C and D and two cats!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        letters_detected: data.letters_detected,
        two_cats_detected: data.two_cats_detected,
      });
      if (correct) {
        speak('Great job!');
