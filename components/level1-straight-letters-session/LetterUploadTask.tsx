/**
 * AI Task: Write any straight-line letter (I, L, T, H, E, F) on paper and upload photo.
...
const INSTRUCTION = 'Write any of these letters on paper and upload a photo: I, L, T, H, E, F';
...
export function LetterUploadTask({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your letter.');
      return;
    }
  ...
  return (
    ...
    >
      <View style={styles.content}>
        ...
        <View style={styles.letterExamples}>
          {['I', 'L', 'T', 'H', 'E', 'F'].map((l) => (
            <Text key={l} style={styles.exLetter}>{l}</Text>
          ))}
        </View>
        {imageUri ? (
...
const styles = StyleSheet.create({
  content: { flex: 1 },
  instruction: { fontSize: 18, fontWeight: '700', color: '#5B21B6', textAlign: 'center', marginBottom: 12, lineHeight: 26 },
  letterExamples: { flexDirection: 'row', justifyContent: 'center', gap: 12, marginBottom: 20 },
  exLetter: { fontSize: 30, fontWeight: '900', color: '#7C3AED' },
