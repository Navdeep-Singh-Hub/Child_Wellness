/**
 * MusicNotebookUpload — Music Notebook Activity
 * Option 1: Draw 7 musical notes and write 7. Option 2: Two rhyming instrument words (e.g. bell, shell).
 */
...
const INSTRUCTIONS =
  'Option 1: Draw 7 musical notes in your notebook and write the number 7.\n\nOR\n\nOption 2: Write two rhyming instrument-related words. Example: bell, shell.';

export function MusicNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-music-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
...
const styles = StyleSheet.create({
  ...
  previewWrap: { marginTop: 20, borderRadius: 16, overflow: 'hidden', borderWidth: 3, borderColor: '#8B5CF6' },
  ...
  resultOk: { backgroundColor: '#EDE9FE', borderColor: '#8B5CF6' },
  resultFail: { backgroundColor: '#FEE2E2', borderColor: '#EF4444' },
  resultEmoji: { fontSize: 48, marginBottom: 8 },
  resultText: { fontSize: 24, fontWeight: '800', color: '#5B21B6' },
