/**
 * MusicSession7 — Special Ed Level 2 · Session 7: Music Party
 * Flow: Intro → Game 1 (Find Music Sounds) → Game 2 (I Spy Rhymes) → Game 3 (Clap Instrument Words) → Game 4 (Count Notes) → Notebook → Result
 * Design: Purple #8B5CF6, Music Blue #3B82F6, Golden Yellow #FACC15. Colorful music party, tablet/touch.
 */
...
import { MusicCleanUp } from './MusicCleanUp';
import { MusicRhymeGame } from './MusicRhymeGame';
import { MusicSyllableGame } from './MusicSyllableGame';
import { MusicDragToCount } from './MusicDragToCount';
import { MusicNotebookUpload } from './MusicNotebookUpload';
...
const GOLDEN_YELLOW = '#FACC15';

interface MusicSession7Props {
  onExit?: () => void;
}

export function MusicSession7({ onExit }: MusicSession7Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🎵', title: 'Find the Music Sounds', desc: 'Initial sound /d/' },
      { icon: '👀', title: 'I Spy Music Rhymes', desc: 'Rhyming hunt' },
      { icon: '👏', title: 'Clap the Instrument Words', desc: 'Syllables' },
      { icon: '🎶', title: 'Count the Musical Notes', desc: 'Tap notes 1–10' },
    ];
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Music Party</Text>
            <Text style={styles.introSub}>
              Join the music party and learn sounds, rhymes, and counting!
            </Text>
          </View>
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.notesRow}>
            {['🎵', '⭐', '🥁', '🎶', '✨', '🎵'].map((e, i) => (
              <Text key={i} style={styles.noteEmoji}>{e}</Text>
            ))}
          </View>
          <Text style={styles.resultTitle}>You made amazing music while learning!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>🎵</Text>
            <Text style={styles.resultBadgeText}>Music Star Badge</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <MusicCleanUp onComplete={advance} />}
        {step === 2 && <MusicRhymeGame onComplete={advance} />}
        {step === 3 && <MusicSyllableGame onComplete={advance} />}
        {step === 4 && <MusicDragToCount onComplete={advance} />}
        {step === 5 && <MusicNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  backButtonText: { fontSize: 17, fontWeight: '700', color: '#6D28D9' },
  header: {
  ...
  progressFill: { height: '100%', backgroundColor: PURPLE, borderRadius: 7 },
  progressText: { fontSize: 16, fontWeight: '700', color: '#5B21B6', minWidth: 48 },
  ...
  introTitle: { fontSize: 28, fontWeight: '800', color: '#5B21B6', textAlign: 'center', marginBottom: 12 },
  introSub: { fontSize: 18, color: '#6D28D9', textAlign: 'center', lineHeight: 26 },
  ...
  notesRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginBottom: 16 },
  noteEmoji: { fontSize: 32 },
  resultTitle: { fontSize: 24, fontWeight: '800', color: '#5B21B6', textAlign: 'center', marginBottom: 24 },
  resultBadge: {
    flexDirection: 'row',
    alignItems: 'center',
