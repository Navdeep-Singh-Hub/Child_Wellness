/**
 * Word Family Finder — -UN words (Game 1 for Grouper Session 3). AAC-friendly.
 */
...
const CORRECT = [
  ...
  { id: 'bun', label: 'bun', image: 'https://placehold.co/140x140/FED7AA/9A3412?text=bun' },
  { id: 'run', label: 'run', image: 'https://placehold.co/140x140/DBEAFE/1E40AF?text=run' },
  { id: 'fun', label: 'fun', image: 'https://placehold.co/140x140/DDD6FE/5B21B6?text=fun' },
];
const WRONG = [
...
export function WordFamilyFinderUN({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    speak('Tap the words that belong to the -UN family.', 0.75);
  }, []);
  ...
  return (
    <GameLayout
      title="Find the -UN words"
      instruction="Tap the words that belong to the -UN family."
    >
