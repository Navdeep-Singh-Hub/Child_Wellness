/**
 * Sort the Objects — Animals (pet, dog) vs Vehicles (jet, car). Game 4 for Grouper Session 7.
 */
...
const ITEMS: { id: string; label: string; category: Category; image: string }[] = [
  { id: 'pet', label: 'pet', category: 'animals', image: 'https://placehold.co/100x100/FED7AA/9A3412?text=pet' },
  { id: 'dog', label: 'dog', category: 'animals', image: 'https://placehold.co/100x100/FDE68A/92400E?text=dog' },
  { id: 'jet', label: 'jet', category: 'vehicles', image: 'https://placehold.co/100x100/DBEAFE/1E40AF?text=jet' },
  { id: 'car', label: 'car', category: 'vehicles', image: 'https://placehold.co/100x100/FEE2E2/991B1B?text=car' },
];

export function ObjectSortingET({ onComplete }: { onComplete: () => void }) {
  ...
  const [currentItem, setCurrentItem] = useState<string | null>('pet');
