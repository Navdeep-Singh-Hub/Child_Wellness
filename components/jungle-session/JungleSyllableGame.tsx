/**
 * JungleSyllableGame — Game 3: Clap Jungle Words
 * gorilla (3), toucan (2), tiger (2), parrot (2). Bouncing leaves, star on correct.
 */
...
const WORDS = [
  { word: 'Gorilla', syllables: 3 },
  { word: 'Toucan', syllables: 2 },
  { word: 'Tiger', syllables: 2 },
  { word: 'Parrot', syllables: 2 },
];

export function JungleSyllableGame({ onComplete }: { onComplete: () => void }) {
  const [round, setRound] = useState(0);
  const [showSuccess, setShowSuccess] = useState(false);
  const [leafAnims] = useState(() =>
  ...
  const triggerLeaves = useCallback((n: number) => {
    for (let i = 0; i < 3; i++) {
      const a = leafAnims[i];
      if (!a) continue;
      a.setValue(0);
  ...
  const handleClapChoice = useCallback(
    ...
    [current, round, onComplete, triggerLeaves, triggerStar]
  );
    ...
    >
      <View style={styles.content}>
        ...
        <View style={styles.leavesRow}>
          {[0, 1, 2].map((i) => {
            const opacity = leafAnims[i]?.interpolate({ inputRange: [0, 1], outputRange: [0.4, 1] });
            return (
              <Animated.View
                key={i}
                style={[styles.leaf, opacity != null ? { opacity } : { opacity: 0.4 }]}
              >
...
const styles = StyleSheet.create({
  ...
  leavesRow: { flexDirection: 'row', gap: 16, marginBottom: 24 },
