/**
 * JungleSession3 — Special Ed Level 2 · Session 3: Jungle Safari
 * Flow: Intro → Game 1 (Jungle Clean-Up) → Game 2 (I Spy Rhymes) → Game 3 (Clap Jungle Words) → Game 4 (Monkey Dot Maze) → Notebook → Result
 * Design: Jungle Green #22C55E, Leaf Green #4ADE80, Sun Yellow #FACC15.
...
import { JungleCleanUp } from './JungleCleanUp';
import { JungleRhymeGame } from './JungleRhymeGame';
import { JungleSyllableGame } from './JungleSyllableGame';
import { JungleDotMaze } from './JungleDotMaze';
import { JungleNotebookUpload } from './JungleNotebookUpload';

const TOTAL_GAMES = 5;
const GAME_COUNT = 4;
const SECTION_NUMBER = 2;
const SESSION_NUMBER = 3;
const JUNGLE_GREEN = '#22C55E';
const LEAF_GREEN = '#4ADE80';
const SUN_YELLOW = '#FACC15';

interface JungleSession3Props {
  onExit?: () => void;
}

export function JungleSession3({ onExit }: JungleSession3Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧹', title: 'Jungle Clean-Up', desc: 'Find the sound' },
      { icon: '👀', title: 'I Spy Jungle Rhymes', desc: 'Rhyming hunt' },
      { icon: '👏', title: 'Clap Jungle Words', desc: 'Syllables' },
      { icon: '🐒', title: 'Monkey Dot Maze', desc: 'Count the dots' },
    ];
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Jungle Safari</Text>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great job exploring the jungle!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>🏅</Text>
            <Text style={styles.resultBadgeText}>Jungle Explorer Badge</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Session 3</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <JungleCleanUp onComplete={advance} />}
        {step === 2 && <JungleRhymeGame onComplete={advance} />}
        {step === 3 && <JungleSyllableGame onComplete={advance} />}
        {step === 4 && <JungleDotMaze onComplete={advance} />}
        {step === 5 && <JungleNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  backButtonText: { fontSize: 17, fontWeight: '700', color: '#166534' },
  header: {
    ...
    paddingBottom: 16,
    borderBottomWidth: 3,
    borderBottomColor: JUNGLE_GREEN,
  },
  ...
  progressFill: { height: '100%', backgroundColor: JUNGLE_GREEN, borderRadius: 7 },
  ...
  leavesRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginBottom: 16 },
  leafEmoji: { fontSize: 32 },
