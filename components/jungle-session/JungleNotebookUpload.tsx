/**
 * JungleNotebookUpload — Jungle Notebook Activity
 * Option 1: Two rhyming jungle words + drawings (e.g. tree, bee). Option 2: Jungle word with syllables marked (go-ril-la → 3) + illustration.
 */
...
const INSTRUCTIONS =
  'Option 1: Write two rhyming jungle words and draw them (e.g. tree, bee).\n\nOR\n\nOption 2: Write a jungle word and mark the syllables (e.g. go-ril-la → 3). Then illustrate the word.';

export function JungleNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-jungle-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const correct =
        data.correct ??
        (data.drawing_present && (data.rhymes_present || data.syllables_marked)) ??
        true;
      const feedback = data.feedback || (correct ? 'Great drawing!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        rhymes_present: data.rhymes_present,
        syllables_marked: data.syllables_marked,
  ...
  return (
    <GameLayout title="Jungle Notebook Activity" instruction={INSTRUCTIONS}>
