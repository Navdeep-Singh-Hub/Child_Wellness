/**
 * AI Task: Draw curved lines on paper and upload photo.
...
const INSTRUCTION = 'Draw curved lines (waves, circles, or arcs) on paper and upload a photo.';
...
export function VowelUploadTask({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your curves.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      setResult({
        ...
        feedback: data.feedback || (success ? 'Great curve drawing!' : 'Try again. Draw curved lines on paper.'),
      });
...
const styles = StyleSheet.create({
  ...
  vowelExamples: { flexDirection: 'row', justifyContent: 'center', gap: 14, marginBottom: 20 },
  vowelChar: { fontSize: 32, fontWeight: '900', color: '#7C3AED' },
