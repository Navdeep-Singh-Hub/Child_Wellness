/**
 * Builder Session 4 — Real World Task: Show something BLUE.
 * Upload photo → AI verifies presence of blue colored object.
 */
...
const INSTRUCTIONS = 'Show something BLUE.\n\nTake a photo or upload a picture of something blue.';
...
export function BuilderNotebookUploadBlue({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see something blue!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see something blue!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Show something blue.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show something blue. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
