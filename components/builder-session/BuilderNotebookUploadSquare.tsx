/**
 * Builder Session 7 — Real World Task: Show a SQUARE object.
 * Upload photo → AI verifies square shape.
 */
...
const INSTRUCTIONS = 'Show a SQUARE object.\n\nTake a photo or upload a picture of something square.';
...
export function BuilderNotebookUploadSquare({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see a square!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see a square!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Show something square.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show a square object. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
