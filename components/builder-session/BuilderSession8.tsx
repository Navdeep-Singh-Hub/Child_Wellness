/**
 * Level 3 Builder — Session 8: Emotions, Colors, Letters & Habitats
 * Flow: Intro → Game 1 (Emoji Emotion) → Game 2 (Color Sorting) → Game 3 (Letter Puzzle) → Game 4 (Animal Habitat) → Real World Task (Smile) → Result
 */
...
import { EmojiEmotionGame } from './EmojiEmotionGame';
import { ColorSortingGame } from './ColorSortingGame';
import { LetterPuzzleGame } from './LetterPuzzleGame';
import { AnimalHabitatGame } from './AnimalHabitatGame';
import { BuilderNotebookUploadSmile } from './BuilderNotebookUploadSmile';
...
interface BuilderSession8Props {
  onExit?: () => void;
}

export function BuilderSession8({ onExit }: BuilderSession8Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '😊', title: 'Emotion Recognition', desc: 'Tap the HAPPY face' },
      { icon: '🎨', title: 'Color Sorting', desc: 'Sort objects by color' },
      { icon: '☀️', title: 'Letter Puzzle', desc: 'Arrange letters to form SUN' },
      { icon: '🐾', title: 'Animal Habitat', desc: 'Match animals to habitat' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Emotions, Colors & More</Text>
            <Text style={styles.introSub}>Session 8 · The Builder</Text>
            <Text style={styles.introDesc}>Play 4 games, then smile and take a photo!</Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Smile Master</Text>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Emotions, Colors & More</Text>
        <Text style={styles.subtitle}>Session 8 · The Builder</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <EmojiEmotionGame onComplete={advance} />}
        {step === 2 && <ColorSortingGame onComplete={advance} />}
        {step === 3 && <LetterPuzzleGame onComplete={advance} />}
        {step === 4 && <AnimalHabitatGame onComplete={advance} />}
        {step === 5 && <BuilderNotebookUploadSmile onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
