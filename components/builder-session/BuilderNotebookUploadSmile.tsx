/**
 * Builder Session 8 — Real World Task: Smile and take a photo.
 * Upload/take photo → AI checks facial emotion (smile) detection.
 */
...
const INSTRUCTIONS = 'Smile and take a photo.\n\nTake a selfie or upload a picture where you are smiling.';
...
export function BuilderNotebookUploadSmile({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see your smile!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see your smile!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Smile for the camera!');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Smile and take a photo. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
