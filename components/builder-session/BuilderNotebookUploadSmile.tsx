/**
 * Builder Session 8 — Smile Scout Studio
 * Smile and upload photo → AI verifies facial expression.
 */
...
import { BUILDER_SESSION, SMILE_SCOUT_THEME as T } from './builderSessionTheme';
...
export function BuilderNotebookUploadSmile({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakBuilderHint(
      'Smile Scout mission! Give a big happy smile, then take or upload a photo of your face.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Glow is checking your smile…');
    setMsgTone('info');
    setActiveStep(3);

    try {
      const formData = new FormData();
      const filename = 'smile-photo.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-builder-s8-smile-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see your smile!' : 'Try again — show a big happy smile.');
      ...
      if (correct) {
        ...
        speakBuilderHint('Success! We see your smile!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakBuilderHint('Try again. Smile big for the camera!');
        try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your smile.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakBuilderHint('Great! Give a big smile, then take or upload a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Smile Scout Complete!"
          subtitle="You shared a wonderful smile!"
          badgeEmoji="😊"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Show a <Text style={styles.smileWord}>big smile</Text>, then snap a selfie or upload a photo.
            </Text>
          </View>
          <Pressable
            onPress={() => speakBuilderHint('Smile big and take or upload a photo.')}
        ...
        <View style={styles.targetShowcase}>
          <Animated.View style={[styles.smileGlow, pulseStyle]} pointerEvents="none" />
          <Text style={styles.showcaseLabel}>Your target mood</Text>
          <Text style={styles.showcaseEmoji}>😊</Text>
          <Text style={styles.showcaseHint}>Big happy smile</Text>
        </View>
          ...
          <View style={styles.frame}>
            {photo ? (
              ...
              <View style={styles.placeholder}>
                ...
                <Text style={styles.placeholderHint}>Show your smile clearly</Text>
              </View>
            )}
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#D97706']} style={styles.primaryBtn}>
                <Ionicons name="happy" size={20} color={T.ink} />
                <Text style={styles.primaryBtnTextDark}>I Can Smile!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  smileWord: { fontWeight: '900', color: T.accentDeep, fontSize: 16 },
  listenBtn: {
    width: 40,
    height: 40,
    borderRadius: 20,
  ...
  showcaseEmoji: { fontSize: 56, marginVertical: 4 },
