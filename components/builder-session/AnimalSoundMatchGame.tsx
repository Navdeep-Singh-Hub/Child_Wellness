/**
 * Builder Session 3 — Game 4: Animal Match
 * Match animal with its sound. Tap animal then tap the correct sound.
 */
...
export function AnimalSoundMatchGame({ onComplete }: AnimalSoundMatchGameProps) {
  ...
  const handleSoundTap = useCallback(
    (soundId: string) => {
      ...
      setSelectedId(null);
      if (matched.size + 1 >= shuffledAnimals.length) {
        setShowSuccess(true);
        setTimeout(() => onComplete(), 2200);
      }
    },
    [selectedId, matched, onComplete, triggerWrong, shuffledAnimals.length]
  );
...
const styles = StyleSheet.create({
  ...
  soundText: { fontSize: 16, fontWeight: '800', color: '#374151' },
