/**
 * Builder Session 1 — Real World Task: Find something RED and place it on a table.
 * Upload photo → AI checks if photo contains a red object. SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS = 'Find something RED around you and place it on a table.\n\nThen take a photo or upload a picture.';
...
export function BuilderNotebookUploadRed({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! You found something red!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! You found something red!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Find something red.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Find something RED. Put it on a table. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
