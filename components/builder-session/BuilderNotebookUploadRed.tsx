/**
 * Builder Session 1 — Scarlet Scout Dispatch
 * Find something RED, place on table, upload photo → AI validates.
 */
...
import { BUILDER_SESSION, RED_SCOUT_THEME as T } from './builderSessionTheme';
...
export function BuilderNotebookUploadRed({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakBuilderHint(
      'Scarlet Scout mission! Find something red around you, put it on a table, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Ruby is scanning for red…');
    setMsgTone('info');
    setActiveStep(3);

    try {
      const formData = new FormData();
      const filename = 'red-task.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-builder-s1-red-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const feedback =
        data.feedback || (correct ? 'SUCCESS! You found something red!' : 'Try again — look for a red object.');
      ...
      if (correct) {
        ...
        speakBuilderHint('Success! You found something red!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakBuilderHint('Try again. Find something red and place it on the table.');
        try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your red object.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakBuilderHint('Great! Put your red object on a table, then take or upload a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="You found red in the real world!"
          badgeEmoji="🔴"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Find something <Text style={styles.redWord}>RED</Text>, place it on a table, then snap a photo.
            </Text>
          </View>
          <Pressable
            onPress={() => speakBuilderHint('Find something red and put it on a table.')}
        ...
        <View style={styles.targetShowcase}>
          <Animated.View style={[styles.redGlow, pulseStyle]} pointerEvents="none" />
          <Text style={styles.showcaseLabel}>Your target</Text>
          <Text style={styles.showcaseRed}>RED</Text>
          ...
          <View style={styles.frame}>
            {photo ? (
              ...
              <View style={styles.placeholder}>
                ...
                <Text style={styles.placeholderHint}>Show the red object clearly</Text>
              </View>
            )}
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#D97706']} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>I Found Red!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  redWord: { fontWeight: '900', color: T.accent, fontSize: 16 },
  listenBtn: {
    width: 40,
    height: 40,
    borderRadius: 20,
  ...
  stepDotActive: { backgroundColor: 'rgba(248, 113, 113, 0.35)', borderColor: T.accentSoft },
  ...
  showcaseRed: { fontSize: 52, fontWeight: '900', color: T.accent, lineHeight: 58 },
