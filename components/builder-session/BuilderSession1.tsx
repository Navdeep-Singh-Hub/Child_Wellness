/**
 * Level 3 Builder — Session 1: Object & Shape Fun
 * Flow: Intro → Game 1 (Object Recognition) → Game 2 (Shape Recognition) → Game 3 (Shadow Match) → Game 4 (Color Match) → Real World Task (Find Red) → Result
 * Playful, bright, child-friendly UI. Confetti + sound on success.
 */
...
import { BuilderNotebookUploadRed } from './BuilderNotebookUploadRed';

const GAME_COUNT = 4;
const TOTAL_STEPS = 6; // 4 games + real-world task + result
const DESIGN = {
  primary: '#8B5CF6',
  success: '#22C55E',
  background: '#F5F3FF',
...
interface BuilderSession1Props {
  onExit?: () => void;
}

export function BuilderSession1({ onExit }: BuilderSession1Props = {}) {
  const [step, setStep] = useState(0); // 0 = intro, 1–4 = games, 5 = task, 6 = result
  ...
  if (step === 0) {
    const cards = [
      { icon: '🐱', title: 'Object Recognition', desc: 'Tap the CAT' },
      { icon: '⭕', title: 'Shape Recognition', desc: 'Tap the circle' },
      { icon: '🖼️', title: 'Match to Shadows', desc: 'Match objects with shadows' },
      { icon: '🎨', title: 'Color Match', desc: 'Put colors in the right boxes' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Object & Shape Fun</Text>
            <Text style={styles.introSub}>Session 1 · The Builder</Text>
            <Text style={styles.introDesc}>Play 4 games, then find something RED!</Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Builder Starter</Text>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Object & Shape Fun</Text>
        <Text style={styles.subtitle}>Session 1 · The Builder</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <ObjectRecognitionGame onComplete={advance} />}
        {step === 2 && <ShapeRecognitionGame onComplete={advance} />}
        {step === 3 && <ShadowMatchGame onComplete={advance} />}
        {step === 4 && <ColorMatchGame onComplete={advance} />}
        {step === 5 && <BuilderNotebookUploadRed onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
