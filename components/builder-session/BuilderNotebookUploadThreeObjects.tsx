/**
 * Builder Session 3 — Real World Task: Place 3 objects on a table.
 * Upload photo → AI counts objects and checks if there are 3.
 */
...
const INSTRUCTIONS = 'Place 3 objects on a table.\n\nThen take a photo or upload a picture.';
...
export function BuilderNotebookUploadThreeObjects({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see 3 objects!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see 3 objects!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Place 3 objects on a table.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Place 3 objects on a table. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
