/**
 * Builder Session 3 — Triple Table Tally
 * Place 3 objects on a table, upload photo → AI counts objects.
 */
...
import { BUILDER_SESSION, TRIPLE_TABLE_THEME as T } from './builderSessionTheme';
...
export function BuilderNotebookUploadThreeObjects({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakBuilderHint(
      'Triple Table mission! Place three objects on a table, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Tally is counting objects on your table…');
    setMsgTone('info');
    setActiveStep(3);

    try {
      const formData = new FormData();
      const filename = 'three-objects.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-builder-s3-three-objects-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see 3 objects!' : 'Try again — place exactly 3 objects on the table.');
      ...
      if (correct) {
        ...
        speakBuilderHint('Success! We see three objects!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakBuilderHint('Try again. Place three objects on a table and take a clear photo.');
        try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your 3 objects.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakBuilderHint('Great! Place three objects on a table, then take or upload a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Table Tally Complete!"
          subtitle="You placed three objects in the real world!"
          badgeEmoji="3️⃣"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Place <Text style={styles.countWord}>3 objects</Text> on a table, then snap a photo.
            </Text>
          </View>
          <Pressable
            onPress={() => speakBuilderHint('Place three objects on a table, then take or upload a photo.')}
        ...
        <View style={styles.targetShowcase}>
          <Animated.View style={[styles.tableGlow, pulseStyle]} pointerEvents="none" />
          <Text style={styles.showcaseLabel}>Your target count</Text>
          <Text style={styles.showcaseCount}>3</Text>
          ...
          <View style={styles.frame}>
            {photo ? (
              ...
              <View style={styles.placeholder}>
                ...
                <Text style={styles.placeholderHint}>Show all three objects clearly</Text>
              </View>
            )}
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#D97706']} style={styles.primaryBtn}>
                <Ionicons name="grid" size={20} color={T.ink} />
                <Text style={styles.primaryBtnTextDark}>I Placed 3 Objects!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  showcaseCount: { fontSize: 52, fontWeight: '900', color: T.accent, lineHeight: 58 },
