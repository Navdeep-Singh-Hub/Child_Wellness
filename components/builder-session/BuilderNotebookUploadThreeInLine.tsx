/**
 * Builder Session 10 — Line Scout Summit
 * Arrange THREE objects in a straight line, upload photo → AI verifies.
 */
...
import { BUILDER_SESSION, LINE_SCOUT_THEME as T } from './builderSessionTheme';
...
export function BuilderNotebookUploadThreeInLine({
  ...
  useEffect(() => {
    speakBuilderHint(
      'Line Scout mission! Pick three objects and arrange them in a straight line, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Scanning for three objects in a line…');
    setMsgTone('info');
    setActiveStep(3);
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct
          ? 'SUCCESS! Three objects in a straight line!'
          : 'Try again — arrange three objects in a row.');
      ...
      if (correct) {
        ...
        speakBuilderHint('Success! Three objects in a straight line!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakBuilderHint('Try again. Line up three objects in a straight row.');
        try {
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakBuilderHint('Great! Line up three objects, then take or upload a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Line Scout Complete!"
          subtitle="Three objects in a perfect row!"
          badgeEmoji="📐"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <View style={styles.badgeRow}>
            ...
            <View style={styles.capstonePill}>
              <Text style={styles.capstoneText}>🏆 Master Task</Text>
            </View>
          </View>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Arrange <Text style={styles.shapeWord}>three objects</Text> in a straight line, then snap a photo.
            </Text>
          </View>
          <Pressable
            onPress={() =>
              speakBuilderHint('Line up three objects in a row and take or upload a photo.')
            }
        ...
        <View style={styles.targetShowcase}>
          <Animated.View style={[styles.lineGlow, pulseStyle]} pointerEvents="none" />
          <Text style={styles.showcaseLabel}>Your target layout</Text>
          ...
          <Text style={styles.showcaseHint}>Three in a row</Text>
        </View>
          ...
          <View style={styles.frame}>
            {photo ? (
              ...
              <View style={styles.placeholder}>
                ...
                <Text style={styles.placeholderHint}>Show three objects lined up clearly</Text>
              </View>
            )}
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#D97706']} style={styles.primaryBtn}>
                <Ionicons name="reorder-three-outline" size={20} color={T.ink} />
                <Text style={styles.primaryBtnTextDark}>I Have Three Objects!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  stepDotActive: { backgroundColor: 'rgba(250, 204, 21, 0.35)', borderColor: T.accentSoft },
