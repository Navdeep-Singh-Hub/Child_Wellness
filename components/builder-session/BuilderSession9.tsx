/**
 * Level 3 Builder — Session 9: Spot the Difference, Count, Build & Shapes
 * Flow: Intro → Game 1 (Spot the Difference) → Game 2 (Counting) → Game 3 (Build House) → Game 4 (Shape ID) → Real World Task (Rectangle) → Result
 */
...
import { DragToBuildHouseGame } from './DragToBuildHouseGame';
import { ShapeIdentificationGame } from './ShapeIdentificationGame';
import { BuilderNotebookUploadRectangle } from './BuilderNotebookUploadRectangle';
...
interface BuilderSession9Props {
  onExit?: () => void;
}

export function BuilderSession9({ onExit }: BuilderSession9Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔍', title: 'Spot the Difference', desc: 'Find the difference between two pictures' },
      { icon: '⭐', title: 'Counting Game', desc: 'Count stars' },
      { icon: '🏠', title: 'Build the House', desc: 'Arrange parts to form house' },
      { icon: '▭', title: 'Shape Identification', desc: 'Select rectangle' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Spot the Difference & Shapes</Text>
            <Text style={styles.introSub}>Session 9 · The Builder</Text>
            <Text style={styles.introDesc}>Play 4 games, then show a rectangle object!</Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Shape Detective</Text>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Spot the Difference & Shapes</Text>
        <Text style={styles.subtitle}>Session 9 · The Builder</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <SpotTheDifferenceGame onComplete={advance} />}
        {step === 2 && <CountingGame onComplete={advance} />}
        {step === 3 && <DragToBuildHouseGame onComplete={advance} />}
        {step === 4 && <ShapeIdentificationGame onComplete={advance} />}
        {step === 5 && <BuilderNotebookUploadRectangle onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
