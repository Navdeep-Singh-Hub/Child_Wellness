/**
 * Builder Session 2 — Real World Task: Draw a CIRCLE on paper.
 * Upload photo → AI verifies if a circle drawing exists.
 */
...
const INSTRUCTIONS = 'Draw a CIRCLE on paper.\n\nThen take a photo or upload a picture of your drawing.';
...
export function BuilderNotebookUploadCircle({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see your circle!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see your circle!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Draw a circle on paper.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Draw a circle on paper. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
