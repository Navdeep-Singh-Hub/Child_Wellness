/**
 * Builder Session 2 — Compass Sketch Atelier
 * Draw a CIRCLE on paper, upload photo → AI verifies.
 */
...
import { BUILDER_SESSION, CIRCLE_SKETCH_THEME as T } from './builderSessionTheme';
...
export function BuilderNotebookUploadCircle({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakBuilderHint(
      'Compass Sketch mission! Draw a circle on paper, then take or upload a photo of your drawing.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Compass is scanning your circle…');
    setMsgTone('info');
    setActiveStep(3);

    try {
      const formData = new FormData();
      const filename = 'circle-drawing.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-builder-s2-circle-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const feedback =
        data.feedback || (correct ? 'SUCCESS! We see your circle!' : 'Try again — draw a round circle on paper.');
      ...
      if (correct) {
        ...
        speakBuilderHint('Success! We see your circle!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakBuilderHint('Try again. Draw a circle on paper and take a clear photo.');
        try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your circle drawing.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakBuilderHint('Great! Draw a circle on paper, then take or upload a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="You drew a circle in the real world!"
          badgeEmoji="⭕"
          variant="indigo"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Draw a <Text style={styles.circleWord}>CIRCLE</Text> on paper, then snap a photo of your sketch.
            </Text>
          </View>
          <Pressable
            onPress={() => speakBuilderHint('Draw a circle on paper, then take or upload a photo.')}
        ...
        <View style={styles.targetShowcase}>
          <Animated.View style={[styles.circleGlow, pulseStyle]} pointerEvents="none" />
          <Text style={styles.showcaseLabel}>Your target shape</Text>
          <View style={styles.circleRing}>
            <View style={styles.circleInner} />
          </View>
          <Text style={styles.showcaseHint}>✏️ 📄 ⭕ 🧭</Text>
        </View>
          ...
          <View style={styles.frame}>
            {photo ? (
              ...
              <View style={styles.placeholder}>
                ...
                <Text style={styles.placeholderHint}>Show your circle drawing clearly</Text>
              </View>
            )}
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#D97706']} style={styles.primaryBtn}>
                <Ionicons name="pencil" size={20} color={T.ink} />
                <Text style={styles.primaryBtnTextDark}>I Drew a Circle!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  circleWord: { fontWeight: '900', color: T.accent, fontSize: 16 },
  listenBtn: {
    width: 40,
    height: 40,
    borderRadius: 20,
  ...
  stepDotActive: { backgroundColor: 'rgba(56, 189, 248, 0.35)', borderColor: T.accentSoft },
