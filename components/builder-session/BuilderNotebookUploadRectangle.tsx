/**
 * Builder Session 9 — Rectangle Scout Dispatch
 * Show a RECTANGLE object, upload photo → AI verifies.
 */
...
import { BUILDER_SESSION, RECT_SCOUT_THEME as T } from './builderSessionTheme';
...
export function BuilderNotebookUploadRectangle({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakBuilderHint(
      'Rectangle Scout mission! Find something rectangular — like a book, door, or phone — then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Frame is scanning for a rectangle…');
    setMsgTone('info');
    setActiveStep(3);

    try {
      const formData = new FormData();
      const filename = 'rectangle-object.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-builder-s9-rectangle-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see a rectangle!' : 'Try again — show a clear rectangular shape.');
      ...
      if (correct) {
        ...
        speakBuilderHint('Success! We see a rectangle!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakBuilderHint('Try again. Find something longer than it is tall and take a clear photo.');
        try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your rectangle object.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakBuilderHint('Great! Hold up a rectangle object, then take or upload a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Rectangle Scout Complete!"
          subtitle="You found a rectangle in the real world!"
          badgeEmoji="▭"
          variant="indigo"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Find a <Text style={styles.shapeWord}>rectangle</Text> — like a book, door, or phone — then snap a photo.
            </Text>
          </View>
          <Pressable
            onPress={() => speakBuilderHint('Find something rectangular and take or upload a photo.')}
        ...
        <View style={styles.targetShowcase}>
          <Animated.View style={[styles.rectGlow, pulseStyle]} pointerEvents="none" />
          <Text style={styles.showcaseLabel}>Your target shape</Text>
          <View style={styles.rectDemo} />
          <Text style={styles.showcaseHint}>Something rectangular</Text>
        </View>
          ...
          <View style={styles.frame}>
            {photo ? (
              ...
              <View style={styles.placeholder}>
                ...
                <Text style={styles.placeholderHint}>Show a rectangle object clearly</Text>
              </View>
            )}
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#D97706']} style={styles.primaryBtn}>
                <Ionicons name="tablet-landscape-outline" size={20} color={T.ink} />
                <Text style={styles.primaryBtnTextDark}>I Found a Rectangle!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  stepDotActive: { backgroundColor: 'rgba(165, 180, 252, 0.35)', borderColor: T.accentSoft },
