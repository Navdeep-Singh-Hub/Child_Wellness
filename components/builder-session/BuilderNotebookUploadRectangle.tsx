/**
 * Builder Session 9 — Real World Task: Show a RECTANGLE object.
 * Upload photo → AI verifies rectangle shape.
 */
...
const INSTRUCTIONS = 'Show a RECTANGLE object.\n\nTake a photo or upload a picture of something rectangular.';
...
export function BuilderNotebookUploadRectangle({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see a rectangle!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see a rectangle!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Show something rectangular.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show a rectangle object. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
