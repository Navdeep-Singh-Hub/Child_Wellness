/**
 * Builder Session 5 — Real World Task: Show THREE fingers.
 * Upload/take photo → AI checks hand gesture (3 fingers).
 */
...
const INSTRUCTIONS = 'Show THREE fingers.\n\nTake a photo or upload a picture of your hand showing 3 fingers.';
...
export function BuilderNotebookUploadThreeFingers({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see three fingers!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see three fingers!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Show three fingers.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show three fingers. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
