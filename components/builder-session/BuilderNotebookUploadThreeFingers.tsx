/**
 * Builder Session 5 — Triple Finger Dispatch
 * Show THREE fingers, upload photo → AI verifies hand gesture.
 */
...
import { BUILDER_SESSION, FINGER_SCOUT_THEME as T } from './builderSessionTheme';
...
export function BuilderNotebookUploadThreeFingers({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakBuilderHint(
      'Triple Finger mission! Hold up your hand showing three fingers, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(async (uri: string) => {
    setUploading(true);
    setMsg('Palm is counting your fingers…');
    setMsgTone('info');
    setActiveStep(3);

    try {
      const formData = new FormData();
      const filename = 'three-fingers.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-builder-s5-three-fingers-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see three fingers!' : 'Try again — show exactly three fingers.');
      ...
      if (correct) {
        ...
        speakBuilderHint('Success! We see three fingers!');
        setCelebrating(true);
        ...
        setMsgTone('error');
        setActiveStep(2);
        speakBuilderHint('Try again. Hold up three fingers clearly and take a photo.');
        try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your hand.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakBuilderHint('Great! Hold up three fingers, then take or upload a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Finger Dispatch Complete!"
          subtitle="You showed three fingers in the real world!"
          badgeEmoji="✋"
          variant="sunset"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Show <Text style={styles.countWord}>3 fingers</Text> on your hand, then snap a photo.
            </Text>
          </View>
          <Pressable
            onPress={() => speakBuilderHint('Hold up three fingers and take or upload a photo.')}
        ...
        <View style={styles.targetShowcase}>
          <Animated.View style={[styles.handGlow, pulseStyle]} pointerEvents="none" />
          <Text style={styles.showcaseLabel}>Your target gesture</Text>
          <Text style={styles.showcaseHand}>✋</Text>
          <Text style={styles.showcaseCount}>3 fingers</Text>
        </View>
          ...
          <View style={styles.frame}>
            {photo ? (
              ...
              <View style={styles.placeholder}>
                ...
                <Text style={styles.placeholderHint}>Show three fingers clearly</Text>
              </View>
            )}
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[T.goldSoft, T.gold, '#D97706']} style={styles.primaryBtn}>
                <Ionicons name="hand-left" size={20} color={T.ink} />
                <Text style={styles.primaryBtnTextDark}>I Show 3 Fingers!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  countWord: { fontWeight: '900', color: T.accent, fontSize: 16 },
  listenBtn: {
    width: 40,
    height: 40,
    borderRadius: 20,
  ...
  stepDotActive: { backgroundColor: 'rgba(251, 146, 60, 0.35)', borderColor: T.accentSoft },
  ...
  showcaseHand: { fontSize: 52, marginVertical: 4 },
  showcaseCount: { fontSize: 18, fontWeight: '900', color: T.accent },
