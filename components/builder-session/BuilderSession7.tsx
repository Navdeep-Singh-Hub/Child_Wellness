/**
 * Level 3 Builder — Session 7: Size, Numbers & Patterns
 * Flow: Intro → Game 1 (Size Sorting) → Game 2 (Number Recognition) → Game 3 (Pattern Drag) → Game 4 (Shape Builder) → Real World Task (Square) → Result
 */
...
import { PatternDragGame } from './PatternDragGame';
import { ShapeBuilderGame } from './ShapeBuilderGame';
import { BuilderNotebookUploadSquare } from './BuilderNotebookUploadSquare';
...
interface BuilderSession7Props {
  onExit?: () => void;
}

export function BuilderSession7({ onExit }: BuilderSession7Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '📏', title: 'Size Sorting', desc: 'Sort objects by size' },
      { icon: '5️⃣', title: 'Number Recognition', desc: 'Tap number 5' },
      { icon: '🔁', title: 'Pattern Drag', desc: 'Complete visual pattern' },
      { icon: '⬜', title: 'Shape Builder', desc: 'Build square from sticks' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Size, Numbers & Patterns</Text>
            <Text style={styles.introSub}>Session 7 · The Builder</Text>
            <Text style={styles.introDesc}>Play 4 games, then show a square object!</Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Shape Master</Text>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Size, Numbers & Patterns</Text>
        <Text style={styles.subtitle}>Session 7 · The Builder</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <SizeSortingGame onComplete={advance} />}
        {step === 2 && <NumberRecognitionGame onComplete={advance} />}
        {step === 3 && <PatternDragGame onComplete={advance} />}
        {step === 4 && <ShapeBuilderGame onComplete={advance} />}
        {step === 5 && <BuilderNotebookUploadSquare onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
