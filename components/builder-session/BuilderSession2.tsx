/**
 * Level 3 Builder — Session 2: Word Builder & More
 * Flow: Intro → Game 1 (Word Builder) → Game 2 (Picture Match) → Game 3 (Sorting) → Game 4 (Shape Puzzle) → Real World Task (Draw Circle) → Result
 */
...
import { BuilderNotebookUploadCircle } from './BuilderNotebookUploadCircle';
...
interface BuilderSession2Props {
  onExit?: () => void;
}

export function BuilderSession2({ onExit }: BuilderSession2Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔤', title: 'Word Builder', desc: 'Spell CAT' },
      { icon: '🍎', title: 'Picture Match', desc: 'Tap the APPLE' },
      { icon: '🧺', title: 'Sorting Game', desc: 'Fruits and toys' },
      { icon: '⬜', title: 'Shape Puzzle', desc: 'Complete the square' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Word Builder & More</Text>
            <Text style={styles.introSub}>Session 2 · The Builder</Text>
            <Text style={styles.introDesc}>Play 4 games, then draw a circle!</Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Word Builder</Text>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Word Builder & More</Text>
        <Text style={styles.subtitle}>Session 2 · The Builder</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <WordBuilderGame onComplete={advance} />}
        {step === 2 && <PictureMatchGame onComplete={advance} />}
        {step === 3 && <SortingGame onComplete={advance} />}
        {step === 4 && <ShapePuzzleGame onComplete={advance} />}
        {step === 5 && <BuilderNotebookUploadCircle onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
