/**
 * Level 3 Builder — Session 3: Memory & Match
 * Flow: Intro → Game 1 (Memory Card Flip) → Game 2 (Big vs Small) → Game 3 (Build DOG) → Game 4 (Animal Sound Match) → Real World Task (3 objects) → Result
 */
...
import { BigVsSmallGame } from './BigVsSmallGame';
import { DragLettersGame } from './DragLettersGame';
import { AnimalSoundMatchGame } from './AnimalSoundMatchGame';
import { BuilderNotebookUploadThreeObjects } from './BuilderNotebookUploadThreeObjects';
...
interface BuilderSession3Props {
  onExit?: () => void;
}

export function BuilderSession3({ onExit }: BuilderSession3Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🃏', title: 'Memory Card Flip', desc: 'Match identical cards' },
      { icon: '📏', title: 'Big vs Small', desc: 'Tap the bigger object' },
      { icon: '🐕', title: 'Build DOG', desc: 'Spell DOG with letters' },
      { icon: '🔊', title: 'Animal Match', desc: 'Match animal with sound' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Memory & Match</Text>
            <Text style={styles.introSub}>Session 3 · The Builder</Text>
            <Text style={styles.introDesc}>Play 4 games, then place 3 objects on a table!</Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Memory Master</Text>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Memory & Match</Text>
        <Text style={styles.subtitle}>Session 3 · The Builder</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <MemoryCardGame onComplete={advance} />}
        {step === 2 && <BigVsSmallGame onComplete={advance} />}
        {step === 3 && <DragLettersGame onComplete={advance} />}
        {step === 4 && <AnimalSoundMatchGame onComplete={advance} />}
        {step === 5 && <BuilderNotebookUploadThreeObjects onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
