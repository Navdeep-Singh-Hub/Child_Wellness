 ...
 * Hub → 4 games + triple table task → completion
 */
...
const PLAY_STEPS = 5;
const SESSION_TITLE = 'Session 3 · Memory & Match';
...
export function BuilderSession3({ onExit }: BuilderSession3Props = {}) {
        ...
        <SafeAreaView style={styles.hubSafe}>
          ...
          <ScrollView contentContainerStyle={styles.hubScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.hubHeader}>
              <Text style={styles.hubEyebrow}>Builder · Session 3</Text>
              <Text style={styles.hubTitle}>{H.name}</Text>
              <Text style={styles.hubSession}>Memory & Match</Text>
              ...
              <View style={styles.speechBubble}>
                ...
                <View style={styles.bubbleBody}>
                  ...
                  <Text style={styles.hubPrompt}>
                    Remember, compare, spell, and match — then tally three objects on a real table!
                  </Text>
                </View>
              </View>
            </View>

            {BUILDER_S3_QUESTS.map((quest) => {
      ...
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle={`${SESSION_TITLE}\nCompleted ${doneCount}/${PLAY_STEPS} builds${
            taskCorrect === false ? '\nTriple table: try again next time!' : ''
          }`}
          badgeEmoji="🧠"
          variant="indigo"
        />
  ...
  if (step === 5) {
    return (
      <BuilderNotebookUploadThreeObjects
        sessionTitle={SESSION_TITLE}
        currentStep={5}
        totalSteps={PLAY_STEPS}
        onBack={() => setStep(0)}
