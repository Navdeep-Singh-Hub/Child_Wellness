 ...
 * Hub → 4 games + finger dispatch task → completion
 */
...
const PLAY_STEPS = 5;
const SESSION_TITLE = 'Session 5 · Trace, Count & Sort';
...
export function BuilderSession5({ onExit }: BuilderSession5Props = {}) {
        ...
        <SafeAreaView style={styles.hubSafe}>
          ...
          <ScrollView contentContainerStyle={styles.hubScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.hubHeader}>
              <Text style={styles.hubEyebrow}>Builder · Session 5</Text>
              <Text style={styles.hubTitle}>{H.name}</Text>
              <Text style={styles.hubSession}>Trace, Count & Sort</Text>
              ...
              <View style={styles.speechBubble}>
                ...
                <View style={styles.bubbleBody}>
                  ...
                  <Text style={styles.hubPrompt}>
                    Trace, tally, count, and sort — then finish with a three-finger photo mission!
                  </Text>
                </View>
              </View>
            </View>

            {BUILDER_S5_QUESTS.map((quest) => {
      ...
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle={`${SESSION_TITLE}\nCompleted ${doneCount}/${PLAY_STEPS} builds${
            taskCorrect === false ? '\nFinger dispatch: try again next time!' : ''
          }`}
          badgeEmoji="✏️"
          variant="indigo"
        />
  ...
  if (step === 5) {
    return (
      <BuilderNotebookUploadThreeFingers
        sessionTitle={SESSION_TITLE}
        currentStep={5}
        totalSteps={PLAY_STEPS}
        onBack={() => setStep(0)}
