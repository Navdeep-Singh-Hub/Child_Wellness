/**
 * Level 3 Builder — Session 5: Trace, Count & Sort
 * Flow: Intro → Game 1 (Trace Shape) → Game 2 (Count Objects) → Game 3 (Drag Numbers) → Game 4 (Picture Category) → Real World Task (Show 3 Fingers) → Result
 */
...
import { TraceShapeGame } from './TraceShapeGame';
import { CountObjectsGame } from './CountObjectsGame';
import { DragNumbersGame } from './DragNumbersGame';
import { PictureCategoryGame } from './PictureCategoryGame';
import { BuilderNotebookUploadThreeFingers } from './BuilderNotebookUploadThreeFingers';
...
interface BuilderSession5Props {
  onExit?: () => void;
}

export function BuilderSession5({ onExit }: BuilderSession5Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔺', title: 'Trace the Shape', desc: 'Trace a triangle' },
      { icon: '🍎', title: 'Count Objects', desc: 'Count apples' },
      { icon: '🔢', title: 'Arrange Numbers', desc: '1, 2, 3' },
      { icon: '🧺', title: 'Picture Category', desc: 'Animals and fruits' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Trace, Count & Sort</Text>
            <Text style={styles.introSub}>Session 5 · The Builder</Text>
            <Text style={styles.introDesc}>Play 4 games, then show three fingers!</Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Trace & Count Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <Text style={[styles.title, { color: DESIGN.primary }]}>Trace, Count & Sort</Text>
        <Text style={styles.subtitle}>Session 5 · The Builder</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <TraceShapeGame onComplete={advance} />}
        {step === 2 && <CountObjectsGame onComplete={advance} />}
        {step === 3 && <DragNumbersGame onComplete={advance} />}
        {step === 4 && <PictureCategoryGame onComplete={advance} />}
        {step === 5 && <BuilderNotebookUploadThreeFingers onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
