/**
 * Mixed Word Notebook — Write one word from each family (-at, -in, -un). Grouper Session 4.
 * AI checks: at_word_present, in_word_present, un_word_present.
 */
...
const INSTRUCTIONS = 'Write one word from each family: -at, -in, -un. Examples: cat, pin, sun.';
...
export function MixedWordNotebookUpload({
  ...
  const [result, setResult] = useState<{
    correct: boolean;
    feedback: string;
    at_word_present?: boolean;
    in_word_present?: boolean;
    un_word_present?: boolean;
  } | null>(null);
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const atPresent = data.at_word_present ?? true;
      const inPresent = data.in_word_present ?? true;
      const unPresent = data.un_word_present ?? true;
      const correct = data.correct ?? (atPresent && inPresent && unPresent);
      const feedback = data.feedback || (correct ? 'Great job!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        at_word_present: data.at_word_present,
        in_word_present: data.in_word_present,
        un_word_present: data.un_word_present,
      });
      if (correct) {
        speak('Great job!');
  ...
  return (
    <GameLayout
      ...
      instruction="Write one word from each family. Then upload or take a photo."
    >
      <View style={styles.content}>
        <Text style={styles.instructions}>{INSTRUCTIONS}</Text>
        <View style={styles.exampleRow}>
          <Text style={styles.exampleLabel}>Examples: </Text>
          <Text style={styles.exampleWords}>-at: cat · -in: pin · -un: sun</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  exampleRow: { flexDirection: 'row', flexWrap: 'wrap', marginBottom: 28, alignItems: 'center', justifyContent: 'center' },
  exampleLabel: { fontSize: 18, color: '#4b5563' },
  exampleWords: { fontSize: 18, fontWeight: '800', color: '#4F46E5', marginLeft: 4 },
