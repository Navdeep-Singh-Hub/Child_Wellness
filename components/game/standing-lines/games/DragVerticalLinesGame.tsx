// Game 4: Drag the Vertical Lines
...
interface DragVerticalLinesGameProps {
...
export default function DragVerticalLinesGame({ onComplete, onBack }: DragVerticalLinesGameProps) {
  ...
  useEffect(() => {
    ...
    setDraggedId(null);
    speakFeedback('Drag the standing lines into the box.');
    return () => stopAllAudio();
  }, [round]);
    ...
    onPanResponderRelease: () => {
      ...
      if (line) {
        ...
        if (inBox && isVertical) {
          ...
          speakFeedback('Great! That is a standing line!');
        } else if (inBox && !isVertical) {
          playSoundEffect('incorrect');
          speakFeedback('That is not a standing line. Try again!');
          // Move line back
          setLines((prev) =>
            prev.map((l) =>
              l.id === draggedId
      ...
      // Check if all vertical lines are in box
      const verticalLines = lines.filter((l) => l.type === 'vertical');
      const verticalInBox = lines.filter((l) => l.type === 'vertical' && l.inBox);
      if (verticalInBox.length === verticalLines.length && verticalLines.length > 0) {
        setTimeout(() => {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Drag the Vertical Lines</Text>
        <Text style={styles.roundText}>Round {round + 1}/{TOTAL_ROUNDS}</Text>
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Drag standing lines into the box</Text>
      </View>
      ...
      <View style={styles.gameArea} {...panResponder.panHandlers}>
          ...
          {lines.map((line) => {
            ...
            return (
              <Line
                ...
                x2={endX}
                y2={endY}
                stroke={isVertical ? '#3B82F6' : '#9CA3AF'}
                strokeWidth="6"
                strokeLinecap="round"
                opacity={line.inBox && isVertical ? 0.5 : 1}
              />
            );
          })}
        ...
        <View style={[styles.boxLabel, { left: BOX_X, top: BOX_Y - 30 }]}>
          <Text style={styles.boxLabelText}>Standing Lines</Text>
        </View>
      </View>
