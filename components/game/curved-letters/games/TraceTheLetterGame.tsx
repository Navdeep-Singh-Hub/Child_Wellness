...
interface TraceTheLetterGameProps {
  onComplete: (stats: { correct: number; total: number; accuracy: number; gameId: string }) => void;
  onBack: () => void;
}

export default function TraceTheLetterGame({ onComplete, onBack }: TraceTheLetterGameProps) {
  const { width, height } = useWindowDimensions();
  const CANVAS_WIDTH = width * 0.9;
  const CANVAS_HEIGHT = height * 0.5;
  const TOLERANCE = 50; // Larger tolerance for curves
  ...
  useEffect(() => {
    ...
    setAccuracy(0);
    speakFeedback('Trace the letter C.');
    return () => stopAllAudio();
  }, [round]);
  ...
  const calculateAccuracy = (path: Point[]): number => {
    ...
    let correctPoints = 0;
    const targetPath: Point[] = LETTER_C.dots;
  ...
  const scaledDots = LETTER_C.dots.map((dot) => ({
