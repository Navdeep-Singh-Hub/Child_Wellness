...
interface TraceTheNumberGameProps {
  onComplete: (stats: { correct: number; total: number; accuracy: number; gameId: string }) => void;
  onBack: () => void;
}

export default function TraceTheNumberGame({ onComplete, onBack }: TraceTheNumberGameProps) {
  ...
  useEffect(() => {
    setCurrentNumber(NUMBERS[round % NUMBERS.length]);
    setTracedPath([]);
    setRoundComplete(false);
    setAccuracy(0);
    speakFeedback(`Trace the number ${currentNumber.number}.`);
    return () => stopAllAudio();
  }, [round]);
  ...
  const calculateAccuracy = (path: Point[]): number => {
    ...
    let correctPoints = 0;
    const targetPath: Point[] = currentNumber.dots;
      ...
      onPanResponderRelease: () => {
        ...
        if (!roundComplete && tracedPath.length > 0) {
          ...
          if (finalAcc >= 80) {
            ...
            speakFeedback('Try to trace closer to the number!');
          }
        }
      },
    })
  ...
  const scaledDots = currentNumber.dots.map((dot) => ({
