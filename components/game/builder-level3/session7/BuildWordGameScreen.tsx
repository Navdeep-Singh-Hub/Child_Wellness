// Game 3: Build the Word - Drag and drop letters for CUP
...
export default function BuildWordGameScreen({ onComplete, onBack }: BuildWordGameScreenProps) {
  ...
  const SLOT_START_X = (width - (SLOT_SPACING * (WORD_CUP.letters.length - 1) + SLOT_WIDTH)) / 2;

  const [letters, setLetters] = useState<LetterState[]>(
    WORD_CUP.letters.map((letter, idx) => ({
  ...
  useEffect(() => {
    speakInstruction('Drag the letters to spell CUP.').catch(() => {});
    return () => stopAllAudio();
  }, []);
      ...
      setLetters((prev) =>
        prev.map((l) => {
          if (l.id === draggedId) {
            ...
            for (let i = 0; i < WORD_CUP.letters.length; i++) {
    ...
    onPanResponderRelease: () => {
      ...
      if (letter && letter.slotIndex !== null) {
        ...
        setLetters((prev) => {
          ...
          if (sortedSlots.length === WORD_CUP.letters.length) {
            const isCorrect = sortedSlots.every((l, idx) => l.letter === WORD_CUP.letters[idx]);
            if (isCorrect) {
              setCompleted(true);
              setTimeout(async () => {
                await playSoundEffect('celebration');
                await speakWord('CUP');
                await speakFeedback('Great! You spelled CUP!');
                setTimeout(() => {
                  onComplete();
                }, 2000);
              }, 100);
        ...
        const originalIndex = WORD_CUP.letters.findIndex((l, idx) => 
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Drag letters to spell CUP</Text>
      </View>
