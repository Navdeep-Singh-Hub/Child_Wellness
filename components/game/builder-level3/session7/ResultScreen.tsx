// Result Screen - Show detection results for Session 7
...
interface ResultScreenProps {
  result: {
    cupWordDetected: boolean;
    heartShapeDetected: boolean;
    mirrorSymmetryDetected: boolean;
  };
  onRetry: () => void;
  onComplete: () => void;
}
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  const allDetected =
    result.cupWordDetected &&
    result.heartShapeDetected &&
    result.mirrorSymmetryDetected;
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            <Ionicons
              name={result.cupWordDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.cupWordDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Word CUP {result.cupWordDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.heartShapeDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.heartShapeDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Heart shape {result.heartShapeDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.mirrorSymmetryDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.mirrorSymmetryDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Mirror symmetry {result.mirrorSymmetryDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
        </Animated.View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You built the word CUP and practiced mirror symmetry! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.cupWordDetected && 'We couldn\'t find the word CUP. '}
                {!result.heartShapeDetected && 'We couldn\'t find a heart shape. '}
                {!result.mirrorSymmetryDetected && 'We couldn\'t find a symmetrical heart. '}
