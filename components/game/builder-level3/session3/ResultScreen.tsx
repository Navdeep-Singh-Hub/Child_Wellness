// Result Screen - Show detection results for Session 3
...
interface ResultScreenProps {
  result: {
    dogWordDetected: boolean;
    triangleDetected: boolean;
    triangleEdgesDetected: boolean;
    dogDrawingDetected: boolean;
  };
  onRetry: () => void;
  onComplete: () => void;
}
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  const allDetected =
    result.dogWordDetected &&
    result.triangleDetected &&
    result.dogDrawingDetected;
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            ...
            />
            <Text style={styles.resultText}>
              Triangle shape {result.triangleDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          {result.triangleEdgesDetected && (
            <View style={styles.resultItem}>
              <Ionicons name="checkmark-circle" size={32} color="#10B981" />
              <Text style={styles.resultText}>Triangle edges detected ✓</Text>
            </View>
          )}
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You built the word DOG, drew a triangle, and drew a dog! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.dogWordDetected && 'We couldn\'t find the word DOG. '}
                {!result.triangleDetected && 'We couldn\'t find the triangle shape. '}
                {!result.dogDrawingDetected && 'We couldn\'t find the dog drawing. '}
