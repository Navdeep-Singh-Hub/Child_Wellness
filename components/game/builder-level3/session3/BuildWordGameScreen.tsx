// Game 3: Build the Word - Drag and drop letters for DOG
...
export default function BuildWordGameScreen({ onComplete, onBack }: BuildWordGameScreenProps) {
  ...
  useEffect(() => {
    speakInstruction('Drag the letters to spell DOG.').catch(() => {});
    return () => stopAllAudio();
  }, []);
          ...
          if (sortedSlots.length === WORD_DOG.letters.length) {
            ...
            if (isCorrect) {
              ...
              setTimeout(async () => {
                ...
                await speakWord('DOG');
                await speakFeedback('Great! You spelled DOG!');
                setTimeout(() => {
                  onComplete();
                }, 2000);
              }, 100);
