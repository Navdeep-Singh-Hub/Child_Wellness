// Result Screen - Show detection results for Session 8
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            <Ionicons
              name={result.catDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.catDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Word CAT {result.catDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.dogDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.dogDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Word DOG {result.dogDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.batDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.batDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Word BAT {result.batDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You practiced your CVC words and shapes! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.catDetected && 'We couldn\'t find the word CAT. '}
                {!result.dogDetected && 'We couldn\'t find the word DOG. '}
                {!result.batDetected && 'We couldn\'t find the word BAT. '}
