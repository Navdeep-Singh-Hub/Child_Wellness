// Game 3: Build the Word - Drag and drop letters for BAT
...
export default function BuildWordGameScreen({ onComplete, onBack }: BuildWordGameScreenProps) {
  ...
  const SLOT_START_X = (width - (SLOT_SPACING * (WORD_BAT.letters.length - 1) + SLOT_WIDTH)) / 2;
  ...
  useEffect(() => {
    speakInstruction('Drag the letters to spell BAT.').catch(() => {});
    return () => stopAllAudio();
  }, []);
      ...
      setLetters((prev) =>
        prev.map((l) => {
          if (l.id === draggedId) {
            ...
            for (let i = 0; i < WORD_BAT.letters.length; i++) {
    ...
    onPanResponderRelease: () => {
      ...
      if (letter && letter.slotIndex !== null) {
        ...
        setLetters((prev) => {
          ...
          if (sortedSlots.length === WORD_BAT.letters.length) {
            const isCorrect = sortedSlots.every((l, idx) => l.letter === WORD_BAT.letters[idx]);
            if (isCorrect) {
              setCompleted(true);
              setTimeout(async () => {
                await playSoundEffect('celebration');
                await speakWord('BAT');
                await speakFeedback('Great! You spelled BAT!');
                setTimeout(() => {
                  onComplete();
                }, 2000);
              }, 100);
        ...
        const originalIndex = WORD_BAT.letters.findIndex((l, idx) => 
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Drag letters to spell BAT</Text>
      </View>
