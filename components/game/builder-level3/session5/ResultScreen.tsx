// Result Screen - Show detection results for Session 5
...
interface ResultScreenProps {
  result: {
    sunWordDetected: boolean;
    ovalDetected: boolean;
    ovalCurveDetected: boolean;
    sunDrawingDetected: boolean;
  };
  onRetry: () => void;
  onComplete: () => void;
}
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  const allDetected =
    result.sunWordDetected &&
    result.ovalDetected &&
    result.sunDrawingDetected;
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            ...
            />
            <Text style={styles.resultText}>
              Oval shape {result.ovalDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          {result.ovalCurveDetected && (
            <View style={styles.resultItem}>
              <Ionicons name="checkmark-circle" size={32} color="#10B981" />
              <Text style={styles.resultText}>Oval curve detected ✓</Text>
            </View>
          )}
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You built the word SUN, drew an oval, and drew the sun! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.sunWordDetected && 'We couldn\'t find the word SUN. '}
                {!result.ovalDetected && 'We couldn\'t find the oval shape. '}
                {!result.sunDrawingDetected && 'We couldn\'t find the sun drawing. '}
