// Game 4: Shape Match - Tap oval objects
...
import { OVAL_OBJECTS } from './gameData';
...
export default function ShapeMatchGameScreen({ onComplete, onBack }: ShapeMatchGameScreenProps) {
  ...
  const objectScales = OVAL_OBJECTS.map(() => useSharedValue(1));

  useEffect(() => {
    speakInstruction('Tap the oval shapes.').catch(() => {});
    return () => stopAllAudio();
  }, []);

  const handleObjectTap = async (objectId: string, isOval: boolean) => {
    if (completed) return;

    const index = OVAL_OBJECTS.findIndex(o => o.id === objectId);
    ...
    if (isOval) {
      ...
      await speakFeedback('Great! That is an oval!');

      // Check if all ovals are selected
      const allOvals = OVAL_OBJECTS.filter(o => o.isOval);
      if (allOvals.every(oval => newSelected.has(oval.id))) {
        setTimeout(() => {
          setCompleted(true);
          playSoundEffect('celebration');
          speakFeedback('Excellent! You found all the ovals!');
          setTimeout(() => {
            onComplete();
          }, 2000);
        }, 500);
      ...
      await speakFeedback('That is not an oval. Try again!');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.objectsContainer}>
        {OVAL_OBJECTS.map((obj, index) => {
          const isSelected = selectedObjects.has(obj.id);
          const isCorrect = obj.isOval;
          ...
          return (
            ...
            >
              <Pressable
                onPress={() => handleObjectTap(obj.id, obj.isOval)}
