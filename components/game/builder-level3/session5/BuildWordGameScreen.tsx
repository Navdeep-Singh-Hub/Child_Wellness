// Game 3: Build the Word - Drag and drop letters for SUN
...
export default function BuildWordGameScreen({ onComplete, onBack }: BuildWordGameScreenProps) {
  ...
  const SLOT_START_X = (width - (SLOT_SPACING * (WORD_SUN.letters.length - 1) + SLOT_WIDTH)) / 2;

  const [letters, setLetters] = useState<LetterState[]>(
    WORD_SUN.letters.map((letter, idx) => ({
  ...
  useEffect(() => {
    speakInstruction('Drag the letters to spell SUN.').catch(() => {});
    return () => stopAllAudio();
  }, []);
      ...
      setLetters((prev) =>
        prev.map((l) => {
          if (l.id === draggedId) {
            ...
            for (let i = 0; i < WORD_SUN.letters.length; i++) {
    ...
    onPanResponderRelease: () => {
      ...
      if (letter && letter.slotIndex !== null) {
        ...
        setLetters((prev) => {
          ...
          if (sortedSlots.length === WORD_SUN.letters.length) {
            const isCorrect = sortedSlots.every((l, idx) => l.letter === WORD_SUN.letters[idx]);
            if (isCorrect) {
              setCompleted(true);
              setTimeout(async () => {
                await playSoundEffect('celebration');
                await speakWord('SUN');
                await speakFeedback('Great! You spelled SUN!');
                setTimeout(() => {
                  onComplete();
                }, 2000);
              }, 100);
        ...
        const originalIndex = WORD_SUN.letters.findIndex((l, idx) => 
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Drag letters to spell SUN</Text>
      </View>
