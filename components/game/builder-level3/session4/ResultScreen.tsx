// Result Screen - Show detection results for Session 4
...
interface ResultScreenProps {
  result: {
    hatWordDetected: boolean;
    rectangleDetected: boolean;
    rectangleProportionValid: boolean;
    hatDrawingDetected: boolean;
  };
  onRetry: () => void;
  onComplete: () => void;
}
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  const allDetected =
    result.hatWordDetected &&
    result.rectangleDetected &&
    result.hatDrawingDetected;
  ...
  useEffect(() => {
    ...
    if (allDetected) {
      speakFeedback('Great job! You completed all tasks!').catch(() => {});
    } else {
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInUp.delay(200)} style={styles.titleContainer}>
          ...
          <Text style={styles.subtitle}>Session 4 Complete</Text>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            <Ionicons
              name={result.hatWordDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.hatWordDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Word HAT {result.hatWordDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.rectangleDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.rectangleDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Rectangle shape {result.rectangleDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          {result.rectangleProportionValid && (
            <View style={styles.resultItem}>
              <Ionicons name="checkmark-circle" size={32} color="#10B981" />
              <Text style={styles.resultText}>Rectangle proportions valid ✓</Text>
            </View>
          )}

          <View style={styles.resultItem}>
            <Ionicons
              name={result.hatDrawingDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.hatDrawingDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Hat drawing {result.hatDrawingDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
        </Animated.View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You built the word HAT, drew a rectangle, and drew a hat! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.hatWordDetected && 'We couldn\'t find the word HAT. '}
                {!result.rectangleDetected && 'We couldn\'t find the rectangle shape. '}
                {!result.hatDrawingDetected && 'We couldn\'t find the hat drawing. '}
