// Game 3: Build the Word - Drag and drop letters for HAT
...
export default function BuildWordGameScreen({ onComplete, onBack }: BuildWordGameScreenProps) {
  ...
  const SLOT_START_X = (width - (SLOT_SPACING * (WORD_HAT.letters.length - 1) + SLOT_WIDTH)) / 2;

  const [letters, setLetters] = useState<LetterState[]>(
    WORD_HAT.letters.map((letter, idx) => ({
  ...
  useEffect(() => {
    speakInstruction('Drag the letters to spell HAT.').catch(() => {});
    return () => stopAllAudio();
  }, []);
      ...
      setLetters((prev) =>
        prev.map((l) => {
          if (l.id === draggedId) {
            ...
            for (let i = 0; i < WORD_HAT.letters.length; i++) {
    ...
    onPanResponderRelease: () => {
      ...
      if (letter && letter.slotIndex !== null) {
        ...
        setLetters((prev) => {
          ...
          if (sortedSlots.length === WORD_HAT.letters.length) {
            const isCorrect = sortedSlots.every((l, idx) => l.letter === WORD_HAT.letters[idx]);
            if (isCorrect) {
              setCompleted(true);
              setTimeout(async () => {
                await playSoundEffect('celebration');
                await speakWord('HAT');
                await speakFeedback('Great! You spelled HAT!');
                setTimeout(() => {
                  onComplete();
                }, 2000);
              }, 100);
        ...
        const originalIndex = WORD_HAT.letters.findIndex((l, idx) => 
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Drag letters to spell HAT</Text>
      </View>
