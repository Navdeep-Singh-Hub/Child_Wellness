// Game 2: Choose the Word - Multiple choice for HAT
...
import { WORD_OPTIONS_SESSION4 } from './gameData';
...
export default function ChooseWordGameScreen({ onComplete, onBack }: ChooseWordGameScreenProps) {
  ...
  const buttonScales = WORD_OPTIONS_SESSION4.map(() => useSharedValue(1));
  const animationScale = useSharedValue(0);
  const animationOpacity = useSharedValue(0);

  useEffect(() => {
    speakInstruction('Tap the word HAT').catch(() => {});
    return () => {
      stopAllAudio();
    };
  }, []);
  ...
  const handleWordSelect = async (word: string) => {
    ...
    const index = WORD_OPTIONS_SESSION4.findIndex(w => w.word === word);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the word HAT</Text>
      </View>

      {/* Word Buttons */}
      <View style={styles.wordsContainer}>
        {WORD_OPTIONS_SESSION4.map((option, index) => {
