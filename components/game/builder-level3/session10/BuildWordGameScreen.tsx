// Game 3: Build Random CVC Word - Arrange scrambled letters
...
export default function BuildWordGameScreen({ onComplete, onBack }: BuildWordGameScreenProps) {
  ...
  useEffect(() => {
    if (currentWord) {
      ...
      speakInstruction(`Arrange the letters to form ${currentWord.word}`).catch(() => {});
    }
    return () => stopAllAudio();
  }, [currentWordIndex]);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Arrange letters to form {currentWord.word}</Text>
      </View>
