// Game 1: Word Recognition - Tap the word you hear
...
import { CHALLENGE_WORDS } from './gameData';
...
export default function WordRecognitionGameScreen({ onComplete, onBack }: WordRecognitionGameScreenProps) {
  ...
  const buttonScales = CHALLENGE_WORDS.map(() => useSharedValue(1));
  const resultScale = useSharedValue(0);

  useEffect(() => {
    if (currentWordIndex < CHALLENGE_WORDS.length) {
      const currentWord = CHALLENGE_WORDS[currentWordIndex];
  ...
  const handleWordSelect = async (word: string) => {
    ...
    const index = CHALLENGE_WORDS.findIndex(w => w.word === word);
    ...
    if (isCorrect) {
      ...
      setCorrectCount((prev) => {
        ...
        if (newCount >= CHALLENGE_WORDS.length) {
          setTimeout(() => {
            setCompleted(true);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.progressContainer}>
        <Text style={styles.progressText}>
          Word {currentWordIndex + 1} of {CHALLENGE_WORDS.length}
        </Text>
      </View>
      ...
      <View style={styles.wordsContainer}>
        {CHALLENGE_WORDS.map((word, index) => {
