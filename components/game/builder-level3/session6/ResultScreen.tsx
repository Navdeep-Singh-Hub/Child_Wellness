// Result Screen - Show detection results for Session 6
...
interface ResultScreenProps {
  result: {
    penWordDetected: boolean;
    centerLineDetected: boolean;
    symmetricalShapesDetected: boolean;
  };
  onRetry: () => void;
  onComplete: () => void;
}
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  const allDetected =
    result.penWordDetected &&
    result.centerLineDetected &&
    result.symmetricalShapesDetected;
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            <Ionicons
              name={result.penWordDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.penWordDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Word PEN {result.penWordDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.centerLineDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.centerLineDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Center line {result.centerLineDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.symmetricalShapesDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.symmetricalShapesDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Symmetrical shapes {result.symmetricalShapesDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
        </Animated.View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You built the word PEN and learned about symmetry! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.penWordDetected && 'We couldn\'t find the word PEN. '}
                {!result.centerLineDetected && 'We couldn\'t find the center line. '}
                {!result.symmetricalShapesDetected && 'We couldn\'t find matching shapes on both sides. '}
