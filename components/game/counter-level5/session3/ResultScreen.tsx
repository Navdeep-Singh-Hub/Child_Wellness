// Result Screen - Show detection results for Level 5 Session 3
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  useEffect(() => {
    ...
    if (allDetected) {
      speakFeedback('Great job! You learned the word IS and solved addition!').catch(() => {});
    } else {
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            <Ionicons
              name={result.isWordDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.isWordDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Word IS {result.isWordDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
          ...
          <View style={styles.resultItem}>
            ...
            />
            <Text style={styles.resultText}>
              Addition 2+2=4 {result.additionCorrect ? 'correct ✓' : 'not found or incorrect'}
            </Text>
          </View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You learned the word IS and solved addition! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.isWordDetected && 'We couldn\'t find the word IS. '}
                {!result.additionCorrect && 'We couldn\'t find 2+2=4 solved correctly. '}
