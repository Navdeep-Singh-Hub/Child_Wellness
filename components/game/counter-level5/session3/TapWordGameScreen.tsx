// Game 2: Tap the Word - Tap the word "IS"
...
import { WORD_OPTIONS_SESSION3 } from './gameData';

interface TapWordGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}

const CORRECT_WORD = 'IS';

export default function TapWordGameScreen({ onComplete, onBack }: TapWordGameScreenProps) {
  const [selectedWord, setSelectedWord] = useState<string | null>(null);
  const [isCorrect, setIsCorrect] = useState<boolean | null>(null);
  const [showAnimation, setShowAnimation] = useState(false);
  const [completed, setCompleted] = useState(false);

  const buttonScales = WORD_OPTIONS_SESSION3.map(() => useSharedValue(1));
  const animationScale = useSharedValue(0);
  const animationOpacity = useSharedValue(0);

  useEffect(() => {
    speakInstruction('Tap the word IS').then(() => {
      speakWord('IS');
    });
    return () => {
      stopAllAudio();
  ...
  const handleWordSelect = async (word: string) => {
    ...
    const index = WORD_OPTIONS_SESSION3.findIndex(w => w.word === word);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Tap the Word</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the word IS</Text>
      </View>

      {/* Word Buttons */}
      <View style={styles.wordsContainer}>
        {WORD_OPTIONS_SESSION3.map((option, index) => {
