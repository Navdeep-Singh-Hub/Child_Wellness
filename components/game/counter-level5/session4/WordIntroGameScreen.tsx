// Game 1: Sight Word Introduction - Word "TO"
...
import { SIGHT_WORD_TO } from './gameData';
...
export default function WordIntroGameScreen({ onNext, onBack }: WordIntroGameScreenProps) {
  ...
  useEffect(() => {
    speakInstruction('This word is TO').then(() => {
      speakWord('TO').then(() => {
        ...
        setTimeout(() => {
          ...
          speakInstruction('Go to school').then(() => {
            setTimeout(() => {
              setShowImage(true);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.wordContainer}>
        <Animated.View style={[styles.wordBox, wordAnimatedStyle]}>
          <Text style={styles.wordText}>{SIGHT_WORD_TO.word}</Text>
        </Animated.View>
      </View>
      ...
      {showSentence && (
        <Animated.View style={[styles.sentenceContainer, sentenceAnimatedStyle]} entering={FadeIn.delay(200)}>
          <Text style={styles.sentenceText}>{SIGHT_WORD_TO.sentence}</Text>
        </Animated.View>
      )}
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.imageWrapper}>
            <Text style={styles.schoolEmoji}>{SIGHT_WORD_TO.emoji}</Text>
          </View>
        </Animated.View>
      )}
