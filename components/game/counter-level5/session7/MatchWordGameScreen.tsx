// Game 2: Match the Word - Tap the word "SEE"
...
import { WORD_OPTIONS_SESSION7 } from './gameData';

interface MatchWordGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}

const CORRECT_WORD = 'SEE';

export default function MatchWordGameScreen({ onComplete, onBack }: MatchWordGameScreenProps) {
  const [selectedWord, setSelectedWord] = useState<string | null>(null);
  const [isCorrect, setIsCorrect] = useState<boolean | null>(null);
  const [showAnimation, setShowAnimation] = useState(false);
  const [completed, setCompleted] = useState(false);

  const buttonScales = WORD_OPTIONS_SESSION7.map(() => useSharedValue(1));
  const animationScale = useSharedValue(0);
  const animationOpacity = useSharedValue(0);

  useEffect(() => {
    speakInstruction('Tap the word SEE').then(() => {
      speakWord('SEE');
    });
    return () => {
      stopAllAudio();
  ...
  const handleWordSelect = async (word: string) => {
    ...
    const index = WORD_OPTIONS_SESSION7.findIndex(w => w.word === word);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Match the Word</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the word SEE</Text>
      </View>

      {/* Word Buttons */}
      <View style={styles.wordsContainer}>
        {WORD_OPTIONS_SESSION7.map((option, index) => {
