// Game 4: Number Sorting - Drag number to match objects (6 stars)
...
import { NUMBER_OPTIONS_SESSION8 } from './gameData';

interface NumberSortingGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}
...
export default function NumberSortingGameScreen({ onComplete, onBack }: NumberSortingGameScreenProps) {
  ...
  const [tiles, setTiles] = useState<NumberTileState[]>(
    NUMBER_OPTIONS_SESSION8.map((num, index) => ({
      id: `tile-${num}`,
      number: num,
      x: width * 0.15 + (index * (width * 0.7) / NUMBER_OPTIONS_SESSION8.length),
      y: TILE_START_Y,
      isDragging: false,
      matched: false,
    }))
    ...
    onPanResponderRelease: async () => {
      ...
      if (isPointInMatchBox(draggedTile.x, draggedTile.y)) {
        ...
        if (isCorrect) {
          ...
          await speakFeedback('Perfect! Six stars match the number 6!');
          ...
          const originalIndex = NUMBER_OPTIONS_SESSION8.indexOf(draggedTile.number);
          const originalX = width * 0.15 + (originalIndex * (width * 0.7) / NUMBER_OPTIONS_SESSION8.length);
          setTiles((prev) =>
            prev.map((t) =>
              t.id === draggedId
                ? {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Number Sorting</Text>
        <View style={{ width: 40 }} />
      </View>
