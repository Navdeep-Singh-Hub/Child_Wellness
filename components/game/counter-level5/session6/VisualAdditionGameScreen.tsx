// Game 3: Visual Addition - Count stars (4+1)
...
import { ADDITION_4_PLUS_1 } from './gameData';
...
export default function VisualAdditionGameScreen({ onComplete, onBack }: VisualAdditionGameScreenProps) {
  ...
  useEffect(() => {
    speakInstruction('How many stars?').catch(() => {});
    return () => stopAllAudio();
  }, []);
  ...
  const handleAnswerSelect = async (answer: number) => {
    ...
    if (isCorrect) {
      ...
      await speakFeedback('Perfect! 4 plus 1 equals 5!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>How many stars?</Text>
      </View>

      {/* Equation Display */}
      <View style={styles.equationContainer}>
        <View style={styles.starGroup}>
          <Animated.View style={starAnimatedStyle} entering={FadeInDown.delay(200)}>
            <View style={styles.starBox}>
              <Text style={styles.starEmoji}>{ADDITION_4_PLUS_1.objects[0]}</Text>
            </View>
          </Animated.View>
          <Animated.View style={starAnimatedStyle} entering={FadeInDown.delay(300)}>
            <View style={styles.starBox}>
              <Text style={styles.starEmoji}>{ADDITION_4_PLUS_1.objects[1]}</Text>
            </View>
          </Animated.View>
          <Animated.View style={starAnimatedStyle} entering={FadeInDown.delay(400)}>
            <View style={styles.starBox}>
              <Text style={styles.starEmoji}>{ADDITION_4_PLUS_1.objects[2]}</Text>
            </View>
          </Animated.View>
          <Animated.View style={starAnimatedStyle} entering={FadeInDown.delay(500)}>
            <View style={styles.starBox}>
              <Text style={styles.starEmoji}>{ADDITION_4_PLUS_1.objects[3]}</Text>
            </View>
          </Animated.View>
        </View>
        <Text style={styles.plusSign}>+</Text>
        <View style={styles.starGroup}>
          <Animated.View style={starAnimatedStyle} entering={FadeInDown.delay(600)}>
            <View style={styles.starBox}>
              <Text style={styles.starEmoji}>{ADDITION_4_PLUS_1.objects[4]}</Text>
            </View>
          </Animated.View>
        </View>
