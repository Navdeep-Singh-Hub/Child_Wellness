// Game 4: Addition Puzzle - Drag number to answer box (4+1=?)
...
import { ADDITION_4_PLUS_1, NUMBER_OPTIONS_SESSION6 } from './gameData';
...
export default function AdditionPuzzleGameScreen({ onComplete, onBack }: AdditionPuzzleGameScreenProps) {
  ...
  const [tiles, setTiles] = useState<NumberTileState[]>(
    NUMBER_OPTIONS_SESSION6.map((num, index) => ({
      id: `tile-${num}`,
      number: num,
      x: width * 0.2 + (index * (width * 0.6) / NUMBER_OPTIONS_SESSION6.length),
      y: TILE_START_Y,
      isDragging: false,
      inAnswerBox: false,
    }))
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderRelease: async () => {
      ...
      if (isPointInAnswerBox(draggedTile.x, draggedTile.y)) {
        const isCorrect = draggedTile.number === ADDITION_4_PLUS_1.answer;
        ...
        if (isCorrect) {
          ...
          const originalIndex = NUMBER_OPTIONS_SESSION6.indexOf(draggedTile.number);
          const originalX = width * 0.2 + (originalIndex * (width * 0.6) / NUMBER_OPTIONS_SESSION6.length);
          setTiles((prev) =>
            prev.map((t) =>
              t.id === draggedId
                ? {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.equationContainer}>
        <Text style={styles.equationText}>4 + 1 = ?</Text>
      </View>
      ...
      {completed && (
        <Animated.View entering={FadeInDown.delay(200)} style={styles.successContainer}>
          <Text style={styles.successText}>Perfect! 4 + 1 = 5! 🎉</Text>
        </Animated.View>
      )}
    </SafeAreaView>
  );
