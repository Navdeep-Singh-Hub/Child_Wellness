// Game 4: Addition Puzzle - Drag number to answer box (2+1=?)
...
import { ADDITION_2_PLUS_1, NUMBER_OPTIONS } from './gameData';

interface AdditionPuzzleGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}
...
export default function AdditionPuzzleGameScreen({ onComplete, onBack }: AdditionPuzzleGameScreenProps) {
  ...
  const [tiles, setTiles] = useState<NumberTileState[]>(
    NUMBER_OPTIONS.map((num, index) => ({
      id: `tile-${num}`,
      number: num,
      x: width * 0.2 + (index * (width * 0.6) / NUMBER_OPTIONS.length),
      y: TILE_START_Y,
      isDragging: false,
      inAnswerBox: false,
    }))
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderRelease: async () => {
      ...
      if (isPointInAnswerBox(draggedTile.x, draggedTile.y)) {
        const isCorrect = draggedTile.number === ADDITION_2_PLUS_1.answer;
        ...
        if (isCorrect) {
          ...
          await speakFeedback('Try again! That\'s not the right answer.');
          
          const originalIndex = NUMBER_OPTIONS.indexOf(draggedTile.number);
          const originalX = width * 0.2 + (originalIndex * (width * 0.6) / NUMBER_OPTIONS.length);
          setTiles((prev) =>
            prev.map((t) =>
              t.id === draggedId
                ? {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Addition Puzzle</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Drag the correct number</Text>
        <Text style={styles.equationText}>2 + 1 = ?</Text>
      </View>
      ...
      <View style={styles.answerBoxContainer} {...panResponder.panHandlers}>
        ...
        >
          <Text style={styles.answerBoxLabel}>?</Text>
        </View>
      </View>
      ...
      {completed && (
        <Animated.View entering={FadeInDown.delay(200)} style={styles.successContainer}>
          <Text style={styles.successText}>2 + 1 = 3! 🎉</Text>
        </Animated.View>
      )}
    </SafeAreaView>
  );
