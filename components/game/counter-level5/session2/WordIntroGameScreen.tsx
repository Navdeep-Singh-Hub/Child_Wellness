// Game 1: Sight Word Introduction - Word "THE"
...
import { SIGHT_WORD_THE } from './gameData';
...
export default function WordIntroGameScreen({ onNext, onBack }: WordIntroGameScreenProps) {
  ...
  useEffect(() => {
    speakInstruction('This word is THE').then(() => {
      speakWord('THE').then(() => {
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.wordContainer}>
        <Animated.View style={[styles.wordBox, wordAnimatedStyle]}>
          <Text style={styles.wordText}>{SIGHT_WORD_THE.word}</Text>
        </Animated.View>
      </View>
      ...
      {showSentence && (
        <Animated.View style={[styles.sentenceContainer, sentenceAnimatedStyle]} entering={FadeIn.delay(200)}>
          <Text style={styles.sentenceText}>{SIGHT_WORD_THE.sentence}</Text>
        </Animated.View>
      )}
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.imageWrapper}>
            <Text style={styles.dogEmoji}>{SIGHT_WORD_THE.emoji}</Text>
          </View>
        </Animated.View>
      )}
