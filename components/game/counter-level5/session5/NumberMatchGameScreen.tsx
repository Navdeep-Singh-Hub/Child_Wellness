// Game 4: Number Match - Drag number to match objects (5 stars)
...
import { NUMBER_OPTIONS_SESSION5 } from './gameData';
...
export default function NumberMatchGameScreen({ onComplete, onBack }: NumberMatchGameScreenProps) {
  ...
  const [tiles, setTiles] = useState<NumberTileState[]>(
    NUMBER_OPTIONS_SESSION5.map((num, index) => ({
      id: `tile-${num}`,
      number: num,
      x: width * 0.15 + (index * (width * 0.7) / NUMBER_OPTIONS_SESSION5.length),
      y: TILE_START_Y,
      isDragging: false,
      matched: false,
    }))
  ...
  useEffect(() => {
    speakInstruction('Drag the correct number').catch(() => {});
    return () => stopAllAudio();
  }, []);
    ...
    onPanResponderRelease: async () => {
      ...
      if (isPointInMatchBox(draggedTile.x, draggedTile.y)) {
        ...
        if (isCorrect) {
          ...
          await speakFeedback('Perfect! Five stars match the number 5!');
          ...
          const originalIndex = NUMBER_OPTIONS_SESSION5.indexOf(draggedTile.number);
          const originalX = width * 0.15 + (originalIndex * (width * 0.7) / NUMBER_OPTIONS_SESSION5.length);
          setTiles((prev) =>
            prev.map((t) =>
              t.id === draggedId
                ? {
