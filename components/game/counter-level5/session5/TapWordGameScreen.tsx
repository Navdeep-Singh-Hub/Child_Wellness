// Game 2: Tap the Word - Tap the word "GO"
...
import { WORD_OPTIONS_SESSION5 } from './gameData';
...
export default function TapWordGameScreen({ onComplete, onBack }: TapWordGameScreenProps) {
  ...
  const buttonScales = WORD_OPTIONS_SESSION5.map(() => useSharedValue(1));
  const animationScale = useSharedValue(0);
  const animationOpacity = useSharedValue(0);

  useEffect(() => {
    speakInstruction('Tap the word GO').then(() => {
      speakWord('GO');
    });
    return () => {
      stopAllAudio();
  ...
  const handleWordSelect = async (word: string) => {
    ...
    const index = WORD_OPTIONS_SESSION5.findIndex(w => w.word === word);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the word GO</Text>
      </View>

      {/* Word Buttons */}
      <View style={styles.wordsContainer}>
        {WORD_OPTIONS_SESSION5.map((option, index) => {
