// Result Screen - Show detection results for Level 5 Session 1
...
interface ResultScreenProps {
  result: {
    wordADetected: boolean;
    additionCorrect: boolean;
    twoApplesDetected: boolean;
  };
  onRetry: () => void;
  onComplete: () => void;
}
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  const allDetected =
    result.wordADetected &&
    result.additionCorrect &&
    result.twoApplesDetected;
  ...
  useEffect(() => {
    ...
    if (allDetected) {
      speakFeedback('Great job! You learned the word A and solved addition!').catch(() => {});
    } else {
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInUp.delay(200)} style={styles.titleContainer}>
          ...
          <Text style={styles.subtitle}>Session 1 Complete</Text>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            <Ionicons
              name={result.wordADetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.wordADetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Word A {result.wordADetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
          ...
          <View style={styles.resultItem}>
            ...
            />
            <Text style={styles.resultText}>
              Addition 1+1=2 {result.additionCorrect ? 'correct ✓' : 'not found or incorrect'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.twoApplesDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.twoApplesDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Two apples {result.twoApplesDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
        </Animated.View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You learned the word A and solved addition! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.wordADetected && 'We couldn\'t find the word A. '}
                {!result.additionCorrect && 'We couldn\'t find 1+1=2 solved correctly. '}
                {!result.twoApplesDetected && 'We couldn\'t find two apples. '}
