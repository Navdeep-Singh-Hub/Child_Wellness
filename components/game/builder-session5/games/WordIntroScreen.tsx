...
import { WORD_SUN } from '../utils/gameData';
...
export default function WordIntroScreen({ onComplete, onBack }: WordIntroScreenProps) {
  ...
  useEffect(() => {
    if (hasStarted && currentLetterIndex < WORD_SUN.letters.length) {
      letterScale.value = withSequence(
        withTiming(0, { duration: 0 }),
        withSpring(1, { damping: 10, stiffness: 100 }),
        withTiming(1, { duration: 1000 })
      );
      speakLetter(WORD_SUN.letters[currentLetterIndex]);
      playSoundEffect('letter');
      
      const timer = setTimeout(() => {
        if (currentLetterIndex < WORD_SUN.letters.length - 1) {
          setCurrentLetterIndex((prev) => prev + 1);
        } else {
          setTimeout(() => {
            speakWord(WORD_SUN.word);
            setShowImage(true);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {/* Word Display */}
      {hasStarted && currentLetterIndex === WORD_SUN.letters.length - 1 && (
        <View style={styles.wordContainer}>
          <Text style={styles.wordText}>{WORD_SUN.word}</Text>
        </View>
      )}
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]}>
          <Text style={styles.imageEmoji}>{WORD_SUN.image}</Text>
          <Text style={styles.imageText}>Sun</Text>
        </Animated.View>
      )}
