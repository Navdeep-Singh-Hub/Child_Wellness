...
import { WORD_OPTIONS, WORD_SUN } from '../utils/gameData';
...
export default function ChooseWordGameScreen({ onComplete, onBack }: ChooseWordGameScreenProps) {
  ...
  useEffect(() => {
    ...
    speakInstruction('Tap the word SUN.');
    return () => stopAllAudio();
  }, [round]);
  ...
  const handleSelect = (word: string) => {
    ...
    if (correct) {
      ...
      speakFeedback('Correct! That spells SUN!');
    } else {
      playSoundEffect('incorrect');
      speakFeedback('Try again! Look for the word SUN.');
    }
