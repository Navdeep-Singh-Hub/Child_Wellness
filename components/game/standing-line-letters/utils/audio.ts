// Audio utilities for Capital Letters — Standing Line Letters module
...
const SOUNDS = {
  ...
  tracing: 'https://actions.google.com/sounds/v1/cartoon/slide_whistle.ogg',
};

let soundCache: Map<string, ExpoAudio.Sound> = new Map();

export async function playSoundEffect(type: 'correct' | 'incorrect' | 'celebration' | 'click' | 'tracing'): Promise<void> {
  try {
    ...
    console.warn('[Standing Line Letters Audio] Error playing sound:', error);
  }
}

export async function speakLetter(letter: string): Promise<void> {
  try {
    await speak(`This is the letter ${letter}`, { rate: 0.8 });
  } catch (error) {
    console.warn('[Standing Line Letters Audio] Error speaking letter:', error);
  }
}
...
export async function speakInstruction(instruction: string): Promise<void> {
  try {
    ...
    console.warn('[Standing Line Letters Audio] Error speaking instruction:', error);
  }
}
...
export async function speakFeedback(message: string): Promise<void> {
  try {
    ...
    console.warn('[Standing Line Letters Audio] Error speaking feedback:', error);
  }
}
...
export async function stopAllAudio(): Promise<void> {
  try {
    ...
    console.warn('[Standing Line Letters Audio] Error stopping audio:', error);
  }
}
