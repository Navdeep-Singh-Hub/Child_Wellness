...
import { RECOGNITION_LETTERS, LETTER_I } from '../utils/gameData';

interface TapTheLetterGameProps {
  onComplete: (stats: { correct: number; total: number; accuracy: number; gameId: string }) => void;
  onBack: () => void;
}

export default function TapTheLetterGame({ onComplete, onBack }: TapTheLetterGameProps) {
  ...
  useEffect(() => {
    // Create options: 1 letter I + 3 other letters
    ...
    setSelectedId(null);
    setIsCorrect(null);
    speakInstruction('Tap the letter I.');
    return () => stopAllAudio();
  }, [round]);
  ...
  const handleSelect = (letter: string) => {
    ...
    if (correct) {
      ...
      speakFeedback('Correct! That is the letter I!');
    } else {
      playSoundEffect('incorrect');
      speakFeedback('Try again! Look for the letter I.');
    }
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Tap the Letter</Text>
        <Text style={styles.roundText}>Round {round + 1}/{TOTAL_ROUNDS}</Text>
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the letter I</Text>
      </View>
