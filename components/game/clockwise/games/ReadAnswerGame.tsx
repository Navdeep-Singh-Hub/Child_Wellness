...
import { playSoundEffect, speakSentence, speakQuestion, speakFeedback, stopAllAudio } from '../utils/audio';
import { READING_QUESTIONS, shuffleArray } from '../utils/gameData';

interface ReadAnswerGameProps {
...
export default function ReadAnswerGame({ onComplete, onBack }: ReadAnswerGameProps) {
  const [round, setRound] = useState(0);
  const [score, setScore] = useState(0);
  const [currentQuestion, setCurrentQuestion] = useState<typeof READING_QUESTIONS[0] | null>(null);
  ...
  useEffect(() => {
    ...
    if (currentQuestion) {
      ...
      soundTimeoutRef.current = setTimeout(async () => {
        ...
        await speakSentence(currentQuestion.sentence).catch(() => {});
        await new Promise(resolve => setTimeout(resolve, 800));
        await speakQuestion(currentQuestion.question).catch(() => {});
        soundTimeoutRef.current = null;
      }, 600);
    }
  ...
  const loadRound = () => {
    ...
    const question = READING_QUESTIONS[round % READING_QUESTIONS.length];
  ...
  const handleAnswerSelect = async (answer: string) => {
    if (!canSelect || !currentQuestion) return;
  ...
  const handleReplay = () => {
    if (currentQuestion) {
      ...
      setTimeout(() => speakQuestion(currentQuestion.question), 800);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructions}>
        <Text style={styles.instructionsText}>Read the sentence and answer the question</Text>
      </View>
