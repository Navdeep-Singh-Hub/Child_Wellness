// Game 4: Shape Match - Square Objects
...
import { SQUARE_OBJECTS } from '../utils/gameData';
...
export default function ShapeMatchGameScreen({ onComplete, onBack }: ShapeMatchGameScreenProps) {
  ...
  useEffect(() => {
    ...
    setDraggedId(null);
    speakInstruction('Tap the square objects.');
    return () => stopAllAudio();
  }, [round]);
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderRelease: () => {
      ...
      if (obj) {
        const isCorrect = obj.isSquare && obj.inBox;
        
        if (isCorrect) {
          setScore((prev) => prev + 1);
          playSoundEffect('correct');
          speakFeedback(`Great! ${obj.name} is a square!`);
          ...
          // Check if all squares are in box
          const allSquaresInBox = objects.every((o) => 
            o.id === draggedId || 
            (o.isSquare && o.inBox) ||
            !o.isSquare
          );
          ...
          if (allSquaresInBox) {
            setTimeout(() => {
              if (round < TOTAL_ROUNDS - 1) {
                ...
                const accuracy = (score / (SQUARE_OBJECTS.filter((o) => o.isSquare).length * TOTAL_ROUNDS)) * 100;
                onComplete({
                  correct: score,
                  total: SQUARE_OBJECTS.filter((o) => o.isSquare).length * TOTAL_ROUNDS,
                  accuracy,
                  gameId: 'shape-match',
                });
              }
        ...
        } else if (obj.inBox && !obj.isSquare) {
          playSoundEffect('incorrect');
          speakFeedback(`${obj.name} is not a square. Try again!`);
          
          // Move back
          const originalIndex = SQUARE_OBJECTS.findIndex((o) => o.id === obj.id);
          setObjects((prev) =>
            prev.map((o) =>
              o.id === draggedId
                ? {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.gameArea} {...panResponder.panHandlers}>
        ...
        >
          <Text style={styles.boxLabel}>Square Objects</Text>
          <View style={styles.boxContent}>
            {objects.filter((o) => o.inBox && o.isSquare).map((obj) => (
