...
interface ResultScreenProps {
  ...
  notebookResult: { batWordDetected: boolean; squareDetected: boolean; batDrawingDetected: boolean } | null;
  onBack: () => void;
}
...
export default function ResultScreen({ gamesCompleted, notebookResult, onBack }: ResultScreenProps) {
  ...
  const allNotebookTasks = notebookResult?.batWordDetected && notebookResult?.squareDetected && notebookResult?.batDrawingDetected;
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Animated.View style={[styles.content, animatedStyle]}>
          ...
          <Text style={styles.encouragement}>Great job! You built the word BAT!</Text>
          ...
          {notebookResult && (
            <View style={styles.notebookResults}>
              ...
              <View style={styles.notebookItem}>
                <Ionicons
                  name={notebookResult.batWordDetected ? 'checkmark-circle' : 'close-circle'}
                  size={20}
                  color={notebookResult.batWordDetected ? '#10B981' : '#EF4444'}
                />
                <Text style={styles.notebookText}>Word BAT</Text>
              </View>
              <View style={styles.notebookItem}>
                <Ionicons
                  name={notebookResult.squareDetected ? 'checkmark-circle' : 'close-circle'}
                  size={20}
                  color={notebookResult.squareDetected ? '#10B981' : '#EF4444'}
                />
                <Text style={styles.notebookText}>Square</Text>
              </View>
              <View style={styles.notebookItem}>
                <Ionicons
                  name={notebookResult.batDrawingDetected ? 'checkmark-circle' : 'close-circle'}
                  size={20}
                  color={notebookResult.batDrawingDetected ? '#10B981' : '#EF4444'}
                />
                <Text style={styles.notebookText}>Bat Drawing</Text>
              </View>
            </View>
          )}
