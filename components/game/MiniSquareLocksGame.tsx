...
const SQUARE_SIZE = 20; // Small square
...
const MiniSquareLocksGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const [squarePathStr, setSquarePathStr] = useState('');
  const [progressPathStr, setProgressPathStr] = useState('');

  const pathPoints = useRef<Array<{ x: number; y: number }>>([]);

  const objectX = useSharedValue(40);
  const objectY = useSharedValue(40);
  ...
  const updatePaths = useCallback(() => {
    if (pathPoints.current.length < 4) return;

    const p1 = pathPoints.current[0];
    const p2 = pathPoints.current[1];
    const p3 = pathPoints.current[2];
    const p4 = pathPoints.current[3];
    setSquarePathStr(`M ${p1.x} ${p1.y} L ${p2.x} ${p2.y} L ${p3.x} ${p3.y} L ${p4.x} ${p4.y} Z`);

    if (progress.value > 0) {
      let progressPath = `M ${p1.x} ${p1.y}`;
      const totalSegments = 4;
      const progressPerSegment = 1 / totalSegments;
      
      for (let i = 0; i < totalSegments; i++) {
        const segmentProgress = Math.min(1, (progress.value - i * progressPerSegment) / progressPerSegment);
        if (segmentProgress <= 0) break;
        
        const startPt = pathPoints.current[i];
        const endPt = pathPoints.current[(i + 1) % 4];
        if (segmentProgress >= 1) {
          progressPath += ` L ${endPt.x} ${endPt.y}`;
        } else {
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((e) => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const p2 = pathPoints.current[(i + 1) % 4];
    ...
    .onEnd(() => {
      ...
      if (progress.value >= 0.85) {
        sparkleX.value = pathPoints.current[0].x;
        sparkleY.value = pathPoints.current[0].y;
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Locks Traced!</Text>
            <Text style={styles.resultSubtitle}>
              You traced {finalStats.correct} locks out of {finalStats.total}!
            </Text>
            <ResultCard
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Mini Square Locks</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • 🔒 Score: {score}
        </Text>
        <Text style={styles.helper}>
          Trace the mini square lock with precision!
        </Text>
      </View>
          ...
          <Animated.View style={styles.gestureArea}>
            ...
            <Animated.View style={[styles.objectContainer, objectStyle]}>
              <View
                style={[
                  styles.object,
                  {
                    backgroundColor: isOffTrack ? '#EF4444' : '#6366F1',
                  },
                ]}
              >
            ...
            {isOffTrack && (
              <View style={styles.warningBox}>
                <Text style={styles.warningText}>Stay on the lock! ⚠️</Text>
              </View>
            )}
          </Animated.View>
        </GestureDetector>
      ...
      <View style={styles.footerBox}>
        <Text style={styles.footerMain}>
          Skills: finger control • precision • small shape tracing
        </Text>
        <Text style={styles.footerSub}>
          Use your finger to trace the mini square!
        </Text>
      </View>
    </SafeAreaView>
  );
