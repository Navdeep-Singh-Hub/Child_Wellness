...
type ShapeType = 'triangle' | 'pentagon' | 'hexagon';

const DotBorderShapesGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const [currentShape, setCurrentShape] = useState<ShapeType>('triangle');
  ...
  const generateShape = useCallback((type: ShapeType) => {
    ...
    if (type === 'triangle') {
      points.push({ x: centerX, y: centerY - size / 2 });
  ...
  const updatePaths = useCallback(() => {
    ...
    if (progress.value > 0) {
      ...
      for (let i = 0; i < totalSegments; i++) {
        ...
        const endPt = pathPoints.current[(i + 1) % numPoints];
      ...
      if (progress.value >= 0.85) {
        ...
        setScore(s => {
          ...
          if (newScore >= TOTAL_ROUNDS) {
            ...
            setTimeout(() => {
              setRound(r => r + 1);
              const shapes: ShapeType[] = ['triangle', 'pentagon', 'hexagon'];
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Shapes Traced!</Text>
            <Text style={styles.resultSubtitle}>
              You traced {finalStats.correct} shapes out of {finalStats.total}!
            </Text>
            <ResultCard
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Dot Border Shapes</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • ⚫ Score: {score}
        </Text>
        <Text style={styles.helper}>
          Trace the dotted border shape with precision!
        </Text>
      </View>
