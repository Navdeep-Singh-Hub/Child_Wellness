/**
 * Body Position Planning — OT Level 8 Session 5 shared engine.
 *
 * Each round shows a target body position (reach high / low / side, turn, or a
 * body shape). The child plans the move, gets into the position and holds it to
 * confirm. Detected from the camera (APK + web) with a guided fallback.
 *
 * Modes: highReach · lowReach · sideReach · turnPosition · shapeBody
 */
...
import { BodyPositionOverlay } from '@/components/game/occupational/level8/session5/components/BodyPositionOverlay';
import { evalPosition, type PositionSpec } from '@/components/game/occupational/level8/session5/bodyPosition';
import { BODYPOS_GAME_THEMES, BODYPOS_SHELL, type BodyPositionMode } from '@/components/game/occupational/level8/session5/bodyPositionTheme';
import { SESSION5_PACING as P, SESSION5_THRESHOLDS as TH } from '@/components/game/occupational/level8/session5/session5Pacing';
...
const VOICE_PRAISE = ['Perfect position!', 'You held it!', 'Brilliant!', 'Just right!', 'Super move!'];
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));

type Phase = 'intro' | 'calibrate' | 'play';

export const BodyPositionGame: React.FC<{
  mode: BodyPositionMode;
  onBack?: () => void;
  onComplete?: () => void;
}> = ({ mode, onBack, onComplete }) => {
  const router = useRouter();
  const T = BODYPOS_GAME_THEMES[mode];
  ...
  const [spec, setSpec] = useState<PositionSpec | null>(null);
  ...
  const specRef = useRef<PositionSpec | null>(null);
  const lastSpecIdxRef = useRef(-1);
  ...
  const endGame = useCallback(() => {
    ...
    const posAcc = accCountRef.current > 0 ? accSumRef.current / accCountRef.current : 0;
    const moveQuality = qualCountRef.current > 0 ? qualSumRef.current / qualCountRef.current : 0;
    const headline = Math.round((completion * 0.5 + posAcc * 0.5) * 100);
    const xp = Math.round(completed * 18 + posAcc * 100 * 0.4 + moveQuality * 100 * 0.4);
    ...
    recordGame(xp)
      .then(() =>
        logGameAndAward({
          type: `body-position-${mode}` as any,
          correct: completed,
          total: totalRounds,
          accuracy: headline,
          xpAwarded: xp,
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      let idx = Math.floor(Math.random() * T.positions.length);
      let guard = 0;
      while (idx === lastSpecIdxRef.current && guard++ < 4) idx = Math.floor(Math.random() * T.positions.length);
      lastSpecIdxRef.current = idx;
      const chosen = T.positions[idx]!;
      specRef.current = chosen;
      setSpec(chosen);
      ...
      schedule(() => {
        ...
        setBanner('Hold the position!');
        Haptics.selectionAsync().catch(() => {});
        schedule(() => setBanner(''), 850);
      }, P.planDelayMs);
    },
    [schedule, T.hintText, T.positions, T.voicePlan],
  );
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const prog = clamp01(elapsed / P.fallbackHoldMs);
    ...
    const ev = evalPosition(m, base, s, TH);
    setScore(ev.score);
    setMatched(ev.ok);
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: BODYPOS_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: BODYPOS_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🧭 POSITION QUEST</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: BODYPOS_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: BODYPOS_SHELL.statLabel }]}>Round</Text>
              <Text style={[styles.statValue, { color: BODYPOS_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: BODYPOS_SHELL.statBorder }]}>
              <Text style={styles.coinEmoji}>{T.collectible}</Text>
              <Text style={[styles.statValue, { color: BODYPOS_SHELL.statValue }]}>{coins}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={BODYPOS_SHELL.sparkleColor} count={16} />
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Stand back so the camera sees your whole body — then move into each position!'
                    : 'Guided mode: plan each position and move along!'}
                </Text>
...
export default BodyPositionGame;
