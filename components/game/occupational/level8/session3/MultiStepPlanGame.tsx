/**
 * Multi-Step Sequencing — OT Level 8 Session 3 shared engine.
 *
 * Each round builds an ORDERED CHAIN of 2–5 movement primitives (growing with
 * progress). A PLAN beat previews the whole chain, then the child executes each
 * step in order, detected from the camera (APK + web) with a guided fallback.
 *
 * Modes: actionChain · starSequence · missionSteps · pirateCommands · rainbowSequence
 */
...
import { MultiStepOverlay } from '@/components/game/occupational/level8/session3/components/MultiStepOverlay';
import { RAINBOW_COLORS, SEQ_GAME_THEMES, SEQ_SHELL, type SeqGameTheme, type SeqMode } from '@/components/game/occupational/level8/session3/seqTheme';
import { SESSION3_PACING as P } from '@/components/game/occupational/level8/session3/session3Pacing';
...
const VOICE_PRAISE = ['Perfect order!', 'Great chain!', 'You remembered it all!', 'Brilliant sequence!', 'Awesome!'];
...
/** Build the ordered chain for a round; length grows every 2 rounds. */
function buildSequence(theme: SeqGameTheme, round: number): Step[] {
  const len = Math.min(theme.maxLen, theme.minLen + Math.floor(round / 2));

  if (theme.builder === 'reachSeq') {
    const kind: ActionKind = theme.rainbow ? 'touch' : 'reach';
    const idxs = shuffle(theme.anchors.map((_, i) => i)).slice(0, Math.min(len, theme.anchors.length));
    return idxs.map((ai) =>
      makeStep(kind, theme.anchors[ai], theme.labelMap?.[kind], theme.rainbow ? RAINBOW_COLORS[ai % RAINBOW_COLORS.length] : undefined),
    );
  }

  const pool = theme.pool ?? ['clap', 'jump', 'turn', 'reach'];
  const steps: Step[] = [];
  let anchorCycle = 0;
  let lastKind: ActionKind | null = null;
  for (let i = 0; i < len; i++) {
    let kind = pool[Math.floor(Math.random() * pool.length)]!;
    let guard = 0;
    while (kind === lastKind && guard++ < 4) kind = pool[Math.floor(Math.random() * pool.length)]!;
    lastKind = kind;
    const anchor = isTargeted(kind) ? theme.anchors[anchorCycle++ % theme.anchors.length]! : null;
    steps.push(makeStep(kind, anchor, theme.labelMap?.[kind]));
  }
  return steps;
}

export const MultiStepPlanGame: React.FC<{
  mode: SeqMode;
  onBack?: () => void;
  onComplete?: () => void;
}> = ({ mode, onBack, onComplete }) => {
  const router = useRouter();
  const T = SEQ_GAME_THEMES[mode];
  ...
  const endGame = useCallback(() => {
    ...
    const seqAcc = accCountRef.current > 0 ? accSumRef.current / accCountRef.current : 0;
    const moveQuality = qualCountRef.current > 0 ? qualSumRef.current / qualCountRef.current : 0;
    const headline = Math.round((completion * 0.5 + seqAcc * 0.5) * 100);
    const xp = Math.round(completed * 20 + seqAcc * 100 * 0.4 + moveQuality * 100 * 0.4);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            mode,
            sequenceAccuracy: Math.round(seqAcc * 100),
  ...
  const completeRound = useCallback(() => {
    ...
    recordHold(DEFAULT_THRESHOLDS.reachHoldMs);
  ...
  const onStepDone = useCallback(
    (stepAccuracy: number) => {
      ...
      if (nextIdx >= seqRef.current.length) {
        ...
        roundActiveRef.current = false;
        setRoundActive(false);
        const nextStep = seqRef.current[nextIdx];
        if (nextStep) {
          setBanner(`Next: ${nextStep.label} ${nextStep.icon}`);
          speakTTS(`Now ${nextStep.label}`, 0.9).catch(() => {});
        }
        schedule(() => beginStepAt(nextIdx), P.betweenStepsMs);
      }
    },
    [beginStepAt, completeRound, schedule],
  );
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      setNear(false);
      setHandPos(null);
      const chainText = built.map((s) => s.label).join(' → ');
      setBanner(`Plan: ${chainText}`);
      setCoachCue(T.hintText);
      if (r === 0 || r % 2 === 0) speakTTS(T.voicePlan, 0.85).catch(() => {});

      const planMs = P.planBaseMs + built.length * P.planPerStepMs;
      schedule(() => {
        if (doneRef.current) return;
  ...
  const tick = useCallback(() => {
    ...
    const step = seqRef.current[idxRef.current];
    if (!step) {
    ...
    const ev = evaluateAction(step, m, prev, base, anchor, DEFAULT_THRESHOLDS);
    ...
    if (roundActiveRef.current) {
      if (ev.transient) {
        ...
        setProgress(prog);
        if (prog >= 1) {
          const acc = step.targeted ? clamp01(1 - minDistRef.current / (DEFAULT_THRESHOLDS.reachRadius * 1.6)) : 0.9;
          onStepDone(acc);
        }
      } else {
    ...
    prevMetricsRef.current = m;
  }, [onStepDone, recordPostureBreak, recordTick, T.hintText, T.positionCue]);
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: SEQ_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: SEQ_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🧠 MOTOR PLANNING LAB III</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: SEQ_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: SEQ_SHELL.statLabel }]}>Round</Text>
              <Text style={[styles.statValue, { color: SEQ_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: SEQ_SHELL.statBorder }]}>
              <Text style={styles.coinEmoji}>{T.collectible}</Text>
              <Text style={[styles.statValue, { color: SEQ_SHELL.statValue }]}>{coins}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={SEQ_SHELL.sparkleColor} count={16} />
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Stand back so the camera sees your whole body — clear space to move!'
                    : 'Guided mode: follow the coach and do the whole chain in order!'}
                </Text>
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: '#A7F3D0', fontSize: 12, fontWeight: '900', letterSpacing: 1 },
  ...
  linkText: { color: '#A7F3D0', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default MultiStepPlanGame;
