/**
 * Vestibular Sequencing — OT Level 7 Session 8 shared engine.
 *
 * Drives all five sequencing games as an ordered action sequence (step / steady /
 * turn / stop / go) emphasizing motor planning and movement memory, detected from
 * camera weight shift, body turn and motion (APK + web), with guided fallback.
 *
 * Modes: followMySteps · movementPatternCopy · starSequencePath · spaceMission · pirateJourney
 */
...
import {
  ...
  type PostureMetrics,
  type ShiftDir,
} from '@/components/game/occupational/level7/session8/poseUtils';
import { SESSION8_PACING } from '@/components/game/occupational/level7/session8/session8Pacing';
import { SEQUENCING_GAME_THEMES, SEQUENCING_SHELL, type SequencingMode } from '@/components/game/occupational/level7/session8/sequencingTheme';
...
const VOICE_PRAISE = ['Great move!', 'Nice sequence!', 'Steady!', 'You did it!', 'Keep going!'];

type Phase = 'intro' | 'calibrate' | 'play';

const buildSequence = (mode: SequencingMode): DynamicAction[] => {
  switch (mode) {
    case 'followMySteps':
      return [...FOLLOW_STEPS_SEQ];
    case 'movementPatternCopy':
      return [...PATTERN_COPY_SEQ];
    case 'starSequencePath':
      return [...STAR_SEQUENCE_SEQ];
    case 'spaceMission':
      return [...SPACE_MISSION_SEQ];
    case 'pirateJourney':
      return [...PIRATE_JOURNEY_SEQ];
  }
};
...
export const VestibularSequencingGame: React.FC<{
  mode: SequencingMode;
  onBack?: () => void;
  onComplete?: () => void;
}> = ({ mode, onBack, onComplete }) => {
  const router = useRouter();
  const T = SEQUENCING_GAME_THEMES[mode];
  const showTrail = !!T.showTrail;
  ...
  const endGame = useCallback(() => {
    ...
    recordGame(xp)
      .then(() =>
        logGameAndAward({
          type: `vestibular-sequencing-${mode}` as any,
          correct: finalScore,
          total: totalRounds,
          accuracy,
          xpAwarded: xp,
  ...
  const finishAction = useCallback(
    (earned: boolean) => {
      ...
      phaseGuardComplete(() => {
        ...
        setMatchedNow(false);
        setActionProgress(0);
        setCoachCue(earned ? 'Great move! Next in the sequence!' : 'Good try — follow the next move!');
        advanceRound();
      });
    },
  ...
  const beginCalibration = useCallback(() => {
    ...
    setCoachCue('Stand tall in the middle, facing the camera — whole body in view!');
  ...
  return (
    <LinearGradient colors={SEQUENCING_SHELL.gradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: SEQUENCING_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: SEQUENCING_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🧭 SEQUENCE QUEST</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: SEQUENCING_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: SEQUENCING_SHELL.statLabel }]}>Step</Text>
              <Text style={[styles.statValue, { color: SEQUENCING_SHELL.statValue }]}>
                {round}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: SEQUENCING_SHELL.statBorder }]}>
              <Text style={styles.starEmoji}>⭐</Text>
              <Text style={[styles.statValue, { color: SEQUENCING_SHELL.statValue }]}>{score}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: SEQUENCING_SHELL.statBorder }]}>
              <Text style={styles.coinEmoji}>🪙</Text>
              <Text style={[styles.statValue, { color: SEQUENCING_SHELL.statValue }]}>{coins}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={SEQUENCING_SHELL.sparkleColor} count={14} />
                ...
                <Text style={styles.introText}>
                  {cameraSupported
                    ? hasCamera
                      ...
                      : 'Starting camera…'
                    : 'Guided mode: follow the movement sequence!'}
                </Text>
                <Pressable
                  ...
                  onPress={handleStart}
                >
                  <Text style={styles.primaryBtnText}>{T.hero} Start Sequence</Text>
                </Pressable>
                {cameraSupported && (
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: '#D8B4FE', fontSize: 12, fontWeight: '900', letterSpacing: 1 },
  header: { alignItems: 'center', marginTop: 8 },
  title: { color: '#fff', fontSize: 23, fontWeight: '900', textAlign: 'center' },
  subtitle: { color: '#E9D5FF', fontSize: 14, fontWeight: '600', marginTop: 2, textAlign: 'center' },
  ...
  calibTrack: { width: '100%', height: 12, borderRadius: 6, backgroundColor: 'rgba(30,27,75,0.75)', overflow: 'hidden' },
  ...
  linkText: { color: '#D8B4FE', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default VestibularSequencingGame;
