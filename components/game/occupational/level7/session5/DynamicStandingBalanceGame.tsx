/**
 * Dynamic Standing Balance — OT Level 7 Session 5 shared engine.
 *
 * Drives all five games as an action sequence (step / steady / turn / stop / go),
 * detected from camera weight shift, body turn and motion (APK + web), or guided
 * fallback.
 *
 * Modes: balanceBridge · steppingStones · islandHopper · riverCrossing · starTrail
 */
...
import {
  ...
  type PostureMetrics,
  type ShiftDir,
} from '@/components/game/occupational/level7/session5/poseUtils';
import { SESSION5_PACING } from '@/components/game/occupational/level7/session5/session5Pacing';
import { BALANCE_GAME_THEMES, BALANCE_SHELL, type BalanceMode } from '@/components/game/occupational/level7/session5/dynamicBalanceTheme';
...
const VOICE_PRAISE = ['Great balance!', 'Nice step!', 'Steady!', 'You did it!', 'Keep going!'];

type Phase = 'intro' | 'calibrate' | 'play';

const buildSequence = (mode: BalanceMode): DynamicAction[] => {
  switch (mode) {
    case 'balanceBridge':
      return [...BALANCE_BRIDGE_SEQ];
    case 'steppingStones':
      return [...STEPPING_STONES_SEQ];
    case 'islandHopper':
      return [...ISLAND_HOPPER_SEQ];
    case 'riverCrossing':
      return [...RIVER_CROSSING_SEQ];
    case 'starTrail':
      return [...STAR_TRAIL_SEQ];
  }
};
...
export const DynamicStandingBalanceGame: React.FC<{
  mode: BalanceMode;
  onBack?: () => void;
  onComplete?: () => void;
  ...
  const endGame = useCallback(() => {
    ...
    recordGame(xp)
      .then(() =>
        logGameAndAward({
          type: `dynamic-balance-${mode}` as any,
          correct: finalScore,
          total: totalRounds,
          accuracy,
          xpAwarded: xp,
  ...
  return (
    <LinearGradient colors={BALANCE_SHELL.gradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          ...
          <Text style={styles.academyLabel}>🌊 BALANCE CROSSING</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: BALANCE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: BALANCE_SHELL.statLabel }]}>Step</Text>
            ...
            <View style={[styles.statPill, { borderColor: BALANCE_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: BALANCE_SHELL.statValue }]}>{score}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: BALANCE_SHELL.statBorder }]}>
              <Text style={styles.coinEmoji}>🪙</Text>
              <Text style={[styles.statValue, { color: BALANCE_SHELL.statValue }]}>{coins}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={BALANCE_SHELL.sparkleColor} count={14} />
                ...
                <Text style={styles.introText}>
                  {cameraSupported
                    ? hasCamera
                      ...
                      : 'Starting camera…'
                    : 'Guided mode: follow the cues and move along!'}
                </Text>
                <Pressable
                  ...
                  onPress={handleStart}
                >
                  <Text style={styles.primaryBtnText}>{T.hero} Start Crossing</Text>
                </Pressable>
                {cameraSupported && (
...
const styles = StyleSheet.create({
  ...
  calibTrack: { width: '100%', height: 12, borderRadius: 6, backgroundColor: 'rgba(8,47,73,0.75)', overflow: 'hidden' },
...
export default DynamicStandingBalanceGame;
