/**
 * Vestibular Endurance — OT Level 7 Session 9 shared engine.
 *
 * Drives all five endurance games as a long ordered action sequence (go / steady /
 * step / turn / stop) emphasizing sustained movement and balance over distance,
 * detected from camera weight shift, body turn and motion (APK + web), with
 * guided fallback.
 *
 * Modes: longTrailWalk · rainbowJourney · forestExplorer · endlessTracks · balanceMarathon
 */
...
import {
  ...
  type PostureMetrics,
  type ShiftDir,
} from '@/components/game/occupational/level7/session9/poseUtils';
import { SESSION9_PACING } from '@/components/game/occupational/level7/session9/session9Pacing';
import { ENDURANCE_GAME_THEMES, ENDURANCE_SHELL, type EnduranceMode } from '@/components/game/occupational/level7/session9/enduranceTheme';
...
const VOICE_PRAISE = ['Keep going!', 'Great pace!', 'Steady!', 'You did it!', 'Strong move!'];

type Phase = 'intro' | 'calibrate' | 'play';

const buildSequence = (mode: EnduranceMode): DynamicAction[] => {
  switch (mode) {
    case 'longTrailWalk':
      return [...LONG_TRAIL_SEQ];
    case 'rainbowJourney':
      return [...RAINBOW_JOURNEY_SEQ];
    case 'forestExplorer':
      return [...FOREST_EXPLORER_SEQ];
    case 'endlessTracks':
      return [...ENDLESS_TRACKS_SEQ];
    case 'balanceMarathon':
      return [...BALANCE_MARATHON_SEQ];
  }
};
...
export const VestibularEnduranceGame: React.FC<{
  mode: EnduranceMode;
  onBack?: () => void;
  onComplete?: () => void;
  ...
  const endGame = useCallback(() => {
    ...
    const xp = Math.round(finalScore * 20 + snap.avgPostureQuality * 0.42 + snap.uprightPct * 0.32);
    ...
    recordGame(xp)
      .then(() =>
        logGameAndAward({
          type: `vestibular-endurance-${mode}` as any,
          correct: finalScore,
          total: totalRounds,
          accuracy,
          xpAwarded: xp,
  ...
  const finishAction = useCallback(
    (earned: boolean) => {
      ...
      phaseGuardComplete(() => {
        ...
        setMatchedNow(false);
        setActionProgress(0);
        setCoachCue(earned ? 'Great pace! Keep going!' : 'Good try — keep moving!');
        advanceRound();
      });
    },
  ...
  return (
    <LinearGradient colors={ENDURANCE_SHELL.gradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: ENDURANCE_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: ENDURANCE_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🏞️ ENDURANCE QUEST</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: ENDURANCE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: ENDURANCE_SHELL.statLabel }]}>Step</Text>
              <Text style={[styles.statValue, { color: ENDURANCE_SHELL.statValue }]}>
                {round}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: ENDURANCE_SHELL.statBorder }]}>
              <Text style={styles.starEmoji}>⭐</Text>
              <Text style={[styles.statValue, { color: ENDURANCE_SHELL.statValue }]}>{score}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: ENDURANCE_SHELL.statBorder }]}>
              <Text style={styles.coinEmoji}>🪙</Text>
              <Text style={[styles.statValue, { color: ENDURANCE_SHELL.statValue }]}>{coins}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={ENDURANCE_SHELL.sparkleColor} count={14} />
            ...
            {cameraSupported && error ? (
              ...
              <>
                <Text style={styles.introText}>
                  {cameraSupported
                    ? hasCamera
                      ? 'Stand back so the camera sees your whole body — get ready for a long journey!'
                      : 'Starting camera…'
                    : 'Guided mode: keep moving the whole way!'}
                </Text>
                <Pressable
                  ...
                  onPress={handleStart}
                >
                  <Text style={styles.primaryBtnText}>{T.hero} Start Journey</Text>
                </Pressable>
                {cameraSupported && (
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: '#86EFAC', fontSize: 12, fontWeight: '900', letterSpacing: 1 },
  ...
  linkText: { color: '#86EFAC', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default VestibularEnduranceGame;
