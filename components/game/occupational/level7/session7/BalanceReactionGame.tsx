/**
 * Balance Reactions — OT Level 7 Session 7 shared engine.
 *
 * Drives all five reaction games as an action sequence (go / stop-freeze / step /
 * turn / steady), detected from camera weight shift, body turn and motion
 * (APK + web), with emphasis on fast reactions and balance recovery. Falls back
 * to a guided mode when the camera is unavailable.
 *
 * Modes: freezeBalance · waveRider · surfChallenge · obstacleBalance · recoveryMaster
 */
...
import {
  ...
  type PostureMetrics,
  type ShiftDir,
} from '@/components/game/occupational/level7/session7/poseUtils';
import { SESSION7_PACING } from '@/components/game/occupational/level7/session7/session7Pacing';
import { REACTION_GAME_THEMES, REACTION_SHELL, type ReactionMode } from '@/components/game/occupational/level7/session7/reactionTheme';
...
const VOICE_PRAISE = ['Great reaction!', 'Nice freeze!', 'Steady!', 'You did it!', 'Quick recovery!'];

type Phase = 'intro' | 'calibrate' | 'play';

const buildSequence = (mode: ReactionMode): DynamicAction[] => {
  switch (mode) {
    case 'freezeBalance':
      return [...FREEZE_BALANCE_SEQ];
    case 'waveRider':
      return [...WAVE_RIDER_SEQ];
    case 'surfChallenge':
      return [...SURF_CHALLENGE_SEQ];
    case 'obstacleBalance':
      return [...OBSTACLE_BALANCE_SEQ];
    case 'recoveryMaster':
      return [...RECOVERY_MASTER_SEQ];
  }
};
...
export const BalanceReactionGame: React.FC<{
  mode: ReactionMode;
  onBack?: () => void;
  onComplete?: () => void;
}> = ({ mode, onBack, onComplete }) => {
  const router = useRouter();
  const T = REACTION_GAME_THEMES[mode];
  ...
  const endGame = useCallback(() => {
    ...
    recordGame(xp)
      .then(() =>
        logGameAndAward({
          type: `balance-reaction-${mode}` as any,
          correct: finalScore,
          total: totalRounds,
          accuracy,
          xpAwarded: xp,
  ...
  const finishAction = useCallback(
    (earned: boolean) => {
      ...
      phaseGuardComplete(() => {
        ...
        setMatchedNow(false);
        setActionProgress(0);
        setCoachCue(earned ? 'Great reaction! Keep going!' : 'Good try — react to the next cue!');
        advanceRound();
      });
    },
  ...
  return (
    <LinearGradient colors={REACTION_SHELL.gradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: REACTION_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: REACTION_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>⚡ REACTION ARENA</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: REACTION_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: REACTION_SHELL.statLabel }]}>Round</Text>
              <Text style={[styles.statValue, { color: REACTION_SHELL.statValue }]}>
                {round}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: REACTION_SHELL.statBorder }]}>
              <Text style={styles.starEmoji}>⭐</Text>
              <Text style={[styles.statValue, { color: REACTION_SHELL.statValue }]}>{score}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: REACTION_SHELL.statBorder }]}>
              <Text style={styles.coinEmoji}>🪙</Text>
              <Text style={[styles.statValue, { color: REACTION_SHELL.statValue }]}>{coins}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={REACTION_SHELL.sparkleColor} count={14} />
            ...
            {cameraSupported && error ? (
              ...
              <>
                <Text style={styles.introText}>
                  {cameraSupported
                    ? hasCamera
                      ? 'Stand back so the camera sees your whole body — room to move and react!'
                      : 'Starting camera…'
                    : 'Guided mode: follow the cues and react fast!'}
                </Text>
                <Pressable
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: '#67E8F9', fontSize: 12, fontWeight: '900', letterSpacing: 1 },
...
export default BalanceReactionGame;
