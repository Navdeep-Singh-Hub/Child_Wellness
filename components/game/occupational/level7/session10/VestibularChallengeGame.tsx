/**
 * Vestibular Challenge Course — OT Level 7 Session 10 shared engine (Level 7 finale).
 *
 * Drives all five challenge-course games as an integrated action sequence (go /
 * step / turn / stop / steady) combining every vestibular skill from the level,
 * detected from camera weight shift, body turn and motion (APK + web), with
 * guided fallback.
 *
 * Modes: pirateIsland · spaceExplorer · jungleExpedition · mountainAdventure · vestibularChampion
 */
...
import {
  ...
  EMPTY_METRICS,
  frameMotionFull,
  JUNGLE_EXPEDITION_SEQ,
  MOUNTAIN_ADVENTURE_SEQ,
  PIRATE_ISLAND_SEQ,
  shiftZone,
  SPACE_EXPLORER_SEQ,
  swayStillness,
  turnProxy,
  VESTIBULAR_CHAMPION_SEQ,
  weightBalanceScore,
  weightShift,
  type DynamicAction,
  ...
  type PostureMetrics,
  type ShiftDir,
} from '@/components/game/occupational/level7/session10/poseUtils';
import { SESSION10_PACING } from '@/components/game/occupational/level7/session10/session10Pacing';
import { CHALLENGE_GAME_THEMES, CHALLENGE_SHELL, type ChallengeMode } from '@/components/game/occupational/level7/session10/challengeTheme';
...
const VOICE_PRAISE = ['Champion move!', 'Brilliant!', 'Steady!', 'You did it!', 'Keep going!'];

type Phase = 'intro' | 'calibrate' | 'play';

const buildSequence = (mode: ChallengeMode): DynamicAction[] => {
  switch (mode) {
    case 'pirateIsland':
      return [...PIRATE_ISLAND_SEQ];
    case 'spaceExplorer':
      return [...SPACE_EXPLORER_SEQ];
    case 'jungleExpedition':
      return [...JUNGLE_EXPEDITION_SEQ];
    case 'mountainAdventure':
      return [...MOUNTAIN_ADVENTURE_SEQ];
    case 'vestibularChampion':
      return [...VESTIBULAR_CHAMPION_SEQ];
  }
};
...
export const VestibularChallengeGame: React.FC<{
  mode: ChallengeMode;
  onBack?: () => void;
  onComplete?: () => void;
}> = ({ mode, onBack, onComplete }) => {
  const router = useRouter();
  const T = CHALLENGE_GAME_THEMES[mode];
  ...
  const endGame = useCallback(() => {
    ...
    recordGame(xp)
      .then(() =>
        logGameAndAward({
          type: `vestibular-challenge-${mode}` as any,
          correct: finalScore,
          total: totalRounds,
          accuracy,
          xpAwarded: xp,
  ...
  const finishAction = useCallback(
    (earned: boolean) => {
      ...
      phaseGuardComplete(() => {
        ...
        setMatchedNow(false);
        setActionProgress(0);
        setCoachCue(earned ? 'Champion move! Keep going!' : 'Good try — on to the next obstacle!');
        advanceRound();
      });
    },
  ...
  return (
    <LinearGradient colors={CHALLENGE_SHELL.gradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: CHALLENGE_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: CHALLENGE_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🏆 CHALLENGE COURSE</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: CHALLENGE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: CHALLENGE_SHELL.statLabel }]}>Step</Text>
              <Text style={[styles.statValue, { color: CHALLENGE_SHELL.statValue }]}>
                {round}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: CHALLENGE_SHELL.statBorder }]}>
              <Text style={styles.starEmoji}>⭐</Text>
              <Text style={[styles.statValue, { color: CHALLENGE_SHELL.statValue }]}>{score}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: CHALLENGE_SHELL.statBorder }]}>
              <Text style={styles.coinEmoji}>🪙</Text>
              <Text style={[styles.statValue, { color: CHALLENGE_SHELL.statValue }]}>{coins}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={CHALLENGE_SHELL.sparkleColor} count={14} />
            ...
            {cameraSupported && error ? (
              ...
              <>
                <Text style={styles.introText}>
                  {cameraSupported
                    ? hasCamera
                      ? 'Stand back so the camera sees your whole body — ready for the challenge course!'
                      : 'Starting camera…'
                    : 'Guided mode: complete the obstacle course!'}
                </Text>
                <Pressable
                  ...
                  onPress={handleStart}
                >
                  <Text style={styles.primaryBtnText}>{T.hero} Start Course</Text>
                </Pressable>
                {cameraSupported && (
...
const styles = StyleSheet.create({
  ...
  calibTrack: { width: '100%', height: 12, borderRadius: 6, backgroundColor: 'rgba(12,10,29,0.75)', overflow: 'hidden' },
...
export default VestibularChallengeGame;
