/**
 * Stadium pitch play area for Goal Pass (OT L4 S1 Game 1).
 */
import { GOAL_PASS_THEME as T } from '@/components/game/occupational/level4/session1/goalPass/goalPassTheme';
...
export const BallTransferPlayArea: React.FC<Props> = ({ roundActive, showGuide, isDragging }) => {
  const goalGlow = useSharedValue(0.6);
  const arrowX = useSharedValue(0);
  const spotlight = useSharedValue(0.4);
  ...
  }, [roundActive, goalGlow, spotlight]);
  ...
  const spotlightLeftStyle = useAnimatedStyle(() => ({
    opacity: spotlight.value,
  }));

  const spotlightRightStyle = useAnimatedStyle(() => ({
    opacity: spotlight.value * 0.85,
  }));
  ...
  return (
    <>
      ...
      <Animated.View style={[styles.floodlight, styles.floodLeft, spotlightLeftStyle]} />
      <Animated.View style={[styles.floodlight, styles.floodRight, spotlightRightStyle]} />

      <View style={styles.centerLine} />
      <View style={styles.centerCircle} />

      <View style={[styles.penaltyBox, styles.penaltyLeft]}>
        <Text style={styles.zoneTag}>KICK OFF</Text>
      ...
      <Animated.View style={[styles.passArrow, arrowStyle]}>
        ...
        <Text style={styles.passArrowHint}>Slide to score</Text>
      </Animated.View>
    </>
  );
};
...
const styles = StyleSheet.create({
  ...
  floodLeft: { top: '-8%', left: '-6%' },
  floodRight: { top: '-8%', right: '-6%' },
  centerLine: {
    position: 'absolute',
    left: '50%',
    top: '10%',
