...
const BallTransferGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const ballX = useSharedValue(SCREEN_WIDTH * 0.15);
  ...
  const panGesture = Gesture.Pan().runOnJS(true)
    ...
    .onEnd(() => {
      ...
      const distance = Math.sqrt(
        Math.pow(ballX.value - rightBoxX.value, 2) + Math.pow(ballY.value - rightBoxY.value, 2)
      );
      ...
      if (distance <= TOLERANCE) {
        ...
        speakTTS('Perfect transfer!', 0.9, 'en-US' );
      } else {
        // Reset ball
        resetBall();
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {});
        speakTTS('Drag the ball to the right box!', 0.8, 'en-US' );
      }
    });

  const resetBall = useCallback(() => {
    ballX.value = withSpring(leftBoxX.value);
    ballY.value = withSpring(leftBoxY.value);
  }, [ballX, ballY, leftBoxX, leftBoxY]);
  ...
  useEffect(() => {
    if (!showInfo && !done) {
      resetBall();
      speakTTS('Drag the ball from left to right box!', 0.8, 'en-US' );
    }
  ...
  if (showInfo) {
    return (
      <GameInfoScreen
        ...
        skills={['Midline crossing', 'Brain hemispheres connect']}
        suitableFor="Children learning midline crossing and brain hemisphere connection"
        onStart={() => {
          setShowInfo(false);
        }}
        onBack={() => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>Ball Transfer</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • ⚽ Score: {score}
        </Text>
        <Text style={styles.instruction}>
          Drag the ball from left to right box!
        </Text>
      </View>
      ...
      <View style={styles.footer}>
        ...
        <Text style={styles.footerSubtext}>
          Drag the ball from left side to right box!
        </Text>
      </View>
    </SafeAreaView>
  );
