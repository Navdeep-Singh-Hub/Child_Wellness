...
const ReversePathGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const [pathType, setPathType] = useState<'straight' | 'curve' | 'zigzag'>('straight');
  ...
  const generatePath = useCallback(() => {
    const types: ('straight' | 'curve' | 'zigzag')[] = ['straight', 'curve', 'zigzag'];
  ...
  const panGesture = Gesture.Pan().runOnJS(true)
    ...
    .onEnd(() => {
      ...
      if (distance <= TOLERANCE) {
        ...
        speakTTS('Perfect reverse!', 0.9, 'en-US' );
      } else {
        resetObject();
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {});
        speakTTS('Follow the reverse path!', 0.8, 'en-US' );
      }
    });
  ...
  useEffect(() => {
    if (!showInfo && !done) {
      generatePath();
      resetObject();
      speakTTS('Follow the reverse path from right to left!', 0.8, 'en-US' );
    }
  }, [showInfo, round, done, generatePath, resetObject]);
  ...
  const updatePath = useCallback(() => {
    ...
    switch (pathType) {
      ...
      case 'curve':
        const midX = (sx + ex) / 2;
        const midY = sy - height * 0.15;
        path = `M ${sx} ${sy} Q ${midX} ${midY} ${ex} ${ey}`;
        break;
      case 'zigzag':
        const z1x = (sx + ex) / 2;
        const z1y = sy - height * 0.1;
        const z2x = (sx + ex) / 2;
        const z2y = sy + height * 0.1;
  ...
  if (showInfo) {
    return (
      <GameInfoScreen
        ...
        suitableFor="Children learning cognitive flexibility through reverse path following"
        onStart={() => {
          setShowInfo(false);
        }}
        onBack={() => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>Reverse Path</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • 🔄 Score: {score}
        </Text>
        <Text style={styles.instruction}>
          Follow the reverse path from right to left!
        </Text>
      </View>
      ...
      <View style={styles.footer}>
        ...
        <Text style={styles.footerSubtext}>
          Follow the same path but in reverse direction!
        </Text>
      </View>
    </SafeAreaView>
  );
