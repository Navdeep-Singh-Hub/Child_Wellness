...
> = ({
  ...
  const startRoundPlay = useCallback(() => {
    ...
    setRoundActive(true);
    layoutPositions();
    if (mode === 'colorMatch') setMatchEmoji(randomMatchEmoji());
    setStatusHint('Top-left → bottom-right!');
    speakTTS(ttsDrag, 0.78).catch(() => {});
  }, [layoutPositions, mode, ttsDrag]);
  ...
  const targetEmoji = mode === 'colorMatch' ? matchEmoji : T.targetEmoji;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <GestureDetector gesture={panGesture}>
        ...
        >
          ...
          {roundActive && (
            <>
              <View style={[styles.cornerZone, styles.tlZone, { borderColor: T.zoneBorder }]}>
                <Text style={[styles.zoneLabel, { color: T.accentDark }]}>FROM</Text>
              </View>
              <View style={[styles.cornerZone, styles.brZone, { borderColor: T.zoneBorder }]}>
                <Text style={[styles.zoneLabel, { color: T.accentDark }]}>TO</Text>
                <Text style={styles.zoneEmoji}>{targetEmoji}</Text>
              </View>
              <View style={[styles.diagonalGuide, { backgroundColor: `${T.accent}33` }]} />
            </>
          )}
