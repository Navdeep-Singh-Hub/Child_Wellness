...
const RobotWireFixGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
    ...
    async (finalScore: number) => {
      ...
      try {
        ...
        const result = await logGameAndAward({
          ...
          accuracy,
          xpAwarded: xp,
          skillTags: ['direction-change', 'motor-planning', 'zig-zag-tracking', 'wire-following'],
        });
        setLogTimestamp(result?.last?.at ?? null);
        router.setParams({ refreshStats: Date.now().toString() });
      } catch (e) {
        console.error('Failed to log robot wire fix game:', e);
      }

      speakTTS('Robot wire fixed perfectly!', 0.78 );
    },
    [router],
  );
      ...
      if (distToEnd <= LINE_TOLERANCE && progress.value >= 0.99 && !hasGoneOffTrack && !hasGoneOffTrackRef.current) {
        ...
        try {
          ...
          if (hasGoneOffTrack) {
            ...
            speakTTS('Follow the zig-zag wire!', 0.78 );
          }
        } catch {}
      }
    });
  ...
  useEffect(() => {
    try {
      speakTTS('Follow the zig-zag wire to fix the robot!', 0.78 );
    } catch {}
  ...
  if (showInfo) {
    return (
      <GameInfoScreen
        ...
        skills={['Direction change', 'Motor planning', 'Zig-zag tracking', 'Wire following']}
        suitableFor="Children developing direction change skills and wire following abilities"
        onStart={() => {
          setShowInfo(false);
        }}
        onBack={handleBack}
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Robot Wire Fix Complete!</Text>
            <Text style={styles.resultSubtitle}>
              You fixed {finalStats.correct} robot wires out of {finalStats.total}!
            </Text>
            <ResultCard
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Robot Wire Fix</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • 🤖 Score: {score}
        </Text>
        <Text style={styles.helper}>
          Follow the zig-zag wire to fix the robot. Stay on the path!
        </Text>
      </View>
      ...
      <View style={styles.footerBox}>
        ...
        <Text style={styles.footerSub}>
          Follow the zig-zag wire carefully!
        </Text>
      </View>
    </SafeAreaView>
  );
