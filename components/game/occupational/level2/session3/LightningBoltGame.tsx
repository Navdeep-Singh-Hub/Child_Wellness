...
const LightningBoltGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const endGame = useCallback(
    async (finalScore: number) => {
      ...
      try {
        ...
        console.error('Failed to log lightning bolt game:', e);
      }

      speakTTS('Lightning bolt traced perfectly!', 0.78 );
    },
    [router],
  );
      ...
      if (distToEnd <= LINE_TOLERANCE && progress.value >= 0.99 && !hasGoneOffTrack && !hasGoneOffTrackRef.current) {
        ...
        try {
          ...
          if (hasGoneOffTrack) {
            ...
            speakTTS('Trace the lightning bolt!', 0.78 );
          }
        } catch {}
      }
    });
  ...
  useEffect(() => {
    try {
      speakTTS('Trace the lightning bolt with sharp angles!', 0.78 );
    } catch {}
    // Generate lightning bolt path with sharp angles (more angular zig-zag)
    pathPoints.current = [];
    const startX = 20;
    const startY = 75;
    const endX = 80;
    const endY = 25;
    const numSegments = 7; // More segments for sharper angles
    ...
    for (let i = 1; i < numSegments; i++) {
      ...
      const sharpAngle = (i % 2 === 0 ? 1 : -1) * 12; // Larger angle for sharper turns
      pathPoints.current.push({ x: baseX + sharpAngle, y: baseY });
    }
  ...
  if (showInfo) {
    return (
      <GameInfoScreen
        ...
        skills={['Direction change', 'Motor planning', 'Sharp angles', 'Precise tracking']}
        suitableFor="Children developing direction change skills and sharp angle tracing abilities"
        onStart={() => {
          setShowInfo(false);
        }}
        onBack={handleBack}
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Lightning Bolt Complete!</Text>
            <Text style={styles.resultSubtitle}>
              You traced {finalStats.correct} lightning bolts out of {finalStats.total}!
            </Text>
            <ResultCard
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Lightning Bolt</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • ⚡ Score: {score}
        </Text>
        <Text style={styles.helper}>
          Trace the lightning bolt with sharp angles. Stay on the path!
        </Text>
      </View>
      ...
      <View style={styles.footerBox}>
        <Text style={styles.footerMain}>
          Skills: direction change • motor planning • sharp angles
        </Text>
        <Text style={styles.footerSub}>
          Trace the lightning bolt with sharp angles!
        </Text>
      </View>
    </SafeAreaView>
  );
