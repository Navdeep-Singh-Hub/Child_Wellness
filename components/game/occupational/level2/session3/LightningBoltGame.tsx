...
const LightningBoltGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const [lightningPathStr, setLightningPathStr] = useState('');
  ...
  const objectY = useSharedValue(15);
    ...
    async (finalScore: number) => {
      ...
      try {
        ...
        const result = await logGameAndAward({
          ...
          accuracy,
          xpAwarded: xp,
          skillTags: ['direction-change', 'motor-planning', 'sharp-angles'],
        });
      ...
      Speech.speak('Zap! Lightning traced!', { rate: 0.78 });
    },
    [router],
  );
    ...
    .onEnd(() => {
      ...
      if (distToEnd <= LINE_TOLERANCE && progress.value >= 0.75) {
        ...
        try {
          ...
          Speech.speak('Trace the lightning!', { rate: 0.78 });
        } catch {}
      }
    });
  ...
  useEffect(() => {
    ...
    const startX = 50;
    const startY = 15;
    const endX = 50 + (Math.random() * 20 - 10);
    const endY = 85;
    ...
    for (let i = 1; i < numSegments; i++) {
      ...
      const offsetX = Math.cos((angle * Math.PI) / 180) * (15 + Math.random() * 10);
      const offsetY = Math.sin((angle * Math.PI) / 180) * (15 + Math.random() * 10);
      points.push({
        x: baseX + offsetX,
        y: baseY + offsetY,
      });
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Lightning Traced!</Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        ...
        <Text style={styles.helper}>
          Trace the lightning bolt with sharp angles! Follow the zig-zag path.
        </Text>
      </View>
          ...
          <Animated.View style={styles.gestureArea}>
            ...
            {isOffTrack && (
              <View style={styles.warningBox}>
                <Text style={styles.warningText}>Stay on the bolt! ⚠️</Text>
              </View>
            )}
          </Animated.View>
        </GestureDetector>
      ...
      <View style={styles.footerBox}>
        ...
        <Text style={styles.footerSub}>
          Trace the lightning bolt with sharp direction changes!
        </Text>
      </View>
    </SafeAreaView>
  );
