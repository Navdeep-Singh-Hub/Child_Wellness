...
const ZigZagRaceGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const endGame = useCallback(
    async (finalScore: number) => {
      ...
      speakTTS('Great zig-zag race!', 0.78 );
    },
    [router],
  );
      ...
      if (distToEnd <= LINE_TOLERANCE && progress.value >= 0.99 && !hasGoneOffTrack && !hasGoneOffTrackRef.current) {
        ...
        try {
          ...
          if (hasGoneOffTrack) {
            ...
            speakTTS('Complete the zig-zag race!', 0.78 );
          }
        } catch {}
      }
    });
  ...
  useEffect(() => {
    try {
      speakTTS('Complete the zig-zag race at your own pace!', 0.78 );
    } catch {}
    // Generate zig-zag race path (timed but slow-paced)
    pathPoints.current = [];
    const startX = 20;
    const startY = 70;
    const endX = 80;
  ...
  if (showInfo) {
    return (
      <GameInfoScreen
        ...
        skills={['Direction change', 'Motor planning', 'Zig-zag tracking', 'Timed movement']}
        suitableFor="Children developing direction change skills and timed movement abilities"
        onStart={() => {
          setShowInfo(false);
        }}
        onBack={handleBack}
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Zig-Zag Race Complete!</Text>
            <Text style={styles.resultSubtitle}>
              You completed {finalStats.correct} races out of {finalStats.total}!
            </Text>
            <ResultCard
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        ...
        <Text style={styles.helper}>
          Complete the zig-zag race at your own pace. Stay on the path!
        </Text>
      </View>
      ...
      <View style={styles.footerBox}>
        ...
        <Text style={styles.footerSub}>
          Complete the zig-zag race at your own pace!
        </Text>
      </View>
    </SafeAreaView>
  );
