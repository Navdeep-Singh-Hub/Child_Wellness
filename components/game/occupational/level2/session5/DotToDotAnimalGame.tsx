...
const DotToDotAnimalGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const generateDots = useCallback(() => {
    const numDots = 3 + Math.floor(Math.random() * 2); // 3-4 dots
    const newDots: Dot[] = [];
    
    // Generate dots in a shape pattern (triangle or square)
    if (numDots === 3) {
      // Triangle
      newDots.push({ x: 50, y: 30, number: 1, connected: false });
      newDots.push({ x: 30, y: 70, number: 2, connected: false });
      newDots.push({ x: 70, y: 70, number: 3, connected: false });
    } else {
      // Square
      newDots.push({ x: 40, y: 40, number: 1, connected: false });
      newDots.push({ x: 60, y: 40, number: 2, connected: false });
      newDots.push({ x: 60, y: 60, number: 3, connected: false });
      newDots.push({ x: 40, y: 60, number: 4, connected: false });
    }
    ...
    async (finalScore: number) => {
      ...
      try {
        ...
        const result = await logGameAndAward({
          ...
          accuracy,
          xpAwarded: xp,
          skillTags: ['planning', 'number-sequence', 'dot-connecting'],
        });
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Animal Complete!</Text>
            <Text style={styles.resultSubtitle}>
              You completed {finalStats.correct} animals out of {finalStats.total}!
            </Text>
            <ResultCard
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Dot-to-Dot Animal</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • 🐾 Score: {score}
        </Text>
        <Text style={styles.helper}>
          Connect the dots in order (1, 2, 3...) to reveal the animal!
        </Text>
      </View>
      ...
      >
          ...
          {dots.map((dot) => (
            <React.Fragment key={dot.number}>
              <Circle
                ...
                cy={dot.y}
                r={DOT_RADIUS}
                fill={dot.connected ? '#3B82F6' : '#E5E7EB'}
                stroke={dot.connected ? '#1E40AF' : '#9CA3AF'}
                strokeWidth="2"
              />
              <SvgText
                x={dot.x}
        ...
        {currentDotIndex === 0 && (
          <View style={styles.instructionBox}>
            <Text style={styles.instructionText}>Tap dot 1 to start! 👆</Text>
          </View>
        )}
      </View>
      ...
      <View style={styles.footerBox}>
        <Text style={styles.footerMain}>
          Skills: planning • number sequence • dot connecting
        </Text>
        <Text style={styles.footerSub}>
          Connect dots in number order to complete the shape!
        </Text>
      </View>
    </SafeAreaView>
  );
