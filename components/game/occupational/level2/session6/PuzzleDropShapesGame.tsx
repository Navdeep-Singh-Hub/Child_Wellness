...
const PuzzleDropShapesGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const generateRound = useCallback(() => {
    ...
    const outlineX = 50 + (Math.random() - 0.5) * 30;
    const outlineY = 60 + Math.random() * 20;
  ...
  const renderShape = (type: ShapeType, x: number, y: number, isOutline: boolean = false) => {
    ...
    const opacity = isOutline ? 0.3 : 1;
    const fill = isOutline ? 'none' : '#8B5CF6';
    const stroke = isOutline ? '#6D28D9' : '#7C3AED';
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Puzzle Complete!</Text>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Puzzle Drop – Shapes</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • 🧩 Score: {score}
        </Text>
        <Text style={styles.helper}>
          Drag the shape to match the outline!
        </Text>
      </View>
      ...
      <View style={styles.footerBox}>
        <Text style={styles.footerMain}>
          Skills: visual discrimination • motor accuracy • shape matching
        </Text>
        <Text style={styles.footerSub}>
          Drag the shape into its matching outline!
        </Text>
      </View>
    </SafeAreaView>
  );
