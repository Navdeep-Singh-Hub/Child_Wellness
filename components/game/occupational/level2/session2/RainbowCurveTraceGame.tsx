/**
 * OT Level 2 · Session 2 · Game 2 — Rainbow Curve Trace
 * Theme: "Rainbow Bridge" — trace the rainbow arc.
 */
...
const RainbowCurveTraceGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const r = useSharedValue(30); const sa = useSharedValue(Math.PI * 0.2); const ea = useSharedValue(Math.PI * 0.8);
  ...
  const endGame = useCallback((finalScore: number) => {
    ...
    speakTTS('Rainbow bridge complete!', 0.78);
    recordGame(xp).then(() =>
      logGameAndAward({ type: 'rainbowTrace', correct: finalScore, total, accuracy: (finalScore / total) * 100, xpAwarded: xp,
  ...
  useEffect(() => {
    speakTTS('Trace the rainbow arc from left to right!', 0.78);
  ...
  if (showCongratulations && done && finalStats) {
    return (
      <CongratulationsScreen message="Rainbow Rider!" showButtons correct={finalStats.correct} total={finalStats.total} xpAwarded={finalStats.xp}
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>🌈 Rainbow Bridge</Text>
        <Text style={styles.subtitle}>Trace the arc from left to right</Text>
