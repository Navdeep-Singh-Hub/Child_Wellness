...
const DontTouchGrassGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
    ...
    async (finalScore: number) => {
      ...
      try {
        ...
        const result = await logGameAndAward({
          ...
          accuracy,
          xpAwarded: xp,
          skillTags: ['continuous-tracking', 'sustained-attention', 'boundary-awareness'],
        });
        setLogTimestamp(result?.last?.at ?? null);
        router.setParams({ refreshStats: Date.now().toString() });
      } catch (e) {
        console.error('Failed to log dont touch grass game:', e);
      }

      speakTTS('Perfect! No grass touched!', 0.78 );
    },
    [router],
  );
  ...
  const panGesture = Gesture.Pan().runOnJS(true)
    ...
    .onUpdate((e) => {
      ...
      // If off the road, you're touching grass
    ...
    .onEnd(() => {
      ...
      if (distToEnd <= LINE_TOLERANCE * 2 && progress.value >= 0.75) {
        ...
        try {
          ...
          speakTTS('Stay on the path, don\'t touch the grass!', { rate: 0.78 });
        } catch {}
      }
    });
  ...
  useEffect(() => {
    try {
      speakTTS('Stay on the path! Don\'t touch the grass on either side.', 0.78 );
    } catch {}
    ...
    // Winding path
    points.push({ x: 30, y: 45 });
    points.push({ x: 40, y: 55 });
    points.push({ x: 50, y: 40 });
    points.push({ x: 60, y: 60 });
    points.push({ x: 70, y: 45 });
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Grass Avoided!</Text>
            <Text style={styles.resultSubtitle}>
              You avoided grass {finalStats.correct} times out of {finalStats.total}!
            </Text>
            <ResultCard
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Don't Touch the Grass</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • 🚶 Score: {score}
        </Text>
        <Text style={styles.helper}>
          Stay on the path! Don't touch the grass on either side.
        </Text>
      </View>
        ...
        <GestureDetector gesture={panGesture}>
          <Animated.View style={styles.gestureArea}>
            <Svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none" style={styles.svg}>
              {/* Grass background */}
              <Rect x="0" y="0" width="100" height="100" fill="#86EFAC" opacity={0.3} />
              {/* Road path */}
              <Path
                d={pathStr}
                stroke="#FBBF24"
            ...
            {isOffTrack && (
              <View style={styles.warningBox}>
                <Text style={styles.warningText}>Don't touch the grass! ⚠️</Text>
              </View>
            )}
          </Animated.View>
        </GestureDetector>
      ...
      <View style={styles.footerBox}>
        <Text style={styles.footerMain}>
          Skills: continuous tracking • sustained attention • boundary awareness
        </Text>
        <Text style={styles.footerSub}>
          Stay on the path and avoid touching the grass!
        </Text>
      </View>
    </SafeAreaView>
  );
