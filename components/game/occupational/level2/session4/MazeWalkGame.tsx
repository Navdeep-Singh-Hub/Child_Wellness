...
const MazeWalkGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const [mazePathStr, setMazePathStr] = useState('');
    ...
    async (finalScore: number) => {
      ...
      try {
        ...
        const result = await logGameAndAward({
          ...
          accuracy,
          xpAwarded: xp,
          skillTags: ['continuous-tracking', 'sustained-attention', 'path-following'],
        });
  ...
  useEffect(() => {
    ...
    // Simple path: right, down, right, up, right
    points.push({ x: 40, y: startY });
    points.push({ x: 40, y: 65 });
    points.push({ x: 60, y: 65 });
    points.push({ x: 60, y: 35 });
    points.push({ x: endX, y: 35 });
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Maze Walk</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • 🧩 Score: {score}
        </Text>
        <Text style={styles.helper}>
          Follow the clear path through the maze. Stay on track!
        </Text>
      </View>
      ...
      <View style={styles.footerBox}>
        <Text style={styles.footerMain}>
          Skills: continuous tracking • sustained attention • path following
        </Text>
        <Text style={styles.footerSub}>
          Follow the clear path through the maze carefully!
        </Text>
      </View>
    </SafeAreaView>
  );
