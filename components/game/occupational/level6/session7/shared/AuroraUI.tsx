/**
 * Dedicated UI chrome for OT Level 6 Session 7 trunk-rotation games
 */
import type { RotateGameTheme } from '@/components/game/occupational/level6/session7/auroraTheme';
import { LinearGradient } from 'expo-linear-gradient';
import React, { useEffect, useRef } from 'react';
import { Pressable, StyleSheet, Text, View } from 'react-native';
import Animated, { useAnimatedStyle, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';

export function AuroraHUD({ theme, round, totalRounds, score, coins }: {
  theme: RotateGameTheme; round: number; totalRounds: number; score: number; coins: number;
}) {
  const S = theme.shell;
...
export function AuroraIntroPanel({ theme, introText, errorText, cameraSupported, hasCamera, onStart, onRetry, onGuided }: {
  theme: RotateGameTheme; introText?: string; errorText?: string;
  ...
  const defaultIntro = cameraSupported
    ? hasCamera ? 'Stand back so the camera can see your body and arms — give yourself room to reach!' : 'Starting camera…'
