/**
 * Dedicated UI chrome for OT Level 6 Session 6 dynamic-balance games
 */
import type { DynamicGameTheme } from '@/components/game/occupational/level6/session6/trailTheme';
import { LinearGradient } from 'expo-linear-gradient';
import React, { useEffect, useRef } from 'react';
import { Pressable, StyleSheet, Text, View } from 'react-native';
import Animated, { useAnimatedStyle, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';

export function TrailHUD({ theme, round, totalRounds, score, coins }: {
  theme: DynamicGameTheme; round: number; totalRounds: number; score: number; coins: number;
}) {
  const S = theme.shell;
    ...
    <View style={styles.hud}>
      ...
      <View style={styles.statsRow}>
        <View style={[styles.pill, { borderColor: S.statBorder, backgroundColor: S.glassBg }]}>
          <Text style={[styles.lbl, { color: S.statLabel }]}>STEP</Text>
...
export function TrailIntroPanel({ theme, introText, errorText, cameraSupported, hasCamera, onStart, onRetry, onGuided }: {
  theme: DynamicGameTheme; introText?: string; errorText?: string;
  ...
  const defaultIntro = cameraSupported
    ? hasCamera ? 'Stand back so the camera sees your whole body — give yourself room to step and turn!' : 'Starting camera…'
