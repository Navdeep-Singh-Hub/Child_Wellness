import { RO, RO_CHIPS, RO_INTRO_STEPS } from './royalObservatoryTokens';
...
export function RoyalObservatoryIntro({
  errorText,
  cameraSupported,
  permissionGranted,
  hasCamera,
  ...
  const current = RO_INTRO_STEPS[step]!;
      ...
      <View style={[styles.card, styles.cardBorder]}>
        ...
        <View style={styles.btnRow}>
          <Pressable style={styles.primary} onPress={onRetry}>
            <LinearGradient colors={[RO.accent, RO.accentDeep]} style={styles.primaryGrad}>
  ...
  if (!cameraSupported) {
    return (
      <View style={[styles.card, styles.cardBorder]}>
        ...
        <Text style={styles.body}>Camera tracking is not available here. Guided mode still protects the crown!</Text>
        <Pressable style={styles.primary} onPress={onGuided}>
          <LinearGradient colors={[RO.accent, RO.accentDeep]} style={styles.primaryGrad}>
            <Text style={styles.primaryTxt}>👑 Start Guided Watch</Text>
          </LinearGradient>
        </Pressable>
      </View>
    );
  ...
  return (
    <Animated.View style={[styles.card, styles.cardBorder, cardAnim]}>
      ...
      {step < RO_INTRO_STEPS.length - 1 ? (
        ...
        <>
          <Text style={styles.camHint}>
            {!permissionGranted
              ? 'Tap Enter Palace — you will be asked to allow camera access.'
              : hasCamera
          ...
          <Pressable style={styles.primary} onPress={onStart}>
            <LinearGradient colors={[RO.accent, RO.accentDeep]} style={styles.primaryGrad}>
              <Text style={styles.primaryTxt}>👑 Enter Palace</Text>
            </LinearGradient>
          </Pressable>
      ...
      {step > 0 && step < RO_INTRO_STEPS.length - 1 ? (
...
const styles = StyleSheet.create({
  card: { backgroundColor: RO.glass, borderRadius: 24, padding: 20, alignItems: 'center', gap: 10, marginBottom: 8 },
  cardBorder: { borderWidth: 1.5, borderColor: RO.glassBorder },
  ...
  stepDotActive: { width: 22, backgroundColor: RO.accentBright },
  stepDotDone: { backgroundColor: RO.good },
  stepIcon: { fontSize: 44 },
  heroEmoji: { fontSize: 48 },
  title: { fontSize: 24, fontWeight: '900', color: RO.textLight },
  stepTitle: { fontSize: 20, fontWeight: '900', color: RO.textLight },
  body: { fontSize: 14, fontWeight: '600', color: RO.textMuted, textAlign: 'center', lineHeight: 21 },
  camHint: { fontSize: 13, fontWeight: '600', color: RO.textMuted, textAlign: 'center' },
  ...
  chip: {
    ...
    borderRadius: 999,
    borderWidth: 1,
    borderColor: `${RO.accent}44`,
    backgroundColor: 'rgba(255,255,255,0.06)',
  },
  chipTxt: { fontSize: 11, fontWeight: '700', color: RO.textMuted },
  ...
  secondary: { paddingHorizontal: 18, paddingVertical: 12, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.12)' },
  secondaryTxt: { color: RO.textLight, fontSize: 14, fontWeight: '800' },
  btnRow: { flexDirection: 'row', gap: 10, width: '100%' },
  link: { fontSize: 13, fontWeight: '700', color: RO.accentBright, textDecorationLine: 'underline', paddingVertical: 4 },
  emoji: { fontSize: 36 },
