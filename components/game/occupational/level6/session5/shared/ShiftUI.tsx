/**
 * Dedicated UI chrome for OT Level 6 Session 5 weight-shifting games
 */
import type { ShiftGameTheme } from '@/components/game/occupational/level6/session5/adventureTheme';
import { LinearGradient } from 'expo-linear-gradient';
import React, { useEffect, useRef } from 'react';
import { Pressable, StyleSheet, Text, View } from 'react-native';
import Animated, { useAnimatedStyle, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';

export function ShiftHUD({ theme, round, totalRounds, score, coins, roundLabel = 'ROUND' }: {
  theme: ShiftGameTheme; round: number; totalRounds: number; score: number; coins: number; roundLabel?: string;
}) {
  const S = theme.shell;
    ...
    <View style={styles.hud}>
      ...
      <View style={styles.statsRow}>
        <View style={[styles.pill, { borderColor: S.statBorder, backgroundColor: S.glassBg }]}>
          <Text style={[styles.lbl, { color: S.statLabel }]}>{roundLabel}</Text>
...
export function ShiftIntroPanel({ theme, introText, errorText, cameraSupported, hasCamera, onStart, onRetry, onGuided }: {
  theme: ShiftGameTheme; introText?: string; errorText?: string;
  ...
  const defaultIntro = cameraSupported
    ? hasCamera ? 'Stand back so the camera can see your whole body — give yourself room to lean!' : 'Starting camera…'
