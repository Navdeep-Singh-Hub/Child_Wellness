/**
 * Dedicated UI chrome for OT Level 6 Session 2 standing posture games
 */
import type { StandGameTheme } from '@/components/game/occupational/level6/session2/forestTheme';
import { LinearGradient } from 'expo-linear-gradient';
import React, { useEffect, useRef } from 'react';
import { Pressable, StyleSheet, Text, View } from 'react-native';
import Animated, { useAnimatedStyle, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';

export function StandHUD({ theme, round, totalRounds, score, coins }: {
  theme: StandGameTheme; round: number; totalRounds: number; score: number; coins: number;
}) {
  const S = theme.shell;
...
export function StandIntroPanel({ theme, introText, errorText, cameraSupported, hasCamera, onStart, onRetry, onGuided }: {
  theme: StandGameTheme; introText?: string; errorText?: string;
  ...
  const defaultIntro = cameraSupported
    ? hasCamera ? 'Stand back so the camera can see your whole body.' : 'Starting camera…'
    ...
    <Animated.View style={[styles.introCard, { backgroundColor: S.glassBg, borderColor: S.glassBorder }, cardAnim]}>
      ...
      {errorText ? (
        ...
        <>
          ...
          <Pressable
            style={[styles.primary, { opacity: cameraSupported && !hasCamera ? 0.6 : 1 }]}
