/**
 * Dedicated UI chrome for OT Level 6 Session 4 static-balance games
 */
import type { BalanceGameTheme } from '@/components/game/occupational/level6/session4/lagoonTheme';
import { LinearGradient } from 'expo-linear-gradient';
import React, { useEffect, useRef } from 'react';
import { Pressable, StyleSheet, Text, View } from 'react-native';
import Animated, { useAnimatedStyle, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';

export function BalanceHUD({ theme, round, totalRounds, score, coins }: {
  theme: BalanceGameTheme; round: number; totalRounds: number; score: number; coins: number;
}) {
  const S = theme.shell;
...
export function BalanceIntroPanel({ theme, introText, errorText, cameraSupported, hasCamera, onStart, onRetry, onGuided }: {
  theme: BalanceGameTheme; introText?: string; errorText?: string;
  ...
  const defaultIntro = cameraSupported
    ? hasCamera ? 'Stand back so the camera can see your whole body and feet.' : 'Starting camera…'
