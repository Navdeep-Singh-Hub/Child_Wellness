/**
 * Dedicated UI chrome for OT Level 6 Session 10 integrated challenge games
 */
import type { ChampionGameTheme } from '@/components/game/occupational/level6/session10/championTheme';
import { LinearGradient } from 'expo-linear-gradient';
import React, { useEffect, useRef } from 'react';
import { Pressable, StyleSheet, Text, View } from 'react-native';
import Animated, { useAnimatedStyle, useSharedValue, withSpring, withTiming } from 'react-native-reanimated';

export function ChampionHUD({ theme, taskLabel, totalTasks, treasures, courseSec }: {
  theme: ChampionGameTheme; taskLabel: number; totalTasks: number; treasures: number; courseSec: number;
}) {
  const S = theme.shell;
  const pop = useSharedValue(1);
  const prev = useRef(treasures);
  useEffect(() => { if (treasures > prev.current) pop.value = withSpring(1.2, {}, () => { pop.value = withSpring(1); }); prev.current = treasures; }, [treasures, pop]);
    ...
    <View style={styles.hud}>
      ...
      <View style={styles.statsRow}>
        <View style={[styles.pill, { borderColor: S.statBorder, backgroundColor: S.glassBg }]}>
          <Text style={[styles.lbl, { color: S.statLabel }]}>TASK</Text>
          <Text style={[styles.val, { color: S.statValue }]}>{taskLabel}/{totalTasks}</Text>
        </View>
        <Animated.View style={[styles.pill, { borderColor: S.statBorder, backgroundColor: S.glassBg }, bump]}>
          <Text style={styles.coin}>{theme.treasure}</Text>
          <Text style={[styles.val, { color: S.statValue }]}>{treasures}</Text>
        </Animated.View>
        <View style={[styles.pill, { borderColor: S.statBorder, backgroundColor: S.glassBg }]}>
          <Text style={styles.coin}>⏱️</Text>
          <Text style={[styles.val, { color: S.statValue }]}>{courseSec}s</Text>
        </View>
      </View>
    </View>
  );
...
export function ChampionIntroPanel({ theme, introText, errorText, cameraSupported, hasCamera, onStart, onRetry, onGuided }: {
  theme: ChampionGameTheme; introText?: string; errorText?: string;
  ...
  const defaultIntro = cameraSupported
    ? hasCamera ? 'Stand back so the camera sees your whole body — clear some space for the course!' : 'Starting camera…'
