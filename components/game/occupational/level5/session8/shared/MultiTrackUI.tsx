/**
 * Shared HUD + intro primitives for OT L5 Session 8 multi-track games.
 * Per-game backdrops live in each game's *Visuals.tsx file.
 */
...
type Theme = TrackThemeTokens;

export function TrackOrb({ size, color, emoji, scale = 1, dimmed, pulse }: { size: number; color: string; emoji: string; scale?: number; dimmed?: boolean; pulse?: boolean }) {
  const glow = useSharedValue(0.5);
  useEffect(() => {
    if (!pulse) return;
    glow.value = withRepeat(withSequence(withTiming(1, { duration: 600, easing: Easing.inOut(Easing.sin) }), withTiming(0.4, { duration: 600 })), -1, false);
  }, [pulse, glow]);
  const anim = useAnimatedStyle(() => ({ transform: [{ scale: scale * (pulse ? 0.95 + glow.value * 0.1 : 1) }] }));
  return (
    <Animated.View style={[styles.orb, { width: size, height: size, borderRadius: size / 2, backgroundColor: color, opacity: dimmed ? 0.55 : 1, borderColor: pulse ? '#FBBF24' : 'rgba(255,255,255,0.55)' }, anim]}>
      <Text style={{ fontSize: size * 0.42 }}>{emoji}</Text>
    </Animated.View>
  );
}

export function TrackHUD({ theme, gameTitle, roundLabel, round, total, score, scoreLabel, hint, phaseLabel, playing }: {
  theme: Theme; gameTitle: string; roundLabel: string; round: number; total: number; score: number; scoreLabel: string; hint: string; phaseLabel?: string; playing?: boolean;
}) {
    ...
    <View style={styles.hudWrap} pointerEvents="none">
      <Glass {...glassProps} style={[styles.hud, { borderColor: theme.hudBorder, backgroundColor: Platform.OS === 'android' ? theme.hudGlass : 'transparent' }]}>
        <View style={styles.hudRow}>
          ...
          <View style={styles.center}><Text style={[styles.title, { color: theme.title }]}>{gameTitle}</Text>{phaseLabel && <Text style={[styles.phase, { color: theme.accent }]}>{playing ? phaseLabel : 'READY'}</Text>}</View>
...
export function TrackIntro({ theme, copy, chips, startLabel, startColors, backdrop, onStart, onBack }: {
  theme: Theme; copy: Pick<TrackCopy, 'title' | 'emoji' | 'subtitle' | 'introDescription'>;
  chips: string[]; startLabel: string; startColors: readonly string[]; backdrop: React.ReactNode; onStart: () => void; onBack: () => void;
}) {
...
export function TrackCountdown({ accent, onDone }: { accent: string; onDone: () => void }) {
  ...
  return (
    <View style={styles.cdOverlay} pointerEvents="none">
      <View style={[styles.cdCard, { borderColor: `${accent}55` }]}>
        <Text style={[styles.cdLbl, { color: accent }]}>TRACK</Text>
...
const styles = StyleSheet.create({
  orb: { justifyContent: 'center', alignItems: 'center', borderWidth: 2.5, shadowColor: '#000', shadowOpacity: 0.35, shadowRadius: 10, elevation: 8 },
