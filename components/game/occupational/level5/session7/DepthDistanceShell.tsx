/** Shared layout shell for OT L5 Session 7 depth games */
...
import { DepthBackdrop } from '@/components/game/occupational/level5/session7/DepthDistanceVisuals';
import type { DepthCopy } from '@/components/game/occupational/level5/session7/depthDistanceThemes';
...
export function DepthDistanceShell({
  theme,
  copy,
  mode,
  showInfo,
      ...
      <SafeAreaView style={[styles.root, { backgroundColor: copy.rootBg }]} edges={['top', 'bottom']}>
        <Session2Intro
          config={{
            ...
            backdrop: <DepthBackdrop theme={theme} backdrop={copy.backdrop} mode={mode} />,
          }}
          onStart={onStart}
          onBack={onExit}
        />
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: copy.rootBg }]} edges={['top']}>
      ...
      <View style={[styles.arena, { borderColor: theme.hudBorder }]}>
        <DepthBackdrop theme={theme} backdrop={copy.backdrop} mode={mode} />
        {children}
      </View>
    </SafeAreaView>
  );
...
export function useDepthExit(onBack?: () => void) {
  return () => {
    stopAllSpeech();
    cleanupSounds();
    onBack?.();
