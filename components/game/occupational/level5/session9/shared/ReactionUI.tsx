/**
 * Dedicated UI for Session 9 visual reaction games
 */
import type { ReactionThemeTokens } from '@/components/game/occupational/level5/session9/reactionTheme';
...
export type ReactionTheme = ReactionThemeTokens;

export function ReactionHUD({ theme, gameTitle, roundLabel, round, total, score, scoreLabel, hint, phaseLabel, playing, extra }: {
  theme: ReactionTheme; gameTitle: string; roundLabel: string; round: number; total: number;
  score: number; scoreLabel: string; hint: string; phaseLabel?: string; playing?: boolean; extra?: React.ReactNode;
}) {
...
export function ReactionIntro({ theme, copy, chips, startLabel, startColors, backdrop, onStart, onBack }: {
  theme: ReactionTheme;
...
export function ReactionCountdown({ accent, onDone }: { accent: string; onDone: () => void }) {
  ...
  return (
    <View style={styles.cdOverlay} pointerEvents="none">
      <View style={[styles.cdCard, { borderColor: `${accent}55` }]}>
        <Text style={[styles.cdLbl, { color: accent }]}>REACT</Text>
