...
import { ChoiceTile, TimerBar } from '@/components/game/occupational/level5/session9/shared/ReactionFX';
import { ReactionCountdown } from '@/components/game/occupational/level5/session9/shared/ReactionUI';
import { SplitSecondBackdrop } from '@/components/game/occupational/level5/session9/splitSecond/SplitSecondVisuals';
...
const QuickChoiceGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const timerHud = (
    <View style={styles.timerWrap}>
      <TimerBar percent={timePercent} accent={SPLIT_SECOND_THEME.accent} />
      <Text style={[styles.timerText, { color: SPLIT_SECOND_THEME.cue }]}>{(timeLeft / 1000).toFixed(1)}s</Text>
    </View>
  );

  return (
    <ReactionShell
      theme={SPLIT_SECOND_THEME} copy={SPLIT_SECOND_COPY} backdrop={<SplitSecondBackdrop />}
    ...
    >
      ...
      >
        {options.map((option) => (
          ...
          >
            <ChoiceTile size={OPTION_SIZE} emoji={option.emoji} scale={option.scale} accent={SPLIT_SECOND_THEME.accent} />
          </View>
        ))}
      </Pressable>
      {showCountdown && (
        <ReactionCountdown
          accent={SPLIT_SECOND_THEME.accent}
          onDone={() => {
