...
const BALL_SIZE = 60;
const TOLERANCE = 50;
const FAST_SPEED = 800; // milliseconds for one cycle

const FastCatchGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const speedX = useRef(4);
  const speedY = useRef(4);
  ...
  const resetBall = useCallback(() => {
    ...
    // Fast speed
    speedX.current = 3 + Math.random() * 2;
    speedY.current = 3 + Math.random() * 2;
  }, [ballX, ballY]);
  ...
  useEffect(() => {
    if (!showInfo && !done) {
      ...
      setTimeout(() => {
        moveBall();
        speakTTS('Catch the fast ball!', 0.8, 'en-US' );
      }, 500);
    }
  ...
  if (showInfo) {
    return (
      <GameInfoScreen
        title="Fast Catch"
        emoji="⚡"
        description="Tap the fast moving ball! Build your reaction speed."
        skills={['Reaction speed']}
        suitableFor="Children learning fast reaction and hand-eye coordination"
        onStart={() => {
          setShowInfo(false);
        }}
        onBack={() => {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <Text style={styles.title}>Fast Catch</Text>
