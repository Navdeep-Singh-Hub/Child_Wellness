/** OT Level 5 · Session 6 · Game 2 — Zen Catch */
...
import { DriftBall, MeadowBackdrop, ZenCatchHUD, ZenCatchInfoScreen } from '@/components/game/occupational/level5/session6/zenCatch/ZenCatchVisuals';
...
const ZenCatchGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  const catch_ = useSpeedCatchGame({ config: SLOW_CATCH_CONFIG, ttsComplete: COPY.ttsComplete, onBack });

  if (catch_.showInfo) return <SafeAreaView style={styles.root} edges={['top', 'bottom']}><ZenCatchInfoScreen onStart={catch_.handleStart} onBack={catch_.handleExit} /></SafeAreaView>;
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      <ZenCatchHUD round={catch_.round} total={catch_.totalRounds} score={catch_.score} hint={catch_.hint} playing={catch_.phase === 'playing'} />
      <Pressable style={styles.arena} onLayout={(e) => catch_.onLayout(e.nativeEvent.layout.width, e.nativeEvent.layout.height)} onPress={catch_.handleGameTap}>
        <MeadowBackdrop />
        {catch_.phase === 'playing' && <Animated.View style={[styles.ballPos, catch_.ballStyle]} pointerEvents="none"><DriftBall size={BALL_SIZE} pulse /></Animated.View>}
...
const styles = StyleSheet.create({
  ...
  exitBtn: { position: 'absolute', top: 52, left: 14, zIndex: 50, backgroundColor: 'rgba(20,83,45,0.9)', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1, borderColor: T.hudBorder },
