/** Shared layout shell for OT L5 Session 6 speed-matching games */
...
import { SpeedMatchBackdrop } from '@/components/game/occupational/level5/session6/SpeedMatchVisuals';
import type { SpeedMatchCopy } from '@/components/game/occupational/level5/session6/speedMatchThemes';
...
export function SpeedMatchShell({
  theme,
  copy,
  showInfo,
  showCongrats,
      ...
      <SafeAreaView style={[styles.root, { backgroundColor: copy.rootBg }]} edges={['top', 'bottom']}>
        <Session2Intro
          config={{
            ...
            backdrop: <SpeedMatchBackdrop theme={theme} backdrop={copy.backdrop} />,
          }}
          onStart={onStart}
          onBack={onExit}
        />
  ...
  return (
    <SafeAreaView style={[styles.root, { backgroundColor: copy.rootBg }]} edges={['top']}>
      ...
      <View style={[styles.arena, { borderColor: theme.hudBorder }]}>
        <SpeedMatchBackdrop theme={theme} backdrop={copy.backdrop} />
        {children}
      </View>
    </SafeAreaView>
  );
...
export function useSpeedMatchExit(onBack?: () => void) {
  return () => {
    stopAllSpeech();
    cleanupSounds();
    onBack?.();
