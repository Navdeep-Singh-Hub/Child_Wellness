/**
 * OT Level 10 · Session 4 · Game 5 — Integration Quest
 *
 * Crystal integration capstone: gather at quest node, integrate
 * balance + reach together, then complete the finale crystal.
 * Camera tracks movement quality, posture, attention and completion.
 */
...
import { IntegrationQuestOverlay } from '@/components/game/occupational/level10/session4/components/IntegrationQuestOverlay';
import { IntegrationQuestVisuals } from '@/components/game/occupational/level10/session4/components/IntegrationQuestVisuals';
import { SensoryMotorMeter } from '@/components/game/occupational/level10/session4/components/SensoryMotorMeter';
import { INTEGRATION_QUEST_PACING as P } from '@/components/game/occupational/level10/session4/integrationQuestPacing';
import {
  INTEGRATION_QUEST_ROUNDS,
  INTEGRATION_QUEST_THEME as T,
  QUEST_SHELL,
  type IntegrationQuestRound,
  type QuestPhaseKind,
} from '@/components/game/occupational/level10/session4/integrationQuestTheme';
import { inQuestZone, integrationQuestQuality } from '@/components/game/occupational/level10/session4/integrationQuestUtils';
...
const VOICE_PRAISE = ['Quest star!', 'Integrated!', 'Champion!', 'Crystal hero!', 'Mastery!'];
...
const IntegrationQuestGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundPhase, setRoundPhase] = useState<QuestPhaseKind>('gather');
  ...
  const [gatherOk, setGatherOk] = useState(false);
  ...
  const questRound = INTEGRATION_QUEST_ROUNDS[Math.min(round, INTEGRATION_QUEST_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<QuestPhaseKind>('gather');
  const roundRef = useRef<IntegrationQuestRound>(questRound);
  ...
  const beginIntegratePhase = useCallback((qr: IntegrationQuestRound) => {
    ...
    setCoachCue(qr.integrateCue);
    phaseStartRef.current = Date.now();
    speakTTS(qr.voiceIntegrate, 0.85).catch(() => {});
    playSound('clap', 0.55, 1.05).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium).catch(() => {});
  }, []);

  const beginCompletePhase = useCallback((qr: IntegrationQuestRound) => {
    roundPhaseRef.current = 'complete';
    setRoundPhase('complete');
    holdMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    setBanner(T.completeLabel);
    setCoachCue(qr.completeCue);
    phaseStartRef.current = Date.now();
    speakTTS(qr.voiceComplete, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    const xp = done * 20 + accuracy + Math.round(motorSumRef.current / Math.max(1, done) * 24);
    setFinalStats({ correct: done, total: totalRounds, xp, accuracy });
    setShowCongrats(true);

    try {
      await recordGame({
        gameId: 'integration-quest',
        score: done,
        accuracy,
        meta: { ...analyticsMeta, integration: motorSumRef.current / Math.max(1, done) },
      });
      await logGameAndAward('integration-quest', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const qr = INTEGRATION_QUEST_ROUNDS[Math.min(r, INTEGRATION_QUEST_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    roundPhaseRef.current = 'gather';
    setRoundPhase('gather');
    setBanner(T.gatherLabel);
    setCoachCue(qr.voiceGather);
    setGatherOk(false);
    setIntegrateOk(false);
    setCompleteOk(false);
    phaseStartRef.current = Date.now();
    speakTTS(qr.voiceGather, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    const holdTarget =
      rp === 'gather' ? P.gatherHoldMs : rp === 'integrate' ? P.integrateHoldMs : P.completeHoldMs;
    const grace =
      rp === 'gather' ? P.gatherGraceMs : rp === 'integrate' ? P.integrateGraceMs : P.completeGraceMs;

    if (!camLiveRef.current) {
      const fbMs =
        rp === 'gather' ? P.fallbackGatherMs : rp === 'integrate' ? P.fallbackIntegrateMs : P.fallbackCompleteMs;
      const elapsed = now - phaseStartRef.current;
      const t = clamp01(elapsed / fbMs);
      setHoldProgress(t);
      setIntegrateScore(0.84);
      setMotorScore(0.82);
      setFocus(0.8);
      setQuality(0.88);
      if (rp === 'gather' && t >= 1) beginIntegratePhase(qr);
      else if (rp === 'integrate' && t >= 1) beginCompletePhase(qr);
      else if (rp === 'complete' && t >= 1) completeRound();
    ...
    const hand = handPoint(qr.hand, s.leftWrist, s.rightWrist);

    if (!s.present || !cursor || (rp === 'integrate' && (!center || !hand))) {
    ...
    if (rp === 'gather') {
      const ok = inQuestZone(cursor, qr.gather);
      const holdPct = clamp01(holdMsRef.current / holdTarget);
      const q = integrationQuestQuality(ok, false, false, holdPct, smooth, s.postureQuality, s.attentionScore, 'gather');

      setGatherOk(ok);
      setIntegrateOk(false);
      setCompleteOk(false);
      setIntegrateScore(ok ? holdPct * 0.5 : 0.25);
      setMotorScore(smooth * 0.4);
      setFocus(s.attentionScore);
      setQuality(q);
      qualSumRef.current += q;
      qualCountRef.current += 1;

      setBanner(ok ? T.holdGatherLabel : T.gatherLabel);
      setCoachCue(ok ? 'Hold at the quest node…' : qr.voiceGather);
      ...
      if (ok) {
        ...
        if (holdMsRef.current >= holdTarget) beginIntegratePhase(qr);
    ...
    if (rp === 'integrate') {
      const balanced = inQuestZone(center, qr.balance) && s.postureQuality > 0.32;
      const reaching = inQuestZone(hand, qr.integrate);
      const ok = balanced && reaching;
      const holdPct = clamp01(holdMsRef.current / holdTarget);
      const q = integrationQuestQuality(
        true,
        ok,
        false,
        holdPct,
      ...
      setIntegrateOk(ok);
      setCompleteOk(false);
      setIntegrateScore(ok ? holdPct * 0.55 + 0.35 : (balanced ? 0.35 : 0.15) + (reaching ? 0.2 : 0));
      setMotorScore(handSmooth * 0.5);
      setFocus(s.attentionScore);
      setQuality(q);
      qualSumRef.current += q;
      qualCountRef.current += 1;
      ...
      setCoachCue(ok ? qr.integrateCue : balanced ? 'Reach to the integrate orb!' : 'Center your balance first!');

      if (ok) {
        if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
        holdMsRef.current += dt;
        setHoldProgress(clamp01(holdMsRef.current / holdTarget));
        if (holdMsRef.current >= holdTarget) beginCompletePhase(qr);
      } else if (now - (lastInZoneRef.current || now) > grace) {
    ...
    const ok = inQuestZone(cursor, qr.complete);
    const holdPct = clamp01(holdMsRef.current / holdTarget);
    const q = integrationQuestQuality(true, true, ok, holdPct, smooth, s.postureQuality, s.attentionScore, 'complete');

    setGatherOk(true);
    setIntegrateOk(true);
    setCompleteOk(ok);
    setIntegrateScore(0.9);
    setMotorScore(ok ? holdPct * 0.65 + smooth * 0.35 : smooth * 0.3);
    setFocus(s.attentionScore);
    setQuality(q);
    qualSumRef.current += q;
    qualCountRef.current += 1;

    setBanner(ok ? T.holdCompleteLabel : T.completeLabel);
    setCoachCue(ok ? qr.completeCue : qr.voiceComplete);
  ...
  }, [beginCompletePhase, beginIntegratePhase, bodyCenter, completeRound, recordTick]);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your full body — quest sensors calibrating…');
  ...
  const renderOverlay = () => (
    <>
      <IntegrationQuestVisuals phase={roundPhase} />
      <SensoryMotorMeter balance={integrateScore} reach={motorScore} focus={focus} />
      {phase === 'play' && (
        <>
          <IntegrationQuestOverlay
            round={questRound}
            phase={roundPhase}
          ...
          <HandReachOverlay
            ...
            requiredHand={roundPhase === 'integrate' ? questRound.hand : 'both'}
          />
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={QUEST_SHELL.sparkleColor} count={28} />
      {phase === 'calibrate' && (
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          ...
          <Text style={styles.academyLabel}>🏆 {QUEST_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: QUEST_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: QUEST_SHELL.statLabel }]}>Quest</Text>
...
export default IntegrationQuestGame;
...
const styles = StyleSheet.create({
  ...
  skillPill: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10, backgroundColor: 'rgba(167,139,250,0.15)' },
