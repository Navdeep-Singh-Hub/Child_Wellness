/**
 * OT Level 10 · Session 8 · Game 3 — Escape Route
 *
 * Exit Quest: scout each blocked exit, then follow the escape route
...
import { EscapeRouteOverlay } from '@/components/game/occupational/level10/session8/components/EscapeRouteOverlay';
import { EscapeRouteVisuals } from '@/components/game/occupational/level10/session8/components/EscapeRouteVisuals';
import { ESCAPE_ROUTE_PACING as P } from '@/components/game/occupational/level10/session8/escapeRoutePacing';
import {
  ESCAPE_ROUTE_ROUNDS,
  ESCAPE_ROUTE_THEME as T,
  ESCAPE_SHELL,
  type EscapeRouteRound,
} from '@/components/game/occupational/level10/session8/escapeRouteTheme';
import { inScoutExitZone, inEscapeRouteZone } from '@/components/game/occupational/level10/session8/escapeRouteUtils';
...
const VOICE_PRAISE = ['Escape star!', 'Route clear!', 'Great scout!', 'Quest champ!', 'Route finder!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'scout' | 'escape';

const EscapeRouteGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [escapeScore, setEscapeScore] = useState(0);
  const [calm, setCalm] = useState(0);
  const [focus, setFocus] = useState(0);
  const [quality, setQuality] = useState(0);
  const [onScout, setOnScout] = useState(false);
  const [onEscape, setOnEscape] = useState(false);
  ...
  const routeRound = ESCAPE_ROUTE_ROUNDS[Math.min(round, ESCAPE_ROUTE_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('scout');
  const roundRef = useRef<EscapeRouteRound>(routeRound);
  ...
  useEffect(() => { roundRef.current = routeRound; }, [routeRound]);
  ...
  const beginEscapePhase = useCallback((pr: EscapeRouteRound) => {
    roundPhaseRef.current = 'escape';
    setRoundPhase('escape');
    holdMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    setShowEscape(true);
    setBanner(T.escapeLabel);
    setCoachCue(pr.escapeCue);
    escapeStartRef.current = Date.now();
    speakTTS(pr.voiceEscape, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      ...
      await logGameAndAward('escape-route', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const pr = ESCAPE_ROUTE_ROUNDS[Math.min(r, ESCAPE_ROUTE_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    ...
    setCoachCue(pr.voiceScout);
    setOnScout(false);
    setOnEscape(false);
    scoutStartRef.current = Date.now();
    speakTTS(pr.voiceScout, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'scout') {
        ...
        setOnScout(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackScoutMs) beginEscapePhase(pr);
      } else {
    ...
    if (rp === 'scout') {
      const atScout = inScoutExitZone(cursor, pr);
      ...
      const q = problemSolvingQuality(atScout, false, holdPct, s.postureQuality, s.attentionScore, smooth, 'try');
      ...
      setCoachCue(atScout ? `${pr.label} — scout the exit!` : pr.voiceScout);

      if (holdMsRef.current >= P.scoutHoldMs) beginEscapePhase(pr);
    ...
    const atEscape =
      inEscapeRouteZone(cursor, pr) &&
      s.postureQuality >= P.minPostureForEscape &&
      s.attentionScore >= P.minAttentionForEscape;
    const holdPct = clamp01(holdMsRef.current / P.escapeHoldMs);
    const scoutPct = clamp01(scoutMsRef.current / P.scoutHoldMs);
    const q = problemSolvingQuality(false, atEscape, holdPct, s.postureQuality, s.attentionScore, smooth, 'adapt');

    setOnScout(false);
    setOnEscape(atEscape);
    setScoutScore(scoutPct);
    setEscapeScore(atEscape ? holdPct * 0.65 + smooth * 0.35 : smooth * 0.25);
    setCalm(atEscape ? smooth * 0.55 + s.attentionScore * 0.45 : smooth * 0.3);
    setFocus(s.attentionScore);
    setQuality(q);
    qualSumRef.current += q;
    qualCountRef.current += 1;

    setBanner(atEscape ? T.holdEscapeLabel : T.escapeLabel);
    setCoachCue(atEscape ? pr.escapeCue : pr.voiceEscape);

    if (atEscape) {
      if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
      holdMsRef.current += dt;
      setHoldProgress(clamp01(holdMsRef.current / P.escapeHoldMs));
      if (holdMsRef.current >= P.escapeHoldMs) completeRound();
    } else if (now - (lastInZoneRef.current || now) > P.escapeGraceMs) {
      holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
      setHoldProgress(clamp01(holdMsRef.current / P.escapeHoldMs));
      lastInZoneRef.current = 0;
    }
    ...
    recordTick(dt, {
      upright: s.postureQuality > P.minPostureForEscape,
      still: smooth > 0.35,
      quality: q,
    });
  }, [beginEscapePhase, completeRound, recordTick]);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your full body — route sensors calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} routes escaped · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <EscapeRouteVisuals escapePhase={roundPhase === 'escape'} />
      <AttentionMeter
        find={scoutScore}
        focus={escapeScore}
        calm={calm}
        title="ROUTE"
        labels={{ find: 'SCOUT', focus: 'ESCAPE', calm: 'CALM' }}
        style={{
          glassBorder: ESCAPE_SHELL.glassBorder,
          statLabel: ESCAPE_SHELL.statLabel,
          accent: T.accent,
          fillColors: ['#8B5CF6', '#F97316', '#FDBA74'],
        }}
      />
      {phase === 'play' && (
        <>
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={ESCAPE_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Route calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: ESCAPE_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: ESCAPE_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🏃 {ESCAPE_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: ESCAPE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: ESCAPE_SHELL.statLabel }]}>Exit</Text>
              <Text style={[styles.statValue, { color: ESCAPE_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: ESCAPE_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🏃</Text>
              <Text style={[styles.statValue, { color: ESCAPE_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: ESCAPE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: ESCAPE_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: ESCAPE_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Escape Route</Text>
            </Pressable>
...
export default EscapeRouteGame;
...
const styles = StyleSheet.create({
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: ESCAPE_SHELL.stageBorder,
    backgroundColor: ESCAPE_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: ESCAPE_SHELL.glassBorder,
  },
  phaseBannerText: { color: ESCAPE_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: ESCAPE_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  errorText: { color: ESCAPE_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
