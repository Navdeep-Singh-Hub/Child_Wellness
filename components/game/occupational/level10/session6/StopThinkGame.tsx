/**
 * OT Level 10 · Session 6 · Game 2 — Stop & Think
 *
 * Pause Path: go to each cue, then STOP and think with
 * calm stillness. Camera tracks movement quality, posture,
...
import { StopThinkOverlay } from '@/components/game/occupational/level10/session6/components/StopThinkOverlay';
import { StopThinkVisuals } from '@/components/game/occupational/level10/session6/components/StopThinkVisuals';
import { STOP_THINK_PACING as P } from '@/components/game/occupational/level10/session6/stopThinkPacing';
import {
  STOP_THINK_ROUNDS,
  STOP_THINK_THEME as T,
  STOP_SHELL,
  type StopThinkRound,
} from '@/components/game/occupational/level10/session6/stopThinkTheme';
import { inGoZone, inStopZone, stopThinkQuality } from '@/components/game/occupational/level10/session6/stopThinkUtils';
...
const VOICE_PRAISE = ['Stop star!', 'Think hero!', 'Calm ace!', 'Great pause!', 'Regulation champ!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'go' | 'stop';

const StopThinkGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundPhase, setRoundPhase] = useState<RoundPhase>('go');
  ...
  const stopRound = STOP_THINK_ROUNDS[Math.min(round, STOP_THINK_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('go');
  const roundRef = useRef<StopThinkRound>(stopRound);
  const doneRef = useRef(false);
  const camLiveRef = useRef(camLive);
  const holdMsRef = useRef(0);
  const goMsRef = useRef(0);
  ...
  useEffect(() => { roundRef.current = stopRound; }, [stopRound]);
  ...
  const beginStopPhase = useCallback((st: StopThinkRound) => {
    roundPhaseRef.current = 'stop';
    setRoundPhase('stop');
    holdMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    setShowStop(true);
    setBanner(T.stopLabel);
    setCoachCue(st.stopCue);
    stopStartRef.current = Date.now();
    speakTTS(st.voiceStop, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      ...
      await logGameAndAward('stop-think', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const st = STOP_THINK_ROUNDS[Math.min(r, STOP_THINK_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    goMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    roundPhaseRef.current = 'go';
    setRoundPhase('go');
    setShowStop(false);
    setBanner(T.goLabel);
    setCoachCue(st.voiceGo);
    setOnGo(false);
    setOnStop(false);
    goStartRef.current = Date.now();
    speakTTS(st.voiceGo, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'go') {
        const elapsed = now - goStartRef.current;
        const t = clamp01(elapsed / P.fallbackGoMs);
        setGoScore(t * 0.85);
        setStopScore(0.35);
        setCalm(0.5);
        setOnGo(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackGoMs) beginStopPhase(st);
      } else {
        const elapsed = now - stopStartRef.current;
        const t = clamp01(elapsed / P.fallbackStopMs);
        setHoldProgress(t);
        setStopScore(0.84);
        setGoScore(0.8);
        setCalm(0.82);
    ...
    if (rp === 'go') {
      const atGo = inGoZone(cursor, st);

      if (atGo) {
        goMsRef.current += dt;
        if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
        holdMsRef.current += dt;
      } else if (now - (lastInZoneRef.current || now) > P.goGraceMs) {
        holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
        lastInZoneRef.current = 0;
      }

      const holdPct = clamp01(holdMsRef.current / P.goHoldMs);
      const q = stopThinkQuality(atGo, false, holdPct, s.postureQuality, s.attentionScore, smooth, 'go');

      setOnGo(atGo);
      setOnStop(false);
      setGoScore(holdPct * 0.65 + (atGo ? 0.35 : smooth * 0.2));
      setStopScore(0.3);
      ...
      setBanner(T.holdGoLabel);
      setCoachCue(atGo ? `${st.label} — go zone!` : st.voiceGo);

      if (holdMsRef.current >= P.goHoldMs) beginStopPhase(st);
    ...
    const atStop =
      inStopZone(cursor, st) &&
      s.postureQuality >= P.minPostureForStop &&
      s.attentionScore >= P.minAttentionForStop &&
      smooth >= P.minStillnessForStop;
    const holdPct = clamp01(holdMsRef.current / P.stopHoldMs);
    const goPct = clamp01(goMsRef.current / P.goHoldMs);
    const q = stopThinkQuality(false, atStop, holdPct, s.postureQuality, s.attentionScore, smooth, 'stop');

    setOnGo(false);
    setOnStop(atStop);
    setGoScore(goPct);
    setStopScore(atStop ? holdPct * 0.6 + smooth * 0.4 : smooth * 0.25);
    setCalm(atStop ? smooth * 0.6 + s.attentionScore * 0.4 : smooth * 0.3);
    setFocus(s.attentionScore);
    setQuality(q);
    qualSumRef.current += q;
    qualCountRef.current += 1;

    setBanner(atStop ? T.holdStopLabel : T.stopLabel);
    setCoachCue(atStop ? st.stopCue : st.voiceStop);

    if (atStop) {
      if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
      holdMsRef.current += dt;
      setHoldProgress(clamp01(holdMsRef.current / P.stopHoldMs));
      if (holdMsRef.current >= P.stopHoldMs) completeRound();
    } else if (now - (lastInZoneRef.current || now) > P.stopGraceMs) {
      holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
      setHoldProgress(clamp01(holdMsRef.current / P.stopHoldMs));
      lastInZoneRef.current = 0;
    }
    ...
    recordTick(dt, {
      upright: s.postureQuality > P.minPostureForStop,
      still: smooth > P.minStillnessForStop,
      quality: q,
    });
  }, [beginStopPhase, completeRound, recordTick]);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your face — stop-and-think calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} pauses completed · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <StopThinkVisuals stopPhase={roundPhase === 'stop'} />
      <AttentionMeter
        find={goScore}
        focus={stopScore}
        calm={calm}
        style={{
          glassBorder: STOP_SHELL.glassBorder,
          statLabel: STOP_SHELL.statLabel,
          accent: STOP_SHELL.accent,
          fillColors: ['#14B8A6', '#EF4444', '#99F6E4'],
        }}
      />
      {phase === 'play' && (
        <>
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={STOP_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Stop-and-think calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: STOP_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: STOP_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🛑 {STOP_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: STOP_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: STOP_SHELL.statLabel }]}>Cue</Text>
              <Text style={[styles.statValue, { color: STOP_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: STOP_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>⏸️</Text>
              <Text style={[styles.statValue, { color: STOP_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: STOP_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: STOP_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: STOP_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Stop & Think</Text>
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: STOP_SHELL.accent, fontSize: 10, fontWeight: '900', letterSpacing: 0.4 },
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: STOP_SHELL.stageBorder,
    backgroundColor: STOP_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: STOP_SHELL.glassBorder,
  },
  phaseBannerText: { color: STOP_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: STOP_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  errorText: { color: STOP_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
