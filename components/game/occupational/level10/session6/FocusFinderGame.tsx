/**
 * OT Level 10 · Session 6 · Game 1 — Focus Finder
 *
 * Spotlight Quest: seek each focus beacon, then hold
 * calm locked attention. Camera tracks movement quality, posture,
...
import { attentionRegulationQuality } from '@/components/game/occupational/level10/session6/attentionRegulationUtils';
import { AttentionMeter } from '@/components/game/occupational/level10/session6/components/AttentionMeter';
import { FocusFinderOverlay } from '@/components/game/occupational/level10/session6/components/FocusFinderOverlay';
import { FocusFinderVisuals } from '@/components/game/occupational/level10/session6/components/FocusFinderVisuals';
import { FOCUS_FINDER_PACING as P } from '@/components/game/occupational/level10/session6/focusFinderPacing';
import {
  FOCUS_FINDER_ROUNDS,
  FOCUS_FINDER_THEME as T,
  FOCUS_SHELL,
  type FocusFinderRound,
} from '@/components/game/occupational/level10/session6/focusFinderTheme';
import { inFocusZone, inSeekZone } from '@/components/game/occupational/level10/session6/focusFinderUtils';
...
const VOICE_PRAISE = ['Focus star!', 'Spotlight hero!', 'Calm ace!', 'Great find!', 'Attention champ!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'seek' | 'focus';

const FocusFinderGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const focusRound = FOCUS_FINDER_ROUNDS[Math.min(round, FOCUS_FINDER_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('seek');
  const roundRef = useRef<FocusFinderRound>(focusRound);
  ...
  useEffect(() => { roundRef.current = focusRound; }, [focusRound]);
  ...
  const beginFocusPhase = useCallback((fr: FocusFinderRound) => {
    ...
    setCoachCue(fr.focusCue);
    focusStartRef.current = Date.now();
    speakTTS(fr.voiceFocus, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      ...
      await logGameAndAward('focus-finder', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const fr = FOCUS_FINDER_ROUNDS[Math.min(r, FOCUS_FINDER_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    seekMsRef.current = 0;
    lastInZoneRef.current = 0;
    ...
    setCoachCue(fr.voiceSeek);
    setOnSeek(false);
    setOnFocus(false);
    seekStartRef.current = Date.now();
    speakTTS(fr.voiceSeek, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'seek') {
        ...
        setOnSeek(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackSeekMs) beginFocusPhase(fr);
      } else {
        const elapsed = now - focusStartRef.current;
        const t = clamp01(elapsed / P.fallbackFocusMs);
        setHoldProgress(t);
        setLockScore(0.84);
        setSeekScore(0.8);
        setCalm(0.82);
    ...
    if (rp === 'seek') {
      const atSeek = inSeekZone(cursor, fr);
      ...
      const q = attentionRegulationQuality(
        atSeek,
        false,
        holdPct,
        s.postureQuality,
      ...
      setCoachCue(atSeek ? `${fr.label} — hold steady!` : fr.voiceSeek);

      if (holdMsRef.current >= P.seekHoldMs) beginFocusPhase(fr);
    ...
    const atFocus =
      inFocusZone(cursor, fr) &&
      s.postureQuality >= P.minPostureForFocus &&
      s.attentionScore >= P.minAttentionForFocus;
    const holdPct = clamp01(holdMsRef.current / P.focusHoldMs);
    const seekPct = clamp01(seekMsRef.current / P.seekHoldMs);
    const q = attentionRegulationQuality(false, atFocus, holdPct, s.postureQuality, s.attentionScore, smooth, 'focus');

    setOnSeek(false);
    setOnFocus(atFocus);
    setSeekScore(seekPct);
    setLockScore(atFocus ? holdPct * 0.65 + smooth * 0.35 : smooth * 0.25);
    setCalm(atFocus ? smooth * 0.55 + s.attentionScore * 0.45 : smooth * 0.3);
    setFocus(s.attentionScore);
    setQuality(q);
    qualSumRef.current += q;
    qualCountRef.current += 1;

    setBanner(atFocus ? T.holdFocusLabel : T.focusLabel);
    setCoachCue(atFocus ? fr.focusCue : fr.voiceFocus);

    if (atFocus) {
      if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
      holdMsRef.current += dt;
      setHoldProgress(clamp01(holdMsRef.current / P.focusHoldMs));
      if (holdMsRef.current >= P.focusHoldMs) completeRound();
    } else if (now - (lastInZoneRef.current || now) > P.focusGraceMs) {
      holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
      setHoldProgress(clamp01(holdMsRef.current / P.focusHoldMs));
      lastInZoneRef.current = 0;
    }
    ...
    recordTick(dt, {
      upright: s.postureQuality > P.minPostureForFocus,
      still: smooth > 0.35,
      quality: q,
    });
  }, [beginFocusPhase, completeRound, recordTick]);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your face — focus spotlight calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} spotlights found · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <FocusFinderVisuals focusPhase={roundPhase === 'focus'} />
      <AttentionMeter
        find={seekScore}
        focus={lockScore}
        calm={calm}
        style={{
          glassBorder: FOCUS_SHELL.glassBorder,
          statLabel: FOCUS_SHELL.statLabel,
          accent: FOCUS_SHELL.accent,
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={FOCUS_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Focus calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: FOCUS_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: FOCUS_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🔍 {FOCUS_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: FOCUS_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: FOCUS_SHELL.statLabel }]}>Spot</Text>
              <Text style={[styles.statValue, { color: FOCUS_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: FOCUS_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>✨</Text>
              <Text style={[styles.statValue, { color: FOCUS_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: FOCUS_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: FOCUS_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: FOCUS_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Focus Finder</Text>
            </Pressable>
...
export default FocusFinderGame;
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: FOCUS_SHELL.accent, fontSize: 10, fontWeight: '900', letterSpacing: 0.4 },
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: FOCUS_SHELL.stageBorder,
    backgroundColor: FOCUS_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: FOCUS_SHELL.glassBorder,
  },
  phaseBannerText: { color: FOCUS_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: FOCUS_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  errorText: { color: FOCUS_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
