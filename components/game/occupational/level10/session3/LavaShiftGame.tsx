/**
 * OT Level 10 · Session 3 · Game 2 — Lava Shift
 *
 * Volcanic safe-stone run: start on Rock A, adapt when lava shifts
 * to escape to Rock B. Camera tracks redirect speed, movement
 * quality, posture, attention and safe-zone holds.
 */
...
import { LavaSafeStones } from '@/components/game/occupational/level10/session3/components/LavaSafeStones';
import { LavaShiftVisuals } from '@/components/game/occupational/level10/session3/components/LavaShiftVisuals';
import { LAVA_SHIFT_PACING as P } from '@/components/game/occupational/level10/session3/lavaShiftPacing';
import {
  LAVA_SHIFT_ROUNDS,
  LAVA_SHIFT_THEME as T,
  LAVA_SHELL,
  type LavaShiftRound,
} from '@/components/game/occupational/level10/session3/lavaShiftTheme';
import { inSafeStone } from '@/components/game/occupational/level10/session3/lavaShiftUtils';
...
const VOICE_PRAISE = ['Safe!', 'Quick escape!', 'Lava hero!', 'Great shift!', 'Volcano star!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'safe-a' | 'shift' | 'escape-b';

const LavaShiftGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundPhase, setRoundPhase] = useState<RoundPhase>('safe-a');
  const [activeStone, setActiveStone] = useState<'a' | 'b'>('a');
  const [stoneASunk, setStoneASunk] = useState(false);
  const [lavaLevel, setLavaLevel] = useState(0.08);
  ...
  const lavaRound = LAVA_SHIFT_ROUNDS[Math.min(round, LAVA_SHIFT_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('safe-a');
  const lavaRef = useRef<LavaShiftRound>(lavaRound);
  ...
  useEffect(() => { lavaRef.current = lavaRound; }, [lavaRound]);
  ...
  const beginEscapePhase = useCallback((lr: LavaShiftRound) => {
    roundPhaseRef.current = 'escape-b';
    setRoundPhase('escape-b');
    setActiveStone('b');
    setStoneASunk(true);
    setLavaLevel(0.28);
    setShiftFlash(false);
    setBanner(T.escapeLabel);
    setCoachCue(lr.escapeCue);
    escapeStartRef.current = Date.now();
    holdMsRef.current = 0;
    setHoldProgress(0);
    speakTTS(lr.voiceEscape, 0.85).catch(() => {});
  }, []);

  const triggerLavaShift = useCallback(
    (lr: LavaShiftRound) => {
      roundPhaseRef.current = 'shift';
      setRoundPhase('shift');
      setShiftFlash(true);
      setBanner(T.shiftLabel);
      setCoachCue(lr.voiceShift);
      setLavaLevel(0.22);
      speakTTS(lr.voiceShift, 0.88).catch(() => {});
      playSound('splash', 0.55, 0.85).catch(() => {});
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
      schedule(() => beginEscapePhase(lr), P.shiftFlashMs);
    },
    [beginEscapePhase, schedule],
  );
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      ...
      await logGameAndAward('lava-shift', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback(
    (r: number) => {
      const lr = LAVA_SHIFT_ROUNDS[Math.min(r, LAVA_SHIFT_ROUNDS.length - 1)]!;
      setRound(r);
      holdMsRef.current = 0;
      ...
      setStoneASunk(false);
      setLavaLevel(0.08);
      roundPhaseRef.current = 'safe-a';
      setRoundPhase('safe-a');
      setActiveStone('a');
      setBanner(T.safeLabel);
      setCoachCue(lr.voiceSafe);
      speakTTS(lr.voiceSafe, 0.82).catch(() => {});

      if (shiftTimerRef.current) clearTimeout(shiftTimerRef.current);
      shiftTimerRef.current = setTimeout(() => triggerLavaShift(lr), P.safePreviewMs + P.shiftDelayMs);
    },
    [triggerLavaShift],
  );
  ...
  const tick = useCallback(() => {
    ...
    const lr = lavaRef.current;
    const rp = roundPhaseRef.current;
    const target = rp === 'escape-b' ? lr.stoneB : lr.stoneA;

    if (rp === 'shift') {
      setLavaLevel((l) => Math.min(0.38, l + 0.008));
      recordTick(dt, { upright: true, still: false, quality: 0.7 });
      return;
    }

    if (!camLiveRef.current) {
      if (rp === 'escape-b') {
        const elapsed = now - escapeStartRef.current;
        const t = clamp01(elapsed / P.fallbackEscapeMs);
        setHoldProgress(t);
        setAdaptation(0.84);
        setFocus(0.82);
        setFlow(0.8);
    ...
    const onTarget = inSafeStone(s.cursor, target);
    const onA = inSafeStone(s.cursor, lr.stoneA);
    const onB = inSafeStone(s.cursor, lr.stoneB);

    if (rp === 'escape-b' && onA && !onB) {
      setLavaLevel((l) => Math.min(0.45, l + 0.006));
      setCoachCue('Leave the lava stone — escape to safety!');
    }

    const adaptSpd =
      rp === 'escape-b' && onB ? adaptSpeedScore(now - escapeStartRef.current, P.quickEscapeBonusMs) : 0.35;
    const holdPct = clamp01(holdMsRef.current / P.holdEscapeMs);
    const q = adaptiveMotorQuality(rp === 'escape-b' && onB, holdPct, adaptSpd, s.postureQuality, smooth, s.attentionScore);

    setAdaptation(rp === 'escape-b' ? adaptSpd : smooth * 0.45);
    ...
    if (rp === 'safe-a') {
      setCoachCue(onTarget ? 'Good — stay on the safe stone…' : lr.voiceSafe);
      setBanner(T.safeLabel);
      setLavaLevel(0.1);
    }
    ...
    if (rp === 'escape-b') {
      if (onB) {
        ...
        const prog = clamp01(holdMsRef.current / P.holdEscapeMs);
        ...
        setHoldProgress(clamp01(holdMsRef.current / P.holdEscapeMs));
        lastInZoneRef.current = 0;
        setBanner(T.escapeLabel);
        setCoachCue(lr.escapeCue);
      }
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your face — lava sensors calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} lava escapes · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const cursorOnActive =
    roundPhase === 'escape-b'
      ? inSafeStone(sample.cursor, lavaRound.stoneB)
      : inSafeStone(sample.cursor, lavaRound.stoneA);

  const renderOverlay = () => (
    <>
      <LavaShiftVisuals lavaLevel={lavaLevel} erupting={shiftFlash || roundPhase === 'shift'} />
      <AdaptiveMeter adaptation={adaptation} focus={focus} flow={flow} />
      {phase === 'play' && (
        <>
          <LavaSafeStones
            stoneA={lavaRound.stoneA}
            stoneB={lavaRound.stoneB}
            activeStone={activeStone}
            showB={roundPhase === 'shift' || roundPhase === 'escape-b'}
            stoneASunk={stoneASunk}
          ...
          {!!banner && (
            <View style={[styles.phaseBanner, shiftFlash && styles.phaseBannerShift]} pointerEvents="none">
              <Text style={styles.phaseBannerText}>{banner}</Text>
            </View>
          )}
        </>
      )}
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={LAVA_SHELL.sparkleColor} count={26} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Lava sensor calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: LAVA_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: LAVA_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🌋 {LAVA_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: LAVA_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: LAVA_SHELL.statLabel }]}>Round</Text>
              <Text style={[styles.statValue, { color: LAVA_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: LAVA_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🪨</Text>
              <Text style={[styles.statValue, { color: LAVA_SHELL.statValue }]}>{escaped}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: LAVA_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: LAVA_SHELL.statLabel }]}>Adapt</Text>
              <Text style={[styles.statValue, { color: LAVA_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Enter Volcanic Run</Text>
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: LAVA_SHELL.gold, fontSize: 10, fontWeight: '900', letterSpacing: 0.4 },
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: LAVA_SHELL.stageBorder,
    backgroundColor: LAVA_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: LAVA_SHELL.glassBorder,
  },
  phaseBannerShift: { borderColor: LAVA_SHELL.warn, backgroundColor: 'rgba(127,29,29,0.8)' },
  phaseBannerText: { color: LAVA_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: LAVA_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  errorText: { color: LAVA_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
