/**
 * OT Level 10 · Session 3 · Game 5 — Surprise Route
 *
 * Trail fork adventure: follow the planned path, then adapt when
 * a surprise fork appears to reach the new trail marker. Camera
 * tracks redirect speed, movement quality, posture, attention and holds.
 */
...
import { SurpriseRouteVisuals } from '@/components/game/occupational/level10/session3/components/SurpriseRouteVisuals';
import { SurpriseTrailOverlay } from '@/components/game/occupational/level10/session3/components/SurpriseTrailOverlay';
import { SURPRISE_ROUTE_PACING as P } from '@/components/game/occupational/level10/session3/surpriseRoutePacing';
import {
  ROUTE_SHELL,
  SURPRISE_ROUTE_ROUNDS,
  SURPRISE_ROUTE_THEME as T,
  type SurpriseRouteRound,
} from '@/components/game/occupational/level10/session3/surpriseRouteTheme';
import { inTrailZone } from '@/components/game/occupational/level10/session3/surpriseRouteUtils';
...
const VOICE_PRAISE = ['Trail star!', 'Fork hero!', 'Surprise ace!', 'Path pro!', 'Explorer win!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'route-a' | 'surprise' | 'adapt-b';

const SurpriseRouteGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const routeRound = SURPRISE_ROUTE_ROUNDS[Math.min(round, SURPRISE_ROUTE_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('route-a');
  const routeRef = useRef<SurpriseRouteRound>(routeRound);
  ...
  useEffect(() => { routeRef.current = routeRound; }, [routeRound]);
  ...
  const beginAdaptPhase = useCallback((rr: SurpriseRouteRound) => {
    roundPhaseRef.current = 'adapt-b';
    setRoundPhase('adapt-b');
    setActiveRoute('b');
    setSurpriseFlash(false);
    setBanner(T.adaptLabel);
    setCoachCue(rr.adaptCue);
    adaptStartRef.current = Date.now();
    holdMsRef.current = 0;
    setHoldProgress(0);
    speakTTS(rr.voiceRouteB, 0.85).catch(() => {});
  }, []);

  const triggerSurprise = useCallback(
    (rr: SurpriseRouteRound) => {
      roundPhaseRef.current = 'surprise';
      setRoundPhase('surprise');
      setSurpriseFlash(true);
      setBanner(T.surpriseLabel);
      setCoachCue(rr.voiceSurprise);
      speakTTS(rr.voiceSurprise, 0.88).catch(() => {});
      playSound('beep', 0.58, 1.05).catch(() => {});
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
      schedule(() => beginAdaptPhase(rr), P.surpriseFlashMs);
    },
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      await recordGame({
        gameId: 'surprise-route',
        score: done,
        accuracy,
        meta: { ...analyticsMeta, adaptation: adaptSumRef.current / Math.max(1, done) },
      });
      await logGameAndAward('surprise-route', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback(
    (r: number) => {
      const rr = SURPRISE_ROUTE_ROUNDS[Math.min(r, SURPRISE_ROUTE_ROUNDS.length - 1)]!;
      setRound(r);
      holdMsRef.current = 0;
      ...
      setCoachCue(rr.voiceRouteA);
      speakTTS(rr.voiceRouteA, 0.82).catch(() => {});

      if (surpriseTimerRef.current) clearTimeout(surpriseTimerRef.current);
      surpriseTimerRef.current = setTimeout(() => triggerSurprise(rr), P.routeAPreviewMs + P.surpriseDelayMs);
    },
    [triggerSurprise],
  );
  ...
  const tick = useCallback(() => {
    ...
    const rr = routeRef.current;
    const rp = roundPhaseRef.current;
    const target = rp === 'adapt-b' ? rr.routeB : rr.routeA;
    ...
    const inZone = inTrailZone(s.cursor, target);
    const onB = inTrailZone(s.cursor, rr.routeB);
    ...
    if (rp === 'route-a') {
      setCoachCue(inZone ? 'Good path — watch for surprise forks…' : rr.voiceRouteA);
      setBanner(T.routeLabel);
    }
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your face — trail sensors calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} surprise routes · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const showA = true;
  const showB = roundPhase === 'surprise' || roundPhase === 'adapt-b';
  const cursorOnActive =
    roundPhase === 'adapt-b'
      ? inTrailZone(sample.cursor, routeRound.routeB)
      : inTrailZone(sample.cursor, routeRound.routeA);

  const renderOverlay = () => (
    <>
      <SurpriseRouteVisuals surpriseFlash={surpriseFlash} />
      <AdaptiveMeter adaptation={adaptation} focus={focus} flow={flow} />
      {phase === 'play' && (
        <>
          <SurpriseTrailOverlay
            routeA={routeRound.routeA}
            routeB={routeRound.routeB}
            activeRoute={activeRoute}
            showA={showA}
            showB={showB}
            surpriseFlash={surpriseFlash}
            holdProgress={holdProgress}
            cursorOnActive={cursorOnActive}
          />
          <ExplorerCursor cursor={sample.cursor} leftWrist={null} rightWrist={null} accent={T.accent} />
          {!!banner && (
            <View style={[styles.phaseBanner, surpriseFlash && styles.phaseBannerSurprise]} pointerEvents="none">
              <Text style={styles.phaseBannerText}>{banner}</Text>
            </View>
          )}
        </>
      )}
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={ROUTE_SHELL.sparkleColor} count={26} />
      {phase === 'calibrate' && (
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: ROUTE_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: ROUTE_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🌿 {ROUTE_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: ROUTE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: ROUTE_SHELL.statLabel }]}>Hike</Text>
              <Text style={[styles.statValue, { color: ROUTE_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: ROUTE_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🦋</Text>
              <Text style={[styles.statValue, { color: ROUTE_SHELL.statValue }]}>{surprises}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: ROUTE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: ROUTE_SHELL.statLabel }]}>Adapt</Text>
              <Text style={[styles.statValue, { color: ROUTE_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Trail</Text>
            </Pressable>
...
export default SurpriseRouteGame;
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: ROUTE_SHELL.accent, fontSize: 10, fontWeight: '900', letterSpacing: 0.4 },
  header: { paddingHorizontal: 20, paddingTop: 8, paddingBottom: 4 },
  title: { color: '#F7FEE7', fontSize: 24, fontWeight: '900' },
  subtitle: { color: '#D9F99D', fontSize: 13, fontWeight: '600', marginTop: 4, lineHeight: 18 },
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: ROUTE_SHELL.stageBorder,
    backgroundColor: ROUTE_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    top: 14,
    alignSelf: 'center',
    backgroundColor: 'rgba(20,33,28,0.88)',
    paddingHorizontal: 20,
    paddingVertical: 8,
    borderRadius: 20,
    borderWidth: 1,
    borderColor: ROUTE_SHELL.glassBorder,
  },
  phaseBannerSurprise: { borderColor: ROUTE_SHELL.warn, backgroundColor: 'rgba(76,29,149,0.72)' },
  phaseBannerText: { color: ROUTE_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  calibWrap: { position: 'absolute', bottom: 48, left: 24, right: 24 },
  calibText: { color: '#F7FEE7', fontWeight: '800', textAlign: 'center', marginBottom: 8 },
  calibTrack: { height: 8, borderRadius: 6, backgroundColor: 'rgba(255,255,255,0.12)', overflow: 'hidden' },
  calibFill: { height: '100%', borderRadius: 6, backgroundColor: '#34D399' },
  cueWrap: {
    position: 'absolute',
    bottom: 14,
    alignSelf: 'center',
  ...
  cueText: { color: ROUTE_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  skillPill: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10, backgroundColor: 'rgba(52,211,153,0.15)' },
  skillText: { color: '#D9F99D', fontSize: 10, fontWeight: '700' },
  ...
  secondaryText: { color: '#D9F99D', fontWeight: '700' },
  errorText: { color: ROUTE_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
