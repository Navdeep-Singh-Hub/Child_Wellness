/**
 * OT Level 10 · Session 9 · Game 2 — Shopping Trip
 *
 * Market Quest: browse each store aisle, then complete checkout
...
import { ShoppingTripOverlay } from '@/components/game/occupational/level10/session9/components/ShoppingTripOverlay';
import { ShoppingTripVisuals } from '@/components/game/occupational/level10/session9/components/ShoppingTripVisuals';
import { realLifeSensoryQuality } from '@/components/game/occupational/level10/session9/realLifeSensoryUtils';
import { SHOPPING_TRIP_PACING as P } from '@/components/game/occupational/level10/session9/shoppingTripPacing';
import {
  SHOPPING_TRIP_ROUNDS,
  SHOPPING_TRIP_THEME as T,
  SHOP_SHELL,
  type ShoppingTripRound,
} from '@/components/game/occupational/level10/session9/shoppingTripTheme';
import { inBrowseAisleZone, inCheckoutZone } from '@/components/game/occupational/level10/session9/shoppingTripUtils';
...
const VOICE_PRAISE = ['Shop star!', 'Great buy!', 'Market champ!', 'Checkout ace!', 'Trip hero!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'browse' | 'buy';

const ShoppingTripGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundPhase, setRoundPhase] = useState<RoundPhase>('browse');
  const [holdProgress, setHoldProgress] = useState(0);
  const [banner, setBanner] = useState('');
  const [coachCue, setCoachCue] = useState(T.hintText);
  const [calibProgress, setCalibProgress] = useState(0);
  const [sparkleKey, setSparkleKey] = useState(0);
  const [browseScore, setBrowseScore] = useState(0);
  ...
  const tripRound = SHOPPING_TRIP_ROUNDS[Math.min(round, SHOPPING_TRIP_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('browse');
  const roundRef = useRef<ShoppingTripRound>(tripRound);
  const doneRef = useRef(false);
  const camLiveRef = useRef(camLive);
  const holdMsRef = useRef(0);
  const browseMsRef = useRef(0);
  const lastInZoneRef = useRef(0);
  const browseStartRef = useRef(0);
  const buyStartRef = useRef(0);
  ...
  useEffect(() => { roundRef.current = tripRound; }, [tripRound]);
  ...
  const beginBuyPhase = useCallback((tr: ShoppingTripRound) => {
    ...
    setHoldProgress(0);
    setShowBuy(true);
    setBanner(T.buyLabel);
    setCoachCue(tr.buyCue);
    buyStartRef.current = Date.now();
    speakTTS(tr.voiceBuy, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      await recordGame({
        gameId: 'shopping-trip',
        score: done,
        accuracy,
        meta: { ...analyticsMeta, adaptive: motorSumRef.current / Math.max(1, done) },
      });
      await logGameAndAward('shopping-trip', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const tr = SHOPPING_TRIP_ROUNDS[Math.min(r, SHOPPING_TRIP_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    browseMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    roundPhaseRef.current = 'browse';
    setRoundPhase('browse');
    setShowBuy(false);
    setBanner(T.browseLabel);
    setCoachCue(tr.voiceBrowse);
    setOnBrowse(false);
    setOnBuy(false);
    browseStartRef.current = Date.now();
    speakTTS(tr.voiceBrowse, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'browse') {
        const elapsed = now - browseStartRef.current;
        const t = clamp01(elapsed / P.fallbackBrowseMs);
        setBrowseScore(t * 0.85);
        setBuyScore(0.35);
        setCalm(0.5);
        setOnBrowse(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackBrowseMs) beginBuyPhase(tr);
      } else {
        const elapsed = now - buyStartRef.current;
        const t = clamp01(elapsed / P.fallbackBuyMs);
        setHoldProgress(t);
        setBuyScore(0.84);
        setBrowseScore(0.8);
        setCalm(0.82);
    ...
    if (rp === 'browse') {
      const atBrowse = inBrowseAisleZone(cursor, tr);

      if (atBrowse) {
        browseMsRef.current += dt;
        if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
        holdMsRef.current += dt;
      } else if (now - (lastInZoneRef.current || now) > P.browseGraceMs) {
        holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
        lastInZoneRef.current = 0;
      }

      const holdPct = clamp01(holdMsRef.current / P.browseHoldMs);
      const q = realLifeSensoryQuality(atBrowse, false, holdPct, s.postureQuality, s.attentionScore, smooth, 'enter');

      setOnBrowse(atBrowse);
      setOnBuy(false);
      setBrowseScore(holdPct * 0.65 + (atBrowse ? 0.35 : smooth * 0.2));
      setBuyScore(0.3);
      ...
      setBanner(T.holdBrowseLabel);
      setCoachCue(atBrowse ? `${tr.label} — browse the aisle!` : tr.voiceBrowse);

      if (holdMsRef.current >= P.browseHoldMs) beginBuyPhase(tr);
    ...
    const atBuy =
      inCheckoutZone(cursor, tr) &&
      s.postureQuality >= P.minPostureForBuy &&
      s.attentionScore >= P.minAttentionForBuy;
    const holdPct = clamp01(holdMsRef.current / P.buyHoldMs);
    const browsePct = clamp01(browseMsRef.current / P.browseHoldMs);
    const q = realLifeSensoryQuality(false, atBuy, holdPct, s.postureQuality, s.attentionScore, smooth, 'participate');

    setOnBrowse(false);
    setOnBuy(atBuy);
    setBrowseScore(browsePct);
    setBuyScore(atBuy ? holdPct * 0.65 + smooth * 0.35 : smooth * 0.25);
    setCalm(atBuy ? smooth * 0.55 + s.attentionScore * 0.45 : smooth * 0.3);
    setFocus(s.attentionScore);
    setQuality(q);
    qualSumRef.current += q;
    qualCountRef.current += 1;

    setBanner(atBuy ? T.holdBuyLabel : T.buyLabel);
    setCoachCue(atBuy ? tr.buyCue : tr.voiceBuy);

    if (atBuy) {
      if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
      holdMsRef.current += dt;
      setHoldProgress(clamp01(holdMsRef.current / P.buyHoldMs));
      if (holdMsRef.current >= P.buyHoldMs) completeRound();
    } else if (now - (lastInZoneRef.current || now) > P.buyGraceMs) {
      holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
      setHoldProgress(clamp01(holdMsRef.current / P.buyHoldMs));
      lastInZoneRef.current = 0;
    }
    ...
    recordTick(dt, {
      upright: s.postureQuality > P.minPostureForBuy,
      still: smooth > 0.35,
      quality: q,
    });
  }, [beginBuyPhase, completeRound, recordTick]);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your full body — store sensors calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} aisles shopped · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <ShoppingTripVisuals buyPhase={roundPhase === 'buy'} />
      <AttentionMeter
        find={browseScore}
        focus={buyScore}
        calm={calm}
        title="SHOP"
        labels={{ find: 'BROWSE', focus: 'BUY', calm: 'CALM' }}
        style={{
          glassBorder: SHOP_SHELL.glassBorder,
          statLabel: SHOP_SHELL.statLabel,
          accent: T.accent,
          fillColors: ['#F97316', '#22C55E', '#A7F3D0'],
        }}
      />
      {phase === 'play' && (
        <>
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={SHOP_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Store calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: SHOP_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: SHOP_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🛒 {SHOP_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: SHOP_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: SHOP_SHELL.statLabel }]}>Aisle</Text>
              <Text style={[styles.statValue, { color: SHOP_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: SHOP_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🛒</Text>
              <Text style={[styles.statValue, { color: SHOP_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: SHOP_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: SHOP_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: SHOP_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Shopping Trip</Text>
            </Pressable>
...
export default ShoppingTripGame;
...
const styles = StyleSheet.create({
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: SHOP_SHELL.stageBorder,
    backgroundColor: SHOP_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: SHOP_SHELL.glassBorder,
  },
  phaseBannerText: { color: SHOP_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: SHOP_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  skillPill: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10, backgroundColor: 'rgba(249,115,22,0.2)' },
  skillText: { color: '#A7F3D0', fontSize: 10, fontWeight: '700' },
  primaryBtn: { borderRadius: 16, paddingVertical: 14, alignItems: 'center' },
  primaryBtnText: { color: '#7C2D12', fontSize: 16, fontWeight: '900' },
  secondaryBtn: { paddingVertical: 8, alignItems: 'center' },
  secondaryText: { color: '#A7F3D0', fontWeight: '700' },
  errorText: { color: SHOP_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
