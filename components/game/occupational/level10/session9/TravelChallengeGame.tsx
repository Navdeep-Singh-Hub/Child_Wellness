/**
 * OT Level 10 · Session 9 · Game 4 — Travel Challenge
 *
 * Journey Adventure: pack at each travel stop, then journey
...
import { TravelChallengeOverlay } from '@/components/game/occupational/level10/session9/components/TravelChallengeOverlay';
import { TravelChallengeVisuals } from '@/components/game/occupational/level10/session9/components/TravelChallengeVisuals';
import { realLifeSensoryQuality } from '@/components/game/occupational/level10/session9/realLifeSensoryUtils';
import { TRAVEL_CHALLENGE_PACING as P } from '@/components/game/occupational/level10/session9/travelChallengePacing';
import {
  TRAVEL_CHALLENGE_ROUNDS,
  TRAVEL_CHALLENGE_THEME as T,
  TRAVEL_SHELL,
  type TravelChallengeRound,
} from '@/components/game/occupational/level10/session9/travelChallengeTheme';
import { inPackZone, inTravelZone } from '@/components/game/occupational/level10/session9/travelChallengeUtils';
...
const VOICE_PRAISE = ['Travel star!', 'Great journey!', 'Trip champ!', 'Adventure ace!', 'Travel hero!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'pack' | 'travel';

const TravelChallengeGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundPhase, setRoundPhase] = useState<RoundPhase>('pack');
  ...
  const [onPack, setOnPack] = useState(false);
  ...
  const travelRound = TRAVEL_CHALLENGE_ROUNDS[Math.min(round, TRAVEL_CHALLENGE_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('pack');
  const roundRef = useRef<TravelChallengeRound>(travelRound);
  const doneRef = useRef(false);
  const camLiveRef = useRef(camLive);
  const holdMsRef = useRef(0);
  const packMsRef = useRef(0);
  const lastInZoneRef = useRef(0);
  const packStartRef = useRef(0);
  ...
  useEffect(() => { roundRef.current = travelRound; }, [travelRound]);
  ...
  const beginTravelPhase = useCallback((tr: TravelChallengeRound) => {
    roundPhaseRef.current = 'travel';
    setRoundPhase('travel');
    holdMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    setShowTravel(true);
    setBanner(T.travelLabel);
    setCoachCue(tr.travelCue);
    travelStartRef.current = Date.now();
    speakTTS(tr.voiceTravel, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      await recordGame({
        gameId: 'travel-challenge',
        score: done,
        accuracy,
        meta: { ...analyticsMeta, adaptive: motorSumRef.current / Math.max(1, done) },
      });
      await logGameAndAward('travel-challenge', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const tr = TRAVEL_CHALLENGE_ROUNDS[Math.min(r, TRAVEL_CHALLENGE_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    packMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    roundPhaseRef.current = 'pack';
    setRoundPhase('pack');
    setShowTravel(false);
    setBanner(T.packLabel);
    setCoachCue(tr.voicePack);
    setOnPack(false);
    setOnTravel(false);
    packStartRef.current = Date.now();
    speakTTS(tr.voicePack, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'pack') {
        const elapsed = now - packStartRef.current;
        const t = clamp01(elapsed / P.fallbackPackMs);
        setPackScore(t * 0.85);
        setTravelScore(0.35);
        setCalm(0.5);
        setOnPack(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackPackMs) beginTravelPhase(tr);
      } else {
        const elapsed = now - travelStartRef.current;
        const t = clamp01(elapsed / P.fallbackTravelMs);
        setHoldProgress(t);
        setTravelScore(0.84);
        setPackScore(0.8);
        setCalm(0.82);
    ...
    if (rp === 'pack') {
      const atPack = inPackZone(cursor, tr);

      if (atPack) {
        packMsRef.current += dt;
        if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
        holdMsRef.current += dt;
      } else if (now - (lastInZoneRef.current || now) > P.packGraceMs) {
        holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
        lastInZoneRef.current = 0;
      }

      const holdPct = clamp01(holdMsRef.current / P.packHoldMs);
      const q = realLifeSensoryQuality(atPack, false, holdPct, s.postureQuality, s.attentionScore, smooth, 'enter');

      setOnPack(atPack);
      setOnTravel(false);
      setPackScore(holdPct * 0.65 + (atPack ? 0.35 : smooth * 0.2));
      setTravelScore(0.3);
      ...
      setBanner(T.holdPackLabel);
      setCoachCue(atPack ? `${tr.label} — pack your bag!` : tr.voicePack);

      if (holdMsRef.current >= P.packHoldMs) beginTravelPhase(tr);
    ...
    const atTravel =
      inTravelZone(cursor, tr) &&
      s.postureQuality >= P.minPostureForTravel &&
      s.attentionScore >= P.minAttentionForTravel;
    const holdPct = clamp01(holdMsRef.current / P.travelHoldMs);
    const packPct = clamp01(packMsRef.current / P.packHoldMs);
    const q = realLifeSensoryQuality(false, atTravel, holdPct, s.postureQuality, s.attentionScore, smooth, 'participate');

    setOnPack(false);
    setOnTravel(atTravel);
    setPackScore(packPct);
    setTravelScore(atTravel ? holdPct * 0.65 + smooth * 0.35 : smooth * 0.25);
    setCalm(atTravel ? smooth * 0.55 + s.attentionScore * 0.45 : smooth * 0.3);
    setFocus(s.attentionScore);
    setQuality(q);
    qualSumRef.current += q;
    qualCountRef.current += 1;

    setBanner(atTravel ? T.holdTravelLabel : T.travelLabel);
    setCoachCue(atTravel ? tr.travelCue : tr.voiceTravel);

    if (atTravel) {
      if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
      holdMsRef.current += dt;
      setHoldProgress(clamp01(holdMsRef.current / P.travelHoldMs));
      if (holdMsRef.current >= P.travelHoldMs) completeRound();
    } else if (now - (lastInZoneRef.current || now) > P.travelGraceMs) {
      holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
      setHoldProgress(clamp01(holdMsRef.current / P.travelHoldMs));
      lastInZoneRef.current = 0;
    }
    ...
    recordTick(dt, {
      upright: s.postureQuality > P.minPostureForTravel,
      still: smooth > 0.35,
      quality: q,
    });
  }, [beginTravelPhase, completeRound, recordTick]);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your full body — travel sensors calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} stops traveled · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <TravelChallengeVisuals travelPhase={roundPhase === 'travel'} />
      <AttentionMeter
        find={packScore}
        focus={travelScore}
        calm={calm}
        title="TRIP"
        labels={{ find: 'PACK', focus: 'TRAVEL', calm: 'CALM' }}
        style={{
          glassBorder: TRAVEL_SHELL.glassBorder,
          statLabel: TRAVEL_SHELL.statLabel,
          accent: T.accent,
          fillColors: ['#0EA5E9', '#F97316', '#BAE6FD'],
        }}
      />
      {phase === 'play' && (
        <>
          <TravelChallengeOverlay
            round={travelRound}
            phase={roundPhase}
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={TRAVEL_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Travel calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: TRAVEL_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: TRAVEL_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>✈️ {TRAVEL_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: TRAVEL_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: TRAVEL_SHELL.statLabel }]}>Stop</Text>
              <Text style={[styles.statValue, { color: TRAVEL_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: TRAVEL_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>✈️</Text>
              <Text style={[styles.statValue, { color: TRAVEL_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: TRAVEL_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: TRAVEL_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: TRAVEL_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Travel Challenge</Text>
            </Pressable>
...
export default TravelChallengeGame;
...
const styles = StyleSheet.create({
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: TRAVEL_SHELL.stageBorder,
    backgroundColor: TRAVEL_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: TRAVEL_SHELL.glassBorder,
  },
  phaseBannerText: { color: TRAVEL_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: TRAVEL_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  errorText: { color: TRAVEL_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
