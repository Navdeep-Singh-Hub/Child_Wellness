/**
 * OT Level 10 · Session 5 · Game 3 — Meal Mission
 *
 * Kitchen Adventure: prepare at each meal station, then hold
 * a meal-ready stance. Camera tracks movement quality, posture,
...
import { MealMissionOverlay } from '@/components/game/occupational/level10/session5/components/MealMissionOverlay';
import { MealMissionVisuals } from '@/components/game/occupational/level10/session5/components/MealMissionVisuals';
import { functionalDailyQuality } from '@/components/game/occupational/level10/session5/functionalDailyUtils';
import { MEAL_MISSION_PACING as P } from '@/components/game/occupational/level10/session5/mealMissionPacing';
import {
  MEAL_MISSION_ROUNDS,
  MEAL_MISSION_THEME as T,
  MEAL_SHELL,
  type MealMissionRound,
} from '@/components/game/occupational/level10/session5/mealMissionTheme';
import { inPrepareZone, inReadyZone } from '@/components/game/occupational/level10/session5/mealMissionUtils';
...
const VOICE_PRAISE = ['Chef star!', 'Meal hero!', 'Kitchen ace!', 'Tasty helper!', 'Daily champ!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'prepare' | 'ready';

const MealMissionGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const mealRound = MEAL_MISSION_ROUNDS[Math.min(round, MEAL_MISSION_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('prepare');
  const roundRef = useRef<MealMissionRound>(mealRound);
  ...
  useEffect(() => { roundRef.current = mealRound; }, [mealRound]);
  ...
  const beginReadyPhase = useCallback((mr: MealMissionRound) => {
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      ...
      await logGameAndAward('meal-mission', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const mr = MEAL_MISSION_ROUNDS[Math.min(r, MEAL_MISSION_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
  ...
  const tick = useCallback(() => {
    ...
    if (rp === 'prepare') {
      const atPrepare = inPrepareZone(cursor, mr);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your face — kitchen routine calibrating…');
  ...
  const renderOverlay = () => (
    <>
      <MealMissionVisuals readyPhase={roundPhase === 'ready'} />
      <FunctionalMeter
        participation={prepareScore}
        focus={focus}
        ready={readyScore}
        style={{
          glassBorder: MEAL_SHELL.glassBorder,
          statLabel: MEAL_SHELL.statLabel,
          accent: MEAL_SHELL.accent,
          fillColors: ['#F97316', '#EF4444', '#FDE68A'],
        }}
      />
      {phase === 'play' && (
        <>
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={MEAL_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Kitchen routine calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: MEAL_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: MEAL_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🍳 {MEAL_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: MEAL_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: MEAL_SHELL.statLabel }]}>Step</Text>
              <Text style={[styles.statValue, { color: MEAL_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: MEAL_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🍳</Text>
              <Text style={[styles.statValue, { color: MEAL_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: MEAL_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: MEAL_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: MEAL_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Meal Mission</Text>
            </Pressable>
...
export default MealMissionGame;
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: MEAL_SHELL.accent, fontSize: 10, fontWeight: '900', letterSpacing: 0.4 },
  header: { paddingHorizontal: 20, paddingTop: 8, paddingBottom: 4 },
  title: { color: '#FFF7ED', fontSize: 24, fontWeight: '900' },
  subtitle: { color: '#FECACA', fontSize: 13, fontWeight: '600', marginTop: 4, lineHeight: 18 },
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: MEAL_SHELL.stageBorder,
    backgroundColor: MEAL_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: MEAL_SHELL.glassBorder,
  },
  phaseBannerText: { color: MEAL_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: MEAL_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  skillPill: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10, backgroundColor: 'rgba(249,115,22,0.15)' },
  skillText: { color: '#FECACA', fontSize: 10, fontWeight: '700' },
  ...
  secondaryText: { color: '#FECACA', fontWeight: '700' },
  errorText: { color: MEAL_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
