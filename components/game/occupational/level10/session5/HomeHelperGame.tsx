/**
 * OT Level 10 · Session 5 · Game 2 — Home Helper
 *
 * Home Helper Quest: prepare at each home chore station, then hold
 * a helper-ready stance. Camera tracks movement quality, posture,
...
import { HomeHelperOverlay } from '@/components/game/occupational/level10/session5/components/HomeHelperOverlay';
import { HomeHelperVisuals } from '@/components/game/occupational/level10/session5/components/HomeHelperVisuals';
import { functionalDailyQuality } from '@/components/game/occupational/level10/session5/functionalDailyUtils';
import { HOME_HELPER_PACING as P } from '@/components/game/occupational/level10/session5/homeHelperPacing';
import {
  HOME_HELPER_ROUNDS,
  HOME_HELPER_THEME as T,
  HOME_SHELL,
  type HomeHelperRound,
} from '@/components/game/occupational/level10/session5/homeHelperTheme';
import { inPrepareZone, inReadyZone } from '@/components/game/occupational/level10/session5/homeHelperUtils';
...
const VOICE_PRAISE = ['Helper star!', 'Home hero!', 'Chore ace!', 'Great helper!', 'Daily champ!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'prepare' | 'ready';

const HomeHelperGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const homeRound = HOME_HELPER_ROUNDS[Math.min(round, HOME_HELPER_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('prepare');
  const roundRef = useRef<HomeHelperRound>(homeRound);
  ...
  useEffect(() => { roundRef.current = homeRound; }, [homeRound]);
  ...
  const beginReadyPhase = useCallback((hr: HomeHelperRound) => {
    ...
    setCoachCue(hr.readyCue);
    readyStartRef.current = Date.now();
    speakTTS(hr.voiceReady, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      ...
      await logGameAndAward('home-helper', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const hr = HOME_HELPER_ROUNDS[Math.min(r, HOME_HELPER_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    ...
    setCoachCue(hr.voicePrepare);
    setOnPrepare(false);
    setOnReady(false);
    prepareStartRef.current = Date.now();
    speakTTS(hr.voicePrepare, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'prepare') {
        ...
        setOnPrepare(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackPrepareMs) beginReadyPhase(hr);
      } else {
    ...
    if (rp === 'prepare') {
      const atPrepare = inPrepareZone(cursor, hr);
      ...
      setCoachCue(atPrepare ? `${hr.label} — hold steady!` : hr.voicePrepare);

      if (holdMsRef.current >= P.prepareHoldMs) beginReadyPhase(hr);

      recordTick(dt, { upright: s.postureQuality > 0.35, still: smooth > 0.35, quality: q });
      return;
    }

    const atReady = inReadyZone(cursor, hr) && s.postureQuality >= P.minPostureForReady;
    ...
    setCoachCue(atReady ? hr.readyCue : hr.voiceReady);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your face — home helper calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} chores completed · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <HomeHelperVisuals readyPhase={roundPhase === 'ready'} />
      <FunctionalMeter
        participation={prepareScore}
        focus={focus}
        ready={readyScore}
        style={{
          glassBorder: HOME_SHELL.glassBorder,
          statLabel: HOME_SHELL.statLabel,
          accent: HOME_SHELL.accent,
          fillColors: ['#FBBF24', '#84CC16', '#FDE68A'],
        }}
      />
      {phase === 'play' && (
        <>
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={HOME_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Home helper calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: HOME_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: HOME_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🏠 {HOME_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: HOME_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: HOME_SHELL.statLabel }]}>Chore</Text>
              <Text style={[styles.statValue, { color: HOME_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: HOME_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>✨</Text>
              <Text style={[styles.statValue, { color: HOME_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: HOME_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: HOME_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: HOME_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Helper Quest</Text>
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: HOME_SHELL.accent, fontSize: 10, fontWeight: '900', letterSpacing: 0.4 },
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: HOME_SHELL.stageBorder,
    backgroundColor: HOME_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: HOME_SHELL.glassBorder,
  },
  phaseBannerText: { color: HOME_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: HOME_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  errorText: { color: HOME_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
