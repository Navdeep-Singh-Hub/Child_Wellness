/**
 * OT Level 10 · Session 5 · Game 4 — Door Challenge
 *
 * Transition Trail: approach each doorway station, then hold
 * a transition-ready stance. Camera tracks movement quality, posture,
...
import { DoorChallengeOverlay } from '@/components/game/occupational/level10/session5/components/DoorChallengeOverlay';
import { DoorChallengeVisuals } from '@/components/game/occupational/level10/session5/components/DoorChallengeVisuals';
import { functionalDailyQuality } from '@/components/game/occupational/level10/session5/functionalDailyUtils';
import { DOOR_CHALLENGE_PACING as P } from '@/components/game/occupational/level10/session5/doorChallengePacing';
import {
  DOOR_CHALLENGE_ROUNDS,
  DOOR_CHALLENGE_THEME as T,
  DOOR_SHELL,
  type DoorChallengeRound,
} from '@/components/game/occupational/level10/session5/doorChallengeTheme';
import { inApproachZone, inReadyZone } from '@/components/game/occupational/level10/session5/doorChallengeUtils';
...
const VOICE_PRAISE = ['Door star!', 'Transition hero!', 'Hallway ace!', 'Great approach!', 'Daily champ!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'prepare' | 'ready';

const DoorChallengeGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const doorRound = DOOR_CHALLENGE_ROUNDS[Math.min(round, DOOR_CHALLENGE_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('prepare');
  const roundRef = useRef<DoorChallengeRound>(doorRound);
  ...
  useEffect(() => { roundRef.current = doorRound; }, [doorRound]);
  ...
  const beginReadyPhase = useCallback((mr: DoorChallengeRound) => {
    ...
    setCoachCue(mr.readyCue);
    readyStartRef.current = Date.now();
    speakTTS(mr.voiceReady, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      await recordGame({
        gameId: 'door-challenge',
        score: done,
        accuracy,
        meta: { ...analyticsMeta, adaptive: motorSumRef.current / Math.max(1, done) },
      });
      await logGameAndAward('door-challenge', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const mr = DOOR_CHALLENGE_ROUNDS[Math.min(r, DOOR_CHALLENGE_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    ...
    setCoachCue(mr.voicePrepare);
    setOnPrepare(false);
    setOnReady(false);
    prepareStartRef.current = Date.now();
    speakTTS(mr.voicePrepare, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'prepare') {
        ...
        setOnPrepare(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackPrepareMs) beginReadyPhase(mr);
      } else {
    ...
    if (rp === 'prepare') {
      const atPrepare = inApproachZone(cursor, mr);
      ...
      setCoachCue(atPrepare ? `${mr.label} — hold steady!` : mr.voicePrepare);

      if (holdMsRef.current >= P.prepareHoldMs) beginReadyPhase(mr);

      recordTick(dt, { upright: s.postureQuality > 0.35, still: smooth > 0.35, quality: q });
      return;
    }

    const atReady = inReadyZone(cursor, mr) && s.postureQuality >= P.minPostureForReady;
    ...
    setCoachCue(atReady ? mr.readyCue : mr.voiceReady);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your face — door transition calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} doors completed · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <DoorChallengeVisuals readyPhase={roundPhase === 'ready'} />
      <FunctionalMeter
        participation={prepareScore}
        focus={focus}
        ready={readyScore}
        style={{
          glassBorder: DOOR_SHELL.glassBorder,
          statLabel: DOOR_SHELL.statLabel,
          accent: DOOR_SHELL.accent,
          fillColors: ['#64748B', '#92400E', '#FDE68A'],
        }}
      />
      {phase === 'play' && (
        <>
          <DoorChallengeOverlay
            round={doorRound}
            phase={roundPhase}
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={DOOR_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Door transition calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: DOOR_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: DOOR_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🚪 {DOOR_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: DOOR_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: DOOR_SHELL.statLabel }]}>Door</Text>
              <Text style={[styles.statValue, { color: DOOR_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: DOOR_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🗝️</Text>
              <Text style={[styles.statValue, { color: DOOR_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: DOOR_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: DOOR_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: DOOR_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Door Challenge</Text>
            </Pressable>
...
export default DoorChallengeGame;
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: DOOR_SHELL.accent, fontSize: 10, fontWeight: '900', letterSpacing: 0.4 },
  header: { paddingHorizontal: 20, paddingTop: 8, paddingBottom: 4 },
  title: { color: '#F8FAFC', fontSize: 24, fontWeight: '900' },
  subtitle: { color: '#CBD5E1', fontSize: 13, fontWeight: '600', marginTop: 4, lineHeight: 18 },
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: DOOR_SHELL.stageBorder,
    backgroundColor: DOOR_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: DOOR_SHELL.glassBorder,
  },
  phaseBannerText: { color: DOOR_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  calibFill: { height: '100%', borderRadius: 6, backgroundColor: '#64748B' },
  cueWrap: {
    position: 'absolute',
    bottom: 14,
    alignSelf: 'center',
  ...
  cueText: { color: DOOR_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  skillPill: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10, backgroundColor: 'rgba(100,116,139,0.2)' },
  skillText: { color: '#CBD5E1', fontSize: 10, fontWeight: '700' },
  primaryBtn: { borderRadius: 16, paddingVertical: 14, alignItems: 'center' },
  primaryBtnText: { color: '#1E293B', fontSize: 16, fontWeight: '900' },
  secondaryBtn: { paddingVertical: 8, alignItems: 'center' },
  secondaryText: { color: '#CBD5E1', fontWeight: '700' },
  errorText: { color: DOOR_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
