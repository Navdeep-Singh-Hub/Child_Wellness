import { HOME_SHELL, type HomeHelperRound } from '@/components/game/occupational/level10/session5/homeHelperTheme';
import React from 'react';
import { StyleSheet, Text, View } from 'react-native';
import Svg, { Circle } from 'react-native-svg';

type Props = {
  round: HomeHelperRound;
  phase: 'prepare' | 'ready';
  holdProgress: number;
  onPrepare: boolean;
  onReady: boolean;
  ...
  return (
    <View style={[styles.padWrap, { left: `${point.x * 100}%`, top: `${point.y * 100}%` }]}>
      ...
      <View
        style={[
          styles.pad,
          { borderColor: variant === 'prepare' ? HOME_SHELL.prepare : color, shadowColor: color },
      ...
      <Text style={[styles.padLabel, { color: variant === 'prepare' ? HOME_SHELL.prepare : HOME_SHELL.ready }]}>
        {label}
      </Text>
    </View>
  );
...
export const HomeHelperOverlay: React.FC<Props> = ({
  round,
  phase,
  holdProgress,
  onPrepare,
