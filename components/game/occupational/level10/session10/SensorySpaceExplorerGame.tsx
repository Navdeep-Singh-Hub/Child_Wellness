/**
 * OT Level 10 · Session 10 · Game 2 — Space Explorer
 *
 * Cosmic Adventure: scan each space zone, then fly
...
import { SpaceExplorerOverlay } from '@/components/game/occupational/level10/session10/components/SpaceExplorerOverlay';
import { SpaceExplorerVisuals } from '@/components/game/occupational/level10/session10/components/SpaceExplorerVisuals';
import { sensoryIntegrationQuality } from '@/components/game/occupational/level10/session10/sensoryIntegrationUtils';
import { SPACE_EXPLORER_PACING as P } from '@/components/game/occupational/level10/session10/spaceExplorerPacing';
import {
  SPACE_EXPLORER_ROUNDS,
  SPACE_EXPLORER_THEME as T,
  SPACE_SHELL,
  type SpaceExplorerRound,
} from '@/components/game/occupational/level10/session10/spaceExplorerTheme';
import { inFlyZone, inScanZone } from '@/components/game/occupational/level10/session10/spaceExplorerUtils';
...
const VOICE_PRAISE = ['Space star!', 'Great flight!', 'Cosmic champ!', 'Explorer ace!', 'Galaxy hero!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'scan' | 'fly';

const SensorySpaceExplorerGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({
  onBack,
  onComplete,
}) => {
  ...
  const [roundPhase, setRoundPhase] = useState<RoundPhase>('scan');
  ...
  const [flyScore, setFlyScore] = useState(0);
  ...
  const explorerRound = SPACE_EXPLORER_ROUNDS[Math.min(round, SPACE_EXPLORER_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('scan');
  const roundRef = useRef<SpaceExplorerRound>(explorerRound);
  ...
  const beginFlyPhase = useCallback((sr: SpaceExplorerRound) => {
    roundPhaseRef.current = 'fly';
    setRoundPhase('fly');
    holdMsRef.current = 0;
    lastInZoneRef.current = 0;
    setHoldProgress(0);
    setShowFly(true);
    setBanner(T.flyLabel);
    setCoachCue(sr.flyCue);
    flyStartRef.current = Date.now();
    speakTTS(sr.voiceFly, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      await recordGame({
        gameId: 'sensory-space-explorer',
        score: done,
        accuracy,
        meta: { ...analyticsMeta, adaptive: motorSumRef.current / Math.max(1, done) },
      });
      await logGameAndAward('sensory-space-explorer', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const sr = SPACE_EXPLORER_ROUNDS[Math.min(r, SPACE_EXPLORER_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    scanMsRef.current = 0;
    lastInZoneRef.current = 0;
    ...
    setCoachCue(sr.voiceScan);
    setOnScan(false);
    setOnFly(false);
    scanStartRef.current = Date.now();
    speakTTS(sr.voiceScan, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'scan') {
        ...
        const t = clamp01(elapsed / P.fallbackScanMs);
        setScanScore(t * 0.85);
        setFlyScore(0.35);
        setCalm(0.5);
        setOnScan(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackScanMs) beginFlyPhase(sr);
      } else {
        const elapsed = now - flyStartRef.current;
        const t = clamp01(elapsed / P.fallbackFlyMs);
        setHoldProgress(t);
        setFlyScore(0.84);
        setScanScore(0.8);
        setCalm(0.82);
    ...
    if (rp === 'scan') {
      const atScan = inScanZone(cursor, sr);

      if (atScan) {
        scanMsRef.current += dt;
        if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
        holdMsRef.current += dt;
      } else if (now - (lastInZoneRef.current || now) > P.scanGraceMs) {
        holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
        lastInZoneRef.current = 0;
      }

      const holdPct = clamp01(holdMsRef.current / P.scanHoldMs);
      const q = sensoryIntegrationQuality(atScan, false, holdPct, s.postureQuality, s.attentionScore, smooth, 'scout');

      setOnScan(atScan);
      setOnFly(false);
      setScanScore(holdPct * 0.65 + (atScan ? 0.35 : smooth * 0.2));
      setFlyScore(0.3);
      ...
      setBanner(T.holdScanLabel);
      setCoachCue(atScan ? `${sr.label} — scan the zone!` : sr.voiceScan);

      if (holdMsRef.current >= P.scanHoldMs) beginFlyPhase(sr);
    ...
    const atFly =
      inFlyZone(cursor, sr) &&
      s.postureQuality >= P.minPostureForFly &&
      s.attentionScore >= P.minAttentionForFly;
    const holdPct = clamp01(holdMsRef.current / P.flyHoldMs);
    const scanPct = clamp01(scanMsRef.current / P.scanHoldMs);
    const q = sensoryIntegrationQuality(false, atFly, holdPct, s.postureQuality, s.attentionScore, smooth, 'trek');

    setOnScan(false);
    setOnFly(atFly);
    setScanScore(scanPct);
    setFlyScore(atFly ? holdPct * 0.65 + smooth * 0.35 : smooth * 0.25);
    setCalm(atFly ? smooth * 0.55 + s.attentionScore * 0.45 : smooth * 0.3);
    setFocus(s.attentionScore);
    setQuality(q);
    qualSumRef.current += q;
    qualCountRef.current += 1;

    setBanner(atFly ? T.holdFlyLabel : T.flyLabel);
    setCoachCue(atFly ? sr.flyCue : sr.voiceFly);

    if (atFly) {
      if (lastInZoneRef.current === 0) lastInZoneRef.current = now;
      holdMsRef.current += dt;
      setHoldProgress(clamp01(holdMsRef.current / P.flyHoldMs));
      if (holdMsRef.current >= P.flyHoldMs) completeRound();
    } else if (now - (lastInZoneRef.current || now) > P.flyGraceMs) {
      holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
      setHoldProgress(clamp01(holdMsRef.current / P.flyHoldMs));
      lastInZoneRef.current = 0;
    }
    ...
    recordTick(dt, {
      upright: s.postureQuality > P.minPostureForFly,
      still: smooth > 0.35,
      quality: q,
    });
  }, [beginFlyPhase, completeRound, recordTick]);
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your full body — space sensors calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} zones flown · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <SpaceExplorerVisuals flyPhase={roundPhase === 'fly'} />
      <AttentionMeter
        find={scanScore}
        focus={flyScore}
        calm={calm}
        title="SPACE"
        labels={{ find: 'SCAN', focus: 'FLY', calm: 'CALM' }}
        style={{
          glassBorder: SPACE_SHELL.glassBorder,
          statLabel: SPACE_SHELL.statLabel,
          accent: T.accent,
          fillColors: ['#6366F1', '#22D3EE', '#C4B5FD'],
        }}
      />
      {phase === 'play' && (
        <>
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={SPACE_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Space calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: SPACE_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: SPACE_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🚀 {SPACE_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: SPACE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: SPACE_SHELL.statLabel }]}>Zone</Text>
              <Text style={[styles.statValue, { color: SPACE_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: SPACE_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🚀</Text>
              <Text style={[styles.statValue, { color: SPACE_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: SPACE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: SPACE_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: SPACE_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Space Explorer</Text>
            </Pressable>
...
export default SensorySpaceExplorerGame;
...
const styles = StyleSheet.create({
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: SPACE_SHELL.stageBorder,
    backgroundColor: SPACE_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: SPACE_SHELL.glassBorder,
  },
  phaseBannerText: { color: SPACE_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: SPACE_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  skillPill: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10, backgroundColor: 'rgba(99,102,241,0.2)' },
  skillText: { color: '#C4B5FD', fontSize: 10, fontWeight: '700' },
  primaryBtn: { borderRadius: 16, paddingVertical: 14, alignItems: 'center' },
  primaryBtnText: { color: '#312E81', fontSize: 16, fontWeight: '900' },
  secondaryBtn: { paddingVertical: 8, alignItems: 'center' },
  secondaryText: { color: '#C4B5FD', fontWeight: '700' },
  errorText: { color: SPACE_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
