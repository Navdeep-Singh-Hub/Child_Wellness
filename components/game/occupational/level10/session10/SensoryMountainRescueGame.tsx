/**
 * OT Level 10 · Session 10 · Game 4 — Mountain Rescue
 *
 * Alpine Quest: spot each rescue site, then climb
...
import { MountainRescueOverlay } from '@/components/game/occupational/level10/session10/components/MountainRescueOverlay';
import { MountainRescueVisuals } from '@/components/game/occupational/level10/session10/components/MountainRescueVisuals';
import { MOUNTAIN_RESCUE_PACING as P } from '@/components/game/occupational/level10/session10/mountainRescuePacing';
import {
  MOUNTAIN_RESCUE_ROUNDS,
  MOUNTAIN_RESCUE_THEME as T,
  MOUNTAIN_SHELL,
  type MountainRescueRound,
} from '@/components/game/occupational/level10/session10/mountainRescueTheme';
import { inRescueZone, inSpotZone } from '@/components/game/occupational/level10/session10/mountainRescueUtils';
...
const VOICE_PRAISE = ['Mountain star!', 'Great rescue!', 'Alpine champ!', 'Rescue ace!', 'Summit hero!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'spot' | 'rescue';

const SensoryMountainRescueGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({
  onBack,
  onComplete,
}) => {
  ...
  const rescueRound = MOUNTAIN_RESCUE_ROUNDS[Math.min(round, MOUNTAIN_RESCUE_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('spot');
  const roundRef = useRef<MountainRescueRound>(rescueRound);
  ...
  useEffect(() => { roundRef.current = rescueRound; }, [rescueRound]);
  ...
  const beginRescuePhase = useCallback((mr: MountainRescueRound) => {
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      await recordGame({
        gameId: 'sensory-mountain-rescue',
        score: done,
        accuracy,
        meta: { ...analyticsMeta, adaptive: motorSumRef.current / Math.max(1, done) },
      });
      await logGameAndAward('sensory-mountain-rescue', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const mr = MOUNTAIN_RESCUE_ROUNDS[Math.min(r, MOUNTAIN_RESCUE_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    spotMsRef.current = 0;
    lastInZoneRef.current = 0;
  ...
  const tick = useCallback(() => {
    ...
    if (rp === 'spot') {
      const atSpot = inSpotZone(cursor, mr);
      ...
      const q = sensoryIntegrationQuality(atSpot, false, holdPct, s.postureQuality, s.attentionScore, smooth, 'scout');
      ...
      setCoachCue(atSpot ? `${mr.label} — spot the site!` : mr.voiceSpot);
    ...
    const q = sensoryIntegrationQuality(false, atRescue, holdPct, s.postureQuality, s.attentionScore, smooth, 'trek');
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your full body — mountain sensors calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} sites rescued · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <MountainRescueVisuals rescuePhase={roundPhase === 'rescue'} />
      <AttentionMeter
        find={spotScore}
        focus={rescueScore}
        calm={calm}
        ...
        style={{
          glassBorder: MOUNTAIN_SHELL.glassBorder,
          statLabel: MOUNTAIN_SHELL.statLabel,
          accent: T.accent,
          fillColors: ['#0EA5E9', '#22C55E', '#E0F2FE'],
        }}
      />
      {phase === 'play' && (
        <>
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={MOUNTAIN_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Mountain calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: MOUNTAIN_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: MOUNTAIN_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🏔️ {MOUNTAIN_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: MOUNTAIN_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: MOUNTAIN_SHELL.statLabel }]}>Site</Text>
              <Text style={[styles.statValue, { color: MOUNTAIN_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: MOUNTAIN_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🏔️</Text>
              <Text style={[styles.statValue, { color: MOUNTAIN_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: MOUNTAIN_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: MOUNTAIN_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: MOUNTAIN_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Mountain Rescue</Text>
            </Pressable>
...
export default SensoryMountainRescueGame;
...
const styles = StyleSheet.create({
  ...
  subtitle: { color: '#E0F2FE', fontSize: 13, fontWeight: '600', marginTop: 4, lineHeight: 18 },
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: MOUNTAIN_SHELL.stageBorder,
    backgroundColor: MOUNTAIN_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: MOUNTAIN_SHELL.glassBorder,
  },
  phaseBannerText: { color: MOUNTAIN_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: MOUNTAIN_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  skillText: { color: '#E0F2FE', fontSize: 10, fontWeight: '700' },
  ...
  secondaryText: { color: '#E0F2FE', fontWeight: '700' },
  errorText: { color: MOUNTAIN_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
