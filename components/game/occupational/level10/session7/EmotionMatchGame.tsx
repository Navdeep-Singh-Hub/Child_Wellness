/**
 * OT Level 10 · Session 7 · Game 2 — Emotion Match
 *
 * Feeling Face Quest: find each emotion cue, then match
...
import { EmotionMatchOverlay } from '@/components/game/occupational/level10/session7/components/EmotionMatchOverlay';
import { EmotionMatchVisuals } from '@/components/game/occupational/level10/session7/components/EmotionMatchVisuals';
import { EMOTION_MATCH_PACING as P } from '@/components/game/occupational/level10/session7/emotionMatchPacing';
import {
  EMOTION_MATCH_ROUNDS,
  EMOTION_MATCH_THEME as T,
  EMOTION_SHELL,
  type EmotionMatchRound,
} from '@/components/game/occupational/level10/session7/emotionMatchTheme';
import { inFindZone, inMatchZone } from '@/components/game/occupational/level10/session7/emotionMatchUtils';
...
const VOICE_PRAISE = ['Match star!', 'Feeling ace!', 'Great match!', 'Emotion champ!', 'Heart hero!'];

type Phase = 'intro' | 'calibrate' | 'play';
type RoundPhase = 'find' | 'match';

const EmotionMatchGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const emotionRound = EMOTION_MATCH_ROUNDS[Math.min(round, EMOTION_MATCH_ROUNDS.length - 1)]!;

  const sampleRef = useRef(sample);
  const phaseRef = useRef<Phase>('intro');
  const roundPhaseRef = useRef<RoundPhase>('find');
  const roundRef = useRef<EmotionMatchRound>(emotionRound);
  ...
  useEffect(() => { roundRef.current = emotionRound; }, [emotionRound]);
  ...
  const beginMatchPhase = useCallback((er: EmotionMatchRound) => {
    ...
    speakTTS(er.voiceMatch, 0.85).catch(() => {});
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      ...
      await logGameAndAward('emotion-match', done, xp);
    } catch {
      // offline
    }
  ...
  const startRound = useCallback((r: number) => {
    const er = EMOTION_MATCH_ROUNDS[Math.min(r, EMOTION_MATCH_ROUNDS.length - 1)]!;
    setRound(r);
    holdMsRef.current = 0;
    findMsRef.current = 0;
    lastInZoneRef.current = 0;
    ...
    setCoachCue(er.voiceFind);
    setOnFind(false);
    setOnMatch(false);
    findStartRef.current = Date.now();
    speakTTS(er.voiceFind, 0.82).catch(() => {});
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (rp === 'find') {
        ...
        setOnFind(t > 0.5);
        setHoldProgress(t);
        if (elapsed >= P.fallbackFindMs) beginMatchPhase(er);
      } else {
    ...
    if (rp === 'find') {
      const atFind = inFindZone(cursor, er);
      ...
      const q = socialSensoryQuality(atFind, false, holdPct, s.postureQuality, s.attentionScore, smooth, 'approach');
      ...
      setCoachCue(atFind ? `${er.label} — hold steady!` : er.voiceFind);

      if (holdMsRef.current >= P.findHoldMs) beginMatchPhase(er);
    ...
    const atMatch =
      inMatchZone(cursor, er) &&
      s.postureQuality >= P.minPostureForMatch &&
      s.attentionScore >= P.minAttentionForMatch;
    const holdPct = clamp01(holdMsRef.current / P.matchHoldMs);
    const findPct = clamp01(findMsRef.current / P.findHoldMs);
    const q = socialSensoryQuality(false, atMatch, holdPct, s.postureQuality, s.attentionScore, smooth, 'greet');

    setOnFind(false);
    setOnMatch(atMatch);
    setFindScore(findPct);
    setMatchScore(atMatch ? holdPct * 0.65 + smooth * 0.35 : smooth * 0.25);
    setCalm(atMatch ? smooth * 0.55 + s.attentionScore * 0.45 : smooth * 0.3);
    setFocus(s.attentionScore);
    setQuality(q);
    qualSumRef.current += q;
    qualCountRef.current += 1;

    setBanner(atMatch ? T.holdMatchLabel : T.matchLabel);
    setCoachCue(atMatch ? er.matchCue : er.voiceMatch);
    ...
    } else if (now - (lastInZoneRef.current || now) > P.matchGraceMs) {
      holdMsRef.current = Math.max(0, holdMsRef.current - dt * 0.5);
      setHoldProgress(clamp01(holdMsRef.current / P.matchHoldMs));
      lastInZoneRef.current = 0;
    }
    ...
    recordTick(dt, {
      upright: s.postureQuality > P.minPostureForMatch,
      still: smooth > 0.35,
      quality: q,
    });
  ...
  const beginCalibration = useCallback(async () => {
    ...
    setPhase('calibrate');
    setCalibProgress(0);
    setCoachCue('Show your full body — emotion sensors calibrating…');
  ...
  if (showCongrats && finalStats) {
    return (
      <CongratulationsScreen
        title={T.congrats}
        subtitle={`${finalStats.correct}/${finalStats.total} emotions matched · ${finalStats.accuracy}% quality`}
        emoji={T.emoji}
        onContinue={() => {
  ...
  const renderOverlay = () => (
    <>
      <EmotionMatchVisuals matchPhase={roundPhase === 'match'} />
      <AttentionMeter
        find={findScore}
        focus={matchScore}
        calm={calm}
        title="FEELING"
        labels={{ find: 'FIND', focus: 'MATCH', calm: 'CALM' }}
        style={{
          glassBorder: EMOTION_SHELL.glassBorder,
          statLabel: EMOTION_SHELL.statLabel,
          accent: T.accent,
          fillColors: ['#2DD4BF', '#F472B6', '#FDE68A'],
        }}
      />
      {phase === 'play' && (
        <>
      ...
      <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={EMOTION_SHELL.sparkleColor} count={24} />
      {phase === 'calibrate' && (
        <View style={styles.calibWrap} pointerEvents="none">
          <Text style={styles.calibText}>Emotion calibration… {Math.round(calibProgress * 100)}%</Text>
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: EMOTION_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: EMOTION_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>😊 {EMOTION_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: EMOTION_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: EMOTION_SHELL.statLabel }]}>Face</Text>
              <Text style={[styles.statValue, { color: EMOTION_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: EMOTION_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>💗</Text>
              <Text style={[styles.statValue, { color: EMOTION_SHELL.statValue }]}>{completed}</Text>
            </View>
            <View style={[styles.statPill, { borderColor: EMOTION_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: EMOTION_SHELL.statLabel }]}>Quality</Text>
              <Text style={[styles.statValue, { color: EMOTION_SHELL.statValue }]}>{Math.round(quality * 100)}%</Text>
            </View>
          </View>
        </View>
        ...
        {phase === 'intro' && (
          <View style={styles.bottomPanel}>
            ...
            >
              <Text style={styles.primaryBtnText}>Start Emotion Match</Text>
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  nativeStage: {
    ...
    overflow: 'hidden',
    borderWidth: 2,
    borderColor: EMOTION_SHELL.stageBorder,
    backgroundColor: EMOTION_SHELL.stageBg,
    position: 'relative',
  },
  phaseBanner: {
    position: 'absolute',
    ...
    borderRadius: 20,
    borderWidth: 1,
    borderColor: EMOTION_SHELL.glassBorder,
  },
  phaseBannerText: { color: EMOTION_SHELL.gold, fontSize: 15, fontWeight: '900', letterSpacing: 1.5 },
  ...
  cueText: { color: EMOTION_SHELL.gold, fontSize: 14, fontWeight: '800', textAlign: 'center' },
  bottomPanel: {
    marginHorizontal: 16,
    marginBottom: 16,
    padding: 16,
  ...
  skillPill: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 10, backgroundColor: 'rgba(45,212,191,0.2)' },
  ...
  errorText: { color: EMOTION_SHELL.warn, textAlign: 'center', fontWeight: '700', fontSize: 12 },
});
