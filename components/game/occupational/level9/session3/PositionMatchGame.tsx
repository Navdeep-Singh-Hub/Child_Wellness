/**
 * OT Level 9 · Session 3 · Game 5 — Position Match (Precision Grid Observatory)
 *
 * Camera tracks focused joint positions on a calibration grid — child locks
 * highlighted joints to target ticks with precision. MediaPipe pose on APK + web.
 */
...
import { PositionMatchOverlay } from '@/components/game/occupational/level9/session3/components/PositionMatchOverlay';
import { POSITION_MATCH_THEME, POSITION_SHELL } from '@/components/game/occupational/level9/session3/jointTheme';
import {
  matchPositionTargets,
  mirroredBodyMarkers,
  positionAxisLabel,
  POSITION_MATCH_TARGETS,
  type AxisPositionScore,
  type PositionMatchTarget,
...
const T = POSITION_MATCH_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Grid locked!', 'Precision match!', 'Joint calibrated!', 'Target hit!', 'Observatory pro!'];

const EMPTY_AXES: AxisPositionScore[] = [];
...
const PositionMatchGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [target, setTarget] = useState<PositionMatchTarget>(POSITION_MATCH_TARGETS[0]!);
  const [axes, setAxes] = useState<AxisPositionScore[]>(EMPTY_AXES);
  ...
  const targetRef = useRef<PositionMatchTarget>(POSITION_MATCH_TARGETS[0]!);
  ...
  const targetForRound = useCallback(
    (r: number) => POSITION_MATCH_TARGETS[Math.min(r, POSITION_MATCH_TARGETS.length - 1)] ?? POSITION_MATCH_TARGETS[0]!,
    [],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const gridAcc = accCountRef.current > 0 ? accSumRef.current / accCountRef.current : 0;
    const moveQuality = qualCountRef.current > 0 ? qualSumRef.current / qualCountRef.current : 0;
    const headline = Math.round((completion * 0.5 + gridAcc * 0.3 + moveQuality * 0.2) * 100);
    const xp = Math.round(completed * 25 + gridAcc * 102 + moveQuality * 74);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            positionMatchAccuracy: Math.round(gridAcc * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      speakTTS(`Lock ${tgt.name}!`, 0.85).catch(() => {});
  ...
  const beginLock = useCallback(() => {
    ...
    schedule(() => finishRound(), P.positionLockMs);
  }, [finishRound, schedule]);

  const buildFallbackAxes = useCallback((tgt: PositionMatchTarget, t: number): AxisPositionScore[] => {
    const blend = 0.55 + t * 0.45;
    const mk = (axis: AxisPositionScore['axis'], val: number): AxisPositionScore => {
      const actual = val * blend;
      const accuracy = clamp01(1 - Math.abs(actual - val) / Math.max(P.positionMatchTolerance, 0.08));
      return {
        axis,
        ...
        target: val,
        accuracy,
        focused: tgt.focus.includes(axis),
      };
    };
    return [
      mk('leftRaise', tgt.leftRaise),
      mk('rightRaise', tgt.rightRaise),
      mk('leftElbow', tgt.leftElbow),
      mk('rightElbow', tgt.rightElbow),
      mk('leftLift', tgt.leftLift),
      mk('rightLift', tgt.rightLift),
      mk('leftKnee', tgt.leftKnee),
      mk('rightKnee', tgt.rightKnee),
    ];
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    const tol = P.positionMatchTolerance;

    if (lockingRef.current) {
      const lp = clamp01((now - lockStartRef.current) / P.positionLockMs);
      setLockProgress(lp);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackPositionMs);
        const fbAxes = buildFallbackAxes(tgt, t);
        const focused = fbAxes.filter((a) => a.focused);
        const fScore = clamp01(focused.reduce((s, a) => s + a.accuracy, 0) / Math.max(1, focused.length));
        const isMatch = t >= 0.55 && fScore >= 0.72;
        setAxes(fbAxes);
        ...
        accCountRef.current += 1;
        if (isMatch) {
          const prog = clamp01((elapsed - P.fallbackPositionMs * 0.5) / P.positionHoldMs);
    ...
    setMarkers(mirroredBodyMarkers(m));
    ...
    const result = matchPositionTargets(m, tgt, tol);
    ...
    if (!result.ok && result.worstAxis) {
      setCoachCue(`Adjust ${positionAxisLabel(result.worstAxis)} — ${T.axisCue}`);
      setBanner('');
    } else if (result.ok) {
    ...
    if (roundActiveRef.current) {
      ...
      if (result.ok) {
        ...
        const prog = clamp01((now - holdStartRef.current) / P.positionHoldMs);
        setHoldProgress(prog);
        if (prog >= 1) beginLock();
    ...
    prevMetricsRef.current = m;
  }, [beginLock, buildFallbackAxes, recordPostureBreak, recordTick]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS(
      'Stand tall with your full body visible. I will learn your pose before the Precision Grid Observatory begins!',
      0.8,
    ).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided grid mode.');
      }
    }
    beginCalibration();
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: POSITION_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: POSITION_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🎯 {POSITION_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: POSITION_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: POSITION_SHELL.statLabel }]}>Round</Text>
              <Text style={[styles.statValue, { color: POSITION_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: POSITION_SHELL.statBorder }]}>
              <Text style={styles.statEmoji}>🔒</Text>
              <Text style={[styles.statValue, { color: POSITION_SHELL.statValue }]}>{locked}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={POSITION_SHELL.sparkleColor} count={26} />
            {phase === 'calibrate' && (
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — lock highlighted joints to the grid targets!'
                    : 'Guided mode: follow the coach and practice precision joint position matching!'}
                </Text>
...
const styles = StyleSheet.create({
  ...
  linkText: { color: '#FBBF24', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default PositionMatchGame;
