/**
 * OT Level 9 · Session 6 · Game 4 — Hero Pose (Champion Hero Colosseum)
 *
 * Camera tracks iconic hero pose matching and controlled heroic power channeling.
...
import { HeroPoseOverlay } from '@/components/game/occupational/level9/session6/components/HeroPoseOverlay';
import { HERO_POSE_THEME, BODY_AWARENESS_SHELL } from '@/components/game/occupational/level9/session6/bodyAwarenessTheme';
import {
  HERO_POSE_ROUNDS,
  bodyZoneStatus,
  heroPoseMatchScore,
...
const T = HERO_POSE_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Unleashed!', 'Hero power!', 'Champion pose!', 'Perfect strike!', 'Colosseum star!'];

const EMPTY_READOUT: FullBodyReadout = { arms: { left: null, right: null }, legs: { left: null, right: null } };

type Phase = 'intro' | 'calibrate' | 'play';

const HeroPoseGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [unleashes, setUnleashes] = useState(0);
  const [completedCount, setCompletedCount] = useState(0);
  const [roundDef, setRoundDef] = useState<HeroPoseRound>(HERO_POSE_ROUNDS[0]!);
  ...
  const [unleashProgress, setUnleashProgress] = useState(0);
  ...
  const roundDefRef = useRef<HeroPoseRound>(HERO_POSE_ROUNDS[0]!);
  const roundActiveRef = useRef(false);
  const previewingRef = useRef(false);
  const unleashingRef = useRef(false);
  const holdStartRef = useRef(0);
  const lastAtTargetRef = useRef(0);
  const unleashStartRef = useRef(0);
  ...
  const targetForRound = useCallback(
    (r: number) => P.heroTargets[Math.min(r, P.heroTargets.length - 1)] ?? 0.64,
    [],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const avgHero = powerCountRef.current > 0 ? powerSumRef.current / powerCountRef.current : 0;
    ...
    const headline = Math.round((completion * 0.5 + avgHero * 0.2 + moveQuality * 0.15 + zoneAccuracy * 0.15) * 100);
    const xp = Math.round(completed * 22 + avgHero * 90 + moveQuality * 70 + zoneAccuracy * 42);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgHeroPower: Math.round(avgHero * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = HERO_POSE_ROUNDS[Math.min(r, HERO_POSE_ROUNDS.length - 1)] ?? HERO_POSE_ROUNDS[0]!;
      roundRef.current = r;
      ...
      unleashStartRef.current = 0;
      peakPowerRef.current = 0;
      previewingRef.current = true;
      unleashingRef.current = false;
      roundActiveRef.current = false;
      setPreviewing(true);
      setUnleashing(false);
      setUnleashProgress(0);
      setHoldProgress(0);
      setHeroPower(0);
      setPoseMatch(0);
      setZoneStatus('light');
      ...
      schedule(() => {
        ...
        speakTTS(T.voicePose, 0.85).catch(() => {});
  ...
  const beginUnleash = useCallback(() => {
    if (doneRef.current || unleashingRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    unleashingRef.current = true;
    setUnleashing(true);
    unleashStartRef.current = Date.now();
    setHoldProgress(1);
    setBanner('UNLEASHED!');
    speakTTS(T.voiceUnleash, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.heroUnleashMs);
  ...
  const tick = useCallback(() => {
    ...
    const band = P.heroBandHalf;

    if (unleashingRef.current) {
      const up = clamp01((now - unleashStartRef.current) / P.heroUnleashMs);
      setUnleashProgress(up);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackHeroMs);
        const match = clamp01(0.46 + t * 0.46);
        ...
        if (status === 'zone' && match >= P.heroFormMin) {
          const zoneElapsed = Math.max(0, elapsed - P.fallbackHeroMs * 0.47);
          const prog = clamp01(zoneElapsed / P.heroHoldMs);
          setHoldProgress(prog);
          if (prog >= 1) beginUnleash();
        } else {
          setHoldProgress(0);
        }
      }
    ...
    const match = heroPoseMatchScore(m, def, P.heroTolerance);
    const p = heroPowerScore(m, def, postureBaseRef.current, forceBaseRef.current, P.heroTolerance, controlled);
    const status = bodyZoneStatus(p, target, band);
    const q = clamp01((positioned ? 0.42 : 0.2) + 0.38 * controlled + 0.2 * (status === 'zone' ? 1 : p));
    setHeroPower(p);
    setPoseMatch(match.score);
    ...
    if (roundActiveRef.current) {
      ...
      if (status === 'zone' && match.score >= P.heroFormMin) zoneTicksRef.current += 1;

      if (status === 'zone' && match.score >= P.heroFormMin) {
        if (holdStartRef.current === 0) holdStartRef.current = now;
        lastAtTargetRef.current = now;
        const prog = clamp01((now - holdStartRef.current) / P.heroHoldMs);
        setHoldProgress(prog);
        if (prog >= 1) beginUnleash();
      } else if (status === 'heavy' || match.score < P.heroFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        setHoldProgress(0);
      }
    }
    ...
    prevMetricsRef.current = m;
  }, [beginUnleash, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS(
      'Stand tall with your full body visible. I will learn your pose before Champion Hero Colosseum begins!',
      0.8,
    ).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided hero mode.');
      }
    }
    beginCalibration();
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: BODY_AWARENESS_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: BODY_AWARENESS_SHELL.statLabel }]}>Pose</Text>
            ...
            <View style={[styles.statPill, { borderColor: BODY_AWARENESS_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: BODY_AWARENESS_SHELL.statValue }]}>{unleashes}</Text>
            </View>
          </View>
        </View>
          ...
          >
            {phase === 'play' && (
              <HeroPoseOverlay
                ...
                unleashProgress={unleashProgress}
                unleashing={unleashing}
                previewing={previewing}
                roundActive={roundActive}
                round={round}
                totalRounds={totalRounds}
                heroCount={completedCount}
                colosseumProgress={colosseumProgress}
                banner={banner}
                quality={quality}
                bandHalf={P.heroBandHalf}
              />
            )}
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — strike each hero pose and channel your power!'
                    : 'Guided mode: follow the coach and practice iconic hero poses with controlled power!'}
                </Text>
