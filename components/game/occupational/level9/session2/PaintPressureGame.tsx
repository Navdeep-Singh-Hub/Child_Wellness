/**
 * OT Level 9 · Session 2 · Game 2 — Paint Pressure (Color Splash Studio)
 *
 * Camera tracks brush press on canvas with graded pressure per stroke.
...
import { PaintOverlay } from '@/components/game/occupational/level9/session2/components/PaintOverlay';
import { PAINT_PRESSURE_THEME, PAINT_SHELL } from '@/components/game/occupational/level9/session2/pressureTheme';
import { paintPressureScore } from '@/components/game/occupational/level9/session2/pressureUtils';
...
const T = PAINT_PRESSURE_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Bold stroke!', 'Perfect brush!', 'Lovely color!', 'So gentle!', 'Masterpiece!'];

type Phase = 'intro' | 'calibrate' | 'play';

const PaintPressureGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const targetForRound = useCallback(
    (r: number) => P.paintTargets[Math.min(r, P.paintTargets.length - 1)] ?? 0.7,
    [],
  );

  const colorForRound = useCallback(
    (r: number) => ({
      emoji: T.paintColors[r % T.paintColors.length] ?? '🔵',
      hex: T.paintHex[r % T.paintHex.length] ?? '#3B82F6',
    }),
    [],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const xp = Math.round(completed * 21 + avgForce * 86 + moveQuality * 66);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgBrushPressure: Math.round(avgForce * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      setBanner(T.brushCue);
      setCoachCue(T.hintText);
      speakTTS(T.voiceBrush, 0.85).catch(() => {});
  ...
  const beginStroke = useCallback(() => {
    if (doneRef.current || strokingRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    ...
    setHoldProgress(1);
    setBanner('STROKE!');
    speakTTS(T.voiceStroke, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {});
    schedule(() => finishRound(), P.paintStrokeMs);
  ...
  const tick = useCallback(() => {
    ...
    if (strokingRef.current) {
      const sp = clamp01((now - strokeStartRef.current) / P.paintStrokeMs);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackBrushMs);
        const f = t * target * 1.04;
        setForce(f);
        setLeftHand({ x: 0.38, y: 0.54 - t * 0.1 });
        setRightHand({ x: 0.62, y: 0.54 - t * 0.1 });
        setHoldProgress(t >= 0.88 ? clamp01((elapsed - P.fallbackBrushMs * 0.88) / P.paintHoldMs) : 0);
        setQuality(0.85);
    ...
    const f = paintPressureScore(m, baselineRef.current);
    ...
    if (roundActiveRef.current) {
      ...
      if (atTarget) {
        ...
        const prog = clamp01((now - holdStartRef.current) / P.paintHoldMs);
        setHoldProgress(prog);
        if (prog >= 1) beginStroke();
    ...
    prevMetricsRef.current = m;
  }, [beginStroke, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS('Stand tall and relax your arms. I will learn how you stand before we paint!', 0.8).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided brush mode.');
      }
    }
    beginCalibration();
  ...
  const { emoji: paintColor, hex: paintHex } = colorForRound(round);

  return (
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: PAINT_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: PAINT_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🎨 {PAINT_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: PAINT_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: PAINT_SHELL.statLabel }]}>Round</Text>
              <Text style={[styles.statValue, { color: PAINT_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: PAINT_SHELL.statBorder }]}>
              <Text style={styles.strokeEmoji}>🖌️</Text>
              <Text style={[styles.statValue, { color: PAINT_SHELL.statValue }]}>{strokes}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={PAINT_SHELL.sparkleColor} count={24} />
            {phase === 'calibrate' && (
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Stand back so the camera sees your arms — press your brush on the canvas with graded pressure!'
                    : 'Guided mode: follow the coach and practice light-to-bold brush strokes!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.brush }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Start Painting</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
export default PaintPressureGame;
