/**
 * OT Level 9 · Session 2 · Game 1 — Decorate The Cake (Sweet Swirl Bakery)
 *
 * Camera tracks piping-bag graded pressure to decorate cake frosting swirls.
...
import { CakeOverlay } from '@/components/game/occupational/level9/session2/components/CakeOverlay';
import { CAKE_SHELL, DECORATE_CAKE_THEME } from '@/components/game/occupational/level9/session2/pressureTheme';
import { decoratePressureScore } from '@/components/game/occupational/level9/session2/pressureUtils';
...
const T = DECORATE_CAKE_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Sweet swirl!', 'Perfect pipe!', 'Lovely frosting!', 'So gentle!', 'Beautiful cake!'];

type Phase = 'intro' | 'calibrate' | 'play';

const DecorateTheCakeGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const targetForRound = useCallback(
    (r: number) => P.decorateTargets[Math.min(r, P.decorateTargets.length - 1)] ?? 0.7,
    [],
  );

  const toppingForRound = useCallback((r: number) => T.toppings[r % T.toppings.length] ?? '🌸', []);
  ...
  const endGame = useCallback(() => {
    ...
    const xp = Math.round(completed * 20 + avgForce * 84 + moveQuality * 64);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgPressure: Math.round(avgForce * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      strokeStartRef.current = 0;
      peakForceRef.current = 0;
      pipingRef.current = false;
      setPiping(false);
      setStrokeProgress(0);
      setHoldProgress(0);
      setForce(0);
      ...
      speakTTS(T.voicePipe, 0.85).catch(() => {});
  ...
  const beginPiping = useCallback(() => {
    if (doneRef.current || pipingRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    pipingRef.current = true;
    setPiping(true);
    strokeStartRef.current = Date.now();
    setHoldProgress(1);
    setBanner('SWIRL!');
    speakTTS(T.voiceSwirl, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light).catch(() => {});
    schedule(() => finishRound(), P.decorateStrokeMs);
  ...
  const tick = useCallback(() => {
    ...
    if (pipingRef.current) {
      const sp = clamp01((now - strokeStartRef.current) / P.decorateStrokeMs);
      setStrokeProgress(sp);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackPipeMs);
        const f = t * target * 1.05;
        setForce(f);
        setLeftHand({ x: 0.4, y: 0.52 });
        setRightHand({ x: 0.6, y: 0.52 });
        setHoldProgress(t >= 0.88 ? clamp01((elapsed - P.fallbackPipeMs * 0.88) / P.decorateHoldMs) : 0);
        setQuality(0.84);
    ...
    const f = decoratePressureScore(m, baselineRef.current);
    ...
    if (roundActiveRef.current) {
      ...
      if (atTarget) {
        ...
        const prog = clamp01((now - holdStartRef.current) / P.decorateHoldMs);
        setHoldProgress(prog);
        if (prog >= 1) beginPiping();
    ...
    prevMetricsRef.current = m;
  }, [beginPiping, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS('Stand tall and relax your arms. I will learn how you stand before we decorate the cake!', 0.8).catch(
      () => {},
    );
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided piping mode.');
      }
    }
    beginCalibration();
  ...
  const topping = toppingForRound(round);

  return (
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: CAKE_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: CAKE_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🧁 {CAKE_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: CAKE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: CAKE_SHELL.statLabel }]}>Round</Text>
              <Text style={[styles.statValue, { color: CAKE_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: CAKE_SHELL.statBorder }]}>
              <Text style={styles.swirlEmoji}>🌸</Text>
              <Text style={[styles.statValue, { color: CAKE_SHELL.statValue }]}>{swirls}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={CAKE_SHELL.sparkleColor} count={22} />
            {phase === 'calibrate' && (
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Stand back so the camera sees your arms — pipe each frosting swirl with graded pressure!'
                    : 'Guided mode: follow the coach and practice gentle-to-firm piping!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accent }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Start Baking</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
const styles = StyleSheet.create({
  ...
  calibTrack: { width: '100%', height: 12, borderRadius: 6, backgroundColor: 'rgba(59,24,16,0.75)', overflow: 'hidden' },
...
export default DecorateTheCakeGame;
