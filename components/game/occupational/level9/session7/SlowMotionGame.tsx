/**
 * OT Level 9 · Session 7 · Game 1 — Slow Motion (Glacial Motion Lab)
 *
 * Camera tracks ultra-slow trajectory progress and controlled calibration effort.
...
import { SlowMotionOverlay } from '@/components/game/occupational/level9/session7/components/SlowMotionOverlay';
import { SLOW_MOTION_THEME, MOVEMENT_CALIBRATION_SHELL } from '@/components/game/occupational/level9/session7/movementCalibrationTheme';
import {
  SLOW_MOTION_ROUNDS,
  calibrationZoneStatus,
  slowMotionPowerScore,
  slowPaceScore,
  slowTrajectoryScore,
  type SlowMotionRound,
...
const T = SLOW_MOTION_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Calibrated!', 'Glacial pro!', 'Slow master!', 'Perfect glide!', 'Motion star!'];

const EMPTY_READOUT: FullBodyReadout = { arms: { left: null, right: null }, legs: { left: null, right: null } };

type Phase = 'intro' | 'calibrate' | 'play';

const SlowMotionGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundDef, setRoundDef] = useState<SlowMotionRound>(SLOW_MOTION_ROUNDS[0]!);
  const [readout, setReadout] = useState<FullBodyReadout>(EMPTY_READOUT);
  const [slowPower, setSlowPower] = useState(0);
  const [trajectory, setTrajectory] = useState(0);
  const [paceScore, setPaceScore] = useState(1);
  ...
  const roundDefRef = useRef<SlowMotionRound>(SLOW_MOTION_ROUNDS[0]!);
  ...
  const peakTrajRef = useRef(0);
  const paceAvgRef = useRef(1);
  ...
  const targetForRound = useCallback(
    (r: number) => P.slowTargets[Math.min(r, P.slowTargets.length - 1)] ?? 0.62,
    [],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const avgSlow = powerCountRef.current > 0 ? powerSumRef.current / powerCountRef.current : 0;
    ...
    const headline = Math.round((completion * 0.5 + avgSlow * 0.2 + moveQuality * 0.15 + zoneAccuracy * 0.15) * 100);
    const xp = Math.round(completed * 22 + avgSlow * 90 + moveQuality * 70 + zoneAccuracy * 42);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgSlowMotion: Math.round(avgSlow * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = SLOW_MOTION_ROUNDS[Math.min(r, SLOW_MOTION_ROUNDS.length - 1)] ?? SLOW_MOTION_ROUNDS[0]!;
      roundRef.current = r;
      ...
      peakTrajRef.current = 0;
      paceAvgRef.current = 1;
  ...
  const beginCalibrate = useCallback(() => {
    ...
    schedule(() => finishRound(), P.slowCalibrateMs);
  ...
  const tick = useCallback(() => {
    ...
    const band = P.slowBandHalf;

    if (calibratingRef.current) {
      const up = clamp01((now - calibrateStartRef.current) / P.slowCalibrateMs);
      setCalibrateProgress(up);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackSlowMs);
        const traj = clamp01(0.42 + t * 0.48);
        const pace = clamp01(0.88 - t * 0.08);
        ...
        setArmsScore(traj);
        setLegsScore(traj);
        setPathProgress(traj);
        setTooFast(false);
        setReadout(EMPTY_READOUT);
        setZoneStatus(status);
        setQuality(0.85);
        if (p > peakPowerRef.current) peakPowerRef.current = p;
        if (traj > peakTrajRef.current) peakTrajRef.current = traj;
        if (status === 'zone' && traj >= P.slowReachMin && pace >= P.slowPaceMin) {
          const zoneElapsed = Math.max(0, elapsed - P.fallbackSlowMs * 0.47);
          const prog = clamp01(zoneElapsed / P.slowHoldMs);
    ...
    const intensity = movementIntensity(motion, P.slowSpeedCeiling);
    const pace = slowPaceScore(motion, P.slowSpeedCeiling);
    paceAvgRef.current = paceAvgRef.current * 0.9 + pace * 0.1;
    const fast = intensity >= P.slowFastCutoff;
    const controlled = clamp01(1 - Math.max(0, intensity - 0.35) / 0.65);
    const traj = slowTrajectoryScore(m, def, P.slowTolerance);
    const p = slowMotionPowerScore(
      m,
      def,
    ...
    const status = calibrationZoneStatus(p, target, band);
    const q = clamp01((positioned ? 0.4 : 0.18) + 0.32 * controlled + 0.14 * pace + 0.14 * (status === 'zone' && !fast ? 1 : p));
    setSlowPower(p);
    setTrajectory(traj.score);
    setPaceScore(pace);
    setArmsScore(traj.armsScore);
    setLegsScore(traj.legsScore);
    setReadout(traj.readout);
    setZoneStatus(status);
    setTooFast(fast);
    setQuality(q);
    if (p > peakPowerRef.current) peakPowerRef.current = p;
    if (traj.score > peakTrajRef.current) peakTrajRef.current = traj.score;
    setPathProgress(peakTrajRef.current);
    ...
    } else if (traj.score < P.slowReachMin) {
    ...
    if (roundActiveRef.current) {
      ...
      if (status === 'zone' && !fast && traj.score >= P.slowReachMin) zoneTicksRef.current += 1;

      const canHold =
        status === 'zone' &&
        !fast &&
        traj.score >= P.slowReachMin &&
        paceAvgRef.current >= P.slowPaceMin;

      if (canHold) {
        if (holdStartRef.current === 0) holdStartRef.current = now;
        lastAtTargetRef.current = now;
        const prog = clamp01((now - holdStartRef.current) / P.slowHoldMs);
        setHoldProgress(prog);
        if (prog >= 1) beginCalibrate();
      } else if (fast || status === 'heavy' || traj.score < P.slowReachMin || paceAvgRef.current < P.slowPaceMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        setHoldProgress(0);
      }
    }
    ...
    recordTick(dt, { upright: positioned, still: pace > 0.7, quality: q });
    prevMetricsRef.current = m;
  }, [beginCalibrate, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS(
      'Stand tall with your full body visible. I will learn your pose before Glacial Motion Lab begins!',
      0.8,
    ).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided slow motion mode.');
      }
    }
    beginCalibration();
  ...
  const labProgress = completedCount / totalRounds;
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: MOVEMENT_CALIBRATION_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: MOVEMENT_CALIBRATION_SHELL.statLabel }]}>Path</Text>
            ...
            <View style={[styles.statPill, { borderColor: MOVEMENT_CALIBRATION_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: MOVEMENT_CALIBRATION_SHELL.statValue }]}>{calibrations}</Text>
            </View>
          </View>
        </View>
          ...
          >
            {phase === 'play' && (
              <SlowMotionOverlay
                ...
                pathCount={completedCount}
                labProgress={labProgress}
                pathProgress={pathProgress}
                tooFast={tooFast}
                banner={banner}
                quality={quality}
                bandHalf={P.slowBandHalf}
              />
            )}
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — glide through each path in ultra-slow motion!'
                    : 'Guided mode: follow the coach and practice glacial slow-motion paths with controlled effort!'}
                </Text>
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: '#DDD6FE', fontSize: 11, fontWeight: '900', letterSpacing: 0.8 },
  header: { alignItems: 'center', marginTop: 8 },
  title: { color: '#fff', fontSize: 23, fontWeight: '900', textAlign: 'center' },
  subtitle: { color: '#DDD6FE', fontSize: 13, fontWeight: '600', marginTop: 2, textAlign: 'center', paddingHorizontal: 8 },
