/**
 * OT Level 9 · Session 5 · Game 1 — Launch Power (Orbital Resistance Bay)
 *
 * Camera tracks squat launch-pad pose and sustained upward ignition power in resistance zone.
...
import { LaunchPowerOverlay } from '@/components/game/occupational/level9/session5/components/LaunchPowerOverlay';
import { LAUNCH_POWER_THEME, RESISTANCE_SHELL } from '@/components/game/occupational/level9/session5/resistanceTheme';
import {
  launchIgnitionBend,
  launchIgnitionRaise,
  launchPadSquat,
...
const T = LAUNCH_POWER_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Blast off!', 'Perfect zone!', 'Steady ignition!', 'Resistance pro!', 'Launch master!'];

type Phase = 'intro' | 'calibrate' | 'play';

const LaunchPowerGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const targetForRound = useCallback(
    (r: number) => P.launchTargets[Math.min(r, P.launchTargets.length - 1)] ?? 0.62,
    [],
  );
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgIgnitionPower: Math.round(avgPower * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      speakTTS(`Launch ${r + 1}!`, 0.85).catch(() => {});
      ...
      schedule(() => {
        ...
        setBanner(T.igniteCue);
        setCoachCue(T.hintText);
        speakTTS(T.voiceIgnite, 0.85).catch(() => {});
  ...
  const beginBlast = useCallback(() => {
    if (doneRef.current || blastingRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    ...
    setHoldProgress(1);
    setBanner('BLAST OFF!');
    speakTTS(T.voiceBlast, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.launchBlastMs);
  ...
  const tick = useCallback(() => {
    ...
    const band = P.launchBandHalf;

    if (blastingRef.current) {
      const bp = clamp01((now - blastStartRef.current) / P.launchBlastMs);
      setBlastProgress(bp);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackLaunchMs);
        const p = clamp01(target * (0.5 + t * 0.55));
        const f = clamp01(0.48 + t * 0.46);
        const status = resistanceZoneStatus(p, target, band);
        setPower(p);
        setForm(f);
        setZoneStatus(status);
        setLeftPalm({ x: 0.34, y: 0.5 - t * 0.1 });
        setRightPalm({ x: 0.66, y: 0.5 - t * 0.1 });
        setQuality(0.85);
        if (p > peakPowerRef.current) peakPowerRef.current = p;
        if (status === 'zone' && f >= P.launchFormMin) {
          const zoneElapsed = Math.max(0, elapsed - P.fallbackLaunchMs * 0.48);
          const prog = clamp01(zoneElapsed / P.launchHoldMs);
          setHoldProgress(prog);
          if (prog >= 1) beginBlast();
        } else {
          setHoldProgress(0);
        }
      }
    ...
    const palms = mirroredLaunchPalms(m);
    ...
    const poseForm = launchPoseForm(m, baselineRef.current);
    const p = launchPowerScore(m, baselineRef.current);
    const status = resistanceZoneStatus(p, target, band);
    ...
    if (poseForm < P.launchFormMin) {
      if (squat < 0.35) setCoachCue(T.stanceCue);
      else if (raise < 0.4 || bend < 0.4 || height < 0.4) setCoachCue(T.formCue);
    ...
    if (roundActiveRef.current) {
      ...
      if (status === 'zone' && poseForm >= P.launchFormMin) zoneTicksRef.current += 1;

      if (status === 'zone' && poseForm >= P.launchFormMin) {
        if (holdStartRef.current === 0) holdStartRef.current = now;
        lastAtTargetRef.current = now;
        const prog = clamp01((now - holdStartRef.current) / P.launchHoldMs);
        setHoldProgress(prog);
        if (prog >= 1) beginBlast();
      } else if (status === 'heavy' || poseForm < P.launchFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        setHoldProgress(0);
      }
    }
    ...
    prevMetricsRef.current = m;
  }, [beginBlast, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS(
      'Stand tall with your full body visible. I will learn your pose before the Orbital Resistance Bay begins!',
      0.8,
    ).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided launch mode.');
      }
    }
    beginCalibration();
  ...
  const bayProgress = completedCount / totalRounds;
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: RESISTANCE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: RESISTANCE_SHELL.statLabel }]}>Launch</Text>
            ...
            <View style={[styles.statPill, { borderColor: RESISTANCE_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: RESISTANCE_SHELL.statValue }]}>{blasts}</Text>
            </View>
          </View>
        </View>
          ...
          >
            {phase === 'play' && (
              <LaunchPowerOverlay
                ...
                launchCount={completedCount}
                bayProgress={bayProgress}
                leftPalm={leftPalm}
                rightPalm={rightPalm}
                banner={banner}
                quality={quality}
                bandHalf={P.launchBandHalf}
              />
            )}
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — squat wide and push upward through ignition resistance!'
                    : 'Guided mode: follow the coach and practice controlled launch ignition power!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Enter Bay</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
export default LaunchPowerGame;
