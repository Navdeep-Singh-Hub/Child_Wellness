/**
 * OT Level 9 · Session 9 · Game 5 — Power Marathon (Thunder Power Marathon)
 *
 * Camera tracks sustained gorilla power pose with escalating endurance holds in power zone.
...
import { PowerMarathonOverlay } from '@/components/game/occupational/level9/session9/components/PowerMarathonOverlay';
import { POWER_MARATHON_THEME, ENDURANCE_EFFORT_SHELL } from '@/components/game/occupational/level9/session9/enduranceEffortTheme';
import {
  POWER_MARATHON_ROUNDS,
  enduranceHoldQuality,
  enduranceStabilityScore,
  mirroredGorillaHands,
  powerMarathonReadout,
  powerMarathonScore,
  type EnduranceZoneStatus,
  type PowerMarathonRound,
...
const T = POWER_MARATHON_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Marathon star!', 'Power mile!', 'Thunder strong!', 'Beast mode!', 'Finish king!'];

type Phase = 'intro' | 'calibrate' | 'play';

const PowerMarathonGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [mileStars, setMileStars] = useState(0);
  const [completedCount, setCompletedCount] = useState(0);
  const [roundDef, setRoundDef] = useState<PowerMarathonRound>(POWER_MARATHON_ROUNDS[0]!);
  ...
  const roundDefRef = useRef<PowerMarathonRound>(POWER_MARATHON_ROUNDS[0]!);
  ...
  const marathonSumRef = useRef(0);
  const marathonCountRef = useRef(0);
  ...
  const targetForRound = useCallback(
    (r: number) => P.marathonTargets[Math.min(r, P.marathonTargets.length - 1)] ?? 0.6,
    [],
  );

  const holdMsForRound = useCallback(
    (r: number) => P.marathonHoldMs[Math.min(r, P.marathonHoldMs.length - 1)] ?? 2000,
    [],
  );

  const fallbackMsForRound = useCallback(
    (r: number) => P.fallbackMarathonMs[Math.min(r, P.fallbackMarathonMs.length - 1)] ?? 3200,
    [],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const avgMarathon = marathonCountRef.current > 0 ? marathonSumRef.current / marathonCountRef.current : 0;
    ...
    const headline = Math.round((completion * 0.5 + avgMarathon * 0.2 + moveQuality * 0.15 + zoneAccuracy * 0.15) * 100);
    const xp = Math.round(completed * 22 + avgMarathon * 92 + moveQuality * 70 + zoneAccuracy * 42);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgPowerMarathon: Math.round(avgMarathon * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = POWER_MARATHON_ROUNDS[Math.min(r, POWER_MARATHON_ROUNDS.length - 1)] ?? POWER_MARATHON_ROUNDS[0]!;
      roundRef.current = r;
  ...
  const finishRound = useCallback(() => {
    ...
    const roundScore = powerMarathonScore(peakPowerRef.current, peakStabilityRef.current);
    marathonSumRef.current += roundScore;
    marathonCountRef.current += 1;
    completedRef.current += 1;
    setCompletedCount(completedRef.current);
    setMarathonProgress(completedRef.current / totalRounds);
  ...
  const beginMile = useCallback(() => {
    if (doneRef.current || finishingRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    finishingRef.current = true;
    setFinishing(true);
    finishStartRef.current = Date.now();
    setHoldProgress(1);
    setBanner('MILE COMPLETE!');
    speakTTS(T.voiceMile, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.marathonFinishMs);
  ...
  const tick = useCallback(() => {
    ...
    if (finishingRef.current) {
      const cp = clamp01((now - finishStartRef.current) / P.marathonFinishMs);
      setFinishProgress(cp);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        if (readout.status === 'zone' && f >= P.marathonFormMin) {
    ...
    const readout = powerMarathonReadout(m, baselineRef.current, target, band);
    const stab = enduranceStabilityScore(effortSamplesRef.current);
    const q = enduranceHoldQuality(m, baselineRef.current, readout, P.marathonFormMin, stab, controlled);
    ...
    if (readout.form < P.marathonFormMin) {
    ...
    if (roundActiveRef.current) {
      ...
      if (readout.status === 'zone' && readout.form >= P.marathonFormMin) zoneTicksRef.current += 1;

      if (readout.status === 'zone' && readout.form >= P.marathonFormMin) {
        ...
        setHoldProgress(prog);
        if (prog >= 1) beginMile();
      } else if (readout.status === 'heavy' || readout.form < P.marathonFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        effortSamplesRef.current = [];
        setHoldProgress(0);
      }
    ...
    prevMetricsRef.current = m;
  }, [beginMile, fallbackMsForRound, holdMsForRound, recordPostureBreak, recordTick, targetForRound]);

  const beginPlay = useCallback(() => {
    resetAnalytics();
    completedRef.current = 0;
    marathonSumRef.current = 0;
    marathonCountRef.current = 0;
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS('Stand tall and relax your arms. I will learn how you stand before Thunder Power Marathon begins!', 0.8).catch(
      () => {},
    );
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided power marathon mode.');
      }
    }
    beginCalibration();
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: ENDURANCE_EFFORT_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: ENDURANCE_EFFORT_SHELL.statLabel }]}>Mile</Text>
            ...
            <View style={[styles.statPill, { borderColor: ENDURANCE_EFFORT_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: ENDURANCE_EFFORT_SHELL.statValue }]}>{mileStars}</Text>
            </View>
          </View>
        </View>
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — hold each power mile in gorilla pose with steady controlled force and endure longer through the marathon!'
                    : 'Guided mode: follow the coach and practice controlled power marathon endurance!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Start Marathon</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
const styles = StyleSheet.create({
  ...
  linkText: { color: '#FCA5A5', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default PowerMarathonGame;
