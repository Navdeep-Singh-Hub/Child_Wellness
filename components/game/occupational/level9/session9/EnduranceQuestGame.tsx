/**
 * OT Level 9 · Session 9 · Game 4 — Endurance Quest (Hero's Quest Vault)
 *
 * Camera tracks sustained overhead quest pillar brace with escalating endurance holds in brace zone.
...
import { EnduranceQuestOverlay } from '@/components/game/occupational/level9/session9/components/EnduranceQuestOverlay';
import { ENDURANCE_QUEST_THEME, ENDURANCE_EFFORT_SHELL } from '@/components/game/occupational/level9/session9/enduranceEffortTheme';
import {
  ENDURANCE_QUEST_ROUNDS,
  enduranceHoldQuality,
  enduranceQuestReadout,
  enduranceQuestScore,
  enduranceStabilityScore,
  mirroredStrengthPalms,
  type EnduranceQuestRound,
...
const T = ENDURANCE_QUEST_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Quest hero!', 'Checkpoint star!', 'Steady brace!', 'Vault master!', 'Legend pro!'];

type Phase = 'intro' | 'calibrate' | 'play';

const EnduranceQuestGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundDef, setRoundDef] = useState<EnduranceQuestRound>(ENDURANCE_QUEST_ROUNDS[0]!);
  ...
  const roundDefRef = useRef<EnduranceQuestRound>(ENDURANCE_QUEST_ROUNDS[0]!);
  ...
  const targetForRound = useCallback(
    (r: number) => P.questTargets[Math.min(r, P.questTargets.length - 1)] ?? 0.6,
    [],
  );

  const holdMsForRound = useCallback(
    (r: number) => P.questHoldMs[Math.min(r, P.questHoldMs.length - 1)] ?? 2000,
    [],
  );

  const fallbackMsForRound = useCallback(
    (r: number) => P.fallbackQuestMs[Math.min(r, P.fallbackQuestMs.length - 1)] ?? 3200,
    [],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const avgQuest = questCountRef.current > 0 ? questSumRef.current / questCountRef.current : 0;
    ...
    const headline = Math.round((completion * 0.5 + avgQuest * 0.2 + moveQuality * 0.15 + zoneAccuracy * 0.15) * 100);
    const xp = Math.round(completed * 22 + avgQuest * 92 + moveQuality * 70 + zoneAccuracy * 42);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgEnduranceQuest: Math.round(avgQuest * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = ENDURANCE_QUEST_ROUNDS[Math.min(r, ENDURANCE_QUEST_ROUNDS.length - 1)] ?? ENDURANCE_QUEST_ROUNDS[0]!;
      roundRef.current = r;
      ...
      checkpointStartRef.current = 0;
      peakBraceRef.current = 0;
  ...
  const finishRound = useCallback(() => {
    ...
    const roundScore = enduranceQuestScore(peakBraceRef.current, peakStabilityRef.current);
    questSumRef.current += roundScore;
    questCountRef.current += 1;
    completedRef.current += 1;
    setCompletedCount(completedRef.current);
    setQuestProgress(completedRef.current / totalRounds);
  ...
  const beginCheckpoint = useCallback(() => {
    if (doneRef.current || checkpointingRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    ...
    setHoldProgress(1);
    setBanner('CHECKPOINT SECURED!');
    speakTTS(T.voiceCheckpoint, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.questCheckpointMs);
  ...
  const tick = useCallback(() => {
    ...
    if (checkpointingRef.current) {
      const cp = clamp01((now - checkpointStartRef.current) / P.questCheckpointMs);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        setQuality(0.85);
        if (readout.power > peakBraceRef.current) peakBraceRef.current = readout.power;
        if (stab > peakStabilityRef.current) peakStabilityRef.current = stab;
        if (readout.status === 'zone' && f >= P.questFormMin) {
    ...
    const readout = enduranceQuestReadout(m, baselineRef.current, target, band);
    const stab = enduranceStabilityScore(effortSamplesRef.current);
    const q = enduranceHoldQuality(m, baselineRef.current, readout, P.questFormMin, stab, controlled);
    ...
    if (readout.form < P.questFormMin) {
      ...
      setCoachCue(T.pillarCue);
    }

    if (roundActiveRef.current) {
      totalTicksRef.current += 1;
      if (readout.status === 'zone' && readout.form >= P.questFormMin) zoneTicksRef.current += 1;

      if (readout.status === 'zone' && readout.form >= P.questFormMin) {
        ...
        setHoldProgress(prog);
        if (prog >= 1) beginCheckpoint();
      } else if (readout.status === 'heavy' || readout.form < P.questFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        effortSamplesRef.current = [];
        setHoldProgress(0);
      }
    ...
    prevMetricsRef.current = m;
  }, [beginCheckpoint, fallbackMsForRound, holdMsForRound, recordPostureBreak, recordTick, targetForRound]);

  const beginPlay = useCallback(() => {
    resetAnalytics();
    completedRef.current = 0;
    questSumRef.current = 0;
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS("Stand tall and relax your arms. I will learn how you stand before Hero's Quest Vault begins!", 0.8).catch(
      () => {},
    );
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided endurance quest mode.');
      }
    }
    beginCalibration();
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: ENDURANCE_EFFORT_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: ENDURANCE_EFFORT_SHELL.statLabel }]}>Stage</Text>
            ...
            <View style={[styles.statPill, { borderColor: ENDURANCE_EFFORT_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: ENDURANCE_EFFORT_SHELL.statValue }]}>{secured}</Text>
            </View>
          </View>
        </View>
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your arms overhead — brace each quest pillar with steady controlled force and hold longer through the adventure!'
                    : 'Guided mode: follow the coach and practice controlled endurance quest holds!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Begin Quest</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
export default EnduranceQuestGame;
