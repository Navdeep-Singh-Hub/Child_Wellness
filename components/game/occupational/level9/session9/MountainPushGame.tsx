/**
 * OT Level 9 · Session 9 · Game 3 — Mountain Push (Alpine Boulder Ridge)
 *
 * Camera tracks sustained mountain boulder wall-push with escalating endurance holds in push zone.
...
import { MountainPushOverlay } from '@/components/game/occupational/level9/session9/components/MountainPushOverlay';
import { MOUNTAIN_PUSH_THEME, ENDURANCE_EFFORT_SHELL } from '@/components/game/occupational/level9/session9/enduranceEffortTheme';
import {
  MOUNTAIN_PUSH_ROUNDS,
  enduranceHoldQuality,
  enduranceStabilityScore,
  mountainPushReadout,
  mountainPushScore,
  type EnduranceZoneStatus,
  type MountainPushRound,
...
const T = MOUNTAIN_PUSH_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Summit star!', 'Boulder boss!', 'Steady push!', 'Ridge climber!', 'Mountain pro!'];

type Phase = 'intro' | 'calibrate' | 'play';

const MountainPushGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [summited, setSummited] = useState(0);
  const [completedCount, setCompletedCount] = useState(0);
  const [roundDef, setRoundDef] = useState<MountainPushRound>(MOUNTAIN_PUSH_ROUNDS[0]!);
  ...
  const roundDefRef = useRef<MountainPushRound>(MOUNTAIN_PUSH_ROUNDS[0]!);
  const roundActiveRef = useRef(false);
  const previewingRef = useRef(false);
  const summitingRef = useRef(false);
  const holdStartRef = useRef(0);
  const lastAtTargetRef = useRef(0);
  const summitStartRef = useRef(0);
  ...
  const targetForRound = useCallback(
    (r: number) => P.pushTargets[Math.min(r, P.pushTargets.length - 1)] ?? 0.6,
    [],
  );

  const holdMsForRound = useCallback(
    (r: number) => P.pushHoldMs[Math.min(r, P.pushHoldMs.length - 1)] ?? 2000,
    [],
  );

  const fallbackMsForRound = useCallback(
    (r: number) => P.fallbackPushMs[Math.min(r, P.fallbackPushMs.length - 1)] ?? 3200,
    [],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const xp = Math.round(completed * 22 + avgPush * 92 + moveQuality * 70 + zoneAccuracy * 42);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgMountainPush: Math.round(avgPush * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = MOUNTAIN_PUSH_ROUNDS[Math.min(r, MOUNTAIN_PUSH_ROUNDS.length - 1)] ?? MOUNTAIN_PUSH_ROUNDS[0]!;
      roundRef.current = r;
      ...
      speakTTS(`${def.name}! Push steady for ${(holdMsForRound(r) / 1000).toFixed(1)} seconds.`, 0.85).catch(() => {});
  ...
  const finishRound = useCallback(() => {
    ...
    const roundScore = mountainPushScore(peakPushRef.current, peakStabilityRef.current);
    pushSumRef.current += roundScore;
  ...
  const beginSummit = useCallback(() => {
    if (doneRef.current || summitingRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    summitingRef.current = true;
    setSummiting(true);
    summitStartRef.current = Date.now();
    setHoldProgress(1);
    setBanner('SUMMIT REACHED!');
    speakTTS(T.voiceSummit, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.pushSummitMs);
  ...
  const tick = useCallback(() => {
    ...
    if (summitingRef.current) {
      const cp = clamp01((now - summitStartRef.current) / P.pushSummitMs);
      setSummitProgress(cp);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        setStability(stab);
        setLeftPalm({ x: 0.38, y: 0.56 });
        setRightPalm({ x: 0.62, y: 0.56 });
        setQuality(0.85);
        if (readout.power > peakPushRef.current) peakPushRef.current = readout.power;
        if (stab > peakStabilityRef.current) peakStabilityRef.current = stab;
        if (readout.status === 'zone' && f >= P.pushFormMin) {
    ...
    setLeftPalm(hands.left);
    setRightPalm(hands.right);
    ...
    const readout = mountainPushReadout(m, baselineRef.current, target, band);
    const stab = enduranceStabilityScore(effortSamplesRef.current);
    const q = enduranceHoldQuality(m, baselineRef.current, readout, P.pushFormMin, stab, controlled);
    ...
    if (readout.form < P.pushFormMin) {
      ...
      setCoachCue(T.stanceCue);
    }

    if (roundActiveRef.current) {
      totalTicksRef.current += 1;
      if (readout.status === 'zone' && readout.form >= P.pushFormMin) zoneTicksRef.current += 1;

      if (readout.status === 'zone' && readout.form >= P.pushFormMin) {
        ...
        setHoldProgress(prog);
        if (prog >= 1) beginSummit();
      } else if (readout.status === 'heavy' || readout.form < P.pushFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        effortSamplesRef.current = [];
        setHoldProgress(0);
      }
    ...
    prevMetricsRef.current = m;
  }, [beginSummit, fallbackMsForRound, holdMsForRound, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS('Stand tall and relax your arms. I will learn how you stand before Alpine Boulder Ridge begins!', 0.8).catch(
      () => {},
    );
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided mountain push mode.');
      }
    }
    beginCalibration();
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          ...
          <Text style={styles.academyLabel}>⛰️ {ENDURANCE_EFFORT_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: ENDURANCE_EFFORT_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: ENDURANCE_EFFORT_SHELL.statLabel }]}>Peak</Text>
            ...
            <View style={[styles.statPill, { borderColor: ENDURANCE_EFFORT_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: ENDURANCE_EFFORT_SHELL.statValue }]}>{summited}</Text>
            </View>
          </View>
        </View>
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your arms — push each boulder with steady controlled force and hold longer as you climb the ridge!'
                    : 'Guided mode: follow the coach and practice controlled mountain push endurance!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Climb Ridge</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: '#D6D3D1', fontSize: 11, fontWeight: '900', letterSpacing: 0.8 },
  header: { alignItems: 'center', marginTop: 8 },
  title: { color: '#fff', fontSize: 23, fontWeight: '900', textAlign: 'center' },
  subtitle: { color: '#D6D3D1', fontSize: 13, fontWeight: '600', marginTop: 2, textAlign: 'center', paddingHorizontal: 8 },
  ...
  introText: { color: '#D6D3D1', fontSize: 14, fontWeight: '600', textAlign: 'center' },
  ...
  linkText: { color: '#A8A29E', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default MountainPushGame;
