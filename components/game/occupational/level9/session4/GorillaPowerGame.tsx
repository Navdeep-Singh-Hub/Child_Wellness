/**
 * OT Level 9 · Session 4 · Game 3 — Gorilla Power (Jungle Power Grove)
 *
 * Camera tracks gorilla chest-beat pose and sustained power effort in target zone.
...
import { GorillaPowerOverlay } from '@/components/game/occupational/level9/session4/components/GorillaPowerOverlay';
import { GORILLA_POWER_THEME, HEAVY_WORK_SHELL } from '@/components/game/occupational/level9/session4/heavyWorkTheme';
import {
  carryZoneStatus,
  gorillaArmRaise,
  gorillaBeatHeight,
...
const T = GORILLA_POWER_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Gorilla roar!', 'Power beat!', 'Jungle strong!', 'Perfect force!', 'Grove king!'];

type Phase = 'intro' | 'calibrate' | 'play';

const GorillaPowerGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roarProgress, setRoarProgress] = useState(0);
  ...
  const targetForRound = useCallback(
    (r: number) => P.gorillaTargets[Math.min(r, P.gorillaTargets.length - 1)] ?? 0.65,
    [],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const avgPower = powerCountRef.current > 0 ? powerSumRef.current / powerCountRef.current : 0;
    ...
    const headline = Math.round((completion * 0.5 + avgPower * 0.2 + moveQuality * 0.15 + zoneAccuracy * 0.15) * 100);
    const xp = Math.round(completed * 21 + avgPower * 88 + moveQuality * 68 + zoneAccuracy * 40);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgGorillaPower: Math.round(avgPower * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      speakTTS(`Gorilla beat ${r + 1}!`, 0.85).catch(() => {});
  ...
  const beginRoar = useCallback(() => {
    if (doneRef.current || roaringRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    roaringRef.current = true;
    setRoaring(true);
    roarStartRef.current = Date.now();
    setHoldProgress(1);
    setBanner('ROAR!');
    speakTTS(T.voiceRoar, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.gorillaRoarMs);
  ...
  const tick = useCallback(() => {
    ...
    const band = P.gorillaBandHalf;

    if (roaringRef.current) {
      const rp = clamp01((now - roarStartRef.current) / P.gorillaRoarMs);
      setRoarProgress(rp);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackGorillaMs);
        const p = clamp01(target * (0.5 + t * 0.55));
        const f = clamp01(0.54 + t * 0.4);
        const status = carryZoneStatus(p, target, band);
        setPower(p);
        setForm(f);
        setZoneStatus(status);
        setLeftHand({ x: 0.32, y: 0.42 - t * 0.06 });
        setRightHand({ x: 0.68, y: 0.42 - t * 0.06 });
        setQuality(0.85);
        if (p > peakPowerRef.current) peakPowerRef.current = p;
        if (status === 'zone' && f >= P.gorillaFormMin) {
          const zoneElapsed = Math.max(0, elapsed - P.fallbackGorillaMs * 0.48);
          const prog = clamp01(zoneElapsed / P.gorillaHoldMs);
    ...
    const poseForm = gorillaPoseForm(m, baselineRef.current);
    const p = gorillaPowerScore(m, baselineRef.current);
    const status = carryZoneStatus(p, target, band);
    ...
    const armRaise = gorillaArmRaise(m);
    const beat = gorillaBeatHeight(m);

    if (poseForm < P.gorillaFormMin) {
      if (stance < armRaise && stance < beat) setCoachCue(T.stanceCue);
    ...
    if (roundActiveRef.current) {
      ...
      if (status === 'zone' && poseForm >= P.gorillaFormMin) zoneTicksRef.current += 1;

      if (status === 'zone' && poseForm >= P.gorillaFormMin) {
        if (holdStartRef.current === 0) holdStartRef.current = now;
        lastAtTargetRef.current = now;
        const prog = clamp01((now - holdStartRef.current) / P.gorillaHoldMs);
        setHoldProgress(prog);
        if (prog >= 1) beginRoar();
      } else if (status === 'heavy' || poseForm < P.gorillaFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        setHoldProgress(0);
      }
    }
    ...
    prevMetricsRef.current = m;
  }, [beginRoar, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS('Stand tall with your full body visible. I will learn your pose before the Jungle Power Grove begins!', 0.8).catch(
      () => {},
    );
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided gorilla mode.');
      }
    }
    beginCalibration();
  ...
  const groveProgress = completedCount / totalRounds;
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            ...
            <View style={[styles.statPill, { borderColor: HEAVY_WORK_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: HEAVY_WORK_SHELL.statValue }]}>{roars}</Text>
            </View>
          </View>
        </View>
          ...
          >
            {phase === 'play' && (
              <GorillaPowerOverlay
                ...
                groveProgress={groveProgress}
                leftHand={leftHand}
                rightHand={rightHand}
                banner={banner}
                quality={quality}
                bandHalf={P.gorillaBandHalf}
              />
            )}
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — wide stance, raised arms and chest beats!'
                    : 'Guided mode: follow the coach and practice controlled gorilla power beats!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Enter Grove</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: '#BEF264', fontSize: 11, fontWeight: '900', letterSpacing: 0.8 },
  header: { alignItems: 'center', marginTop: 8 },
  title: { color: '#fff', fontSize: 23, fontWeight: '900', textAlign: 'center' },
  subtitle: { color: '#BEF264', fontSize: 13, fontWeight: '600', marginTop: 2, textAlign: 'center', paddingHorizontal: 8 },
  ...
  introText: { color: '#BEF264', fontSize: 14, fontWeight: '600', textAlign: 'center' },
...
export default GorillaPowerGame;
