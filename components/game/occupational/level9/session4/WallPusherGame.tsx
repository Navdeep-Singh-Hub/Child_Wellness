/**
 * OT Level 9 · Session 4 · Game 2 — Wall Pusher (Stone Fortress Corridor)
 *
 * Camera tracks wall-push pose form and sustained bilateral thrust to breach fortress walls.
...
import { WallPusherOverlay } from '@/components/game/occupational/level9/session4/components/WallPusherOverlay';
import { HEAVY_WORK_SHELL, WALL_PUSHER_THEME } from '@/components/game/occupational/level9/session4/heavyWorkTheme';
import {
  carryZoneStatus,
  wallPushForceScore,
  wallPushPoseForm,
...
const T = WALL_PUSHER_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Wall breached!', 'Steady push!', 'Perfect thrust!', 'Strong control!', 'Fortress pro!'];

type Phase = 'intro' | 'calibrate' | 'play';

const WallPusherGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const breachingRef = useRef(false);
  const holdStartRef = useRef(0);
  const lastAtTargetRef = useRef(0);
  const breachStartRef = useRef(0);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgPushEffort: Math.round(avgEffort * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      breachStartRef.current = 0;
      peakEffortRef.current = 0;
      previewingRef.current = true;
      breachingRef.current = false;
      ...
      speakTTS(`Push ${T.walls[r % T.walls.length]} wall!`, 0.85).catch(() => {});
  ...
  const beginBreach = useCallback(() => {
    if (doneRef.current || breachingRef.current) return;
    roundActiveRef.current = false;
    setRoundActive(false);
    breachingRef.current = true;
    setBreaching(true);
    breachStartRef.current = Date.now();
    setHoldProgress(1);
    setBanner('BREACHED!');
    speakTTS(T.voiceBreach, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.pushBreachMs);
  ...
  const tick = useCallback(() => {
    ...
    const band = P.pushBandHalf;

    if (breachingRef.current) {
      const bp = clamp01((now - breachStartRef.current) / P.pushBreachMs);
      setBreachProgress(bp);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackWallPushMs);
        ...
        if (status === 'zone' && f >= P.wallFormMin) {
          const zoneElapsed = Math.max(0, elapsed - P.fallbackWallPushMs * 0.48);
          const prog = clamp01(zoneElapsed / P.pushHoldMs);
          setHoldProgress(prog);
          if (prog >= 1) beginBreach();
        } else {
          setHoldProgress(0);
        }
      }
    ...
    const poseForm = wallPushPoseForm(m, baselineRef.current);
    const e = wallPushForceScore(m, baselineRef.current);
    const status = carryZoneStatus(e, target, band);
    ...
    if (roundActiveRef.current) {
      ...
      if (status === 'zone' && poseForm >= P.wallFormMin) zoneTicksRef.current += 1;

      if (status === 'zone' && poseForm >= P.wallFormMin) {
        if (holdStartRef.current === 0) holdStartRef.current = now;
        lastAtTargetRef.current = now;
        const prog = clamp01((now - holdStartRef.current) / P.pushHoldMs);
        setHoldProgress(prog);
        if (prog >= 1) beginBreach();
      } else if (status === 'heavy' || poseForm < P.wallFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        setHoldProgress(0);
      }
    }
    ...
    prevMetricsRef.current = m;
  }, [beginBreach, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS('Stand tall and relax your arms. I will learn how you stand before the fortress corridor begins!', 0.8).catch(
      () => {},
    );
  ...
  const corridorProgress = completedCount / totalRounds;
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            ...
            <View style={[styles.statPill, { borderColor: HEAVY_WORK_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: HEAVY_WORK_SHELL.statValue }]}>{breached}</Text>
            </View>
          </View>
        </View>
          ...
          >
            {phase === 'play' && (
              <WallPusherOverlay
                ...
                breachedCount={completedCount}
                corridorProgress={corridorProgress}
                leftPalm={leftPalm}
                rightPalm={rightPalm}
                banner={banner}
                quality={quality}
                bandHalf={P.pushBandHalf}
              />
            )}
            ...
            {camLive && error && permissionGranted ? (
              <>
                ...
                <View style={styles.btnRow}>
                  <Pressable style={[styles.primaryBtn, { backgroundColor: T.stone }]} onPress={() => setActive(true)}>
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your arms — extend and push both palms into each fortress wall!'
                    : 'Guided mode: follow the coach and practice controlled wall pushing!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Enter Corridor</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
const styles = StyleSheet.create({
  ...
  academyLabel: { color: '#CBD5E1', fontSize: 11, fontWeight: '900', letterSpacing: 0.8 },
  header: { alignItems: 'center', marginTop: 8 },
  title: { color: '#fff', fontSize: 23, fontWeight: '900', textAlign: 'center' },
  subtitle: { color: '#CBD5E1', fontSize: 13, fontWeight: '600', marginTop: 2, textAlign: 'center', paddingHorizontal: 8 },
  ...
  introText: { color: '#CBD5E1', fontSize: 14, fontWeight: '600', textAlign: 'center' },
  ...
  linkText: { color: '#94A3B8', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default WallPusherGame;
