/**
 * OT Level 9 · Session 10 · Game 1 — Jungle Worksite (Jungle Timber Worksite)
 *
 * Camera tracks carry pose form and sustained jungle log haul effort to deliver timber to worksite stations.
...
import { JungleWorksiteOverlay } from '@/components/game/occupational/level9/session10/components/JungleWorksiteOverlay';
import { JUNGLE_WORKSITE_THEME, PROPRIOCEPTIVE_ADVENTURE_SHELL } from '@/components/game/occupational/level9/session10/proprioceptiveAdventureTheme';
import {
  JUNGLE_WORKSITE_ROUNDS,
  adventureHaulQuality,
  jungleWorksiteReadout,
  jungleWorksiteScore,
  mirroredCarryHands,
  type AdventureZoneStatus,
  type JungleWorksiteRound,
...
const T = JUNGLE_WORKSITE_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Log delivered!', 'Jungle haul!', 'Steady carry!', 'Worksite pro!', 'Timber star!'];

type Phase = 'intro' | 'calibrate' | 'play';

const JungleWorksiteGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundDef, setRoundDef] = useState<JungleWorksiteRound>(JUNGLE_WORKSITE_ROUNDS[0]!);
  const [worksiteProgress, setWorksiteProgress] = useState(0);
  ...
  const roundDefRef = useRef<JungleWorksiteRound>(JUNGLE_WORKSITE_ROUNDS[0]!);
  ...
  const targetForRound = useCallback(
    (r: number) => P.worksiteTargets[Math.min(r, P.worksiteTargets.length - 1)] ?? 0.6,
    [],
  );

  const holdMsForRound = useCallback(
    (r: number) => P.worksiteHoldMs[Math.min(r, P.worksiteHoldMs.length - 1)] ?? 1700,
    [],
  );

  const fallbackMsForRound = useCallback(
    (r: number) => P.fallbackWorksiteMs[Math.min(r, P.fallbackWorksiteMs.length - 1)] ?? 3000,
    [],
  );
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgJungleWorksite: Math.round(avgEffort * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = JUNGLE_WORKSITE_ROUNDS[Math.min(r, JUNGLE_WORKSITE_ROUNDS.length - 1)] ?? JUNGLE_WORKSITE_ROUNDS[0]!;
      roundRef.current = r;
      ...
      speakTTS(`${def.name}! Haul steady for ${(holdMsForRound(r) / 1000).toFixed(1)} seconds.`, 0.85).catch(() => {});
      ...
      schedule(() => {
        ...
        speakTTS(T.voiceHaul, 0.85).catch(() => {});
  ...
  const finishRound = useCallback(() => {
    ...
    const roundScore = jungleWorksiteScore(peakPowerRef.current);
    effortSumRef.current += roundScore;
    effortCountRef.current += 1;
    completedRef.current += 1;
    setCompletedCount(completedRef.current);
    setWorksiteProgress(completedRef.current / totalRounds);
  ...
  const beginDeliver = useCallback(() => {
    ...
    setHoldProgress(1);
    setBanner('LOG DELIVERED!');
    speakTTS(T.voiceDeliver, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.worksiteDeliverMs);
  ...
  const tick = useCallback(() => {
    ...
    if (deliveringRef.current) {
      const dp = clamp01((now - deliverStartRef.current) / P.worksiteDeliverMs);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const readout = { form: f, effort: e, status: (e >= target - band && e <= target + band ? 'zone' : e < target - band ? 'light' : 'heavy') as AdventureZoneStatus, power: clamp01(f * 0.46 + e * 0.54) };
        setEffort(e);
        setForm(f);
        ...
        if (readout.status === 'zone' && f >= P.worksiteFormMin) {
    ...
    const readout = jungleWorksiteReadout(m, baselineRef.current, target, band);
    const q = adventureHaulQuality(m, baselineRef.current, readout, P.worksiteFormMin, controlled, positioned);
    ...
    if (readout.form < P.worksiteFormMin) {
    ...
    if (roundActiveRef.current) {
      ...
      if (readout.status === 'zone' && readout.form >= P.worksiteFormMin) zoneTicksRef.current += 1;

      if (readout.status === 'zone' && readout.form >= P.worksiteFormMin) {
        ...
        setHoldProgress(prog);
        if (prog >= 1) beginDeliver();
      } else if (readout.status === 'heavy' || readout.form < P.worksiteFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        setHoldProgress(0);
      }
    }
    ...
    prevMetricsRef.current = m;
  }, [beginDeliver, fallbackMsForRound, holdMsForRound, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS('Stand tall and relax your arms. I will learn how you stand before Jungle Timber Worksite begins!', 0.8).catch(
      () => {},
    );
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided jungle worksite mode.');
      }
    }
    beginCalibration();
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: PROPRIOCEPTIVE_ADVENTURE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: PROPRIOCEPTIVE_ADVENTURE_SHELL.statLabel }]}>Site</Text>
            ...
            <View style={[styles.statPill, { borderColor: PROPRIOCEPTIVE_ADVENTURE_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: PROPRIOCEPTIVE_ADVENTURE_SHELL.statValue }]}>{delivered}</Text>
            </View>
          </View>
        </View>
          ...
          >
            {phase === 'play' && (
              <JungleWorksiteOverlay
                ...
                worksiteProgress={worksiteProgress}
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your arms — haul each jungle log to the worksite with steady controlled effort!'
                    : 'Guided mode: follow the coach and practice controlled jungle timber hauling!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Enter Worksite</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
const styles = StyleSheet.create({
  ...
  linkText: { color: '#84CC16', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default JungleWorksiteGame;
