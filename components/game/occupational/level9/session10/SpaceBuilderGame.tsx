/**
 * OT Level 9 · Session 10 · Game 2 — Space Builder (Orbital Construction Bay)
 *
 * Camera tracks wall-push pose form and sustained bilateral force to install space modules.
...
import { SpaceBuilderOverlay } from '@/components/game/occupational/level9/session10/components/SpaceBuilderOverlay';
import { PROPRIOCEPTIVE_ADVENTURE_SHELL, SPACE_BUILDER_THEME } from '@/components/game/occupational/level9/session10/proprioceptiveAdventureTheme';
import {
  SPACE_BUILDER_ROUNDS,
  adventurePushQuality,
  mirroredPushPalms,
  spaceBuilderReadout,
  spaceBuilderScore,
  type AdventureZoneStatus,
  type SpaceBuilderRound,
...
const T = SPACE_BUILDER_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Module locked!', 'Space push!', 'Steady thrust!', 'Builder pro!', 'Orbital star!'];

type Phase = 'intro' | 'calibrate' | 'play';

const SpaceBuilderGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [lockedStars, setLockedStars] = useState(0);
  const [completedCount, setCompletedCount] = useState(0);
  const [roundDef, setRoundDef] = useState<SpaceBuilderRound>(SPACE_BUILDER_ROUNDS[0]!);
  ...
  const roundDefRef = useRef<SpaceBuilderRound>(SPACE_BUILDER_ROUNDS[0]!);
  ...
  const targetForRound = useCallback(
    (r: number) => P.builderTargets[Math.min(r, P.builderTargets.length - 1)] ?? 0.6,
    [],
  );

  const holdMsForRound = useCallback(
    (r: number) => P.builderHoldMs[Math.min(r, P.builderHoldMs.length - 1)] ?? 1700,
    [],
  );

  const fallbackMsForRound = useCallback(
    (r: number) => P.fallbackBuilderMs[Math.min(r, P.fallbackBuilderMs.length - 1)] ?? 3000,
    [],
  );
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgSpaceBuilder: Math.round(avgEffort * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = SPACE_BUILDER_ROUNDS[Math.min(r, SPACE_BUILDER_ROUNDS.length - 1)] ?? SPACE_BUILDER_ROUNDS[0]!;
      roundRef.current = r;
  ...
  const finishRound = useCallback(() => {
    ...
    const roundScore = spaceBuilderScore(peakPowerRef.current);
    effortSumRef.current += roundScore;
    effortCountRef.current += 1;
    completedRef.current += 1;
    setCompletedCount(completedRef.current);
    setBuildProgress(completedRef.current / totalRounds);
  ...
  const beginLock = useCallback(() => {
    ...
    setHoldProgress(1);
    setBanner('MODULE LOCKED!');
    speakTTS(T.voiceLock, 0.85).catch(() => {});
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy).catch(() => {});
    schedule(() => finishRound(), P.builderLockMs);
  ...
  const tick = useCallback(() => {
    ...
    if (lockingRef.current) {
      const lp = clamp01((now - lockStartRef.current) / P.builderLockMs);
      setLockProgress(lp);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const readout = {
          ...
          power: clamp01(f * 0.44 + e * 0.56),
        };
        setEffort(e);
        setForm(f);
        setZoneStatus(readout.status);
        setLeftPalm({ x: 0.36, y: 0.58 - t * 0.1 });
        setRightPalm({ x: 0.64, y: 0.58 - t * 0.1 });
        setQuality(0.85);
        if (readout.power > peakPowerRef.current) peakPowerRef.current = readout.power;
        if (readout.status === 'zone' && f >= P.builderFormMin) {
    ...
    const readout = spaceBuilderReadout(m, baselineRef.current, target, band);
    const q = adventurePushQuality(m, baselineRef.current, readout, P.builderFormMin, controlled, positioned);
    ...
    if (readout.form < P.builderFormMin) {
    ...
    if (roundActiveRef.current) {
      ...
      if (readout.status === 'zone' && readout.form >= P.builderFormMin) zoneTicksRef.current += 1;

      if (readout.status === 'zone' && readout.form >= P.builderFormMin) {
        ...
        setHoldProgress(prog);
        if (prog >= 1) beginLock();
      } else if (readout.status === 'heavy' || readout.form < P.builderFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        setHoldProgress(0);
      }
    }
    ...
    prevMetricsRef.current = m;
  }, [beginLock, fallbackMsForRound, holdMsForRound, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS('Stand tall and relax your arms. I will learn how you stand before Orbital Construction Bay begins!', 0.8).catch(
      () => {},
    );
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided space builder mode.');
      }
    }
    beginCalibration();
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: PROPRIOCEPTIVE_ADVENTURE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: PROPRIOCEPTIVE_ADVENTURE_SHELL.statLabel }]}>Bay</Text>
            ...
            <View style={[styles.statPill, { borderColor: PROPRIOCEPTIVE_ADVENTURE_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: PROPRIOCEPTIVE_ADVENTURE_SHELL.statValue }]}>{lockedStars}</Text>
            </View>
          </View>
        </View>
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your arms — push each space module into place with steady controlled force!'
                    : 'Guided mode: follow the coach and practice controlled space module installation!'}
                </Text>
...
export default SpaceBuilderGame;
