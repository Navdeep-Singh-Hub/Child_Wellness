/** Jungle Worksite overlay — OT L9 S10 Game 1 */
import type { JungleWorksiteTheme } from '@/components/game/occupational/level9/session10/proprioceptiveAdventureTheme';
import { PROPRIOCEPTIVE_ADVENTURE_SHELL } from '@/components/game/occupational/level9/session10/proprioceptiveAdventureTheme';
import type { AdventureZoneStatus, JungleWorksiteRound } from '@/components/game/occupational/level9/session10/proprioceptiveAdventureUtils';
...
type Props = {
  theme: JungleWorksiteTheme;
  roundDef: JungleWorksiteRound;
  effort: number;
  form: number;
  targetEffort: number;
...
export function JungleWorksiteOverlay({
  theme,
  roundDef,
  effort,
  form,
  ...
  const canopyStyle = useAnimatedStyle(() => ({
    opacity: 0.24 + canopy.value * 0.36,
  }));
  ...
  return (
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      {theme.decor.map((d, i) => (
        <Animated.Text
          key={`jungle-${i}`}
          style={[styles.decor, canopyStyle, { left: `${5 + (i * 16) % 85}%`, top: `${3 + (i % 4) * 7}%` }]}
        >
          {d}
        </Animated.Text>
      ))}
      ...
      {rightHand && (
        <View style={[styles.handDot, { left: `${rightHand.x * 100}%`, top: `${rightHand.y * 100}%`, borderColor: theme.timber }]}>
      ...
      <View style={styles.hudWrap}>
        ...
        <View style={styles.hudRow}>
          ...
          <Text style={[styles.zoneBadge, { color: inZone ? PROPRIOCEPTIVE_ADVENTURE_SHELL.good : theme.timber }]}>
            {inZone ? 'STEADY HAUL' : zoneStatus === 'heavy' ? 'TOO HEAVY' : 'HAUL FIRMER'}
          </Text>
          <Text style={styles.roundText}>
            Site {round + 1}/{totalRounds}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.deliveredRow}>
        {Array.from({ length: totalRounds }).map((_, i) => (
          <Text key={i} style={[styles.deliveredDot, { opacity: i < deliveredCount ? 1 : 0.25 }]}>
            {i < deliveredCount ? theme.sites[i % theme.sites.length] : '·'}
          </Text>
        ))}
      </View>

      <View style={styles.worksiteWrap}>
        <Text style={styles.worksiteLabel}>🌴 WORKSITE TRAIL {Math.round(worksiteProgress * 100)}%</Text>
        <View style={styles.worksiteTrack}>
          <LinearGradient
            colors={[theme.accentDeep, theme.accent, theme.timber]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
            style={[styles.worksiteFill, { width: `${worksiteProgress * 100}%` }]}
          />
        </View>
      </View>
      ...
      {!previewing && (
        <View style={styles.logWrap}>
          <View style={[styles.logCard, { borderColor: inZone ? PROPRIOCEPTIVE_ADVENTURE_SHELL.good : theme.timber }]}>
            <Text style={styles.logEmoji}>{roundDef.icon}</Text>
            <Text style={styles.logLabel}>{roundDef.site}</Text>
            <Text style={styles.holdHint}>Haul {holdSeconds.toFixed(1)}s</Text>
          </View>
      ...
      {previewing && (
        <View style={styles.previewWrap}>
          ...
          <Text style={styles.previewSub}>Haul target {targetPct}% · Hold {holdSeconds.toFixed(1)}s</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  handLabel: { fontSize: 10, fontWeight: '900', color: '#166534' },
  hudWrap: {
    position: 'absolute',
    top: 8,
    left: 10,
  ...
  hudTitle: { color: '#ECFDF5', fontSize: 9, fontWeight: '900', letterSpacing: 0.4 },
  meterTrack: {
    height: 10,
    borderRadius: 5,
  ...
  worksiteLabel: { color: '#BBF7D0', fontSize: 10, fontWeight: '800', marginBottom: 4 },
  worksiteTrack: {
    height: 8,
    borderRadius: 4,
  ...
  worksiteFill: { height: '100%', borderRadius: 4 },
  logWrap: { position: 'absolute', top: '38%', alignSelf: 'center', alignItems: 'center' },
  logCard: {
    width: 120,
    paddingVertical: 18,
    borderRadius: 16,
  ...
  logEmoji: { fontSize: 36 },
  logLabel: { color: '#ECFDF5', fontSize: 9, fontWeight: '900', letterSpacing: 1, marginTop: 4 },
  holdHint: { color: '#84CC16', fontSize: 8, fontWeight: '800', marginTop: 3 },
  holdRing: {
    marginTop: 10,
    width: 100,
    height: 7,
  ...
  previewText: { color: '#ECFDF5', fontSize: 14, fontWeight: '800', marginTop: 8 },
  previewSub: { color: '#84CC16', fontSize: 11, fontWeight: '700', marginTop: 4 },
  deliverText: {
    position: 'absolute',
    bottom: '14%',
    alignSelf: 'center',
