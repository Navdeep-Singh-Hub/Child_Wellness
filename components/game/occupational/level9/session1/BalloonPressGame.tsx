/**
 * OT Level 9 · Session 1 · Game 1 — Balloon Press (Candy Cloud Press)
 *
 * Camera tracks wrist squeeze + forward press to inflate a virtual balloon.
...
import { BalloonOverlay } from '@/components/game/occupational/level9/session1/components/BalloonOverlay';
import { BALLOON_PRESS_THEME, FORCE_SHELL } from '@/components/game/occupational/level9/session1/forceTheme';
import {
...
const T = BALLOON_PRESS_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Great press!', 'Perfect force!', 'Nice control!', 'So gentle!', 'Strong press!'];

type Phase = 'intro' | 'calibrate' | 'play';

const BalloonPressGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const targetForRound = useCallback((r: number) => P.targetForces[Math.min(r, P.targetForces.length - 1)] ?? 0.7, []);
  ...
  const endGame = useCallback(() => {
    ...
    const xp = Math.round(completed * 18 + avgForce * 80 + moveQuality * 60);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgForce: Math.round(avgForce * 100),
  ...
  const startRound = useCallback(
    (r: number) => {
      ...
      setBanner(T.pressCue);
      setCoachCue(T.hintText);
      speakTTS(T.voicePress, 0.85).catch(() => {});
  ...
  const tick = useCallback(() => {
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const t = clamp01(elapsed / P.fallbackPressMs);
        setForce(t * target * 1.1);
        setLeftHand({ x: 0.38, y: 0.55 + t * 0.08 });
        setRightHand({ x: 0.62, y: 0.55 + t * 0.08 });
        setHoldProgress(t);
        setQuality(0.82);
    ...
    const controlled = clamp01(1 - Math.max(0, intensity - 0.55) / 0.45);
    const f = pressForceScore(m, baselineRef.current);
    const q = clamp01((positioned ? 0.4 : 0.2) + 0.35 * controlled + 0.25 * f);
    setForce(f);
    setQuality(q);
    if (f > peakForceRef.current) peakForceRef.current = f;

    if (roundActiveRef.current) {
      const atTarget = f >= target * 0.92;
      if (atTarget) {
    ...
    prevMetricsRef.current = m;
  }, [camLiveRef, completeRound, recordPostureBreak, recordTick, targetForRound]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    setCoachCue('Relax your arms at your sides — I am learning your starting pose.');
    speakTTS('Stand tall and relax your arms. I will learn how you stand before we press!', 0.8).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided press mode.');
      }
    }
    beginCalibration();
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          <TouchableOpacity onPress={handleBack} style={[styles.backBtn, { borderColor: FORCE_SHELL.backBorder }]}>
            <Text style={[styles.backText, { color: FORCE_SHELL.backText }]}>← Back</Text>
          </TouchableOpacity>
          <Text style={styles.academyLabel}>🫧 {FORCE_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: FORCE_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: FORCE_SHELL.statLabel }]}>Round</Text>
              <Text style={[styles.statValue, { color: FORCE_SHELL.statValue }]}>
                {Math.min(round + (phase === 'play' ? 1 : 0), totalRounds)}/{totalRounds}
              </Text>
            </View>
            <View style={[styles.statPill, { borderColor: FORCE_SHELL.statBorder }]}>
              <Text style={styles.popEmoji}>🎈</Text>
              <Text style={[styles.statValue, { color: FORCE_SHELL.statValue }]}>{pops}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          ...
          >
            ...
            <SparkleBurst key={sparkleKey} visible={sparkleKey > 0} color={FORCE_SHELL.sparkleColor} count={18} />
            {phase === 'calibrate' && (
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Stand back so the camera sees your arms — press the balloon with your hands!'
                    : 'Guided mode: follow the coach and practice gentle-to-firm presses!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accent }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Start Press</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
export default BalloonPressGame;
