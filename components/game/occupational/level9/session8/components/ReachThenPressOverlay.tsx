/** Reach Then Press overlay — OT L9 S8 Game 2 */
...
import type { ReachThenPressTheme } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingTheme';
import { PROPRIO_SEQUENCING_SHELL } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingTheme';
import type { ReachThenPressRound, SequenceZoneStatus } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingUtils';
...
type Props = {
  theme: ReachThenPressTheme;
  roundDef: ReachThenPressRound;
  stepIndex: 0 | 1;
  effort: number;
  form: number;
  armsScore: number;
...
export function ReachThenPressOverlay({
  theme,
  roundDef,
  stepIndex,
  effort,
  ...
  useEffect(() => {
    const ratio = Math.min(1, effort / Math.max(0.35, stepIndex === 1 ? targetEffort : form || 0.35));
    padGlow.value = withTiming(completing ? 1 : ratio * 0.65, { duration: 220 });
  }, [completing, effort, form, padGlow, stepIndex, targetEffort]);

  const pulseStyle = useAnimatedStyle(() => ({ opacity: 0.22 + pulse.value * 0.38 }));
  const padStyle = useAnimatedStyle(() => ({
    opacity: 0.35 + padGlow.value * 0.55,
    transform: [{ scale: 1 + padGlow.value * 0.08 }],
  }));
  ...
  const stepAccent = stepIndex === 0 ? theme.reach : theme.press;
    ...
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      ...
      <View style={styles.hudWrap}>
        <Text style={styles.hudTitle}>
          {stepIndex === 0 ? 'REACH' : 'PRESS'} {metricPct}% · FORM {formPct}%
        ...
        <View style={styles.hudRow}>
          ...
          <Text style={[styles.zoneBadge, { color: inZone ? PROPRIO_SEQUENCING_SHELL.good : stepAccent }]}>
            {inZone ? (stepIndex === 0 ? 'ON TARGET' : 'IN ZONE') : zoneStatus === 'heavy' ? 'TOO STRONG' : stepIndex === 0 ? 'REACH MORE' : 'PRESS FIRMER'}
          </Text>
          <Text style={styles.roundText}>
            Seq {round + 1}/{totalRounds}
          </Text>
        </View>
        {stepIndex === 0 && (
      ...
      <View style={styles.planRow}>
        <StepChip n={1} icon="🎯" label={roundDef.reachLabel} state={stepIndex === 0 ? 'active' : stepIndex === 1 ? 'done' : 'pending'} accent={theme.reach} />
        <Text style={styles.arrow}>→</Text>
        <StepChip n={2} icon="🫸" label={roundDef.pressLabel} state={stepIndex === 1 ? 'active' : 'pending'} accent={theme.press} />
      </View>

      <View style={styles.seqWrap}>
        <Text style={styles.seqLabel}>🛰️ SEQUENCE {Math.round(sequenceProgress * 100)}%</Text>
      ...
      <View style={styles.targetsRow}>
        {Array.from({ length: totalRounds }).map((_, i) => (
          <Text key={i} style={[styles.targetDot, { opacity: i < sequenceCount ? 1 : 0.25 }]}>
            {i < sequenceCount ? theme.targets[i % theme.targets.length] : '·'}
          </Text>
        ))}
      </View>
      ...
      {previewing && (
        <View style={styles.previewWrap}>
          ...
          <Text style={styles.previewSeq}>🎯 Reach → 🫸 Press</Text>
        </View>
      )}
      ...
      {!previewing && stepIndex === 0 && (
        <View style={styles.figureRow}>
          <View style={styles.figureCol}>
            ...
            <FullBodyFigure pose={roundDef.reach} accent={theme.reach} jointColor={theme.accentDeep} label={roundDef.target} compact />
          </View>
          <Text style={styles.arrow}>→</Text>
          <View style={styles.figureCol}>
            ...
            <FullBodyFigure
              pose={playerPose}
              accent={inZone ? PROPRIO_SEQUENCING_SHELL.good : theme.accent}
              jointColor={theme.reach}
              matched={inZone}
              compact
            />
          </View>
      ...
      {!previewing && stepIndex === 1 && (
        <Animated.View style={[styles.padWrap, padStyle]}>
          ...
          <Text style={styles.padLabel}>FORCE PAD</Text>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  hudTitle: { color: '#DBEAFE', fontSize: 9, fontWeight: '900', letterSpacing: 0.4 },
  meterTrack: {
    height: 10,
    borderRadius: 5,
  ...
  qualityText: { color: '#93C5FD', fontSize: 11, fontWeight: '700' },
  ...
  chipNum: { color: '#DBEAFE', fontSize: 8, fontWeight: '900' },
  chipIcon: { fontSize: 18, marginTop: 2 },
  chipLabel: { color: '#93C5FD', fontSize: 8, fontWeight: '900', letterSpacing: 0.5, marginTop: 2 },
  arrow: { color: PROPRIO_SEQUENCING_SHELL.gold, fontSize: 16, fontWeight: '900' },
  seqWrap: { position: 'absolute', top: '18%', left: 14, right: 14 },
  seqLabel: { color: '#93C5FD', fontSize: 10, fontWeight: '800', marginBottom: 4 },
  seqBar: {
    height: 8,
    borderRadius: 4,
  ...
  targetsRow: {
    ...
    paddingHorizontal: 10,
    paddingVertical: 4,
    backgroundColor: 'rgba(12,25,41,0.45)',
    borderRadius: 16,
  },
  targetDot: { fontSize: 14 },
  previewWrap: { position: 'absolute', top: '40%', alignSelf: 'center', alignItems: 'center' },
  previewEmoji: { fontSize: 68 },
  previewText: { color: '#DBEAFE', fontSize: 14, fontWeight: '800', marginTop: 8 },
  previewSeq: { color: '#93C5FD', fontSize: 12, fontWeight: '900', marginTop: 4 },
  figureRow: {
    position: 'absolute',
    top: '42%',
    alignSelf: 'center',
  ...
  figureLabel: { color: '#93C5FD', fontSize: 7, fontWeight: '900', letterSpacing: 0.6, marginBottom: 3 },
  padWrap: {
    position: 'absolute',
    top: '44%',
    alignSelf: 'center',
  ...
  padEmoji: { fontSize: 52 },
  padLabel: { color: '#C7D2FE', fontSize: 11, fontWeight: '900', marginTop: 6, letterSpacing: 0.6 },
  hand: {
    position: 'absolute',
    width: 28,
    height: 28,
