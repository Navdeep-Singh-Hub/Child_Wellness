/** Pirate Work Mission overlay — OT L9 S8 Game 4 */
import type { PirateWorkMissionTheme } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingTheme';
import { PROPRIO_SEQUENCING_SHELL } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingTheme';
import type { PirateWorkMissionRound, SequenceZoneStatus } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingUtils';
...
type Props = {
  theme: PirateWorkMissionTheme;
  roundDef: PirateWorkMissionRound;
  stepIndex: 0 | 1;
  effort: number;
  form: number;
  targetEffort: number;
...
export function PirateWorkOverlay({
  theme,
  roundDef,
  stepIndex,
  effort,
  ...
  const lootGlow = useSharedValue(0);
  ...
  useEffect(() => {
    ...
    lootGlow.value = withTiming(completing ? 1 : ratio, { duration: 200 });
  }, [completing, lootGlow, effort, targetEffort]);

  const waveStyle = useAnimatedStyle(() => ({ opacity: 0.22 + wave.value * 0.4 }));
  const lootStyle = useAnimatedStyle(() => ({
    opacity: 0.42 + lootGlow.value * 0.52,
    transform: [{ scale: 1 + lootGlow.value * (stepIndex === 1 ? 0.12 : 0.09) }],
  }));
  ...
  const stepAccent = stepIndex === 0 ? theme.haul : theme.stow;
    ...
    <View style={StyleSheet.absoluteFill} pointerEvents="none">
      ...
      <View style={styles.hudWrap}>
        <Text style={styles.hudTitle}>
          {stepIndex === 0 ? 'HAUL' : 'STOW'} {effortPct}% · FORM {formPct}% · TARGET {targetPct}%
        </Text>
        ...
        <View style={styles.hudRow}>
          ...
          <Text style={styles.roundText}>
            Mission {round + 1}/{totalRounds}
          </Text>
        </View>
      </View>

      <View style={styles.planRow}>
        <StepChip n={1} icon="🪢" label={roundDef.haulLabel} state={stepIndex === 0 ? 'active' : stepIndex === 1 ? 'done' : 'pending'} accent={theme.haul} />
        <Text style={styles.arrow}>→</Text>
        <StepChip n={2} icon="💰" label={roundDef.stowLabel} state={stepIndex === 1 ? 'active' : 'pending'} accent={theme.stow} />
      </View>

      <View style={styles.seqWrap}>
        <Text style={styles.seqLabel}>🏴‍☠️ MISSIONS {Math.round(missionProgress * 100)}%</Text>
        <View style={styles.seqBar}>
          <LinearGradient
            colors={[theme.accentDeep, theme.accent]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
            style={[styles.seqFill, { width: `${missionProgress * 100}%` }]}
          />
        </View>
      </View>
      ...
      <View style={styles.lootRow}>
        {Array.from({ length: totalRounds }).map((_, i) => (
          <Text key={i} style={[styles.lootDot, { opacity: i < missionCount ? 1 : 0.25 }]}>
            {i < missionCount ? theme.loot[i % theme.loot.length] : '·'}
          </Text>
        ))}
      </View>
      ...
      {previewing && (
        <View style={styles.previewWrap}>
          ...
          <Text style={styles.previewSeq}>🪢 Haul → 💰 Stow</Text>
        </View>
      )}

      {!previewing && (
        <Animated.View style={[styles.lootWrap, lootStyle]}>
          <Text style={styles.lootEmoji}>{stepIndex === 0 ? roundDef.icon : '💰'}</Text>
          <Text style={styles.lootLabel}>{stepIndex === 0 ? 'HAUL' : 'STOW'}</Text>
          <Text style={styles.lootName}>{roundDef.loot}</Text>
        </Animated.View>
      )}
      ...
      {completing && (
        <Text style={styles.completeText}>MISSION {Math.round(completeProgress * 100)}%</Text>
      )}
...
const styles = StyleSheet.create({
  ...
  chipNum: { color: '#CCFBF1', fontSize: 8, fontWeight: '900' },
  ...
  lootEmoji: { fontSize: 52 },
  lootLabel: { color: '#FDE68A', fontSize: 11, fontWeight: '900', marginTop: 6, letterSpacing: 0.6 },
  lootName: { color: '#CCFBF1', fontSize: 10, fontWeight: '700', marginTop: 3 },
  hand: {
    position: 'absolute',
    width: 28,
    height: 28,
