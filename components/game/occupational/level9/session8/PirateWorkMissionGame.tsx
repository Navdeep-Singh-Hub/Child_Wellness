/**
 * OT Level 9 · Session 8 · Game 4 — Pirate Work Mission (Pirate Harbor Deck)
 *
 * Camera tracks two-step proprioceptive sequence: haul tow rope with controlled pull,
 * then stow treasure with controlled carry effort. MediaPipe pose on APK + web; guided fallback.
 */
...
import { PirateWorkOverlay } from '@/components/game/occupational/level9/session8/components/PirateWorkOverlay';
import { PIRATE_WORK_MISSION_THEME, PROPRIO_SEQUENCING_SHELL } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingTheme';
import {
  PIRATE_WORK_MISSION_ROUNDS,
  mirroredShipRopes,
  pirateHaulStepScore,
  pirateMissionScore,
  pirateStowStepScore,
  sequenceStepQuality,
  type PirateWorkMissionRound,
...
const T = PIRATE_WORK_MISSION_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Ahoy captain!', 'Deck pro!', 'Perfect haul!', 'Treasure star!', 'Pirate master!'];

type Phase = 'intro' | 'calibrate' | 'play';

const PirateWorkMissionGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundDef, setRoundDef] = useState<PirateWorkMissionRound>(PIRATE_WORK_MISSION_ROUNDS[0]!);
  ...
  const roundDefRef = useRef<PirateWorkMissionRound>(PIRATE_WORK_MISSION_ROUNDS[0]!);
  ...
  const haulTargetForRound = useCallback(
    (r: number) => P.haulTargets[Math.min(r, P.haulTargets.length - 1)] ?? 0.56,
    [],
  );

  const stowTargetForRound = useCallback(
    (r: number) => P.stowTargets[Math.min(r, P.stowTargets.length - 1)] ?? 0.58,
    [],
  );

  const targetForStep = useCallback(
    (r: number, s: 0 | 1) => (s === 0 ? haulTargetForRound(r) : stowTargetForRound(r)),
    [haulTargetForRound, stowTargetForRound],
  );
  ...
  const endGame = useCallback(() => {
    ...
    const avgMission = missionCountRef.current > 0 ? missionSumRef.current / missionCountRef.current : 0;
    ...
    const headline = Math.round((completion * 0.5 + avgMission * 0.2 + moveQuality * 0.15 + zoneAccuracy * 0.15) * 100);
    const xp = Math.round(completed * 22 + avgMission * 90 + moveQuality * 70 + zoneAccuracy * 42);
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgPirateWorkMission: Math.round(avgMission * 100),
  ...
  const beginStep = useCallback(
    (s: 0 | 1) => {
      ...
      setRoundActive(true);
      setBanner(s === 0 ? T.haulCue : T.stowCue);
      setCoachCue(s === 0 ? T.haulFormCue : T.stowFormCue);
      speakTTS(s === 0 ? T.voiceHaul : T.voiceStow, 0.85).catch(() => {});
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = PIRATE_WORK_MISSION_ROUNDS[Math.min(r, PIRATE_WORK_MISSION_ROUNDS.length - 1)] ?? PIRATE_WORK_MISSION_ROUNDS[0]!;
      roundRef.current = r;
      ...
      speakTTS(`${def.name}! Haul first, then stow the ${def.loot}.`, 0.85).catch(() => {});
  ...
  const finishRound = useCallback(() => {
    ...
    const missionPower = pirateMissionScore(peakHaulRef.current, peakStowRef.current);
    missionSumRef.current += missionPower;
    missionCountRef.current += 1;
    completedRef.current += 1;
    setCompletedCount(completedRef.current);
    setMissionProgress(completedRef.current / totalRounds);
    recordHold(P.haulHoldMs + P.stowHoldMs);
  ...
  const beginSequenceComplete = useCallback(() => {
    ...
    schedule(() => finishRound(), P.pirateMissionCompleteMs);
  ...
  const onStepDone = useCallback(
    (stepPower: number) => {
      ...
      if (stepRef.current === 0) {
        ...
        setRoundActive(false);
        setHoldProgress(0);
        setBanner('HAULED! Now stow!');
        speakTTS(T.voiceStow, 0.9).catch(() => {});
  ...
  const tick = useCallback(() => {
    ...
    const formMin = s === 0 ? P.haulFormMin : P.stowFormMin;
    const holdMs = s === 0 ? P.haulHoldMs : P.stowHoldMs;
    const fallbackMs = s === 0 ? P.fallbackHaulMs : P.fallbackStowMs;

    if (completingRef.current) {
      const up = clamp01((now - completeStartRef.current) / P.pirateMissionCompleteMs);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const score = {
          form: f,
          effort: e,
          status: (e >= target - P.bandHalf && e <= target + P.bandHalf ? 'zone' : e < target - P.bandHalf ? 'light' : 'heavy') as SequenceZoneStatus,
          power: clamp01(f * 0.45 + e * 0.55),
        };
        setEffort(e);
        setForm(f);
        ...
        setLeftHand(null);
        setRightHand(null);
        if (score.power > (s === 0 ? peakHaulRef.current : peakStowRef.current)) {
          if (s === 0) peakHaulRef.current = score.power;
          else peakStowRef.current = score.power;
        }
    ...
    const score = s === 0
      ? pirateHaulStepScore(m, forceBaseRef.current, target, P.bandHalf)
      : pirateStowStepScore(m, forceBaseRef.current, target, P.bandHalf);
    const q = sequenceStepQuality(m, postureBaseRef.current, score, formMin);
    const hands = s === 0 ? mirroredShipRopes(m) : mirroredCarryHands(m);
    ...
    } else if (s === 0 && score.form >= formMin) {
      setCoachCue(T.anchorCue);
    } else {
      setCoachCue(T.hintText);
    }
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS(
      'Stand tall with your full body visible. I will learn your pose before Pirate Harbor Deck begins!',
      0.8,
    ).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided pirate work mode.');
      }
    }
    beginCalibration();
    ...
    <LinearGradient colors={T.bgGradient} style={styles.root}>
      <SafeAreaView style={styles.safe}>
        <View style={styles.topRow}>
          ...
          <Text style={styles.academyLabel}>🏴‍☠️ {PROPRIO_SEQUENCING_SHELL.academyLabel}</Text>
          <View style={{ width: 64 }} />
        </View>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: PROPRIO_SEQUENCING_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: PROPRIO_SEQUENCING_SHELL.statLabel }]}>Mission</Text>
            ...
            <View style={[styles.statPill, { borderColor: PROPRIO_SEQUENCING_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: PROPRIO_SEQUENCING_SHELL.statValue }]}>{missions}</Text>
            </View>
          </View>
        </View>
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — haul the tow rope with steady pull force, then stow the treasure with controlled carry effort!'
                    : 'Guided mode: follow the coach and practice haul-then-stow pirate work missions with controlled effort!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Board Deck</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
const styles = StyleSheet.create({
  ...
  linkText: { color: '#14B8A6', fontSize: 13, fontWeight: '700', textDecorationLine: 'underline' },
});

export default PirateWorkMissionGame;
