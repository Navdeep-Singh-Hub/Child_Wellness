/**
 * OT Level 9 · Session 8 · Game 5 — Rainbow Challenge (Rainbow Spectrum Bridge)
 *
 * Camera tracks two-step proprioceptive sequence: shape rainbow arch pose,
 * then glow with controlled overhead hold effort. MediaPipe pose on APK + web; guided fallback.
 */
...
import { RainbowChallengeOverlay } from '@/components/game/occupational/level9/session8/components/RainbowChallengeOverlay';
import { RAINBOW_CHALLENGE_THEME, PROPRIO_SEQUENCING_SHELL } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingTheme';
import {
  RAINBOW_CHALLENGE_ROUNDS,
  mirroredStrengthPalms,
  rainbowArchStepQuality,
  rainbowArchStepScore,
  rainbowChallengeScore,
  rainbowGlowStepScore,
  sequenceStepQuality,
  type RainbowChallengeRound,
...
const T = RAINBOW_CHALLENGE_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Rainbow star!', 'Spectrum pro!', 'Perfect glow!', 'Bridge master!', 'Color champion!'];

const EMPTY_READOUT: FullBodyReadout = { arms: { left: null, right: null }, legs: { left: null, right: null } };

type Phase = 'intro' | 'calibrate' | 'play';

const RainbowChallengeGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundDef, setRoundDef] = useState<RainbowChallengeRound>(RAINBOW_CHALLENGE_ROUNDS[0]!);
  ...
  const roundDefRef = useRef<RainbowChallengeRound>(RAINBOW_CHALLENGE_ROUNDS[0]!);
  ...
  const peakArchRef = useRef(0);
  const peakGlowRef = useRef(0);
  ...
  const glowTargetForRound = useCallback(
    (r: number) => P.glowTargets[Math.min(r, P.glowTargets.length - 1)] ?? 0.58,
    [],
  );
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgRainbowChallenge: Math.round(avgSequence * 100),
  ...
  const beginStep = useCallback((s: 0 | 1) => {
    ...
    setRoundActive(true);
    setBanner(s === 0 ? T.archCue : T.glowCue);
    setCoachCue(s === 0 ? T.archFormCue : T.glowFormCue);
    speakTTS(s === 0 ? T.voiceArch : T.voiceGlow, 0.85).catch(() => {});
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = RAINBOW_CHALLENGE_ROUNDS[Math.min(r, RAINBOW_CHALLENGE_ROUNDS.length - 1)] ?? RAINBOW_CHALLENGE_ROUNDS[0]!;
      roundRef.current = r;
      ...
      stepRef.current = 0;
      setStepIndex(0);
      peakArchRef.current = 0;
      peakGlowRef.current = 0;
      ...
      speakTTS(`${def.colorName} ${def.name}! Arch first, then glow with steady hold effort.`, 0.85).catch(() => {});
  ...
  const finishRound = useCallback(() => {
    ...
    const seqPower = rainbowChallengeScore(peakArchRef.current, peakGlowRef.current);
    powerSumRef.current += seqPower;
    powerCountRef.current += 1;
    completedRef.current += 1;
    setCompletedCount(completedRef.current);
    setChallengeProgress(completedRef.current / totalRounds);
    recordHold(P.archHoldMs + P.glowHoldMs);
  ...
  const beginSequenceComplete = useCallback(() => {
    ...
    schedule(() => finishRound(), P.rainbowChallengeCompleteMs);
  ...
  const onStepDone = useCallback(
    (stepPower: number) => {
      ...
      if (stepRef.current === 0) {
        if (stepPower > peakArchRef.current) peakArchRef.current = stepPower;
        roundActiveRef.current = false;
        setRoundActive(false);
        setHoldProgress(0);
        setBanner('ARCHED! Now glow!');
        speakTTS(T.voiceGlow, 0.9).catch(() => {});
        schedule(() => beginStep(1), P.betweenStepsMs);
      } else {
        if (stepPower > peakGlowRef.current) peakGlowRef.current = stepPower;
  ...
  const tick = useCallback(() => {
    ...
    const glowTarget = glowTargetForRound(r);
    const holdMs = s === 0 ? P.archHoldMs : P.glowHoldMs;
    const fallbackMs = s === 0 ? P.fallbackArchMs : P.fallbackGlowMs;

    if (completingRef.current) {
      const up = clamp01((now - completeStartRef.current) / P.rainbowChallengeCompleteMs);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        const e = s === 0 ? f : clamp01(glowTarget * (0.5 + t * 0.46));
        const status: SequenceZoneStatus =
          s === 0
            ? f >= P.archReachMin
              ? 'zone'
              : 'light'
            : e >= glowTarget - P.bandHalf && e <= glowTarget + P.bandHalf
              ? 'zone'
              : e < glowTarget - P.bandHalf
                ? 'light'
                : 'heavy';
        setEffort(e);
        setForm(f);
        ...
        if (power > (s === 0 ? peakArchRef.current : peakGlowRef.current)) {
          if (s === 0) peakArchRef.current = power;
          else peakGlowRef.current = power;
        }
        const canHold = s === 0 ? status === 'zone' : status === 'zone' && f >= P.glowFormMin;
        if (canHold) {
    ...
    if (s === 0) {
      const arch = rainbowArchStepScore(m, def, P.archTolerance, P.archReachMin);
      const q = rainbowArchStepQuality(m, postureBaseRef.current, arch, P.archReachMin);
      setEffort(arch.effort);
      setForm(arch.form);
      setZoneStatus(arch.status);
      setArmsScore(arch.armsScore);
      setLegsScore(arch.legsScore);
      setReadout(arch.readout);
      setQuality(q);
      setLeftHand(null);
      setRightHand(null);
      if (arch.power > peakArchRef.current) peakArchRef.current = arch.power;

      if (arch.form < P.archReachMin) {
        setCoachCue(T.archFormCue);
      } else {
        setCoachCue(T.shineCue);
      }

      if (roundActiveRef.current) {
        totalTicksRef.current += 1;
        const canHold = arch.status === 'zone';
        ...
        if (canHold) {
          ...
          setHoldProgress(prog);
          if (prog >= 1) onStepDone(arch.power);
        } else if (arch.form < P.archReachMin || now - lastAtTargetRef.current > P.holdGraceMs) {
          holdStartRef.current = 0;
          setHoldProgress(0);
        }
      }
      ...
      recordTick(dt, { upright: positioned, still: arch.status === 'zone', quality: q });
      prevMetricsRef.current = m;
      return;
    }

    const glow = rainbowGlowStepScore(m, forceBaseRef.current, glowTarget, P.bandHalf);
    const q = sequenceStepQuality(m, postureBaseRef.current, glow, P.glowFormMin);
    const hands = mirroredStrengthPalms(m);

    setEffort(glow.effort);
    setForm(glow.form);
    setZoneStatus(glow.status);
    setQuality(q);
    setLeftHand(hands.left);
    setRightHand(hands.right);

    if (glow.power > peakGlowRef.current) peakGlowRef.current = glow.power;

    if (glow.form < P.glowFormMin) {
      setCoachCue(T.glowFormCue);
    } else if (glow.status === 'heavy') {
      setCoachCue(T.heavyCue);
      if (now - lastHeavyRef.current > 2200) {
        lastHeavyRef.current = now;
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
      }
    } else if (glow.status === 'light') {
    ...
    if (roundActiveRef.current) {
      totalTicksRef.current += 1;
      const canHold = glow.status === 'zone' && glow.form >= P.glowFormMin;
      if (canHold) zoneTicksRef.current += 1;
      if (canHold) {
        if (holdStartRef.current === 0) holdStartRef.current = now;
        lastAtTargetRef.current = now;
        const prog = clamp01((now - holdStartRef.current) / holdMs);
        setHoldProgress(prog);
        if (prog >= 1) onStepDone(glow.power);
      } else if (glow.status === 'heavy' || glow.form < P.glowFormMin || now - lastAtTargetRef.current > P.holdGraceMs) {
        holdStartRef.current = 0;
        setHoldProgress(0);
      }
    }
    ...
    qualCountRef.current += 1;
    if (!positioned) recordPostureBreak();
    recordTick(dt, { upright: positioned, still: glow.status === 'zone', quality: q });
    prevMetricsRef.current = m;
  }, [glowTargetForRound, onStepDone, recordPostureBreak, recordTick]);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS(
      'Stand tall with your full body visible. I will learn your pose before Rainbow Spectrum Bridge begins!',
      0.8,
    ).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided rainbow challenge mode.');
      }
    }
    beginCalibration();
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: PROPRIO_SEQUENCING_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: PROPRIO_SEQUENCING_SHELL.statLabel }]}>Color</Text>
            ...
            <View style={[styles.statPill, { borderColor: PROPRIO_SEQUENCING_SHELL.statBorder }]}>
              ...
              <Text style={[styles.statValue, { color: PROPRIO_SEQUENCING_SHELL.statValue }]}>{challenges}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.stageWrap}>
          <CameraStage
            ...
            quality={quality}
            glowColor={T.glowAura}
            hero={T.hero}
            coachCue={phase === 'play' || phase === 'calibrate' ? coachCue : ''}
          >
            {phase === 'play' && (
              <RainbowChallengeOverlay
                theme={T}
                roundDef={roundDef}
                stepIndex={stepIndex}
                effort={effort}
                ...
                legsScore={legsScore}
                readout={readout}
                targetEffort={stepIndex === 1 ? glowTargetForRound(round) : P.archReachMin}
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — shape each rainbow arch, then glow with steady overhead hold effort!'
                    : 'Guided mode: follow the coach and practice arch-then-glow rainbow challenges with controlled effort!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Enter Bridge</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
export default RainbowChallengeGame;
