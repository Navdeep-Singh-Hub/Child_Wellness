/**
 * OT Level 9 · Session 8 · Game 3 — Power Sequence (Thunder Charge Vault)
 *
 * Camera tracks two-step proprioceptive sequence: charge power in stance zone,
 * then blast with controlled upward thrust. MediaPipe pose on APK + web; guided fallback.
 */
...
import { PowerSequenceOverlay } from '@/components/game/occupational/level9/session8/components/PowerSequenceOverlay';
import { POWER_SEQUENCE_THEME, PROPRIO_SEQUENCING_SHELL } from '@/components/game/occupational/level9/session8/proprioceptiveSequencingTheme';
import {
  POWER_SEQUENCE_ROUNDS,
  mirroredGorillaHands,
  mirroredLaunchPalms,
  powerBlastStepScore,
  powerChargeStepScore,
  powerSequenceScore,
  sequenceStepQuality,
  type PowerSequenceRound,
...
const T = POWER_SEQUENCE_THEME;
const clamp01 = (v: number) => Math.max(0, Math.min(1, v));
const VOICE_PRAISE = ['Power surge!', 'Vault pro!', 'Perfect blast!', 'Thunder star!', 'Core master!'];

type Phase = 'intro' | 'calibrate' | 'play';

const PowerSequenceGame: React.FC<{ onBack?: () => void; onComplete?: () => void }> = ({ onBack, onComplete }) => {
  ...
  const [roundDef, setRoundDef] = useState<PowerSequenceRound>(POWER_SEQUENCE_ROUNDS[0]!);
  ...
  const roundDefRef = useRef<PowerSequenceRound>(POWER_SEQUENCE_ROUNDS[0]!);
  ...
  const peakChargeRef = useRef(0);
  const peakBlastRef = useRef(0);
  ...
  const chargeTargetForRound = useCallback(
    (r: number) => P.chargeTargets[Math.min(r, P.chargeTargets.length - 1)] ?? 0.58,
    [],
  );

  const blastTargetForRound = useCallback(
    (r: number) => P.blastTargets[Math.min(r, P.blastTargets.length - 1)] ?? 0.6,
    [],
  );

  const targetForStep = useCallback(
    (r: number, s: 0 | 1) => (s === 0 ? chargeTargetForRound(r) : blastTargetForRound(r)),
    [blastTargetForRound, chargeTargetForRound],
  );
      ...
      .then(() =>
        logGameAndAward({
          ...
          meta: {
            ...
            avgPowerSequence: Math.round(avgSequence * 100),
  ...
  const beginStep = useCallback(
    (s: 0 | 1) => {
      ...
      setRoundActive(true);
      setBanner(s === 0 ? T.chargeCue : T.blastCue);
      setCoachCue(s === 0 ? T.chargeFormCue : T.blastFormCue);
      speakTTS(s === 0 ? T.voiceCharge : T.voiceBlast, 0.85).catch(() => {});
  ...
  const startRound = useCallback(
    (r: number) => {
      if (doneRef.current) return;
      const def = POWER_SEQUENCE_ROUNDS[Math.min(r, POWER_SEQUENCE_ROUNDS.length - 1)] ?? POWER_SEQUENCE_ROUNDS[0]!;
      roundRef.current = r;
      ...
      setStepIndex(0);
      stepAccRef.current = [];
      peakChargeRef.current = 0;
      peakBlastRef.current = 0;
      ...
      speakTTS(`${def.name}! Charge first, then blast the ${def.core}.`, 0.85).catch(() => {});
  ...
  const finishRound = useCallback(() => {
    ...
    const seqPower = powerSequenceScore(peakChargeRef.current, peakBlastRef.current);
    ...
    recordHold(P.chargeHoldMs + P.blastHoldMs);
  ...
  const beginSequenceComplete = useCallback(() => {
    ...
    schedule(() => finishRound(), P.powerSequenceCompleteMs);
  ...
  const onStepDone = useCallback(
    (stepPower: number) => {
      ...
      if (stepRef.current === 0) {
        ...
        setRoundActive(false);
        setHoldProgress(0);
        setBanner('CHARGED! Now blast!');
        speakTTS(T.voiceBlast, 0.9).catch(() => {});
  ...
  const tick = useCallback(() => {
    ...
    const formMin = s === 0 ? P.chargeFormMin : P.blastFormMin;
    const holdMs = s === 0 ? P.chargeHoldMs : P.blastHoldMs;
    const fallbackMs = s === 0 ? P.fallbackChargeMs : P.fallbackBlastMs;

    if (completingRef.current) {
      const up = clamp01((now - completeStartRef.current) / P.powerSequenceCompleteMs);
    ...
    if (!camLiveRef.current) {
      if (roundActiveRef.current) {
        ...
        setLeftHand(null);
        setRightHand(null);
        if (score.power > (s === 0 ? peakChargeRef.current : peakBlastRef.current)) {
          if (s === 0) peakChargeRef.current = score.power;
          else peakBlastRef.current = score.power;
        }
    ...
    const score = s === 0
      ? powerChargeStepScore(m, forceBaseRef.current, target, P.bandHalf)
      : powerBlastStepScore(m, forceBaseRef.current, target, P.bandHalf);
    const q = sequenceStepQuality(m, postureBaseRef.current, score, formMin);
    const hands = s === 0 ? mirroredGorillaHands(m) : mirroredLaunchPalms(m);
  ...
  const beginCalibration = useCallback(() => {
    ...
    speakTTS(
      'Stand tall with your full body visible. I will learn your pose before Thunder Charge Vault begins!',
      0.8,
    ).catch(() => {});
  ...
  const handleStart = useCallback(async () => {
    if (cameraSupported && !forceFallback && !permissionGranted) {
      ...
      if (!granted) {
        setCoachCue('Camera not allowed — playing guided power sequence mode.');
      }
    }
    beginCalibration();
        ...
        <View style={styles.header}>
          ...
          <View style={styles.statsRow}>
            <View style={[styles.statPill, { borderColor: PROPRIO_SEQUENCING_SHELL.statBorder }]}>
              <Text style={[styles.statLabel, { color: PROPRIO_SEQUENCING_SHELL.statLabel }]}>Core</Text>
            ...
            {camLive && error && permissionGranted ? (
              ...
              <>
                <Text style={styles.introText}>
                  {camLive
                    ? 'Step back so the camera sees your full body — charge power in the stance zone, then blast with controlled thrust!'
                    : 'Guided mode: follow the coach and practice charge-then-blast power sequences with controlled effort!'}
                </Text>
                <Pressable style={[styles.primaryBtn, { backgroundColor: T.accentDeep }]} onPress={handleStart}>
                  <Text style={styles.primaryBtnText}>{T.hero} Enter Vault</Text>
                </Pressable>
                {camLive && (
                  <Pressable
...
export default PowerSequenceGame;
