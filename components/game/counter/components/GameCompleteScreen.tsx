// Individual Game Completion Screen for The Counter
import { playSoundEffect, stopAllAudio } from '@/components/game/counter/utils/audio';
...
export default function GameCompleteScreen({
  ...
  const getPerformanceMessage = () => {
    if (accuracy >= 90) {
      return "You're a Smart Counter!";
    } else if (accuracy >= 70) {
      return 'Great Reading & Counting!';
    } else {
  ...
  const getPerformanceColor = () => {
    if (accuracy >= 90) {
      return ['#86EFAC', '#4ADE80'];
    } else if (accuracy >= 70) {
      return ['#C4B5FD', '#A78BFA'];
    } else {
      return ['#FBCFE8', '#F9A8D4'];
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <LinearGradient colors={['#FEF3F8', '#F0F4FF', '#ECFDF5']} style={StyleSheet.absoluteFill} />
      ...
      <Animated.View style={[styles.resultCard, animatedStyle]}>
        ...
        >
          <View style={styles.resultContent}>
            <Ionicons
              name={accuracy >= 80 ? 'trophy' : accuracy >= 70 ? 'star' : 'heart'}
              size={64}
              color="#FFFFFF"
            />
