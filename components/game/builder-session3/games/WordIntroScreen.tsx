...
export default function WordIntroScreen({ onComplete, onBack }: WordIntroScreenProps) {
  ...
  useEffect(() => {
    if (hasStarted && currentLetterIndex < WORD_DOG.letters.length) {
      letterScale.value = withSequence(
        withTiming(0, { duration: 0 }),
        withSpring(1, { damping: 10, stiffness: 100 }),
        withTiming(1, { duration: 1000 })
      );
      speakLetter(WORD_DOG.letters[currentLetterIndex]);
      playSoundEffect('letter');
      
      const timer = setTimeout(() => {
        if (currentLetterIndex < WORD_DOG.letters.length - 1) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {/* Word Display */}
      {hasStarted && currentLetterIndex === WORD_DOG.letters.length - 1 && (
        <View style={styles.wordContainer}>
          <Text style={styles.wordText}>{WORD_DOG.word}</Text>
        </View>
      )}
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]}>
          <Text style={styles.imageEmoji}>{WORD_DOG.image}</Text>
          <Text style={styles.imageText}>Dog</Text>
        </Animated.View>
      )}
