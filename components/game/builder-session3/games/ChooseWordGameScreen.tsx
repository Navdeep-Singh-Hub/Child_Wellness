...
import { WORD_OPTIONS, WORD_DOG } from '../utils/gameData';
...
export default function ChooseWordGameScreen({ onComplete, onBack }: ChooseWordGameScreenProps) {
  ...
  useEffect(() => {
    ...
    speakInstruction('Tap the word DOG.');
    return () => stopAllAudio();
  }, [round]);
  ...
  const handleSelect = (word: string) => {
    ...
    if (correct) {
      ...
      speakFeedback('Correct! That spells DOG!');
    } else {
      playSoundEffect('incorrect');
      speakFeedback('Try again! Look for the word DOG.');
    }
