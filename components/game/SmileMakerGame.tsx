...
// Distance to smile curve (inverted arc)
const distanceToSmile = (
  px: number,
  py: number,
  centerX: number,
  centerY: number,
...
const SmileMakerGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
  ...
  const updatePaths = useCallback(() => {
    ...
    setSmilePathStr(`M ${startX} ${startY} A ${radius.value} ${radius.value} 0 0 1 ${endX} ${endY}`);
  ...
  // Smile curve parameters (inverted arc)
  const centerX = useSharedValue(50);
  const centerY = useSharedValue(60);
  ...
  const objectY = useSharedValue(60);
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Smile Complete!</Text>
            <Text style={styles.resultSubtitle}>
              You created {finalStats.correct} smiles out of {finalStats.total}!
            </Text>
            <ResultCard
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Smile Maker</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • 😊 Score: {score}
        </Text>
        <Text style={styles.helper}>
          Trace the curve to make a smile! Follow the happy arc from left to right.
        </Text>
      </View>
      ...
      <View style={styles.footerBox}>
        <Text style={styles.footerMain}>
          Skills: smooth wrist movement • curved tracking • smile curve tracing
        </Text>
        <Text style={styles.footerSub}>
          Make someone happy by tracing the smile curve!
        </Text>
      </View>
    </SafeAreaView>
  );
