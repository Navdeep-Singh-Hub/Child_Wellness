// Individual Game Completion Screen for The Graduate (with Level 1 Graduate Badge)
import { playSoundEffect, stopAllAudio } from '@/components/game/graduate/utils/audio';
...
export default function GameCompleteScreen({
  ...
  const getPerformanceMessage = () => {
    if (isLevelComplete) {
      return "You've Completed Level 1!";
    }
    if (accuracy >= 90) {
      return "You're a Story Star!";
    } else if (accuracy >= 70) {
      return 'Great Problem Solver!';
    } else {
  ...
  const getPerformanceColor = () => {
    if (isLevelComplete) {
      return ['#FDE68A', '#FCD34D']; // Golden graduation theme
    }
    if (accuracy >= 90) {
      ...
      <Animated.View style={[styles.resultCard, animatedStyle]}>
        ...
        >
          <View style={styles.resultContent}>
            <Ionicons
              name={isLevelComplete ? 'trophy' : accuracy >= 80 ? 'trophy' : accuracy >= 70 ? 'star' : 'book'}
              size={64}
              color="#FFFFFF"
            />
