// Game 4: Visual Word Problem (Subtraction)
...
import { SUBTRACTION_WORD_PROBLEMS, getWordProblemOptions } from '../utils/gameData';

interface WordProblemSubtractionGameProps {
...
export default function WordProblemSubtractionGame({ onComplete, onBack }: WordProblemSubtractionGameProps) {
  const [round, setRound] = useState(0);
  const [score, setScore] = useState(0);
  const [currentProblem, setCurrentProblem] = useState<typeof SUBTRACTION_WORD_PROBLEMS[0] | null>(null);
  ...
  const loadRound = () => {
    if (round >= TOTAL_ROUNDS) {
      ...
      onComplete({
        ...
        total: TOTAL_ROUNDS,
        accuracy,
        gameId: 'word-problem-subtraction',
      });
      return;
    }

    const problem = SUBTRACTION_WORD_PROBLEMS[round % SUBTRACTION_WORD_PROBLEMS.length];
  ...
  const handleRemoveObjects = async () => {
    if (!currentProblem || isAnimating || removedObjects.size >= currentProblem.remove) return;

    setIsAnimating(true);
    setCanSelect(false);

    // Animate removal
    for (let i = 0; i < currentProblem.remove; i++) {
      await new Promise(resolve => setTimeout(resolve, 500));
      const objToRemove = Array.from({ length: currentProblem.start }, (_, idx) => idx)
        .find(idx => !removedObjects.has(idx));
      if (objToRemove !== undefined) {
        setRemovedObjects(prev => new Set([...prev, objToRemove]));
      }
    }
  ...
  const handleAnswerSelect = async (answer: number) => {
    if (!canSelect || !currentProblem || removedObjects.size < currentProblem.remove) return;
  ...
  const remainingCount = currentProblem.start - removedObjects.size;
  const canAnswer = removedObjects.size === currentProblem.remove;
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {showObjects && (
        <View style={styles.objectsContainer}>
          {removedObjects.size < currentProblem.remove && (
            <Pressable
              onPress={handleRemoveObjects}
              disabled={isAnimating || !canSelect}
            ...
            >
              ...
              >
                ...
                <Text style={styles.removeText}>
                  Remove {currentProblem.remove} {currentProblem.objectType}
                </Text>
              </LinearGradient>
            </Pressable>
          )}
          <View style={styles.objectsRow}>
            {Array.from({ length: currentProblem.start }).map((_, i) => {
              const isRemoved = removedObjects.has(i);
              return (
                <Animated.View
                  key={i}
                  style={[
                ...
                >
                  {!isRemoved && <ObjectIcon type={currentProblem.objectType} size={40} />}
                </Animated.View>
              );
            })}
          </View>
