// Game 3: Clock Introduction - Learn about 4:00
...
import { CLOCK_TIME_SESSION4 } from './gameData';
...
export default function ClockIntroGameScreen({ onComplete, onBack }: ClockIntroGameScreenProps) {
  ...
  useEffect(() => {
    const initClock = async () => {
      ...
      await new Promise(resolve => setTimeout(resolve, 500));
      await speakClockTime(CLOCK_TIME_SESSION4.hour);
      
      await new Promise(resolve => setTimeout(resolve, 1000));
      await speakInstruction(CLOCK_TIME_SESSION4.description);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <Animated.View style={[styles.clockWrapper, clockAnimatedStyle]} entering={FadeInDown.delay(200)}>
        <AnalogClock hour={CLOCK_TIME_SESSION4.hour} minute={CLOCK_TIME_SESSION4.minute} />
      </Animated.View>

      {/* Time Display */}
      <View style={styles.timeContainer}>
        <Text style={styles.timeText}>{CLOCK_TIME_SESSION4.time}</Text>
      </View>
        ...
        <Animated.View style={[styles.explanationContainer, explanationAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.explanationBox}>
            <Text style={styles.explanationText}>
              Minute hand → {CLOCK_TIME_SESSION4.minuteHandPosition}
            </Text>
            <Text style={styles.explanationText}>
              Hour hand → {CLOCK_TIME_SESSION4.hourHandPosition}
            </Text>
            <Text style={styles.explanationDescription}>
              {CLOCK_TIME_SESSION4.description}
            </Text>
          </View>
        </Animated.View>
      )}
