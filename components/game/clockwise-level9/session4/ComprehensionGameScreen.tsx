...
import { COMPREHENSION_QUESTION_SESSION4 } from './gameData';
...
export default function ComprehensionGameScreen({ onComplete, onBack }: ComprehensionGameScreenProps) {
  ...
  const buttonScales = COMPREHENSION_QUESTION_SESSION4.options.map(() => useSharedValue(1));

  useEffect(() => {
    speakQuestion(COMPREHENSION_QUESTION_SESSION4.question).catch(() => {});
    return () => stopAllAudio();
  }, []);
  ...
  const handleAnswerSelect = async (answer: string) => {
    ...
    const index = COMPREHENSION_QUESTION_SESSION4.options.findIndex(o => o.answer === answer);
    buttonScales[index].value = withSpring(0.9, {}, () => {
      buttonScales[index].value = withSpring(1);
    });

    setSelectedAnswer(answer);
    const option = COMPREHENSION_QUESTION_SESSION4.options.find(o => o.answer === answer);
    const isCorrect = option?.correct || false;

    if (isCorrect) {
      await playSoundEffect('correct');
      await speakFeedback('Perfect! The dog runs fast!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.questionContainer}>
        <Text style={styles.questionText}>{COMPREHENSION_QUESTION_SESSION4.question}</Text>
      </View>

      {/* Answer Options */}
      <View style={styles.answersContainer}>
        {COMPREHENSION_QUESTION_SESSION4.options.map((option, index) => {
