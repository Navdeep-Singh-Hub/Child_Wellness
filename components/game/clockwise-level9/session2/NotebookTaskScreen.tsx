...
import { CLOCK_TIME_SESSION2, NOTEBOOK_CLOCK_TIME_SESSION2, NOTEBOOK_SENTENCE_SESSION2 } from './gameData';
...
export default function NotebookTaskScreen({ onNext, onBack }: NotebookTaskScreenProps) {
  useEffect(() => {
    speakInstruction(
      `Write the sentence: ${NOTEBOOK_SENTENCE_SESSION2}. Then draw a clock showing ${NOTEBOOK_CLOCK_TIME_SESSION2}.`
    ).catch(() => {});
    return () => stopAllAudio();
  }, []);
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(200)} style={styles.instructionCard}>
          ...
          <View style={styles.sentenceExample}>
            <Text style={styles.sentenceText}>{NOTEBOOK_SENTENCE_SESSION2}</Text>
          </View>
          <View style={styles.taskSection}>
            <Text style={styles.taskNumber}>2.</Text>
            <Text style={styles.taskText}>Draw a clock showing {NOTEBOOK_CLOCK_TIME_SESSION2}</Text>
          </View>
          <View style={styles.clockGuideContainer}>
            <Text style={styles.guideLabel}>Guide:</Text>
            <Text style={styles.guideText}>Hour hand → {CLOCK_TIME_SESSION2.hourHandPosition}</Text>
            <Text style={styles.guideText}>Minute hand → {CLOCK_TIME_SESSION2.minuteHandPosition}</Text>
        ...
        <Animated.View entering={FadeInDown.delay(400)} style={styles.tipsCard}>
          ...
          <Text style={styles.tipText}>• Make sure the hour hand points to {CLOCK_TIME_SESSION2.hourHandPosition}</Text>
          <Text style={styles.tipText}>• Make sure the minute hand points to {CLOCK_TIME_SESSION2.minuteHandPosition}</Text>
