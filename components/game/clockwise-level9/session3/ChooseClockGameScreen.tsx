// Game 4: Choose Correct Clock - Tap the clock showing 3:00
...
import { CLOCK_OPTIONS_SESSION3 } from './gameData';

interface ChooseClockGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}
...
export default function ChooseClockGameScreen({ onComplete, onBack }: ChooseClockGameScreenProps) {
  const [selectedClock, setSelectedClock] = useState<string | null>(null);
  const [completed, setCompleted] = useState(false);

  const buttonScales = CLOCK_OPTIONS_SESSION3.map(() => useSharedValue(1));

  useEffect(() => {
    speakInstruction('Tap the clock showing 3:00').then(() => {
      setTimeout(() => {
        speakClockTime(3).catch(() => {});
      }, 500);
    });
    return () => stopAllAudio();
  }, []);
  ...
  const handleClockSelect = async (time: string) => {
    ...
    const index = CLOCK_OPTIONS_SESSION3.findIndex(c => c.time === time);
    buttonScales[index].value = withSpring(0.9, {}, () => {
      buttonScales[index].value = withSpring(1);
    });

    setSelectedClock(time);
    const option = CLOCK_OPTIONS_SESSION3.find(c => c.time === time);
    const isCorrect = option?.correct || false;

    if (isCorrect) {
      await playSoundEffect('correct');
      await speakFeedback('Perfect! You found 3 o\'clock!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Choose Correct Clock</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the clock showing 3:00</Text>
      </View>

      {/* Clock Options */}
      <View style={styles.clocksContainer}>
        {CLOCK_OPTIONS_SESSION3.map((option, index) => {
