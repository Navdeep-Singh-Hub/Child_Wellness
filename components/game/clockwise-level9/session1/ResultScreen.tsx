// Result Screen - Show detection results for Level 9 Session 1
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  useEffect(() => {
    ...
    if (allDetected) {
      speakFeedback('Great job! You read the story and learned the time!').catch(() => {});
    } else {
        ...
        {allDetected && (
          <Animated.View entering={FadeInDown.delay(300)} style={styles.badgeContainer}>
            <View style={styles.badge}>
              ...
              <Text style={styles.badgeText}>Time Explorer</Text>
            </View>
          </Animated.View>
        )}
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            ...
            <Text style={styles.resultText}>
              Sentence "Sam has a red ball" {result.sentenceDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
          ...
          <View style={styles.resultItem}>
            ...
            />
            <Text style={styles.resultText}>
              Clock time 1:00 {result.clockTimeCorrect ? 'correct ✓' : 'not correct'}
            </Text>
          </View>
        </Animated.View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You read the story and learned the time! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.sentenceDetected && 'We couldn\'t find the sentence "Sam has a red ball". '}
                {!result.clockDetected && 'We couldn\'t find a clock. '}
                {!result.clockTimeCorrect && 'The clock hands are not correct for 1:00. '}
