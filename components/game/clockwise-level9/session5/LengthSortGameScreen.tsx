// Game 4: Length Sort - Drag objects to LONG or SHORT boxes
...
import { SORTING_OBJECTS_SESSION5 } from './gameData';

interface LengthSortGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}
...
export default function LengthSortGameScreen({ onComplete, onBack }: LengthSortGameScreenProps) {
  ...
  const [objects, setObjects] = useState<ObjectState[]>(
    SORTING_OBJECTS_SESSION5.map((obj, idx) => ({
      id: obj.id,
      emoji: obj.emoji,
      label: obj.label,
      length: obj.length as 'long' | 'short',
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Sort by Length</Text>
        <View style={{ width: 40 }} />
      </View>
        ...
        {objects.map((obj) => (
          ...
          >
            <View style={styles.objectCard}>
              ...
              <View style={obj.length === 'long' ? styles.longLine : styles.shortLine} />
            </View>
          </Animated.View>
        ))}
      </View>
