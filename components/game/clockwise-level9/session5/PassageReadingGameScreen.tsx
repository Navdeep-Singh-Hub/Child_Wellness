...
import { PASSAGE_SESSION5 } from './gameData';
...
export default function PassageReadingGameScreen({ onNext, onBack }: PassageReadingGameScreenProps) {
  ...
  useEffect(() => {
    speakStory(PASSAGE_SESSION5.passage).then(() => {
      ...
      {showIllustration && (
        <Animated.View style={[styles.illustrationContainer, illustrationAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.illustrationWrapper}>
            <Text style={styles.illustrationEmoji}>{PASSAGE_SESSION5.illustration.emoji}</Text>
            <View style={styles.longPencilLine} />
          </View>
        </Animated.View>
      )}
