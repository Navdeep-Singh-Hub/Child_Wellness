// Result Screen - Show detection results for Level 9 Session 5
...
import { NOTEBOOK_SENTENCE_SESSION5 } from './gameData';

interface ResultScreenProps {
  result: {
    sentenceDetected: boolean;
    linesDetected: boolean;
    lengthDifferenceDetected: boolean;
  };
  onRetry: () => void;
  onComplete: () => void;
}
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  const allDetected =
    ...
    result.lengthDifferenceDetected;
  ...
  useEffect(() => {
    ...
    if (allDetected) {
      speakFeedback('Great job! You learned about long and short!').catch(() => {});
    } else {
        ...
        {allDetected && (
          <Animated.View entering={FadeInDown.delay(300)} style={styles.badgeContainer}>
            <View style={styles.badge}>
              ...
              <Text style={styles.badgeText}>Measurement Explorer</Text>
            </View>
          </Animated.View>
        )}
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            ...
            <Text style={styles.resultText}>
              Sentence "{NOTEBOOK_SENTENCE_SESSION5}" {result.sentenceDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.linesDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.linesDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Two lines {result.linesDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.lengthDifferenceDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.lengthDifferenceDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Length difference {result.lengthDifferenceDetected ? 'detected ✓' : 'not detected'}
            </Text>
          </View>
        </Animated.View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You read the passage and learned about long and short! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.sentenceDetected && `We couldn't find the sentence "${NOTEBOOK_SENTENCE_SESSION5}". `}
                {!result.linesDetected && 'We couldn\'t find two lines. '}
                {!result.lengthDifferenceDetected && 'The lines are not clearly long and short. '}
