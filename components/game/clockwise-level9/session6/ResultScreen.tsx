// Result Screen - Show detection results for Level 9 Session 6
...
import { NOTEBOOK_SENTENCE_SESSION6 } from './gameData';

interface ResultScreenProps {
  result: {
    sentenceDetected: boolean;
    objectsDetected: boolean;
    heightDifferenceDetected: boolean;
  };
  onRetry: () => void;
  onComplete: () => void;
}
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  const allDetected =
    ...
    result.heightDifferenceDetected;
  ...
  useEffect(() => {
    ...
    if (allDetected) {
      speakFeedback('Great job! You learned about tall and short!').catch(() => {});
    } else {
        ...
        {allDetected && (
          <Animated.View entering={FadeInDown.delay(300)} style={styles.badgeContainer}>
            <View style={styles.badge}>
              ...
              <Text style={styles.badgeText}>Height Explorer</Text>
            </View>
          </Animated.View>
        )}
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            ...
            <Text style={styles.resultText}>
              Sentence "{NOTEBOOK_SENTENCE_SESSION6}" {result.sentenceDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.objectsDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.objectsDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Two objects {result.objectsDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.heightDifferenceDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.heightDifferenceDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Height difference {result.heightDifferenceDetected ? 'detected ✓' : 'not detected'}
            </Text>
          </View>
        </Animated.View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You read the passage and learned about tall and short! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.sentenceDetected && `We couldn't find the sentence "${NOTEBOOK_SENTENCE_SESSION6}". `}
                {!result.objectsDetected && 'We couldn\'t find two objects. '}
                {!result.heightDifferenceDetected && 'The objects are not clearly tall and short. '}
