...
const StarBuilderGame: React.FC<{ onBack?: () => void }> = ({ onBack }) => {
    ...
    async (finalScore: number) => {
      ...
      try {
        ...
        const result = await logGameAndAward({
          ...
          accuracy,
          xpAwarded: xp,
          skillTags: ['planning', 'number-sequence', 'shape-building'],
        });
        setLogTimestamp(result?.last?.at ?? null);
        router.setParams({ refreshStats: Date.now().toString() });
      } catch (e) {
        console.error('Failed to log star builder game:', e);
      }

      Speech.speak('Star complete!', { rate: 0.78 });
    },
    [router],
  );
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultTitle}>Star Complete!</Text>
            <Text style={styles.resultSubtitle}>
              You built {finalStats.correct} stars out of {finalStats.total}!
            </Text>
            <ResultCard
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerBlock}>
        <Text style={styles.title}>Star Builder</Text>
        <Text style={styles.subtitle}>
          Round {round}/{TOTAL_ROUNDS} • ⭐ Score: {score}
        </Text>
        <Text style={styles.helper}>
          Connect the dots in order to build a star!
        </Text>
      </View>
          ...
          {dots.map((dot) => (
            <React.Fragment key={dot.number}>
              <Circle
                ...
                cy={dot.y}
                r={DOT_RADIUS}
                fill={dot.connected ? '#FBBF24' : '#E5E7EB'}
                stroke={dot.connected ? '#F59E0B' : '#9CA3AF'}
                strokeWidth="2"
              />
              <SvgText
                x={dot.x}
      ...
      <View style={styles.footerBox}>
        <Text style={styles.footerMain}>
          Skills: planning • number sequence • shape building
        </Text>
        <Text style={styles.footerSub}>
          Connect dots in order to build a perfect star!
        </Text>
      </View>
    </SafeAreaView>
  );
