// Audio utilities for Scribbling & Gripping module
...
const SOUNDS = {
  ...
  drawing: 'https://actions.google.com/sounds/v1/cartoon/slide_whistle.ogg',
};

let soundCache: Map<string, ExpoAudio.Sound> = new Map();

export async function playSoundEffect(type: 'correct' | 'incorrect' | 'celebration' | 'click' | 'drawing'): Promise<void> {
  try {
    ...
    console.warn('[Scribbling Audio] Error playing sound:', error);
  }
}
...
export async function speakInstruction(instruction: string): Promise<void> {
  try {
    ...
    console.warn('[Scribbling Audio] Error speaking instruction:', error);
  }
}
...
export async function speakFeedback(message: string): Promise<void> {
  try {
    ...
    console.warn('[Scribbling Audio] Error speaking feedback:', error);
  }
}
...
export async function stopAllAudio(): Promise<void> {
  try {
    ...
    console.warn('[Scribbling Audio] Error stopping audio:', error);
  }
}
