// Game 2: Trace the Sleeping Line
...
interface TraceSleepingLineGameProps {
  onComplete: (stats: { correct: number; total: number; accuracy: number; gameId: string }) => void;
  onBack: () => void;
}

export default function TraceSleepingLineGame({ onComplete, onBack }: TraceSleepingLineGameProps) {
  ...
  useEffect(() => {
    ...
    setAccuracy(0);
    speakFeedback('Trace the sleeping line from left to right.');
    return () => stopAllAudio();
  }, [round]);
  ...
  const handleNext = () => {
    if (round < TOTAL_ROUNDS - 1) {
      ...
      onComplete({
        ...
        gameId: 'trace-sleeping-line',
      });
    }
  };

  // Scale pattern to canvas
  const scaleX = CANVAS_WIDTH / 500;
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Trace the Sleeping Line</Text>
