...
import { CURVE_PATTERNS } from '../utils/gameData';
import { isPointOnPath, Point } from '@/utils/pathUtils';

interface TraceCurvedLinesGameProps {
  onComplete: (stats: { correct: number; total: number; accuracy: number; gameId: string }) => void;
  onBack: () => void;
}

export default function TraceCurvedLinesGame({ onComplete, onBack }: TraceCurvedLinesGameProps) {
  const { width, height } = useWindowDimensions();
  const CANVAS_WIDTH = width * 0.9;
  const CANVAS_HEIGHT = height * 0.5;
  const TOLERANCE = 50; // Slightly larger tolerance for curves

  const [round, setRound] = useState(0);
  const [currentPattern, setCurrentPattern] = useState(CURVE_PATTERNS[0]);
  ...
  useEffect(() => {
    setCurrentPattern(CURVE_PATTERNS[round % CURVE_PATTERNS.length]);
    setTracedPath([]);
    setRoundComplete(false);
    setAccuracy(0);
    speakFeedback('Trace the curved line.');
    return () => stopAllAudio();
  }, [round]);
      ...
      onPanResponderRelease: () => {
        ...
        if (!roundComplete && tracedPath.length > 0) {
          ...
          if (finalAcc >= 80) {
            ...
            speakFeedback('Try to trace closer to the curve!');
          }
        }
      },
    })
  ...
  const curveToSvgString = (pattern: typeof CURVE_PATTERNS[0]): string => {
    if (pattern.dots.length < 3) return '';
    const start = pattern.dots[0];
    const mid = pattern.dots[Math.floor(pattern.dots.length / 2)];
    const end = pattern.dots[pattern.dots.length - 1];
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Trace Curved Lines</Text>
