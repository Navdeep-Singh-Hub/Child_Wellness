...
import { TongueAwarenessSessionManager } from '@/components/game/speech/tongue-awareness/modules/TongueAwarenessSessionManager';
import type { TongueAwarenessGameId } from '@/components/game/speech/tongue-awareness/modules/tongueAwarenessTypes';
import {
  MouthPoseFaceGatePulse,
  MouthPoseFooterStatus,
  MouthPosePlayArea,
} from '@/components/game/speech/mouth-pose/shared/mouthPoseShared';
import type { TongueAwarenessSense } from '@/hooks/useTongueAwareness';
...
export const DEFAULT_TONGUE_AWARENESS_ROUNDS = 3;
export const TONGUE_AWARENESS_INTERACTIONS_PER_ROUND = 3;

export function clearTongueAwarenessSpeech() {
...
export function speakTongueAwareness(text: string, rate = DEFAULT_TTS_RATE) {
  clearTongueAwarenessSpeech();
  speakTTS(text, rate);
}

export function hapticTongueAwarenessReward() {
  try {
...
export function useTongueAwarenessSession(gameId: TongueAwarenessGameId, rounds = DEFAULT_TONGUE_AWARENESS_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new TongueAwarenessSessionManager(gameId, rounds));
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      await logGameAndAward({
        ...
        skillTags: [
          'tongue-awareness',
          'oral-sensory-mapping',
          'internal-mouth-awareness',
          'pre-oral',
          'exploration-comfort',
        ],
      ...
      console.warn('[tongue awareness] log failed', e);
    }
  }, [gameId, rounds]);
...
export function TongueAwarenessOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Great tongue exploring!"
...
type TongueAwarenessShellProps = {
  title: string;
  subtitle: string;
  skills: string;
  ...
  onGoodTry?: () => void;
  sense?: TongueAwarenessSense;
};

export function TongueAwarenessShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '👅',
  startTitle = 'Tongue awareness play',
  startHint = 'Tap, watch, and explore your tongue. Every try counts.',
  children,
  onGoodTry,
  sense,
}: TongueAwarenessShellProps) {
  return (
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
          ...
          <>
            ...
            {sense && (
              <View style={styles.meterRow}>
                ...
                <Text style={styles.meterLabel}>{sense.helperVisible ? 'Helper on' : 'Exploring'}</Text>
              </View>
            )}
          </>
        )}
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {TONGUE_AWARENESS_INTERACTIONS_PER_ROUND}{' '}
            explorations
          </Text>
        </View>
      </LinearGradient>
