...
import { MotorPlanningSessionManager } from '@/components/game/speech/motor-planning/modules/MotorPlanningSessionManager';
import type { MotorPlanningGameId } from '@/components/game/speech/motor-planning/modules/motorPlanningTypes';
import type { MotorPlanningSense } from '@/hooks/useMotorPlanning';
...
export const DEFAULT_PLANNING_ROUNDS = 3;
export const PLANNING_INTERACTIONS_PER_ROUND = 3;

export function clearPlanningSpeech() {
...
export function speakPlanning(text: string, rate = DEFAULT_TTS_RATE) {
  clearPlanningSpeech();
  speakTTS(text, rate);
}

export function hapticPlanningReward() {
  try {
...
export function usePlanningSession(gameId: MotorPlanningGameId, rounds = DEFAULT_PLANNING_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new MotorPlanningSessionManager(gameId, rounds));
    ...
    try {
      await logGameAndAward({
        ...
        skillTags: [
          ...
          'oral-sequencing-readiness',
          'speech-readiness',
        ],
        meta: managerRef.current.getAnalytics(),
      });
    } catch (e) {
      console.warn('[motor planning] log failed', e);
    }
  }, [gameId, rounds]);
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Great motor planning trying!"
...
type ShellProps = {
  ...
  sense: MotorPlanningSense;
};

export function MotorPlanningShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '🧠',
  startHint = 'Watch first, prepare, then copy. All attempts count.',
  children,
  sense,
}: ShellProps) {
  return (
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          <View style={styles.startWrap}>
            ...
            <Text style={styles.startTitle}>Motor planning</Text>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {PLANNING_INTERACTIONS_PER_ROUND} tries
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
function MicBanner({ sense }: { sense: MotorPlanningSense }) {
...
export function usePlanningHits({
  canPlay,
  sense,
  hits,
  setHits,
  ...
  manager: MotorPlanningSessionManager;
  ...
  React.useEffect(() => {
    ...
    const next = hits + 1;
    setHits(next);
    manager.recordAttempt(sense.planningProgress);
    if (next >= PLANNING_INTERACTIONS_PER_ROUND) {
