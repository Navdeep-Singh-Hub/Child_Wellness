...
import { CVPreparationSessionManager } from '@/components/game/speech/cv-preparation/modules/CVPreparationSessionManager';
import type { CVPreparationGameId } from '@/components/game/speech/cv-preparation/modules/cvPreparationTypes';
import { useCVPreparation, type CVPreparationSense } from '@/hooks/useCVPreparation';
...
export const DEFAULT_CV_PREPARATION_ROUNDS = 3;
export const CV_PREPARATION_INTERACTIONS_PER_ROUND = 3;

export function clearCVPreparationSpeech() {
...
export function speakCVPreparation(text: string, rate = DEFAULT_TTS_RATE) {
  clearCVPreparationSpeech();
  speakTTS(text, rate);
}

export function hapticCVReward() {
  try {
...
export function useCVPreparationSession(
  gameId: CVPreparationGameId,
  rounds = DEFAULT_CV_PREPARATION_ROUNDS,
) {
  ...
  const managerRef = useRef(new CVPreparationSessionManager(gameId, rounds));
    ...
    try {
      await logGameAndAward({
        ...
        skillTags: [
          ...
          'consonant-vowel-readiness',
          'speech-pattern-sequencing',
        ],
        meta: { ...managerRef.current.getAnalytics(), level6: consumeLevel6Telemetry() },
      });
    } catch (e) {
      console.warn('[cv preparation] log failed', e);
    }
  }, [gameId, rounds]);
...
export function CVPreparationOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Great speech pattern tries!"
...
type ShellProps = {
  ...
  children: (sense: CVPreparationSense) => React.ReactNode;
};

export function CVPreparationShell({
  gameId,
  title,
  subtitle,
  skills,
  ...
  phaseHint,
  startEmoji = '🗣️',
  startTitle = 'Simple speech patterns',
  startHint = 'Watch MA, PA, and BA patterns. Copy with your mouth — humming or any sound counts. Optional voice is extra fun, not required.',
  progressLabel = 'patterns',
  children,
}: ShellProps) {
  const sense = useCVPreparation(canPlay, gameId, round);
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
          ...
          <>
            ...
            {sense.postureHint && (
              <Text style={[styles.postureHint, { color: accent }]}>
                Gentle hint: {sense.postureHint.toLowerCase()} mouth feeling — any try is great!
              </Text>
            )}
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {CV_PREPARATION_INTERACTIONS_PER_ROUND}{' '}
            {progressLabel}
          </Text>
        </View>
      </LinearGradient>
...
export function useCVRewardCounter(
