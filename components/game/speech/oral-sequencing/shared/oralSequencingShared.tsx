...
import { OralSequencingSessionManager } from '@/components/game/speech/oral-sequencing/modules/OralSequencingSessionManager';
import type { OralSequencingGameId } from '@/components/game/speech/oral-sequencing/modules/oralSequencingTypes';
import type { OralSequencingSense } from '@/hooks/useOralSequencing';
...
export const DEFAULT_ORAL_SEQUENCE_ROUNDS = 3;
export const ORAL_SEQUENCE_INTERACTIONS_PER_ROUND = 3;

export function clearOralSequenceSpeech() {
...
export function speakOralSequence(text: string, rate = DEFAULT_TTS_RATE) {
  clearOralSequenceSpeech();
  speakTTS(text, rate);
}

export function hapticOralSequenceReward() {
  try {
...
export function useOralSequenceSession(gameId: OralSequencingGameId, rounds = DEFAULT_ORAL_SEQUENCE_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new OralSequencingSessionManager(gameId, rounds));
    ...
    try {
      await logGameAndAward({
        ...
        skillTags: [
          'oral-sequencing',
          'oral-motor-planning',
          'movement-order-awareness',
          'speech-readiness',
        ],
        meta: managerRef.current.getAnalytics(),
      });
    } catch (e) {
      console.warn('[oral sequencing] log failed', e);
    }
  }, [gameId, rounds]);
...
export function OralSequenceOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Great oral sequencing trying!"
...
type ShellProps = {
  ...
  sense: OralSequencingSense;
};

export function OralSequencingShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '🔢',
  startHint = 'Watch mouth steps and try in your own order. All attempts count.',
  children,
  sense,
}: ShellProps) {
  return (
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          <View style={styles.startWrap}>
            ...
            <Text style={styles.startTitle}>Oral sequencing</Text>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {ORAL_SEQUENCE_INTERACTIONS_PER_ROUND} tries
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
function MicBanner({ sense }: { sense: OralSequencingSense }) {
...
export function useOralSequenceHits({
  canPlay,
  sense,
  hits,
  setHits,
  ...
  manager: OralSequencingSessionManager;
  ...
  React.useEffect(() => {
    ...
    if (next >= ORAL_SEQUENCE_INTERACTIONS_PER_ROUND) {
