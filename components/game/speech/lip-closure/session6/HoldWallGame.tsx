import {
  resistanceDifficultyMs,
  resistanceRoundDifficulty,
} from '@/components/game/speech/lip-closure/modules/LipResistanceSessionManager';
import {
  DEFAULT_RESISTANCE_ROUNDS,
  LipResistanceGameOverlays,
  LipResistanceGameShell,
  LipResistanceProgressRing,
  clearResistanceSpeech,
  speakResistance,
  useLipResistanceGameSession,
  useLipResistanceProgress,
  useLipResistanceSense,
} from '@/components/game/speech/lip-closure/shared/lipResistanceShared';
import React, { useEffect, useState } from 'react';
import { StyleSheet, Text, View } from 'react-native';

type Props = { onBack: () => void; onComplete?: () => void };

export function HoldWallGame({ onBack, onComplete }: Props) {
  const session = useLipResistanceGameSession('hold-wall', DEFAULT_RESISTANCE_ROUNDS);
  const [canPlay, setCanPlay] = useState(false);
  const lip = useLipResistanceSense(canPlay);
  const target = resistanceDifficultyMs(resistanceRoundDifficulty(session.round));
  const [shake, setShake] = useState(0);
  const [castle, setCastle] = useState(false);

  useEffect(() => {
    speakResistance('Hold the Wall! Keep lips strong to hold the wall up.');
    return () => clearResistanceSpeech();
  }, []);

  useEffect(() => {
    if (!canPlay) return;
    session.manager.startDetecting();
    setShake(0);
    setCastle(false);
    speakResistance('Hold steady — do not let the wall shake!');
  }, [session.round, canPlay]);

  useEffect(() => {
    if (!canPlay) return;
    const id = setInterval(() => {
      setShake((s) => {
        if (lip.stableHold) return Math.max(0, s - 0.02);
        return Math.min(1, s + 0.012);
      });
      if (lip.stableHold) session.manager.onHolding();
      else if (!lip.inGracePeriod) session.manager.onWarning();
    }, 60);
    return () => clearInterval(id);
  }, [canPlay, lip.stableHold, lip.inGracePeriod]);

  const progress = useLipResistanceProgress(lip, target, canPlay, () => {
    setCastle(true);
    speakResistance('Castle celebration! The wall stands strong!');
    void session.manager.markSuccess(lip.holdDuration, lip.stabilityScore);
    setTimeout(() => session.completeRound(), 1000);
  });

  const wall = castle ? '🏰✨' : shake > 0.4 ? '🧱💫' : '🧱';

  return (
    <>
      <LipResistanceGameShell
        title="Hold the Wall"
        subtitle="Steady lips keep the wall strong"
        skills="🧱 Endurance • 💪 Lip control"
        gradient={['#FEF3C7', '#FDE68A']}
        accent="#B45309"
        onBack={onBack}
        round={session.round}
        rounds={session.rounds}
        canPlay={canPlay}
        onStart={() => setCanPlay(true)}
        lip={lip}
      >
        <View style={styles.center}>
          <Text style={[styles.wall, { transform: [{ rotate: `${shake * 4 - 2}deg` }] }]}>{wall}</Text>
          <LipResistanceProgressRing progress={progress} accent="#B45309" />
        </View>
      </LipResistanceGameShell>
      <LipResistanceGameOverlays {...session} onBack={onBack} onComplete={onComplete} />
    </>
  );
}

const styles = StyleSheet.create({
  center: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  wall: { fontSize: 88, marginBottom: 16 },
});
