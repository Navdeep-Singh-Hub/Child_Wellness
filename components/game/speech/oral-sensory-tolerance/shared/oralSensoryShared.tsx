...
import {
  OralToleranceSessionManager,
} from '@/components/game/speech/oral-sensory-tolerance/modules/OralToleranceSessionManager';
import type { OralToleranceGameId } from '@/components/game/speech/oral-sensory-tolerance/modules/oralSensoryTypes';
import {
  MouthPoseFaceGatePulse,
  MouthPoseFooterStatus,
  MouthPosePlayArea,
} from '@/components/game/speech/mouth-pose/shared/mouthPoseShared';
import type { OralToleranceSense } from '@/hooks/useOralSensoryTolerance';
...
export const DEFAULT_ORAL_ROUNDS = 3;
export const ORAL_INTERACTIONS_PER_ROUND = 3;

export function clearOralSpeech() {
...
export function speakOral(text: string, rate = DEFAULT_TTS_RATE) {
  clearOralSpeech();
  speakTTS(text, rate);
}

export function hapticOralReward() {
  try {
...
export function useOralGameSession(gameId: OralToleranceGameId, rounds = DEFAULT_ORAL_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new OralToleranceSessionManager(gameId, rounds));
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[oral sensory] log failed', e);
    }
  }, [gameId, rounds]);
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Mouth activities can feel safe."
...
type ShellProps = {
  ...
  onGoodTry?: () => void;
  onCalmDown?: () => void;
  sense?: OralToleranceSense;
};

export function OralShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '🫧',
  startTitle = 'Safe mouth play',
  startHint = 'Tap gently. Watching is okay. Every try counts.',
  children,
  onGoodTry,
  onCalmDown,
  sense,
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          ...
          <>
            ...
            <View style={styles.helperRow}>
              ...
              <Pressable
                style={[styles.helperBtnAlt, { borderColor: accent }]}
                onPress={() => {
                  onCalmDown?.();
                  hapticOralReward();
                }}
              >
                <Text style={[styles.helperBtnAltText, { color: accent }]}>🧘 Calm</Text>
              </Pressable>
            </View>
              ...
              <View style={styles.meterRow}>
                <View style={styles.meter}>
                  <View
                    style={[
                      styles.meterFill,
                      { width: `${Math.round(sense.comfortLevel * 100)}%`, backgroundColor: accent },
                    ]}
                  />
                </View>
                <Text style={styles.meterLabel}>
                  {sense.state === 'HELPING' ? 'Slow and safe' : sense.rewardState !== 'NONE' ? 'Nice!' : 'Comfy'}
                </Text>
              </View>
            )}
          </>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {ORAL_INTERACTIONS_PER_ROUND} calm taps
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  helperRow: { flexDirection: 'row', gap: 10, paddingHorizontal: 12, paddingBottom: 8 },
  ...
  helperBtnAltText: { fontWeight: '900', fontSize: 16 },
  ...
  meterLabel: { fontSize: 12, fontWeight: '800', color: '#334155', minWidth: 90, textAlign: 'right' },
  footer: {
