...
export const TapWhatIShowYouGame: React.FC<Props> = ({
  onBack,
  onComplete,
  requiredRounds = 6,
}) => {
  ...
  // Game state
  const [objects, setObjects] = useState<Array<{ id: number; emoji: string; color: string[]; name: string }>>([]);
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      const result = await logGameAndAward({
        type: 'tap-what-i-show-you',
        correct: correctTaps,
        total: totalAttempts || requiredRounds,
        accuracy,
        xpAwarded: xp,
        mode: 'therapy',
        skillTags: ['visual-instruction', 'attention-to-cues', 'errorless-learning', 'directive-comprehension'],
  ...
  const startRound = useCallback(() => {
    ...
    // Show glowing object and instruction
    setTimeout(() => {
      setGlowingObjectId(target.id);
      
      // Glow animation
      Animated.sequence([
        Animated.parallel([
          Animated.timing(glowScales.get(target.id)!, {
            toValue: 1.3,
            duration: GLOW_DURATION_MS / 2,
            easing: Easing.inOut(Easing.ease),
            useNativeDriver: true,
          }),
          Animated.timing(glowOpacities.get(target.id)!, {
            toValue: 0.7,
            duration: GLOW_DURATION_MS / 2,
            useNativeDriver: true,
          }),
      ...
      // Show instruction after glow starts
      setTimeout(() => {
      ...
      >
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            <Text style={styles.title}>Tap What I Show You</Text>
            <Text style={styles.subtitle}>
              {showInstruction ? 'Tap the glowing object!' : 'Watch carefully...'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.skillsContainer}>
          <View style={styles.skillItem}>
            ...
            <Text style={styles.skillText}>Visual Instruction</Text>
          </View>
          <View style={styles.skillItem}>
            <Ionicons name="flash" size={20} color="#0F172A" />
            <Text style={styles.skillText}>Attention to Cues</Text>
          </View>
          <View style={styles.skillItem}>
            <Ionicons name="checkmark-circle" size={20} color="#0F172A" />
            <Text style={styles.skillText}>Errorless Learning</Text>
          </View>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  glow: {
    position: 'absolute',
    width: OBJECT_SIZE + 50,
    height: OBJECT_SIZE + 50,
    borderRadius: (OBJECT_SIZE + 50) / 2,
