...
export const PointToObjectAppearsGame: React.FC<Props> = ({
  onBack,
  onComplete,
  requiredTaps = 5,
}) => {
  ...
  const finishGame = useCallback(async () => {
    ...
    speak('Amazing! You anticipated where the object would appear!');
    ...
    try {
      ...
      const result = await logGameAndAward({
        ...
        xpAwarded,
        skillTags: ['gesture-anticipation', 'referential-pointing', 'social-prediction'],
        meta: {
          gameType: 'point-to-object-appears',
          totalTaps: requiredTaps,
          correctTaps: hits,
        },
      });
  ...
  const pointAtDirection = (direction: PointDirection) => {
    setIsPointing(true);
  ...
  const handSize = 48;
  const handX = objectCenterX - handSize / 2;
  const handY = objectCenterY + OBJECT_SIZE / 2 + 14;
      ...
      >
        <View style={styles.header}>
          ...
          <View style={styles.headerText}>
            <Text style={styles.title}>Point → Object Appears</Text>
            <Text style={styles.subtitle}>
              {isPointing && !objectVisible ? '👆 I\'m pointing...' : objectVisible ? 'Tap it!' : 'Watch where I point!'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.playArea}>
          ...
          {isPointing && (
            <Animated.View
              pointerEvents="none"
              style={[
                styles.pointingHandContainer,
                {
                  left: handX,
                  top: handY,
                  opacity: pointingOpacity,
                  transform: [{ scale: pointingHandScale }],
                },
              ]}
            >
              <Text style={styles.pointingHandEmoji}>{getPointingHandEmoji()}</Text>
            </Animated.View>
          )}
          ...
          {!objectVisible && !isPointing && (
            <View style={styles.instructionBadge}>
              <Text style={styles.instructionText}>👆 Watch where I point!</Text>
            </View>
          )}
        </View>
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>
            👆 Gesture Anticipation • 🎯 Referential Pointing • 🧠 Social Prediction
          </Text>
