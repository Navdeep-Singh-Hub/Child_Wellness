...
import { BreathJawCoordinationSessionManager } from '@/components/game/speech/breath-jaw-coordination/modules/BreathJawCoordinationSessionManager';
import type { BreathJawCoordinationGameId } from '@/components/game/speech/breath-jaw-coordination/modules/breathJawCoordinationTypes';
import type { BreathJawCoordinationSense } from '@/hooks/useBreathJawCoordination';
...
export const DEFAULT_BREATH_JAW_ROUNDS = 3;
export const BREATH_JAW_INTERACTIONS_PER_ROUND = 3;

export function clearBreathJawSpeech() {
...
export function speakBreathJaw(text: string, rate = DEFAULT_TTS_RATE) {
  clearBreathJawSpeech();
  speakTTS(text, rate);
}

export function hapticBreathJawReward() {
  try {
...
export function useBreathJawSession(gameId: BreathJawCoordinationGameId, rounds = DEFAULT_BREATH_JAW_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new BreathJawCoordinationSessionManager(gameId, rounds));
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[breath jaw coordination] log failed', e);
    }
  }, [gameId, rounds]);
...
export function BreathJawOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Breath and jaw teamwork — every try helps!"
...
type ShellProps = {
  ...
  sense: BreathJawCoordinationSense;
};

export function BreathJawCoordinationShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '💨',
  startHint = 'Use soft air, jaw movement, or both. All attempts count.',
  children,
  sense,
}: ShellProps) {
  return (
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          <View style={styles.startWrap}>
            ...
            <Text style={styles.startTitle}>Breath + jaw teamwork</Text>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {BREATH_JAW_INTERACTIONS_PER_ROUND} tries
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
function MicBanner({ sense }: { sense: BreathJawCoordinationSense }) {
...
export function useBreathJawHits({
  canPlay,
  sense,
  hits,
  setHits,
  ...
  manager: BreathJawCoordinationSessionManager;
  ...
  React.useEffect(() => {
    ...
    const next = hits + 1;
    setHits(next);
    manager.recordAttempt(Math.min(1, next / BREATH_JAW_INTERACTIONS_PER_ROUND));
    if (next >= BREATH_JAW_INTERACTIONS_PER_ROUND) {
