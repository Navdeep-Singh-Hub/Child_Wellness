/**
 * Windmill Spin Game
 * Blow to spin a colorful pinwheel
 */
...
interface RoundResult {
  round: number;
  stars: number;
  rotations: number; // Total rotations completed
  timeElapsed: number;
}

const TOTAL_ROUNDS = 5;
const ROUND_TIME_MS = 40000; // 40 seconds per round
const TARGET_ROTATIONS = 5; // 5 full rotations (1800 degrees)
...
export function WindmillSpinGame({ onBack, onComplete, requiredRounds = TOTAL_ROUNDS }: Props) {
  ...
  useEffect(() => {
    ...
    if (blowState.isSustained) {
      // Rotate pinwheel based on blow intensity
      const rotationSpeed = blowState.intensity * 5; // degrees per frame
      setRotationAngle(prev => {
        const newAngle = prev + rotationSpeed;
        const rotations = newAngle / 360;
        totalRotations.current = rotations;
        return newAngle;
      });
    }
  ...
  const startCalibration = useCallback(() => {
    ...
    if (currentRound === 1) {
      speak(
        'Welcome to Windmill Spin! Blow into your device to spin the colorful pinwheel. ' +
        `Complete ${TARGET_ROTATIONS} full rotations to finish each round! Show your face to the camera to start!`
      );
    } else {
  ...
  const startCountdown = useCallback(() => {
    ...
    countdownRef.current = setInterval(() => {
      setCountdown(prev => {
        ...
        speak(prev - 1 === 0 ? 'Go! Spin the windmill!' : String(prev - 1));
        return prev - 1;
      });
    }, 1000);
  }, []);
  ...
  const startRound = useCallback(() => {
    ...
    timerRef.current = setInterval(() => {
      ...
      if (totalRotations.current >= TARGET_ROTATIONS) {
        endRound();
      }
  ...
  const endRound = useCallback(() => {
    ...
    const rotations = totalRotations.current;
    let stars = 0;

    if (rotations >= TARGET_ROTATIONS && timeElapsed < 20) {
      stars = 3;
    } else if (rotations >= TARGET_ROTATIONS && timeElapsed < 40) {
      stars = 2;
    } else if (rotations >= 3) {
      stars = 1;
    }
  ...
  }, [currentRound, timeElapsed, requiredRounds, startCalibration]);
  ...
  const finishGame = useCallback(async () => {
    ...
    const averageRotations = roundResults.reduce((sum, r) => sum + r.rotations, 0) / roundResults.length;
    const accuracy = Math.round((averageRotations / TARGET_ROTATIONS) * 100);
  ...
  // Pinwheel segments (8 segments with different colors)
  const pinwheelSize = isTablet ? 300 : isMobile ? 200 : 250;
  const centerX = pinwheelSize / 2;
  const centerY = pinwheelSize / 2;
  const radius = pinwheelSize / 2 - 20;
      ...
      <View style={styles.overlay}>
        ...
        {gameState === 'playing' && (
          <View style={styles.counterContainer}>
            <Text style={styles.counterText}>
              {Math.floor(totalRotations.current)} / {TARGET_ROTATIONS} rotations
            </Text>
          </View>
        )}
        ...
        {gameState === 'playing' && (
          <View style={styles.gameContainer}>
            ...
            >
              <Svg width={pinwheelSize} height={pinwheelSize}>
                <Defs>
