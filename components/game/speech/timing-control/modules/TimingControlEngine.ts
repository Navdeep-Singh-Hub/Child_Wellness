...
export class TimingControlEngine {
  ...
  constructor(gameId: TimingControlGameId) {
    this.gameId = gameId;
    if (gameId === 'timing-hero-challenge') this.timingLength = 3;
  }

  configure(difficulty: TimingControlDifficulty) {
    this.difficulty = difficulty;
    this.timingLength = difficulty === 'easy' ? 2 : 3;
  ...
  process(rawLevel: number, now = Date.now()): TimingControlSnapshot {
  ...
  coordinate(now = Date.now()) {
    ...
    this.state = 'TIMING_ACTIVE';
    this.stepIndex = (this.stepIndex + 1) % DEFAULT_STEPS.length;
    this.currentStep = DEFAULT_STEPS[this.stepIndex] ?? DEFAULT_STEPS[0];
    const count = (this.timingAttempt % this.timingLength) || this.timingLength;
    this.progress = clamp01(count / this.timingLength);

    const rewardEvery = this.difficulty === 'easy' ? 1 : this.difficulty === 'medium' ? 2 : 3;
    if (this.timingAttempt % rewardEvery === 0 || this.progress >= 0.95) {
  ...
  triggerReward(kind: TimingRewardState) {
  ...
  private snapshot(now: number): TimingControlSnapshot {
    return {
      state: this.state,
      timingAttempt: this.timingAttempt,
  ...
  getAnalytics(gameCompletion = 0): TimingControlAnalytics {
    return {
      engagementTimeMs: Date.now() - this.startMs,
      timingAttempts: this.timingAttempt,
