...
import { VowelShapingSessionManager } from '@/components/game/speech/vowel-shaping/modules/VowelShapingSessionManager';
import type { VowelShapingGameId } from '@/components/game/speech/vowel-shaping/modules/vowelShapingTypes';
import { useVowelShaping, type VowelShapingSense } from '@/hooks/useVowelShaping';
...
export const DEFAULT_VOWEL_SHAPING_ROUNDS = 3;
export const VOWEL_SHAPING_INTERACTIONS_PER_ROUND = 3;

export function clearVowelShapingSpeech() {
...
export function speakVowelShaping(text: string, rate = DEFAULT_TTS_RATE) {
  clearVowelShapingSpeech();
  speakTTS(text, rate);
}

export function hapticVowelReward() {
  try {
...
export function useVowelShapingSession(gameId: VowelShapingGameId, rounds = DEFAULT_VOWEL_SHAPING_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new VowelShapingSessionManager(gameId, rounds));
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[vowel shaping] log failed', e);
    }
  }, [gameId, rounds]);
...
export function VowelShapingOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Great vowel mouth shapes!"
...
type ShellProps = {
  ...
  startTitle?: string;
  startHint?: string;
  children: (sense: VowelShapingSense) => React.ReactNode;
};

export function VowelShapingShell({
  gameId,
  title,
  subtitle,
  skills,
  ...
  phaseHint,
  startEmoji = '😮',
  startTitle = 'Vowel mouth shapes',
  startHint = 'Watch AAA, OOO, and EEE shapes. Copy with your mouth — any try counts. Optional voice is extra fun, not required.',
  children,
}: ShellProps) {
  const sense = useVowelShaping(canPlay, gameId, round);
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {VOWEL_SHAPING_INTERACTIONS_PER_ROUND}{' '}
            shapes
          </Text>
        </View>
      </LinearGradient>
...
export function useVowelRewardCounter(
