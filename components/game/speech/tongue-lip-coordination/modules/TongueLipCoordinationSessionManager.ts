...
export function tongueLipRoundDifficulty(round: number): TongueLipCoordinationDifficulty {
  if (round <= 1) return 'easy';
  if (round === 2) return 'medium';
  return 'hard';
}

export class TongueLipCoordinationSessionManager {
  readonly gameId: TongueLipCoordinationGameId;
  readonly rounds: number;
  private round = 1;
  private interactionsThisRound = 0;
  private totalInteractions = 0;
  private analytics: TongueLipCoordinationAnalytics = {
    engagementTimeMs: 0,
  ...
  constructor(gameId: TongueLipCoordinationGameId, rounds = 3) {
    this.gameId = gameId;
    this.rounds = rounds;
  }
