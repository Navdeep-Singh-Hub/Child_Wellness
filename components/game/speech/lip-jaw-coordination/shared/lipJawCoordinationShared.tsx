...
import { LipJawCoordinationSessionManager } from '@/components/game/speech/lip-jaw-coordination/modules/LipJawCoordinationSessionManager';
import type { LipJawCoordinationGameId } from '@/components/game/speech/lip-jaw-coordination/modules/lipJawCoordinationTypes';
import type { LipJawCoordinationSense } from '@/hooks/useLipJawCoordination';
...
export const DEFAULT_LIP_JAW_ROUNDS = 3;
export const LIP_JAW_INTERACTIONS_PER_ROUND = 3;

export function clearLipJawSpeech() {
...
export function speakLipJaw(text: string, rate = DEFAULT_TTS_RATE) {
  clearLipJawSpeech();
  speakTTS(text, rate);
}

export function hapticLipJawReward() {
  try {
...
export function useLipJawSession(gameId: LipJawCoordinationGameId, rounds = DEFAULT_LIP_JAW_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new LipJawCoordinationSessionManager(gameId, rounds));
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[lip jaw coordination] log failed', e);
    }
  }, [gameId, rounds]);
...
export function LipJawOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Lips and jaw move together — great teamwork!"
...
type ShellProps = {
  ...
  onGoodTry?: () => void;
  sense?: LipJawCoordinationSense;
};

export function LipJawCoordinationShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '👄',
  startTitle = 'Lips + jaw teamwork',
  startHint = 'Watch the big mouth, copy if you want, and tap I tried. Partial moves count — no pressure.',
  children,
  onGoodTry,
  sense,
}: ShellProps) {
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {LIP_JAW_INTERACTIONS_PER_ROUND} tries
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
export function useCoordinationHits(
  canPlay: boolean,
  sense: LipJawCoordinationSense,
  hits: number,
  setHits: React.Dispatch<React.SetStateAction<number>>,
  manager: LipJawCoordinationSessionManager,
  ...
  React.useEffect(() => {
    ...
    if (next >= LIP_JAW_INTERACTIONS_PER_ROUND) {
