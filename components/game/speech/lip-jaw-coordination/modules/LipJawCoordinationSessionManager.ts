...
export function lipJawRoundDifficulty(round: number): LipJawCoordinationDifficulty {
  if (round <= 1) return 'easy';
  if (round === 2) return 'medium';
  return 'hard';
}

export class LipJawCoordinationSessionManager {
  readonly gameId: LipJawCoordinationGameId;
  readonly rounds: number;
  private round = 1;
  private interactionsThisRound = 0;
  private totalInteractions = 0;
  private analytics: LipJawCoordinationAnalytics = {
    engagementTimeMs: 0,
  ...
  constructor(gameId: LipJawCoordinationGameId, rounds = 3) {
    this.gameId = gameId;
    this.rounds = rounds;
  }
