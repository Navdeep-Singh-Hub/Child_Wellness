...
import { MouthAttentionSessionManager } from '@/components/game/speech/mouth-attention/modules/MouthAttentionSessionManager';
import type { MouthAttentionGameId } from '@/components/game/speech/mouth-attention/modules/mouthAttentionTypes';
import {
  MouthPoseFaceGatePulse,
  MouthPoseFooterStatus,
  MouthPosePlayArea,
} from '@/components/game/speech/mouth-pose/shared/mouthPoseShared';
import type { MouthAttentionSense } from '@/hooks/useMouthAttention';
...
export const DEFAULT_MOUTH_ATTENTION_ROUNDS = 3;
export const MOUTH_ATTENTION_INTERACTIONS_PER_ROUND = 3;

export function clearMouthAttentionSpeech() {
...
export function speakMouthAttention(text: string, rate = DEFAULT_TTS_RATE) {
  clearMouthAttentionSpeech();
  speakTTS(text, rate);
}

export function hapticMouthAttentionReward() {
  try {
...
export function useMouthAttentionSession(gameId: MouthAttentionGameId, rounds = DEFAULT_MOUTH_ATTENTION_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new MouthAttentionSessionManager(gameId, rounds));
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      await logGameAndAward({
        ...
        skillTags: [
          'mouth-attention-shifting',
          'oral-awareness',
          'visual-attention-shifting',
          'joint-attention-readiness',
          'pre-oral',
        ],
        meta: managerRef.current.getAnalytics(),
      });
    } catch (e) {
      console.warn('[mouth attention] log failed', e);
    }
  }, [gameId, rounds]);
...
export function MouthAttentionOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Great noticing and attention shifting!"
...
type MouthAttentionShellProps = {
  ...
  onGoodTry?: () => void;
  sense?: MouthAttentionSense;
};

export function MouthAttentionShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '👄',
  startTitle = 'Mouth attention play',
  startHint = 'Tap and notice different mouth parts. Watching counts too.',
  children,
  onGoodTry,
  sense,
}: MouthAttentionShellProps) {
  return (
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          ...
          <>
            ...
            <MouthPosePlayArea canPlay={canPlay}>
              {sense ? (
                <MouthPoseFaceGatePulse canPlay={canPlay} onPulse={() => sense.interact()} />
              ) : null}
            ...
            {sense && (
              <View style={styles.meterRow}>
                ...
                <Text style={styles.meterLabel}>{sense.helperVisible ? 'Helper on' : 'Engaged'}</Text>
              </View>
            )}
          </>
        )}
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {MOUTH_ATTENTION_INTERACTIONS_PER_ROUND} interactions
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  meterLabel: { fontSize: 12, fontWeight: '800', color: '#334155', minWidth: 80, textAlign: 'right' },
