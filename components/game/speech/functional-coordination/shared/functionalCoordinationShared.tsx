...
import { FunctionalCoordinationSessionManager } from '@/components/game/speech/functional-coordination/modules/FunctionalCoordinationSessionManager';
import type { FunctionalCoordinationGameId } from '@/components/game/speech/functional-coordination/modules/functionalCoordinationTypes';
import type { FunctionalCoordinationSense } from '@/hooks/useFunctionalCoordination';
...
export const FUNCTIONAL_INTERACTIONS_PER_ROUND = 3;
...
export function hapticFunctionalReward() {
  try {
...
export function useFunctionalSession(gameId: FunctionalCoordinationGameId, rounds = DEFAULT_FUNCTIONAL_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new FunctionalCoordinationSessionManager(gameId, rounds));
    ...
    try {
      await logGameAndAward({
        ...
        skillTags: [
          ...
          'oral-integration',
          'speech-readiness',
          'motor-planning-readiness',
        ],
        meta: managerRef.current.getAnalytics(),
      });
    } catch (e) {
      console.warn('[functional coordination] log failed', e);
    }
  }, [gameId, rounds]);
...
export function FunctionalOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="You completed functional coordination!"
...
type ShellProps = {
  ...
  sense: FunctionalCoordinationSense;
};

export function FunctionalCoordinationShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '🎓',
  startHint = 'Watch, prepare, and copy with calm pacing. All attempts count.',
  children,
  sense,
}: ShellProps) {
  return (
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          <View style={styles.startWrap}>
            ...
            <Text style={styles.startTitle}>Functional coordination</Text>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {FUNCTIONAL_INTERACTIONS_PER_ROUND} tries
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
function MicBanner({ sense }: { sense: FunctionalCoordinationSense }) {
...
export function useFunctionalHits({
  canPlay,
  sense,
  hits,
  setHits,
  ...
  manager: FunctionalCoordinationSessionManager;
  ...
  React.useEffect(() => {
    ...
    const next = hits + 1;
    setHits(next);
    manager.recordAttempt(sense.coordinationProgress);
    if (next >= FUNCTIONAL_INTERACTIONS_PER_ROUND) {
