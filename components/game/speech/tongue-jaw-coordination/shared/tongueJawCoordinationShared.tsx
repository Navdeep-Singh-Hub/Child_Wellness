...
import { TongueJawCoordinationSessionManager } from '@/components/game/speech/tongue-jaw-coordination/modules/TongueJawCoordinationSessionManager';
import type { TongueJawCoordinationGameId } from '@/components/game/speech/tongue-jaw-coordination/modules/tongueJawCoordinationTypes';
import type { TongueJawCoordinationSense } from '@/hooks/useTongueJawCoordination';
...
export const DEFAULT_TONGUE_JAW_ROUNDS = 3;
export const TONGUE_JAW_INTERACTIONS_PER_ROUND = 3;

export function clearTongueJawSpeech() {
...
export function speakTongueJaw(text: string, rate = DEFAULT_TTS_RATE) {
  clearTongueJawSpeech();
  speakTTS(text, rate);
}

export function hapticTongueJawReward() {
  try {
...
export function useTongueJawSession(
  gameId: TongueJawCoordinationGameId,
  rounds = DEFAULT_TONGUE_JAW_ROUNDS,
) {
  ...
  const managerRef = useRef(new TongueJawCoordinationSessionManager(gameId, rounds));
    ...
    try {
      await logGameAndAward({
        ...
        skillTags: [
          ...
          'internal-oral-coordination',
          'motor-planning',
        ],
        meta: managerRef.current.getAnalytics(),
      });
    } catch (e) {
      console.warn('[tongue jaw coordination] log failed', e);
    }
  }, [gameId, rounds]);
...
export function TongueJawOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Tongue and jaw teamwork — great trying!"
...
type ShellProps = {
  ...
  onGoodTry?: () => void;
  sense?: TongueJawCoordinationSense;
};

export function TongueJawCoordinationShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '👅',
  startTitle = 'Tongue + jaw teamwork',
  startHint = 'Watch the big mouth, try tongue + jaw together, and tap I tried. Mouth-only tries count too.',
  children,
  onGoodTry,
  sense,
}: ShellProps) {
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
              ...
              <View style={styles.meterRow}>
                <View style={styles.meter}>
                  <View
                    style={[
                      styles.meterFill,
                      { width: `${Math.round(sense.sequenceProgress * 100)}%`, backgroundColor: accent },
                    ]}
                  />
                </View>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {TONGUE_JAW_INTERACTIONS_PER_ROUND} tries
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
export function useTongueJawHits({
  canPlay,
  sense,
  hits,
  setHits,
  ...
  manager: TongueJawCoordinationSessionManager;
  ...
  React.useEffect(() => {
    ...
    if (next >= TONGUE_JAW_INTERACTIONS_PER_ROUND) {
