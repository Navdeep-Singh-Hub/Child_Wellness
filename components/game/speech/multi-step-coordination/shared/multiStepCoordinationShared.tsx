...
import { MultiStepCoordinationSessionManager } from '@/components/game/speech/multi-step-coordination/modules/MultiStepCoordinationSessionManager';
import type { MultiStepCoordinationGameId } from '@/components/game/speech/multi-step-coordination/modules/multiStepCoordinationTypes';
import type { MultiStepCoordinationSense } from '@/hooks/useMultiStepCoordination';
...
export const DEFAULT_MULTI_STEP_ROUNDS = 3;
export const MULTI_STEP_INTERACTIONS_PER_ROUND = 3;

export function clearMultiStepSpeech() {
...
export function speakMultiStep(text: string, rate = DEFAULT_TTS_RATE) {
  clearMultiStepSpeech();
  speakTTS(text, rate);
}

export function hapticMultiStepReward() {
  try {
...
export function useMultiStepSession(gameId: MultiStepCoordinationGameId, rounds = DEFAULT_MULTI_STEP_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new MultiStepCoordinationSessionManager(gameId, rounds));
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[multi step coordination] log failed', e);
    }
  }, [gameId, rounds]);
...
export function MultiStepOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Great multi-step coordination trying!"
...
type ShellProps = {
  ...
  sense: MultiStepCoordinationSense;
};

export function MultiStepCoordinationShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '🧠',
  startHint = 'Watch the sequence and try in your own way. All attempts count.',
  children,
  sense,
}: ShellProps) {
  return (
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          <View style={styles.startWrap}>
            ...
            <Text style={styles.startTitle}>Multi-step coordination</Text>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {MULTI_STEP_INTERACTIONS_PER_ROUND} tries
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
function MicBanner({ sense }: { sense: MultiStepCoordinationSense }) {
...
export function useMultiStepHits({
  canPlay,
  sense,
  hits,
  setHits,
  ...
  manager: MultiStepCoordinationSessionManager;
  ...
  React.useEffect(() => {
    ...
    const next = hits + 1;
    setHits(next);
    manager.recordAttempt(sense.sequenceProgress);
    if (next >= MULTI_STEP_INTERACTIONS_PER_ROUND) {
