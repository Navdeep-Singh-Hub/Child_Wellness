...
import { BreathLipCoordinationSessionManager } from '@/components/game/speech/breath-lip-coordination/modules/BreathLipCoordinationSessionManager';
import type { BreathLipCoordinationGameId } from '@/components/game/speech/breath-lip-coordination/modules/breathLipCoordinationTypes';
import type { BreathLipCoordinationSense } from '@/hooks/useBreathLipCoordination';
...
export const DEFAULT_BREATH_LIP_ROUNDS = 3;
export const BREATH_LIP_INTERACTIONS_PER_ROUND = 3;

export function clearBreathLipSpeech() {
...
export function speakBreathLip(text: string, rate = DEFAULT_TTS_RATE) {
  clearBreathLipSpeech();
  speakTTS(text, rate);
}

export function hapticBreathLipReward() {
  try {
...
export function useBreathLipSession(gameId: BreathLipCoordinationGameId, rounds = DEFAULT_BREATH_LIP_ROUNDS) {
  const [round, setRound] = useState(1);
  const [showRoundSuccess, setShowRoundSuccess] = useState(false);
  const [gameFinished, setGameFinished] = useState(false);
  const [finalStats, setFinalStats] = useState<{ accuracy: number; totalStars: number } | null>(null);
  const finishedRef = useRef(false);
  const managerRef = useRef(new BreathLipCoordinationSessionManager(gameId, rounds));
  ...
  const finishGame = useCallback(async () => {
    ...
    try {
      ...
      console.warn('[breath lip coordination] log failed', e);
    }
  }, [gameId, rounds]);
...
export function BreathLipOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Breath and lips teamwork — every try helps!"
...
type ShellProps = {
  ...
  sense: BreathLipCoordinationSense;
};

export function BreathLipCoordinationShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '💨',
  startHint = 'Use soft air, lip movement, or both. All attempts count.',
  children,
  sense,
}: ShellProps) {
  return (
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          <View style={styles.startWrap}>
            ...
            <Text style={styles.startTitle}>Breath + lips teamwork</Text>
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {BREATH_LIP_INTERACTIONS_PER_ROUND} tries
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
...
function MicBanner({ sense }: { sense: BreathLipCoordinationSense }) {
...
export function useBreathLipHits({
  canPlay,
  sense,
  hits,
  setHits,
  ...
  manager: BreathLipCoordinationSessionManager;
  ...
  React.useEffect(() => {
    ...
    const next = hits + 1;
    setHits(next);
    manager.recordAttempt(Math.min(1, next / BREATH_LIP_INTERACTIONS_PER_ROUND));
    if (next >= BREATH_LIP_INTERACTIONS_PER_ROUND) {
