...
import { SpeechOralImitationSessionManager } from '@/components/game/speech/speech-oral-imitation/modules/SpeechOralImitationSessionManager';
import type { SpeechOralImitationGameId } from '@/components/game/speech/speech-oral-imitation/modules/speechOralImitationTypes';
...
export const DEFAULT_SPEECH_ORAL_ROUNDS = 3;
export const SPEECH_ORAL_INTERACTIONS_PER_ROUND = 3;

export function clearSpeechOralImitationSpeech() {
...
export function speakSpeechOralImitation(text: string, rate = DEFAULT_TTS_RATE) {
  clearSpeechOralImitationSpeech();
  speakTTS(text, rate);
}

export function hapticSpeechOralReward() {
  try {
...
export function useSpeechOralImitationSession(
  gameId: SpeechOralImitationGameId,
  rounds = DEFAULT_SPEECH_ORAL_ROUNDS,
) {
  ...
  const managerRef = useRef(new SpeechOralImitationSessionManager(gameId, rounds));
    ...
    try {
      await logGameAndAward({
        ...
        skillTags: [
          ...
          'oral-imitation',
          'speech-like-mouth-imitation',
          'motor-planning',
        ],
        meta: { ...managerRef.current.getAnalytics(), level6: consumeLevel6Telemetry() },
      });
    } catch (e) {
      console.warn('[speech oral imitation] log failed', e);
    }
  }, [gameId, rounds]);
...
export function SpeechOralImitationOverlays({
  showRoundSuccess,
  gameFinished,
  finalStats,
  onBack,
      ...
      {gameFinished && finalStats && (
        <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
          <CongratulationsScreen
            message="Great speech mouth copying!"
...
export function SpeechOralImitationShell({
  title,
  subtitle,
  skills,
  gradient,
  ...
  phaseHint,
  startEmoji = '🗣️',
  startTitle = 'Speech mouth copy',
  startHint = 'Watch speech mouth shapes and copy. Partial tries count — no pressure.',
  children,
  onGoodTry,
  sense,
  encouragementNote,
      ...
      <LinearGradient colors={gradient} style={styles.flex}>
        ...
        {!canPlay ? (
          ...
          <>
            ...
            {encouragementNote ? (
              <Text style={[styles.encourage, { color: accent }]}>{encouragementNote}</Text>
            ) : null}
        ...
        <View style={styles.footer}>
          ...
          <Text style={styles.progressText}>
            Round {Math.min(round, rounds)} / {rounds} · {hits} / {SPEECH_ORAL_INTERACTIONS_PER_ROUND} copies
          </Text>
        </View>
      </LinearGradient>
    </SafeAreaView>
