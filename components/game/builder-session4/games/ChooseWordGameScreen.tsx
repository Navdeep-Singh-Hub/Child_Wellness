...
import { WORD_OPTIONS, WORD_HAT } from '../utils/gameData';
...
export default function ChooseWordGameScreen({ onComplete, onBack }: ChooseWordGameScreenProps) {
  ...
  useEffect(() => {
    ...
    speakInstruction('Tap the word HAT.');
    return () => stopAllAudio();
  }, [round]);
  ...
  const handleSelect = (word: string) => {
    ...
    if (correct) {
      ...
      speakFeedback('Correct! That spells HAT!');
    } else {
      playSoundEffect('incorrect');
      speakFeedback('Try again! Look for the word HAT.');
    }
