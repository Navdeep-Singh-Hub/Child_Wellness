...
import { WORD_HAT } from '../utils/gameData';
...
export default function WordIntroScreen({ onComplete, onBack }: WordIntroScreenProps) {
  ...
  useEffect(() => {
    if (hasStarted && currentLetterIndex < WORD_HAT.letters.length) {
      letterScale.value = withSequence(
        withTiming(0, { duration: 0 }),
        withSpring(1, { damping: 10, stiffness: 100 }),
        withTiming(1, { duration: 1000 })
      );
      speakLetter(WORD_HAT.letters[currentLetterIndex]);
      playSoundEffect('letter');
      
      const timer = setTimeout(() => {
        if (currentLetterIndex < WORD_HAT.letters.length - 1) {
          setCurrentLetterIndex((prev) => prev + 1);
        } else {
          setTimeout(() => {
            speakWord(WORD_HAT.word);
            setShowImage(true);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {/* Word Display */}
      {hasStarted && currentLetterIndex === WORD_HAT.letters.length - 1 && (
        <View style={styles.wordContainer}>
          <Text style={styles.wordText}>{WORD_HAT.word}</Text>
        </View>
      )}
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]}>
          <Text style={styles.imageEmoji}>{WORD_HAT.image}</Text>
          <Text style={styles.imageText}>Hat</Text>
        </Animated.View>
      )}
