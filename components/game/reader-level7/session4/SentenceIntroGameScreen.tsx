// Game 1: Sentence Introduction - Read "I see the sun"
...
import { SENTENCE_SESSION4 } from './gameData';
...
export default function SentenceIntroGameScreen({ onNext, onBack }: SentenceIntroGameScreenProps) {
  ...
  const wordHighlights = SENTENCE_SESSION4.words.map(() => useSharedValue(0));

  useEffect(() => {
    speakInstruction(SENTENCE_SESSION4.sentence).then(() => {
      ...
      const highlightNextWord = () => {
        if (wordIndex < SENTENCE_SESSION4.words.length) {
      ...
      <View style={styles.sentenceContainer}>
        <Animated.View style={[styles.sentenceBox, sentenceAnimatedStyle]} entering={FadeIn.delay(200)}>
          <Text style={styles.sentenceText}>
            {SENTENCE_SESSION4.words.map((word, index) => (
              <Animated.Text
                key={index}
                style={[
                  styles.sentenceWord,
              ...
              >
                {word}{index < SENTENCE_SESSION4.words.length - 1 ? ' ' : ''}
              </Animated.Text>
            ))}
          </Text>
        </Animated.View>
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.imageWrapper}>
            <Text style={styles.sunEmoji}>{SENTENCE_SESSION4.emoji}</Text>
          </View>
        </Animated.View>
      )}
