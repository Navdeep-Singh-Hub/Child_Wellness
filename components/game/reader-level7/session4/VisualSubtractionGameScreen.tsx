// Game 3: Visual Subtraction - 5 - 1 = 4
...
import { SUBTRACTION_5_MINUS_1 } from './gameData';
...
export default function VisualSubtractionGameScreen({ onComplete, onBack }: VisualSubtractionGameScreenProps) {
  ...
  const appleScales = SUBTRACTION_5_MINUS_1.objects.map(() => useSharedValue(1));
  ...
  const handleAnswerSelect = async (answer: number) => {
    ...
    if (isCorrect) {
      ...
      await speakFeedback('Perfect! 5 minus 1 equals 4!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.objectsContainer}>
        {SUBTRACTION_5_MINUS_1.objects.map((emoji, index) => (
          <Animated.View
            key={index}
            style={appleAnimatedStyles[index]}
            entering={FadeInDown.delay(200 + index * 100)}
          >
            <Pressable onPress={() => handleRemoveApple(index)} disabled={applesRemoved >= 1}>
      ...
      {showEquation && (
        <Animated.View entering={FadeInDown.delay(300)} style={styles.equationContainer}>
          <Text style={styles.equationText}>5 - 1 = ?</Text>
        </Animated.View>
      )}
