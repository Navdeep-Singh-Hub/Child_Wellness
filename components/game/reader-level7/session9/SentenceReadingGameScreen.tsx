// Game 1: Sentence Reading - Read "I see a bat" aloud
...
export default function SentenceReadingGameScreen({ onNext, onBack }: SentenceReadingGameScreenProps) {
  ...
  const wordHighlights = SENTENCE_SESSION9.words.map(() => useSharedValue(0));
  ...
  useEffect(() => {
    speakInstruction(SENTENCE_SESSION9.sentence).then(() => {
      ...
      const highlightNextWord = () => {
        if (wordIndex < SENTENCE_SESSION9.words.length) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Read Sentence</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.sentenceContainer}>
        <Animated.View style={[styles.sentenceBox, sentenceAnimatedStyle]} entering={FadeIn.delay(200)}>
          <Text style={styles.sentenceText}>
            {SENTENCE_SESSION9.words.map((word, index) => (
              <Animated.Text
                key={index}
                style={[
                  styles.sentenceWord,
              ...
              >
                {word}{index < SENTENCE_SESSION9.words.length - 1 ? ' ' : ''}
              </Animated.Text>
            ))}
          </Text>
        </Animated.View>
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.imageWrapper}>
            <Text style={styles.batEmoji}>{SENTENCE_SESSION9.emoji}</Text>
          </View>
        </Animated.View>
      )}
