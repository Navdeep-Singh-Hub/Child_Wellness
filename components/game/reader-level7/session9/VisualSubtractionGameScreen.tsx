// Game 3: Visual Subtraction - 9 - 4 = 5
...
import { SUBTRACTION_9_MINUS_4 } from './gameData';
...
export default function VisualSubtractionGameScreen({ onComplete, onBack }: VisualSubtractionGameScreenProps) {
  ...
  const starScales = SUBTRACTION_9_MINUS_4.objects.map(() => useSharedValue(1));

  useEffect(() => {
    speakInstruction('Take away four stars').catch(() => {});
    return () => stopAllAudio();
  }, []);

  const handleRemoveStar = (index: number) => {
    if (starsRemoved >= 4) return;
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Take away four stars</Text>
      </View>

      {/* Objects Display */}
      <View style={styles.objectsContainer}>
        {SUBTRACTION_9_MINUS_4.objects.map((emoji, index) => (
          <Animated.View
            key={index}
            style={starAnimatedStyles[index]}
            entering={FadeInDown.delay(200 + index * 100)}
          >
            <Pressable onPress={() => handleRemoveStar(index)} disabled={starsRemoved >= 4}>
      ...
      {showEquation && (
        <Animated.View entering={FadeInDown.delay(300)} style={styles.equationContainer}>
          <Text style={styles.equationText}>9 - 4 = 5</Text>
        </Animated.View>
      )}
