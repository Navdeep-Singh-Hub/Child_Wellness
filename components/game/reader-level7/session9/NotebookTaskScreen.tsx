...
import { SENTENCE_SESSION9, SUBTRACTION_9_MINUS_4 } from './gameData';
...
export default function NotebookTaskScreen({ onNext, onBack }: NotebookTaskScreenProps) {
  useEffect(() => {
    speakInstruction(
      `Write the sentence: ${SENTENCE_SESSION9.sentence}. Solve ${SUBTRACTION_9_MINUS_4.equation} equals ${SUBTRACTION_9_MINUS_4.answer}. Then draw ${SUBTRACTION_9_MINUS_4.answer} stars.`
    ).catch(() => {});
    return () => stopAllAudio();
  }, []);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(200)} style={styles.instructionCard}>
          ...
          <View style={styles.subtractionExample}>
            <Text style={styles.subtractionText}>
              {SUBTRACTION_9_MINUS_4.equation} = {SUBTRACTION_9_MINUS_4.answer}
            </Text>
          </View>
          <View style={styles.taskSection}>
            <Text style={styles.taskNumber}>3.</Text>
            <Text style={styles.taskText}>Draw {SUBTRACTION_9_MINUS_4.answer} stars</Text>
          </View>
          <View style={styles.exampleContainer}>
            <Text style={styles.exampleLabel}>Example:</Text>
            <View style={styles.starsExample}>
              {Array.from({ length: SUBTRACTION_9_MINUS_4.answer }).map((_, index) => (
        ...
        <Animated.View entering={FadeInDown.delay(400)} style={styles.tipsCard}>
          ...
          <Text style={styles.tipText}>• Make sure {SUBTRACTION_9_MINUS_4.equation} = {SUBTRACTION_9_MINUS_4.answer} is correct</Text>
          <Text style={styles.tipText}>• Draw {SUBTRACTION_9_MINUS_4.answer} clear stars</Text>
