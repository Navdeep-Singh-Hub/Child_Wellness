...
import { SENTENCE_SESSION3, SUBTRACTION_5_MINUS_2 } from './gameData';
...
export default function NotebookTaskScreen({ onNext, onBack }: NotebookTaskScreenProps) {
  useEffect(() => {
    speakInstruction(
      `Write the sentence: ${SENTENCE_SESSION3.sentence}. Solve ${SUBTRACTION_5_MINUS_2.equation} equals ${SUBTRACTION_5_MINUS_2.answer}. Then draw ${SUBTRACTION_5_MINUS_2.answer} stars.`
    ).catch(() => {});
    return () => stopAllAudio();
  }, []);
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(200)} style={styles.instructionCard}>
          ...
          <View style={styles.sentenceExample}>
            <Text style={styles.sentenceText}>{SENTENCE_SESSION3.sentence}</Text>
          </View>
          <View style={styles.taskSection}>
          ...
          <View style={styles.subtractionExample}>
            <Text style={styles.subtractionText}>
              {SUBTRACTION_5_MINUS_2.equation} = {SUBTRACTION_5_MINUS_2.answer}
            </Text>
          </View>
          <View style={styles.taskSection}>
            <Text style={styles.taskNumber}>3.</Text>
            <Text style={styles.taskText}>Draw {SUBTRACTION_5_MINUS_2.answer} stars</Text>
          </View>
          <View style={styles.exampleContainer}>
            <Text style={styles.exampleLabel}>Example:</Text>
            <View style={styles.starsExample}>
              {Array.from({ length: SUBTRACTION_5_MINUS_2.answer }).map((_, index) => (
        ...
        <Animated.View entering={FadeInDown.delay(400)} style={styles.tipsCard}>
          ...
          <Text style={styles.tipText}>• Make sure {SUBTRACTION_5_MINUS_2.equation} = {SUBTRACTION_5_MINUS_2.answer} is correct</Text>
          <Text style={styles.tipText}>• Draw {SUBTRACTION_5_MINUS_2.answer} clear stars</Text>
