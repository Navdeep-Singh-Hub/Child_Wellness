// Game 4: Drag Remove Objects - Drag away two apples
...
interface DragRemoveObjectsGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}

export default function DragRemoveObjectsGameScreen({ onComplete, onBack }: DragRemoveObjectsGameScreenProps) {
  const [applesInBasket, setApplesInBasket] = useState(5);
  ...
  useEffect(() => {
    speakInstruction('Drag away two apples').catch(() => {});
    return () => stopAllAudio();
  }, []);
  ...
  const handleAppleRemoved = async () => {
    ...
    setApplesInBasket(5 - newRemoved);
    ...
    if (newRemoved === 2) {
      ...
      await speakFeedback('Great! You removed two apples!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Remove Objects</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Drag away two apples</Text>
      </View>
