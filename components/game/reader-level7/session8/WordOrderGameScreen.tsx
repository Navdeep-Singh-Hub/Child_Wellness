// Game 2: Choose Correct Word Order - Tap the correct sentence
...
import { SENTENCE_OPTIONS_SESSION8 } from './gameData';

interface WordOrderGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}

export default function WordOrderGameScreen({ onComplete, onBack }: WordOrderGameScreenProps) {
  const [selectedSentence, setSelectedSentence] = useState<string | null>(null);
  const [completed, setCompleted] = useState(false);

  const buttonScales = SENTENCE_OPTIONS_SESSION8.map(() => useSharedValue(1));

  useEffect(() => {
    speakInstruction('Choose the correct sentence').catch(() => {});
    return () => stopAllAudio();
  }, []);
  ...
  const handleSentenceSelect = async (sentence: string) => {
    ...
    const index = SENTENCE_OPTIONS_SESSION8.findIndex(s => s.sentence === sentence);
    buttonScales[index].value = withSpring(0.9, {}, () => {
      buttonScales[index].value = withSpring(1);
    });

    setSelectedSentence(sentence);
    const option = SENTENCE_OPTIONS_SESSION8.find(s => s.sentence === sentence);
    const isCorrect = option?.correct || false;

    if (isCorrect) {
      await playSoundEffect('correct');
      await speakFeedback('Perfect! You chose the correct sentence!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Choose Correct Word Order</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Choose the correct sentence</Text>
      </View>

      {/* Sentence Options */}
      <View style={styles.sentencesContainer}>
        {SENTENCE_OPTIONS_SESSION8.map((option, index) => {
