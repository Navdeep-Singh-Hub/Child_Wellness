// Game 3: Visual Subtraction - 8 - 3 = 5
...
import { SUBTRACTION_8_MINUS_3 } from './gameData';
...
export default function VisualSubtractionGameScreen({ onComplete, onBack }: VisualSubtractionGameScreenProps) {
  ...
  const appleScales = SUBTRACTION_8_MINUS_3.objects.map(() => useSharedValue(1));

  useEffect(() => {
    speakInstruction('Take away three apples').catch(() => {});
    return () => stopAllAudio();
  }, []);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Take away three apples</Text>
      </View>

      {/* Objects Display */}
      <View style={styles.objectsContainer}>
        {SUBTRACTION_8_MINUS_3.objects.map((emoji, index) => (
          <Animated.View
            key={index}
            style={appleAnimatedStyles[index]}
            entering={FadeInDown.delay(200 + index * 100)}
          >
            <Pressable onPress={() => handleRemoveApple(index)} disabled={applesRemoved >= 3}>
      ...
      {showEquation && (
        <Animated.View entering={FadeInDown.delay(300)} style={styles.equationContainer}>
          <Text style={styles.equationText}>8 - 3 = 5</Text>
        </Animated.View>
      )}
