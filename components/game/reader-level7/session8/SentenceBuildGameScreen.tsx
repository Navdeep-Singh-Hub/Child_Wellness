// Game 1: Build Sentence - Drag words to build the sentence
...
import { CORRECT_ORDER_SESSION8, MIXED_WORDS_SESSION8 } from './gameData';
...
export default function SentenceBuildGameScreen({ onComplete, onBack }: SentenceBuildGameScreenProps) {
  ...
  useEffect(() => {
    speakInstruction('Build the sentence').catch(() => {});
    return () => stopAllAudio();
  }, []);

  const checkOrder = () => {
    const currentOrder = slots.filter(s => s !== null) as string[];
    const isCorrect = JSON.stringify(currentOrder) === JSON.stringify(CORRECT_ORDER_SESSION8);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Build the sentence</Text>
      </View>
      ...
      <View style={styles.wordsContainer}>
        {MIXED_WORDS_SESSION8.map((word, index) => {
