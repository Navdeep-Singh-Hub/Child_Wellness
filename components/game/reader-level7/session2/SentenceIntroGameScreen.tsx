// Game 1: Sentence Introduction - Read "I see a dog"
...
import { SENTENCE_SESSION2 } from './gameData';
...
export default function SentenceIntroGameScreen({ onNext, onBack }: SentenceIntroGameScreenProps) {
  ...
  const wordHighlights = SENTENCE_SESSION2.words.map(() => useSharedValue(0));

  useEffect(() => {
    speakInstruction(SENTENCE_SESSION2.sentence).then(() => {
      ...
      const highlightNextWord = () => {
        if (wordIndex < SENTENCE_SESSION2.words.length) {
      ...
      <View style={styles.sentenceContainer}>
        <Animated.View style={[styles.sentenceBox, sentenceAnimatedStyle]} entering={FadeIn.delay(200)}>
          <Text style={styles.sentenceText}>
            {SENTENCE_SESSION2.words.map((word, index) => (
              <Animated.Text
                key={index}
                style={[
                  styles.sentenceWord,
              ...
              >
                {word}{index < SENTENCE_SESSION2.words.length - 1 ? ' ' : ''}
              </Animated.Text>
            ))}
          </Text>
        </Animated.View>
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.imageWrapper}>
            <Text style={styles.dogEmoji}>{SENTENCE_SESSION2.emoji}</Text>
          </View>
        </Animated.View>
      )}
