// Game 2: Word to Picture Match - Tap the word that matches the picture
...
import { playSoundEffect, speakInstruction, speakFeedback, speakWord, stopAllAudio } from '../../reader/utils/audio';
import { WORD_OPTIONS_SESSION2 } from './gameData';

interface WordPictureMatchGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}

const CORRECT_WORD = 'dog';

export default function WordPictureMatchGameScreen({ onComplete, onBack }: WordPictureMatchGameScreenProps) {
  ...
  useEffect(() => {
    speakInstruction('Tap the word that matches the picture').then(() => {
      speakWord('dog');
    });
    return () => {
      stopAllAudio();
  ...
  const handleWordSelect = async (word: string) => {
    ...
    if (correct) {
      ...
      pictureScale.value = withSpring(1.2, {}, () => {
        pictureScale.value = withSpring(1);
      });
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Word to Picture Match</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the word that matches the picture</Text>
      </View>

      {/* Picture Display */}
      <View style={styles.pictureContainer}>
        <Animated.View style={pictureAnimatedStyle} entering={FadeInDown.delay(200)}>
      ...
      <View style={styles.wordsContainer}>
        {WORD_OPTIONS_SESSION2.map((option, index) => {
          ...
          return (
            <Animated.View key={option.word} entering={FadeInDown.delay(400 + index * 100)} style={buttonAnimatedStyle}>
              <Pressable
