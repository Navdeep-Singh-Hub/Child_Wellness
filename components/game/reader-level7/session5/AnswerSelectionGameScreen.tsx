// Game 4: Choose Correct Answer - 6 - 2 = ?
...
import { ANSWER_OPTIONS_SESSION5, CORRECT_ANSWER } from './gameData';
...
export default function AnswerSelectionGameScreen({ onComplete, onBack }: AnswerSelectionGameScreenProps) {
  ...
  const buttonScales = ANSWER_OPTIONS_SESSION5.map(() => useSharedValue(1));
  ...
  const handleAnswerSelect = async (answer: number) => {
    ...
    const index = ANSWER_OPTIONS_SESSION5.indexOf(answer);
    ...
    if (isCorrect) {
      ...
      await speakFeedback('Perfect! 6 minus 2 equals 4!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Choose Answer</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Question */}
      <View style={styles.questionContainer}>
        <Text style={styles.questionText}>6 - 2 = ?</Text>
      </View>

      {/* Answer Options */}
      <View style={styles.answersContainer}>
        {ANSWER_OPTIONS_SESSION5.map((answer, index) => {
