// Game 3: Visual Subtraction - 6 - 2 = 4
...
import { SUBTRACTION_6_MINUS_2 } from './gameData';
...
export default function VisualSubtractionGameScreen({ onComplete, onBack }: VisualSubtractionGameScreenProps) {
  ...
  const appleScales = SUBTRACTION_6_MINUS_2.objects.map(() => useSharedValue(1));
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.objectsContainer}>
        {SUBTRACTION_6_MINUS_2.objects.map((emoji, index) => (
          <Animated.View
            key={index}
      ...
      {showEquation && (
        <Animated.View entering={FadeInDown.delay(300)} style={styles.equationContainer}>
          <Text style={styles.equationText}>6 - 2 = 4</Text>
        </Animated.View>
      )}
