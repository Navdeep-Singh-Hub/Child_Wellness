...
import { SENTENCE_SESSION5, SUBTRACTION_6_MINUS_2 } from './gameData';
...
export default function NotebookTaskScreen({ onNext, onBack }: NotebookTaskScreenProps) {
  useEffect(() => {
    speakInstruction(
      `Write the sentence: ${SENTENCE_SESSION5.sentence}. Solve ${SUBTRACTION_6_MINUS_2.equation} equals ${SUBTRACTION_6_MINUS_2.answer}. Then draw ${SUBTRACTION_6_MINUS_2.answer} apples.`
    ).catch(() => {});
    return () => stopAllAudio();
  }, []);
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(200)} style={styles.instructionCard}>
          ...
          <View style={styles.subtractionExample}>
            <Text style={styles.subtractionText}>
              {SUBTRACTION_6_MINUS_2.equation} = {SUBTRACTION_6_MINUS_2.answer}
            </Text>
          </View>
          <View style={styles.taskSection}>
            <Text style={styles.taskNumber}>3.</Text>
            <Text style={styles.taskText}>Draw {SUBTRACTION_6_MINUS_2.answer} apples</Text>
          </View>
          <View style={styles.exampleContainer}>
            <Text style={styles.exampleLabel}>Example:</Text>
            <View style={styles.applesExample}>
              {Array.from({ length: SUBTRACTION_6_MINUS_2.answer }).map((_, index) => (
        ...
        <Animated.View entering={FadeInDown.delay(400)} style={styles.tipsCard}>
          ...
          <Text style={styles.tipText}>• Make sure {SUBTRACTION_6_MINUS_2.equation} = {SUBTRACTION_6_MINUS_2.answer} is correct</Text>
          <Text style={styles.tipText}>• Draw {SUBTRACTION_6_MINUS_2.answer} clear apples</Text>
