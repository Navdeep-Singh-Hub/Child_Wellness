// Game 1: Sentence Introduction - Read "I see a hat"
...
import { SENTENCE_SESSION5 } from './gameData';
...
export default function SentenceIntroGameScreen({ onNext, onBack }: SentenceIntroGameScreenProps) {
  ...
  const wordHighlights = SENTENCE_SESSION5.words.map(() => useSharedValue(0));

  useEffect(() => {
    speakInstruction(SENTENCE_SESSION5.sentence).then(() => {
      ...
      const highlightNextWord = () => {
        if (wordIndex < SENTENCE_SESSION5.words.length) {
      ...
      <View style={styles.sentenceContainer}>
        <Animated.View style={[styles.sentenceBox, sentenceAnimatedStyle]} entering={FadeIn.delay(200)}>
          <Text style={styles.sentenceText}>
            {SENTENCE_SESSION5.words.map((word, index) => (
              <Animated.Text
                key={index}
                style={[
                  styles.sentenceWord,
              ...
              >
                {word}{index < SENTENCE_SESSION5.words.length - 1 ? ' ' : ''}
              </Animated.Text>
            ))}
          </Text>
        </Animated.View>
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.imageWrapper}>
            <Text style={styles.hatEmoji}>{SENTENCE_SESSION5.emoji}</Text>
          </View>
        </Animated.View>
      )}
