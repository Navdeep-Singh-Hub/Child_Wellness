// Game 1: Sentence Reading - Read "I see a dog" aloud
...
import { SENTENCE_SESSION7 } from './gameData';
...
export default function SentenceReadingGameScreen({ onNext, onBack }: SentenceReadingGameScreenProps) {
  ...
  const wordHighlights = SENTENCE_SESSION7.words.map(() => useSharedValue(0));

  useEffect(() => {
    speakInstruction(SENTENCE_SESSION7.sentence).then(() => {
      ...
      const highlightNextWord = () => {
        if (wordIndex < SENTENCE_SESSION7.words.length) {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Read Sentence Aloud</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.sentenceContainer}>
        <Animated.View style={[styles.sentenceBox, sentenceAnimatedStyle]} entering={FadeIn.delay(200)}>
          <Text style={styles.sentenceText}>
            {SENTENCE_SESSION7.words.map((word, index) => (
              <Animated.Text
                key={index}
                style={[
                  styles.sentenceWord,
              ...
              >
                {word}{index < SENTENCE_SESSION7.words.length - 1 ? ' ' : ''}
              </Animated.Text>
            ))}
          </Text>
        </Animated.View>
      ...
      {showImage && (
        <Animated.View style={[styles.imageContainer, imageAnimatedStyle]} entering={FadeInDown.delay(400)}>
          <View style={styles.imageWrapper}>
            <Text style={styles.dogEmoji}>{SENTENCE_SESSION7.emoji}</Text>
          </View>
        </Animated.View>
      )}
