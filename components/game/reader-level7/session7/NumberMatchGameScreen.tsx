// Game 4: Match Number Answer - 7 - 2 = ?
...
import { CORRECT_NUMBER_ANSWER, NUMBER_OPTIONS_SESSION7 } from './gameData';
...
export default function NumberMatchGameScreen({ onComplete, onBack }: NumberMatchGameScreenProps) {
  ...
  const buttonScales = NUMBER_OPTIONS_SESSION7.map(() => useSharedValue(1));

  useEffect(() => {
    speakInstruction('Choose the correct number').catch(() => {});
    return () => stopAllAudio();
  }, []);
  ...
  const handleAnswerSelect = async (answer: number) => {
    ...
    const index = NUMBER_OPTIONS_SESSION7.indexOf(answer);
    buttonScales[index].value = withSpring(0.9, {}, () => {
      buttonScales[index].value = withSpring(1);
    });

    setSelectedAnswer(answer);
    const isCorrect = answer === CORRECT_NUMBER_ANSWER;

    if (isCorrect) {
      await playSoundEffect('correct');
      await speakFeedback('Perfect! 7 minus 2 equals 5!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Match Number Answer</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Question */}
      <View style={styles.questionContainer}>
        <Text style={styles.questionText}>7 - 2 = ?</Text>
      </View>

      {/* Number Options */}
      <View style={styles.numbersContainer}>
        {NUMBER_OPTIONS_SESSION7.map((number, index) => {
          const isSelected = selectedAnswer === number;
          const isCorrect = number === CORRECT_NUMBER_ANSWER;
          ...
          return (
            ...
            >
              <Pressable
                onPress={() => handleAnswerSelect(number)}
                disabled={completed}
