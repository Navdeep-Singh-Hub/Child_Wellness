// Result Screen - Show detection results for Level 7 Session 7
...
export default function ResultScreen({ result, onRetry, onComplete }: ResultScreenProps) {
  ...
  useEffect(() => {
    ...
    if (allDetected) {
      speakFeedback('Great job! You practiced reading and subtraction!').catch(() => {});
    } else {
        ...
        <Animated.View entering={FadeInDown.delay(600)} style={styles.resultsContainer}>
          ...
          <View style={styles.resultItem}>
            ...
            <Text style={styles.resultText}>
              Sentence "I see a dog" {result.sentenceDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
          ...
          <View style={styles.resultItem}>
            ...
            />
            <Text style={styles.resultText}>
              Subtraction 7-2=5 {result.subtractionCorrect ? 'correct ✓' : 'not found or incorrect'}
            </Text>
          </View>

          <View style={styles.resultItem}>
            <Ionicons
              name={result.fiveStarsDetected ? 'checkmark-circle' : 'close-circle'}
              size={32}
              color={result.fiveStarsDetected ? '#10B981' : '#EF4444'}
            />
            <Text style={styles.resultText}>
              Five stars {result.fiveStarsDetected ? 'found ✓' : 'not found'}
            </Text>
          </View>
        </Animated.View>
          ...
          {allDetected ? (
            <>
              ...
              <Text style={styles.messageText}>
                You practiced reading and subtraction! Great work!
              </Text>
            </>
          ) : (
            <>
              ...
              <Text style={styles.messageText}>
                {!result.sentenceDetected && 'We couldn\'t find the sentence "I see a dog". '}
                {!result.subtractionCorrect && 'We couldn\'t find 7-2=5 solved correctly. '}
                {!result.fiveStarsDetected && 'We couldn\'t find five stars. '}
