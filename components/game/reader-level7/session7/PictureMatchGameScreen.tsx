// Game 2: Find Correct Picture - Tap the correct picture
...
import { PICTURE_OPTIONS_SESSION7, SENTENCE_SESSION7 } from './gameData';
...
export default function PictureMatchGameScreen({ onComplete, onBack }: PictureMatchGameScreenProps) {
  ...
  const buttonScales = PICTURE_OPTIONS_SESSION7.map(() => useSharedValue(1));

  useEffect(() => {
    speakInstruction('Tap the correct picture').catch(() => {});
    return () => stopAllAudio();
  }, []);
  ...
  const handlePictureSelect = async (label: string) => {
    ...
    const index = PICTURE_OPTIONS_SESSION7.findIndex(p => p.label === label);
    buttonScales[index].value = withSpring(0.9, {}, () => {
      buttonScales[index].value = withSpring(1);
    });

    setSelectedPicture(label);
    const option = PICTURE_OPTIONS_SESSION7.find(p => p.label === label);
    const isCorrect = option?.correct || false;

    if (isCorrect) {
      await playSoundEffect('correct');
      await speakFeedback('Perfect! You found the correct picture!');
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Find Correct Picture</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the correct picture</Text>
      </View>

      {/* Picture Options */}
      <View style={styles.picturesContainer}>
        {PICTURE_OPTIONS_SESSION7.map((option, index) => {
