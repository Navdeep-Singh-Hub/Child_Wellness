// Notebook Task Screen - Write sentence and solve subtraction, draw apples
...
import { SENTENCE_SESSION1, SUBTRACTION_3_MINUS_1 } from './gameData';
...
export default function NotebookTaskScreen({ onNext, onBack }: NotebookTaskScreenProps) {
  useEffect(() => {
    speakInstruction(
      `Write the sentence: ${SENTENCE_SESSION1.sentence}. Solve ${SUBTRACTION_3_MINUS_1.equation} equals ${SUBTRACTION_3_MINUS_1.answer}. Then draw ${SUBTRACTION_3_MINUS_1.answer} apples.`
    ).catch(() => {});
    return () => stopAllAudio();
  }, []);
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Animated.View entering={FadeInDown.delay(200)} style={styles.instructionCard}>
          ...
          <View style={styles.sentenceExample}>
            <Text style={styles.sentenceText}>{SENTENCE_SESSION1.sentence}</Text>
          </View>
          <View style={styles.taskSection}>
          ...
          <View style={styles.subtractionExample}>
            <Text style={styles.subtractionText}>
              {SUBTRACTION_3_MINUS_1.equation} = {SUBTRACTION_3_MINUS_1.answer}
            </Text>
          </View>
          <View style={styles.taskSection}>
            <Text style={styles.taskNumber}>3.</Text>
            <Text style={styles.taskText}>Draw {SUBTRACTION_3_MINUS_1.answer} apples</Text>
          </View>
          <View style={styles.exampleContainer}>
            <Text style={styles.exampleLabel}>Example:</Text>
            <View style={styles.applesExample}>
              {Array.from({ length: SUBTRACTION_3_MINUS_1.answer }).map((_, index) => (
        ...
        <Animated.View entering={FadeInDown.delay(400)} style={styles.tipsCard}>
          ...
          <Text style={styles.tipText}>• Make sure {SUBTRACTION_3_MINUS_1.equation} = {SUBTRACTION_3_MINUS_1.answer} is correct</Text>
          <Text style={styles.tipText}>• Draw {SUBTRACTION_3_MINUS_1.answer} clear apples</Text>
