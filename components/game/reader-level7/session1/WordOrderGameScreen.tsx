// Game 2: Word Order - Arrange words to make a sentence
...
import { SENTENCE_SESSION1 } from './gameData';
...
const CORRECT_ORDER = ['I', 'see', 'a', 'cat'];
const MIXED_WORDS = ['cat', 'see', 'I', 'a'];
...
export default function WordOrderGameScreen({ onComplete, onBack }: WordOrderGameScreenProps) {
  ...
  useEffect(() => {
    speakInstruction('Arrange the words to make a sentence').catch(() => {});
    return () => stopAllAudio();
  }, []);

  const checkOrder = () => {
    const currentOrder = slots.filter(s => s !== null) as string[];
    const isCorrect = JSON.stringify(currentOrder) === JSON.stringify(CORRECT_ORDER);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Word Order</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Arrange the words to make a sentence</Text>
      </View>
