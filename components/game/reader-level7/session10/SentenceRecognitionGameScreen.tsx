// Game 1: Sentence Recognition - Tap the sentence you hear
...
import { SENTENCE_OPTIONS_SESSION10 } from './gameData';

interface SentenceRecognitionGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}

export default function SentenceRecognitionGameScreen({ onComplete, onBack }: SentenceRecognitionGameScreenProps) {
  const [selectedSentence, setSelectedSentence] = useState<string | null>(null);
  const [completed, setCompleted] = useState(false);

  const correctSentence = SENTENCE_OPTIONS_SESSION10.find(s => s.correct)?.sentence || '';
  const buttonScales = SENTENCE_OPTIONS_SESSION10.map(() => useSharedValue(1));

  useEffect(() => {
    const speakSentence = async () => {
      await speakInstruction('Tap the sentence you hear');
      await new Promise(resolve => setTimeout(resolve, 1000));
      await speakInstruction(correctSentence);
    };
    speakSentence().catch(() => {});
    return () => stopAllAudio();
  }, []);
  ...
  const handleSentenceSelect = async (sentence: string) => {
    ...
    const index = SENTENCE_OPTIONS_SESSION10.findIndex(s => s.sentence === sentence);
    buttonScales[index].value = withSpring(0.9, {}, () => {
      buttonScales[index].value = withSpring(1);
    });

    setSelectedSentence(sentence);
    const option = SENTENCE_OPTIONS_SESSION10.find(s => s.sentence === sentence);
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Sentence Recognition</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Tap the sentence you hear</Text>
      </View>

      {/* Sentence Options */}
      <View style={styles.sentencesContainer}>
        {SENTENCE_OPTIONS_SESSION10.map((option, index) => {
