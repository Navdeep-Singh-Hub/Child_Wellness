// Game 2: Build Sentence - Drag words to build the sentence
...
import { CORRECT_ORDER_SESSION10, MIXED_WORDS_SESSION10 } from './gameData';

interface SentenceBuildGameScreenProps {
  onComplete: () => void;
  onBack: () => void;
}

export default function SentenceBuildGameScreen({ onComplete, onBack }: SentenceBuildGameScreenProps) {
  ...
  useEffect(() => {
    speakInstruction('Arrange the words to build the sentence').catch(() => {});
    return () => stopAllAudio();
  }, []);

  const checkOrder = () => {
    const currentOrder = slots.filter(s => s !== null) as string[];
    const isCorrect = JSON.stringify(currentOrder) === JSON.stringify(CORRECT_ORDER_SESSION10);
  ...
  const handleCorrect = async () => {
    ...
    await speakFeedback('Perfect! You built the sentence!');
    setCompleted(true);
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Build Sentence</Text>
        <View style={{ width: 40 }} />
      </View>

      {/* Instruction */}
      <View style={styles.instructionContainer}>
        <Text style={styles.instructionText}>Arrange the words to build the sentence</Text>
      </View>
      ...
      <View style={styles.wordsContainer}>
        {MIXED_WORDS_SESSION10.map((word, index) => {
