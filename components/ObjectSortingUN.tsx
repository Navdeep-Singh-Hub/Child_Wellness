/**
 * Sort the Objects — Outdoor (sun, run) vs Food (bun, apple). Game 4 for Grouper Session 3.
 */
...
const ITEMS: { id: string; label: string; category: Category; image: string }[] = [
  { id: 'sun', label: 'sun', category: 'outdoor', image: 'https://placehold.co/100x100/FEF9C3/854D0E?text=sun' },
  { id: 'run', label: 'run', category: 'outdoor', image: 'https://placehold.co/100x100/DBEAFE/1E40AF?text=run' },
  { id: 'bun', label: 'bun', category: 'food', image: 'https://placehold.co/100x100/FED7AA/9A3412?text=bun' },
  { id: 'apple', label: 'apple', category: 'food', image: 'https://placehold.co/100x100/FEE2E2/991B1B?text=apple' },
];

export function ObjectSortingUN({ onComplete }: { onComplete: () => void }) {
  ...
  const [currentItem, setCurrentItem] = useState<string | null>('sun');
