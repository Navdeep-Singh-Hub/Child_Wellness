/**
 * UN Word Session 3 — Level 4 (Grouper) Session 3: The -UN Word Family
 * Flow: Intro → Game 1 (Word Family Finder) → Game 2 (Rhyme Match) → Game 3 (Clap Word) → Game 4 (Object Sort) → Notebook → Result
 * AAC-friendly: minimal, predictable, sensory-friendly. Colors: Primary #4F46E5, Success #22C55E, Background #F3F4F6.
 */
...
import { WordFamilyFinderUN } from '@/components/WordFamilyFinderUN';
import { RhymeMatchUN } from '@/components/RhymeMatchUN';
import { ClapWordUN } from '@/components/ClapWordUN';
import { ObjectSortingUN } from '@/components/ObjectSortingUN';
import { UNWordNotebookUpload } from '@/components/UNWordNotebookUpload';
...
interface UNWordSession3Props {
  onExit?: () => void;
}

export function UNWordSession3({ onExit }: UNWordSession3Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔍', title: 'Find the -UN words', desc: 'Word family' },
      { icon: '🎵', title: 'Find the rhyme', desc: 'Rhyming' },
      { icon: '👏', title: 'Clap the word', desc: 'Syllables' },
      { icon: '📦', title: 'Sort the items', desc: 'Classification' },
    ];
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>The -UN Family</Text>
            <Text style={styles.introSub}>
              Let's learn words that end with -un.
            </Text>
          </View>
          <View style={styles.exampleBox}>
            <Text style={styles.exampleTitle}>Example words:</Text>
            <View style={styles.exampleWordsRow}>
              <Text style={styles.exampleWord}>sun</Text>
              <Text style={styles.exampleWord}>bun</Text>
              <Text style={styles.exampleWord}>run</Text>
              <Text style={styles.exampleWord}>fun</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning the -UN family!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>UN Family Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        <Text style={styles.title}>The -UN Family</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <WordFamilyFinderUN onComplete={advance} />}
        {step === 2 && <RhymeMatchUN onComplete={advance} />}
        {step === 3 && <ClapWordUN onComplete={advance} />}
        {step === 4 && <ObjectSortingUN onComplete={advance} />}
        {step === 5 && <UNWordNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
