/**
 * IN Word Notebook — Write 3 -IN words and draw them. Photo upload + AI check. Grouper Session 2.
 */
...
const INSTRUCTIONS = 'Write 3 -IN words and draw them. Examples: pin, tin, bin.';
...
export function INWordNotebookUpload({
  ...
  const [result, setResult] = useState<{
    correct: boolean;
    feedback: string;
    in_words_present?: boolean;
    three_words_present?: boolean;
    drawings_present?: boolean;
  } | null>(null);
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      setResult({
        correct,
        feedback,
        in_words_present: data.in_words_present,
        three_words_present: data.three_words_present,
        drawings_present: data.drawings_present,
      });
      if (correct) {
        speak('Great job!');
  ...
  return (
    <GameLayout
      ...
      instruction="Write 3 -IN words and draw them. Then upload or take a photo."
    >
