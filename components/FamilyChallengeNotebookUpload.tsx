/**
 * Family Challenge Notebook — Write 5 rhyming words (e.g. cat, hat, bat, rat, mat). Grouper Session 9.
 * AI checks: rhyming_words_present, five_words_present.
 */
...
const INSTRUCTIONS = 'Write 5 rhyming words. Example: cat, hat, bat, rat, mat.';
...
export function FamilyChallengeNotebookUpload({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const fivePresent = data.five_words_present ?? true;
      const correct = data.correct ?? (rhymingPresent && fivePresent);
      const feedback = data.feedback || (correct ? 'Great job!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        rhyming_words_present: data.rhyming_words_present,
        five_words_present: data.five_words_present,
      });
      if (correct) {
        speak('Great job!');
  ...
  return (
    <GameLayout
      ...
      instruction="Write 5 rhyming words. Then upload or take a photo."
    >
      <View style={styles.content}>
        <Text style={styles.instructions}>{INSTRUCTIONS}</Text>
        <View style={styles.exampleRow}>
          <Text style={styles.exampleWords}>cat · hat · bat · rat · mat</Text>
        </View>
