/**
 * Game 5: Alphabet Passport — trace A–Z on paper and upload.
 * POST /api/verify-az-tracing
 */
...
export function AlphabetUploadTask({
  currentStep,
  totalSteps,
  onBack,
  onComplete,
  ...
  useEffect(() => {
    ...
    if (!spokeIntro.current) {
      ...
      speak('Trace letters A to Z on paper using dotted guides, then photograph your page for the passport!', 0.72);
    }
    return () => stopTTS();
  }, []);
  ...
  const mascotHint =
    phase === 1
      ? 'Trace A through Z on paper with dotted guides!'
      : phase === 2
        ...
        : result?.success
          ...
          : result
            ? result.feedback
            : 'The ranger is reviewing your page…';
    ...
    async (uri: string) => {
      ...
      try {
        ...
        if (Platform.OS === 'web' && (uri.startsWith('blob:') || uri.startsWith('data:'))) {
          ...
          fd.append('file', blob, 'alphabet-tracing.jpg');
        } else {
          fd.append('file', { uri, name: 'alphabet-tracing.jpg', type: 'image/jpeg' } as any);
        }
        const h = await authHeaders({ multipart: true });
        delete (h as Record<string, string>)['Content-Type'];
        const res = await fetch(`${API_BASE_URL}/api/verify-az-tracing`, { method: 'POST', headers: h, body: fd });
        if (!res.ok) {
        ...
        setResult({
          success: ok,
          message: ok ? 'Stamped!' : 'Try Again',
          feedback: data.feedback || (ok ? 'Great A–Z tracing!' : 'Try again with clearer letters.'),
        });
        if (ok) {
          setShowCelebrate(true);
          speak('Your alphabet passport is complete! Expedition success!', 0.72);
  ...
  const handleReady = () => {
    setPhase(2);
    speak('Now photograph your A to Z tracing page!', 0.72);
  ...
  const handleRetry = () => {
    ...
    setImageUri(null);
    setPhase(1);
    speak('Let us try again! Trace A to Z on paper first.', 0.72);
  };

  const stepDots = Array.from({ length: totalSteps }, (_, i) => i + 1);
  const phases = [
    { n: 1, label: 'Trace A–Z', icon: '✏️' },
    { n: 2, label: 'Photograph', icon: '📷' },
    { n: 3, label: 'Get stamp', icon: '📜' },
  ];
      ...
      <View style={styles.root}>
        ...
        <View style={styles.celebrateOverlay}>
          ...
          <View style={styles.celebrateCard}>
            ...
            <Text style={styles.celebrateTitle}>Passport Stamped!</Text>
            <Text style={styles.celebrateSub}>Your A–Z expedition is official</Text>
          </View>
        </View>
      </View>
    );
      ...
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            <Text style={styles.gameLabel}>ALPHABET PASSPORT</Text>
            <Text style={styles.gameTitle}>A–Z Paper Entry</Text>
          </View>
        </View>
        ...
        {phase === 1 ? (
          <View style={styles.paperCard}>
            <View style={styles.paperFrame}>
              ...
              <Text style={styles.paperTitle}>Full Alphabet Page</Text>
              <Text style={styles.paperDesc}>
                Trace every letter from A to Z on paper using dotted guides, then get your passport stamped!
              </Text>
            </View>
            <Pressable onPress={handleReady} style={({ pressed }) => [styles.primaryBtn, pressed && styles.pressed]}>
              ...
              <Text style={styles.primaryBtnText}>I traced A–Z!</Text>
            </Pressable>
          </View>
        ) : (
          <View style={styles.uploadArea}>
            {imageUri ? (
              <View style={styles.frameOuter}>
                ...
                <Text style={styles.frameCaption}>Passport Page</Text>
              </View>
            ) : (
            ...
            {uploading ? (
              <View style={styles.loadingRow}>
                ...
                <Text style={styles.loadingText}>Ranger is reviewing…</Text>
              </View>
            ) : (
...
const styles = StyleSheet.create({
  ...
  phaseDotActive: { backgroundColor: 'rgba(251,191,36,0.2)', borderColor: '#FBBF24' },
