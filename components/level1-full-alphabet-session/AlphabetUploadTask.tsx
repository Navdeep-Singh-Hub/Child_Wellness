/**
 * AI Task: Trace A to Z using dots on paper and upload photo.
...
const INSTRUCTION = 'Trace letters A to Z on paper using dotted guides, then upload a photo.';
...
export function AlphabetUploadTask({
  ...
  const upload = async (uri: string) => {
    ...
    try {
      ...
      setResult({ success: ok, message: data.message || (ok ? 'SUCCESS' : 'TRY AGAIN'), feedback: data.feedback || (ok ? 'Great tracing!' : 'Try again.') });
  ...
  return (
    <GameContainerGrip title="A–Z Task" currentStep={currentStep} totalSteps={totalSteps} mascot="📷" mascotHint={INSTRUCTION} onBack={onBack}>
...
const styles = StyleSheet.create({
  content: { flex: 1 },
  instruction: { fontSize: 18, fontWeight: '700', color: '#5B21B6', textAlign: 'center', marginBottom: 20, lineHeight: 26 },
  ...
  btnPri: { backgroundColor: '#5B21B6' },
  btnSec: { backgroundColor: '#EDE9FE' },
  btnText: { fontSize: 18, fontWeight: '800', color: '#FFF' },
  btnTextSec: { color: '#5B21B6' },
  pressed: { opacity: 0.9 },
