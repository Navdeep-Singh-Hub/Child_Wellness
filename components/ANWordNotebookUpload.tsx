/**
 * AN Word Notebook — Write 3 -AN words (fan, man, pan). Grouper Session 6.
 * AI checks: an_words_present, three_words_present.
 */
...
const INSTRUCTIONS = 'Write 3 -AN words. Example: fan, man, pan.';
...
export function ANWordNotebookUpload({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const anPresent = data.an_words_present ?? true;
      const threeWords = data.three_words_present ?? true;
      const correct = data.correct ?? (anPresent && threeWords);
      const feedback = data.feedback || (correct ? 'Great job!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        an_words_present: data.an_words_present,
  ...
  return (
    <GameLayout
      ...
      instruction="Write 3 -AN words. Then upload or take a photo."
    >
      <View style={styles.content}>
        <Text style={styles.instructions}>{INSTRUCTIONS}</Text>
        <View style={styles.exampleRow}>
          <Text style={styles.exampleWords}>fan · man · pan</Text>
        </View>
