/**
 * Level 4 Grouper — Session 6: The -AN Word Family
 * Hub → 4 games + terracotta journal task → completion
 */
import { ConfettiEffect } from '@/components/games/Level1/ConfettiEffect';
import { SuccessCelebration } from '@/components/ui/SuccessCelebration';
import { markSessionGameComplete } from '@/components/special-education/shared/useSessionIntroProgress';
import { WordFamilyFinderGame } from '@/components/grouper-session/session6/WordFamilyFinderGame';
import { RhymeMatchGame } from '@/components/grouper-session/session6/RhymeMatchGame';
import { ClapWordGame } from '@/components/grouper-session/session6/ClapWordGame';
import { ObjectSortingGame } from '@/components/grouper-session/session6/ObjectSortingGame';
import { ANWordNotebookUpload } from '@/components/grouper-session/session6/ANWordNotebookUpload';
import {
  GROUPER_S6_HUB_THEME as H,
  GROUPER_S6_QUESTS,
  GROUPER_SESSION,
} from '@/components/grouper-session/grouperSessionTheme';
import { TerracottaTrailBackground } from '@/components/grouper-session/TerracottaTrailBackground';
...
const PLAY_STEPS = 5;
const SESSION_TITLE = 'Session 6 · The -AN Family';
        ...
        <SafeAreaView style={styles.hubSafe}>
          ...
          <ScrollView contentContainerStyle={styles.hubScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.hubHeader}>
              <Text style={styles.hubEyebrow}>Grouper · Session 6 · Terracotta Trail</Text>
              <Text style={styles.hubTitle}>{H.name}</Text>
              <Text style={styles.hubSession}>The -AN Word Family</Text>

              <View style={styles.exampleChip}>
                <Text style={styles.exampleText}>fan · man · pan · can</Text>
              </View>
              ...
              <View style={styles.speechBubble}>
                ...
                <View style={styles.bubbleBody}>
                  ...
                  <Text style={styles.hubPrompt}>
                    Along the terracotta trail, -AN words line up together — scout them, echo rhymes, clap, sort, then fill your journal!
                  </Text>
                </View>
              </View>
            </View>

            {GROUPER_S6_QUESTS.map((quest) => {
  ...
  if (step === 6) {
    return (
      <View style={styles.root}>
        ...
        <ConfettiEffect />
        <SuccessCelebration
          title="Great job learning the -AN family!"
          subtitle={`AN Family Star 🥫\n${SESSION_TITLE}\nCompleted ${doneCount}/${PLAY_STEPS} builds${
            taskCorrect === false ? '\nTerracotta Journal: try again next time!' : ''
          }`}
          badgeEmoji="⭐"
          variant="sunset"
        />
