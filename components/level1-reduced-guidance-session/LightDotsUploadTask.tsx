/**
 * AI Task: Write A to Z using light dots on paper and upload photo.
...
const INSTRUCTION = 'Write A to Z using light dots on paper, then upload a photo.';
...
export function LightDotsUploadTask({
  ...
  const upload = async (uri: string) => {
    ...
    try {
      ...
      setResult({ success: ok, message: data.message || (ok ? 'SUCCESS' : 'TRY AGAIN'), feedback: data.feedback || (ok ? 'Great work!' : 'Try again.') });
  ...
  return (
    <GameContainerGrip title="A–Z Light Dots" currentStep={currentStep} totalSteps={totalSteps} mascot="📷" mascotHint={INSTRUCTION} onBack={onBack}>
...
const styles = StyleSheet.create({
  content: { flex: 1 },
  instruction: { fontSize: 18, fontWeight: '700', color: '#374151', textAlign: 'center', marginBottom: 20, lineHeight: 26 },
  ...
  btnPri: { backgroundColor: '#374151' },
  btnSec: { backgroundColor: '#F3F4F6' },
  btnText: { fontSize: 18, fontWeight: '800', color: '#FFF' },
  btnTextSec: { color: '#374151' },
  pressed: { opacity: 0.9 },
  tryAgain: { marginTop: 12, alignSelf: 'center' },
  tryAgainText: { fontSize: 16, fontWeight: '700', color: '#374151' },
});
