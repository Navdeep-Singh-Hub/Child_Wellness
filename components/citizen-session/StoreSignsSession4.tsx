 ...
 * Flow: Intro → Game 1 (Store Sign Recognition) → Game 2 (Sign Meaning) → Game 3 (Price Tag) → Game 4 (Buy Toy) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Store Signs</Text>
            <Text style={styles.introSub}>Let's learn signs we see in stores.</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '🏪', title: 'Tap the correct sign', desc: 'OPEN, CLOSED, SALE' },
              { icon: '📋', title: 'Match sign with meaning', desc: 'discount, store is open' },
              { icon: '🏷️', title: 'Find the price', desc: '₹10, ₹20, ₹50' },
              { icon: '🧸', title: 'Buy the toy', desc: 'Choose the right coin' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning store signs!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Store Signs Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: CITIZEN_COLOR }]}>
        <Text style={[styles.title, { color: '#BE185D' }]}>Store Signs</Text>
        <Text style={[styles.subtitle, { color: CITIZEN_COLOR }]}>The Citizen · Session 4</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <StoreSignsNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
