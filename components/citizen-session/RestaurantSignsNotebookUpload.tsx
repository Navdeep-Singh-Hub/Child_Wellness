/**
 * Notebook Task — Sign Sketch: Draw a MENU sign. Upload photo, AI verifies.
 */
    ...
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <View style={sketch.signPlate}>
          <Text style={sketch.signIcon}>🍽️</Text>
          <Text style={sketch.signText}>MENU</Text>
        </View>
        <Text style={sketch.caption}>blue sign · MENU letters · food list symbol</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ MENU text</Text>
...
function CheckRow({ label, ok, pending }: { label: string; ok?: boolean; pending?: boolean }) {
  return (
    <View style={check.row}>
      ...
      >
        {pending ? (
          <ActivityIndicator size="small" color={DINING.glow} />
        ) : (
...
export function RestaurantSignsNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-restaurant-signs-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const menuOk = data.menu_sign_detected ?? true;
      const drawingOk = data.drawing_valid ?? true;
      const correct = data.correct ?? (menuOk && drawingOk);
      const feedback =
        data.feedback || (correct ? 'Great job!' : "Let's try again! Draw a clearer MENU sign.");

      setResult({
        correct,
        feedback,
        menu_sign_detected: data.menu_sign_detected,
  ...
  const coachLine = uploading
    ? 'Scanning for a MENU sign drawing…'
    : result?.correct
      ? 'Verified! Your MENU sign looks great.'
      : result
        ? 'Write MENU big and clear — then capture again.'
        : 'Draw MENU in bold letters — add a food icon if you like!';

  return (
    <CitizenGameShell
      studio="SIGN SKETCH · MISSION"
      title="Notebook proof"
      instruction="Draw a MENU sign in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
      onReplayVoice={playVoice}
    >
      <View style={styles.badge}>
        <Text style={styles.badgeTxt}>SESSION 7 · NOTEBOOK TASK</Text>
      </View>
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[DINING.accent, DINING.rose]} style={styles.actionGrad}>
            {uploading ? (
      ...
      {(uploading || result) && (
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              <ActivityIndicator color={DINING.glow} size="large" />
              <Text style={styles.resultTitle}>Sign scan in progress…</Text>
              <CheckRow label="MENU sign detected" pending />
            ...
            <>
              ...
              <CheckRow label="MENU sign detected" ok={result.menu_sign_detected ?? result.correct} />
...
const sketch = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3, color: DINING.glow, marginBottom: 8 },
  glow: {
    position: 'absolute',
    top: 44,
    width: 130,
  ...
  signText: { fontSize: 24, fontWeight: '900', color: DINING.glow, letterSpacing: 2 },
  caption: { fontSize: 14, fontWeight: '800', color: CZ.textLight, textAlign: 'center' },
  hintRow: { flexDirection: 'row', gap: 12, marginTop: 8 },
  hint: { fontSize: 11, fontWeight: '700', color: DINING.glow },
});
...
const styles = StyleSheet.create({
  ...
  badgeTxt: { fontSize: 9, fontWeight: '900', letterSpacing: 1, color: DINING.glow },
  ...
  stepTitle: { fontSize: 11, fontWeight: '800', color: DINING.glow },
  ...
  actionTxtAlt: { color: DINING.glow },
  pressed: { opacity: 0.9 },
  ...
  retryBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 12, backgroundColor: DINING.accent },
