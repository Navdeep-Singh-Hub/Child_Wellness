/**
 * Game 4 — Pay for the juice. Juice costs ₹10; choose the correct coin (₹10). Session 7: Restaurant Signs.
 */
...
export function PayJuiceGame({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (coin: string) => {
      if (coin !== CORRECT) {
        speak('Try again. The juice costs ten rupees.');
        triggerShake();
        return;
      }
      speak('Correct! You paid for the juice!');
      setShowSuccess(true);
  ...
  if (showSuccess) return <SuccessCelebration variant="indigo" title="Great Job!" subtitle="You paid for the juice!" />;
