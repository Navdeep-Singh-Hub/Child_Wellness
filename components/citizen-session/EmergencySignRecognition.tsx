/**
 * Game 1 — Tap the correct emergency sign. Display EXIT, FIRE, HELP. Instruction: "Tap the EXIT sign." Session 8: Emergency Signs.
 */
...
const SIGNS = [
  { id: 'EXIT', label: 'EXIT', color: '#166534', bg: '#DCFCE7' },
  { id: 'FIRE', label: 'FIRE', color: '#991B1B', bg: '#FEE2E2' },
  { id: 'HELP', label: 'HELP', color: '#1E40AF', bg: '#DBEAFE' },
];
const CORRECT = 'EXIT';
...
export function EmergencySignRecognition({ onComplete }: { onComplete: () => void }) {
  ...
  if (showSuccess) return <SuccessCelebration variant="indigo" title="Great Job!" subtitle="You found EXIT!" />;

  const shakeX = shakeAnim.interpolate({ inputRange: [0, 1], outputRange: [0, 10] });

  return (
    <GameLayout
      title="Tap the correct emergency sign"
      instruction="Tap the EXIT sign."
      icon="🚨"
      backgroundVariant="indigo"
    >
      <View style={styles.content}>
        <Text style={styles.prompt}>Which one is the EXIT sign?</Text>
