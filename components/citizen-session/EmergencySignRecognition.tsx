/**
 * Game 1 — Sign Scan: Tap the correct emergency sign. EXIT, FIRE, HELP. Answer: EXIT.
 */
...
const SIGNS = [
  ...
  { id: 'FIRE', label: 'FIRE', color: '#991B1B', glow: '#FCA5A5', icon: '🔥' },
  { id: 'HELP', label: 'HELP', color: '#1E40AF', glow: '#93C5FD', icon: '🆘' },
] as const;

type SignId = (typeof SIGNS)[number]['id'];
const CORRECT: SignId = 'EXIT';
const VOICE = 'Tap the EXIT sign.';
const EMERGENCY = { accent: '#DC2626', glow: '#FCA5A5' } as const;
...
export function EmergencySignRecognition({ onComplete }: { onComplete: () => void }) {
  ...
  const handleTap = useCallback(
    (id: SignId) => {
      ...
      if (id === CORRECT) {
        ...
        speak('Try again. Tap the EXIT sign.', 0.7);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  const coachLine =
    attempts === 0
      ? 'EXIT shows the safe way out — find that sign!'
      : 'Look for EXIT — not fire or help.';
  ...
  return (
    ...
    >
      <View style={styles.laneFrame}>
        <LinearGradient
          colors={[`${EMERGENCY.accent}33`, 'transparent', `${CZ.amber}22`]}
          style={styles.laneGlow}
        />
        <Text style={styles.laneLabel}>EMERGENCY LANE</Text>
        <Text style={styles.prompt}>Which one is EXIT?</Text>
        ...
        <Text style={styles.hint}>🚪 green · safe way to leave the building</Text>
      </View>
    </CitizenGameShell>
  );
}
...
const styles = StyleSheet.create({
  ...
  laneGlow: { ...StyleSheet.absoluteFillObject },
  laneLabel: {
    fontSize: 9,
    fontWeight: '900',
    letterSpacing: 1.4,
