 ...
 * Flow: Intro → Game 1 (Traffic Sign Recognition) → Game 2 (Sign Meaning) → Game 3 (Coin Counting) → Game 4 (Parking Payment) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Traffic Signs</Text>
            <Text style={styles.introSub}>Let's learn signs we see on the road.</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '🛑', title: 'Tap the correct sign', desc: 'STOP, NO ENTRY, PARKING' },
              { icon: '📋', title: 'Match sign with meaning', desc: 'stop moving, cannot go' },
              { icon: '🪙', title: 'Count the coins', desc: '₹5 + ₹5 + ₹5 = ?' },
              { icon: '🅿️', title: 'Pay the parking fee', desc: 'Choose the right coin' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning traffic signs!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Traffic Safety Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: CITIZEN_COLOR }]}>
        <Text style={[styles.title, { color: '#BE185D' }]}>Traffic Signs</Text>
        <Text style={[styles.subtitle, { color: CITIZEN_COLOR }]}>The Citizen · Session 5</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <TrafficSignsNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
