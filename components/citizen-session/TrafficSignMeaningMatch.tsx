/**
 * Game 2 — Traffic Match: NO ENTRY→cannot go, STOP→stop moving, PARKING→place to park a vehicle.
 */
...
const SIGN_STYLE: Record<string, { color: string; glow: string; emoji: string }> = {
  'NO ENTRY': { color: '#1E40AF', glow: '#93C5FD', emoji: '🚫' },
  STOP: { color: '#DC2626', glow: '#FCA5A5', emoji: '🛑' },
  PARKING: { color: '#16A34A', glow: '#86EFAC', emoji: '🅿️' },
};
...
const QUESTIONS: {
  ...
  {
    sign: 'NO ENTRY',
    correctMeaning: 'cannot go',
    options: ['cannot go', 'stop moving', 'place to park a vehicle'],
    voice: 'What does NO ENTRY mean? Tap cannot go.',
  },
  {
    sign: 'STOP',
    correctMeaning: 'stop moving',
    options: ['cannot go', 'stop moving', 'place to park a vehicle'],
    voice: 'What does STOP mean? Tap stop moving.',
  },
  {
    sign: 'PARKING',
    correctMeaning: 'place to park a vehicle',
    options: ['cannot go', 'stop moving', 'place to park a vehicle'],
    voice: 'What does PARKING mean? Tap place to park a vehicle.',
  },
];
...
export function TrafficSignMeaningMatch({ onComplete }: { onComplete: () => void }) {
  ...
  const coachLine =
    q.sign === 'NO ENTRY'
      ? 'NO ENTRY blocks the way — which phrase matches?'
      : q.sign === 'STOP'
        ? 'STOP means pause completely — find the right meaning!'
        : 'PARKING shows where cars can stop — pick the matching phrase!';
  ...
  if (showSuccess) {
    return (
      <SuccessCelebration
        variant="sunset"
        title="Traffic Match!"
        subtitle="You know the traffic signs!"
        badgeEmoji="📋"
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <View style={styles.matchFrame}>
        <LinearGradient
          colors={[`${signStyle.color}33`, 'transparent', `${ROAD.amber}22`]}
