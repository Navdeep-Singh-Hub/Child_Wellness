/**
 * Notebook Task — Sign Sketch: Draw an OPEN sign. Upload photo, AI verifies.
 */
...
const VOICE = 'Draw an OPEN sign in your notebook. Then upload or take a photo.';
const SHOP = { accent: '#F97316', glow: '#FDBA74', amber: '#F59E0B' } as const;

const STEPS = [
  { icon: '✏️', title: 'Draw', body: 'OPEN sign.' },
  { icon: '📓', title: 'Notebook', body: 'Green text.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
] as const;

function OpenSignSketch() {
    ...
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <View style={sketch.signPlate}>
          ...
          <Text style={sketch.signText}>OPEN</Text>
        </View>
        <Text style={sketch.caption}>green letters · clear OPEN text · shop sign</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ OPEN text</Text>
...
function CheckRow({ label, ok, pending }: { label: string; ok?: boolean; pending?: boolean }) {
  return (
    <View style={check.row}>
      ...
      >
        {pending ? (
          <ActivityIndicator size="small" color={SHOP.glow} />
        ) : (
...
export function StoreSignsNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-store-signs-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const openOk = data.open_sign_detected ?? true;
      const drawingOk = data.drawing_valid ?? true;
      const correct = data.correct ?? (openOk && drawingOk);
      const feedback =
        data.feedback || (correct ? 'Great job!' : "Let's try again! Draw a clearer OPEN sign.");

      setResult({
        correct,
        feedback,
        open_sign_detected: data.open_sign_detected,
  ...
  const coachLine = uploading
    ? 'Scanning for an OPEN sign drawing…'
    : result?.correct
      ? 'Verified! Your OPEN sign looks great.'
      : result
        ? 'Write OPEN big and clear — then capture again.'
        : 'Use green if you can — draw OPEN bold in your notebook!';

  return (
    <CitizenGameShell
      studio="SIGN SKETCH · MISSION"
      title="Notebook proof"
      instruction="Draw an OPEN sign in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
      onReplayVoice={playVoice}
    >
      <View style={styles.badge}>
        <Text style={styles.badgeTxt}>SESSION 4 · NOTEBOOK TASK</Text>
      </View>
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[SHOP.accent, SHOP.amber]} style={styles.actionGrad}>
            {uploading ? (
      ...
      {(uploading || result) && (
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              <ActivityIndicator color={SHOP.glow} size="large" />
              <Text style={styles.resultTitle}>Sign scan in progress…</Text>
              <CheckRow label="OPEN sign detected" pending />
            ...
            <>
              ...
              <CheckRow label="OPEN sign detected" ok={result.open_sign_detected ?? result.correct} />
...
const sketch = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3, color: SHOP.glow, marginBottom: 8 },
  glow: {
    position: 'absolute',
    top: 44,
    width: 120,
  ...
  signIcon: { fontSize: 32 },
  signText: { fontSize: 28, fontWeight: '900', color: '#86EFAC', letterSpacing: 2 },
  caption: { fontSize: 14, fontWeight: '800', color: CZ.textLight, textAlign: 'center' },
  hintRow: { flexDirection: 'row', gap: 12, marginTop: 8 },
  hint: { fontSize: 11, fontWeight: '700', color: SHOP.glow },
});
...
const styles = StyleSheet.create({
  ...
  badgeTxt: { fontSize: 9, fontWeight: '900', letterSpacing: 1, color: SHOP.glow },
  ...
  stepTitle: { fontSize: 11, fontWeight: '800', color: SHOP.glow },
  ...
  retryBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 12, backgroundColor: SHOP.accent },
