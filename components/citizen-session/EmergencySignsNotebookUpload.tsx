/**
 * Notebook Task — Sign Sketch: Draw an emergency EXIT sign. Upload photo, AI verifies.
 */
...
const VOICE =
  'Draw an emergency EXIT sign in your notebook. Then upload or take a photo.';
const EMERGENCY = { accent: '#DC2626', glow: '#FCA5A5', amber: '#F59E0B', exit: '#166534' } as const;

const STEPS = [
  { icon: '✏️', title: 'Draw', body: 'EXIT sign.' },
  { icon: '📓', title: 'Notebook', body: 'Green + arrow.' },
  ...
  return (
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        ...
        <Text style={sketch.caption}>green sign · EXIT letters · arrow pointing out</Text>
...
function CheckRow({ label, ok, pending }: { label: string; ok?: boolean; pending?: boolean }) {
  return (
    <View style={check.row}>
      ...
      >
        {pending ? (
          <ActivityIndicator size="small" color={EMERGENCY.glow} />
        ) : (
...
export function EmergencySignsNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-emergency-signs-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
  ...
  const coachLine = uploading
    ...
    : result?.correct
      ...
      : result
        ? 'Write EXIT big and green — add an arrow, then capture again.'
        : 'Draw EXIT in bold green letters — point the arrow to the exit!';

  return (
    <CitizenGameShell
      studio="SIGN SKETCH · MISSION"
      title="Notebook proof"
      instruction="Draw an emergency EXIT sign. Then upload or take a photo."
      mascot="📓"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[EMERGENCY.accent, EMERGENCY.amber]} style={styles.actionGrad}>
            {uploading ? (
      ...
      {(uploading || result) && (
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              <ActivityIndicator color={EMERGENCY.glow} size="large" />
...
const sketch = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3, color: EMERGENCY.glow, marginBottom: 8 },
  glow: {
    position: 'absolute',
    top: 44,
    width: 140,
    height: 60,
    borderRadius: 12,
    backgroundColor: EMERGENCY.exit,
  },
  frame: {
    width: '92%',
  ...
  signText: { fontSize: 24, fontWeight: '900', color: '#86EFAC', letterSpacing: 2 },
  caption: { fontSize: 14, fontWeight: '800', color: CZ.textLight, textAlign: 'center' },
  hintRow: { flexDirection: 'row', gap: 12, marginTop: 8 },
  hint: { fontSize: 11, fontWeight: '700', color: EMERGENCY.glow },
});
...
const styles = StyleSheet.create({
  ...
  badgeTxt: { fontSize: 9, fontWeight: '900', letterSpacing: 1, color: EMERGENCY.glow },
  ...
  stepTitle: { fontSize: 11, fontWeight: '800', color: EMERGENCY.glow },
  ...
  actionTxtAlt: { color: EMERGENCY.glow },
  pressed: { opacity: 0.9 },
  ...
  retryBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 12, backgroundColor: EMERGENCY.accent },
