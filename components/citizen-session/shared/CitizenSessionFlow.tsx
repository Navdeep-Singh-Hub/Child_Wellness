/**
 * Shared intro · play · complete flow for The Citizen session orchestrators.
 */
import { CitizenBackdrop } from '@/components/citizen-session/shared/CitizenBackdrop';
import type { CitizenSessionConfig } from '@/components/citizen-session/shared/citizenSessionConfigs';
...
export function CitizenSessionFlow({ config, onExit, renderGame }: Props) {
          ...
          <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.hubBadge}>
              <Text style={[styles.hubBadgeTxt, { color: P.glow }]}>
                THE CITIZEN · SESSION {config.sessionNumber}
              </Text>
            </View>
              ...
              {config.cards.map((card, i) => {
                ...
                return (
                  <Pressable
                    ...
                    style={({ pressed }) => [
                      styles.introCard,
                      { borderColor: unlocked ? `${P.accent}88` : CZ.glassBorder },
                  ...
                  >
                    ...
                    {!unlocked ? (
                      <Ionicons name="lock-closed" size={18} color={CZ.textMuted} />
                    ) : (
          ...
          <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
            ...
            <View
              style={[
                styles.resultBadge,
                {
                  borderColor: config.finale ? P.accent : CZ.good,
...
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: CZ.bg[0] },
  safe: { flex: 1 },
  backRow: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  cardTextCol: { flex: 1 },
  introCardTitle: { fontSize: 16, fontWeight: '800', color: CZ.textLight },
  introCardDesc: { fontSize: 13, fontWeight: '600', color: CZ.textMuted, marginTop: 2 },
  ...
  header: {
    ...
    paddingBottom: 14,
    borderBottomWidth: 2,
    backgroundColor: 'rgba(26,10,18,0.45)',
  },
  headerTitle: { fontSize: 20, fontWeight: '900', color: CZ.textLight, textAlign: 'center' },
  ...
  resultBadgeText: { fontSize: 20, fontWeight: '900', color: CZ.goodGlow },
  resultStats: { marginBottom: 14 },
  resultStat: { fontSize: 16, fontWeight: '600', color: CZ.textMuted, marginBottom: 4, textAlign: 'center' },
  backBtn: {
    marginTop: 20,
    paddingVertical: 14,
