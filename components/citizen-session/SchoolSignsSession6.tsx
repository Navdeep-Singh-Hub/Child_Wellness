 ...
 * Flow: Intro → Game 1 (School Sign Recognition) → Game 2 (Icon–Place Match) → Game 3 (Coin Sorting) → Game 4 (Buy Pencil) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>School Signs</Text>
            <Text style={styles.introSub}>Let's learn signs we see in school.</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '📚', title: 'Tap the correct place', desc: 'LIBRARY, CLASSROOM, PLAYGROUND' },
              { icon: '📋', title: 'Match icon with place', desc: 'library, classroom, playground' },
              { icon: '🪙', title: 'Sort the coins', desc: '₹1, ₹2, ₹5, ₹10' },
              { icon: '✏️', title: 'Buy the pencil', desc: 'Choose the right coin' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning school signs!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>School Signs Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: CITIZEN_COLOR }]}>
        <Text style={[styles.title, { color: '#BE185D' }]}>School Signs</Text>
        <Text style={[styles.subtitle, { color: CITIZEN_COLOR }]}>The Citizen · Session 6</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <SchoolSignsNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
