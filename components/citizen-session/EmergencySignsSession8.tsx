 ...
 * Flow: Intro → Game 1 (Sign Recognition) → Game 2 (Sign Meaning) → Game 3 (Coin Counting) → Game 4 (Pay Water) → Notebook → Result
...
export function EmergencySignsSession8({ onExit }: EmergencySignsSession8Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🚨', title: 'Tap the correct sign', desc: 'EXIT, FIRE, HELP' },
      { icon: '📋', title: 'Match sign with meaning', desc: 'danger, way to leave safely' },
      { icon: '🪙', title: 'Count the coins', desc: '₹2 + ₹2 + ₹5 = ?' },
      { icon: '🫙', title: 'Pay the correct amount', desc: 'Choose the right coin' },
    ];
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Emergency Signs</Text>
            <Text style={styles.introSub}>Let's learn signs used during emergencies.</Text>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning emergency signs!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Emergency Safety Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: CITIZEN_COLOR }]}>
        <Text style={[styles.title, { color: '#BE185D' }]}>Emergency Signs</Text>
        <Text style={[styles.subtitle, { color: CITIZEN_COLOR }]}>The Citizen · Session 8</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <EmergencySignsNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
