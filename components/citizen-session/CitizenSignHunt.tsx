/**
 * Game 1 — Sign Hunt: Find the EXIT sign in the scene. Display EXIT, STOP, TOILET. Session 10: Citizen Master Challenge.
 */
...
export function CitizenSignHunt({ onComplete }: { onComplete: () => void }) {
  ...
  const handleTap = useCallback(
    (id: string) => {
      if (id !== CORRECT) {
        speak('Try again. Find the EXIT sign.');
        triggerShake();
        return;
      }
      speak('Correct! You found EXIT!');
      setShowSuccess(true);
  ...
  if (showSuccess) return <SuccessCelebration variant="indigo" title="Great Job!" subtitle="You found the EXIT sign!" />;
...
const styles = StyleSheet.create({
  ...
  sceneRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 16 },
  signCard: {
    borderRadius: 20,
    padding: 24,
    borderWidth: 3,
