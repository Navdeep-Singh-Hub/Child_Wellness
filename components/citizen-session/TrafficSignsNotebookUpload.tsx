/**
 * Notebook Task — Sign Sketch: Draw a NO ENTRY sign. Upload photo, AI verifies.
 */
...
const VOICE =
  'Draw a NO ENTRY sign in your notebook. Red circle with a white bar. Then upload or take a photo.';
const ROAD = { accent: '#EF4444', glow: '#FCA5A5', amber: '#F59E0B' } as const;

const STEPS = [
  { icon: '✏️', title: 'Draw', body: 'NO ENTRY.' },
  { icon: '📓', title: 'Notebook', body: 'Red circle.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
] as const;

function NoEntrySignSketch() {
    ...
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <View style={sketch.signPlate}>
          <View style={sketch.circleOuter}>
            <View style={sketch.bar} />
          </View>
          <Text style={sketch.signIcon}>🚫</Text>
          <Text style={sketch.signText}>NO ENTRY</Text>
        </View>
        <Text style={sketch.caption}>red circle · white bar · clear symbol</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ circle + bar</Text>
...
function CheckRow({ label, ok, pending }: { label: string; ok?: boolean; pending?: boolean }) {
  return (
    <View style={check.row}>
      ...
      >
        {pending ? (
          <ActivityIndicator size="small" color={ROAD.glow} />
        ) : (
...
export function TrafficSignsNotebookUpload({
  onComplete,
}: {
  ...
  const [result, setResult] = useState<{
    correct: boolean;
    feedback: string;
    no_entry_sign_detected?: boolean;
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-traffic-signs-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const noEntryOk = data.no_entry_sign_detected ?? true;
      const drawingOk = data.drawing_valid ?? true;
      const correct = data.correct ?? (noEntryOk && drawingOk);
      const feedback =
        data.feedback ||
        (correct ? 'Great job!' : "Let's try again! Draw a clearer NO ENTRY sign.");

      setResult({
        correct,
        feedback,
        no_entry_sign_detected: data.no_entry_sign_detected,
  ...
  const coachLine = uploading
    ? 'Scanning for a NO ENTRY sign drawing…'
    : result?.correct
      ? 'Verified! Your NO ENTRY sign looks great.'
      : result
        ? 'Draw a red circle with a white bar — then capture again.'
        : 'Use red for the circle — add a white bar across the middle!';

  return (
    <CitizenGameShell
      studio="SIGN SKETCH · MISSION"
      title="Notebook proof"
      instruction="Draw a NO ENTRY sign in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
      onReplayVoice={playVoice}
    >
      <View style={styles.badge}>
        <Text style={styles.badgeTxt}>SESSION 5 · NOTEBOOK TASK</Text>
      </View>
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[ROAD.accent, ROAD.amber]} style={styles.actionGrad}>
            {uploading ? (
      ...
      {(uploading || result) && (
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              <ActivityIndicator color={ROAD.glow} size="large" />
              <Text style={styles.resultTitle}>Sign scan in progress…</Text>
              <CheckRow label="NO ENTRY sign detected" pending />
            ...
            <>
              ...
              <CheckRow
                label="NO ENTRY sign detected"
                ok={result.no_entry_sign_detected ?? result.correct}
              />
...
const sketch = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3, color: ROAD.glow, marginBottom: 8 },
  glow: {
    position: 'absolute',
    top: 48,
    width: 72,
  ...
  signIcon: { fontSize: 28 },
  signText: { fontSize: 16, fontWeight: '900', color: ROAD.glow, letterSpacing: 1.5 },
  caption: { fontSize: 14, fontWeight: '800', color: CZ.textLight, textAlign: 'center' },
  hintRow: { flexDirection: 'row', gap: 12, marginTop: 8 },
  hint: { fontSize: 11, fontWeight: '700', color: ROAD.glow },
});
...
const check = StyleSheet.create({
  ...
  iconPending: { backgroundColor: 'rgba(252,165,165,0.15)' },
...
const styles = StyleSheet.create({
  ...
  badgeTxt: { fontSize: 9, fontWeight: '900', letterSpacing: 1, color: ROAD.glow },
  ...
  stepTitle: { fontSize: 11, fontWeight: '800', color: ROAD.glow },
  ...
  retryBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 12, backgroundColor: ROAD.accent },
