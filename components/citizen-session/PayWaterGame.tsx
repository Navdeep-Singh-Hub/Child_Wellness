/**
 * Game 4 — Pay for the water bottle. Water bottle costs ₹10; choose the correct coin (₹10). Session 8: Emergency Signs.
 */
...
export function PayWaterGame({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (coin: string) => {
      if (coin !== CORRECT) {
        speak('Try again. The water bottle costs ten rupees.');
        triggerShake();
        return;
      }
      speak('Correct! You paid the correct amount!');
      setShowSuccess(true);
  ...
  if (showSuccess) return <SuccessCelebration variant="indigo" title="Great Job!" subtitle="You paid for the water!" />;
...
const styles = StyleSheet.create({
  ...
  itemName: { fontSize: 20, fontWeight: '800', color: '#1f2937', marginBottom: 4 },
