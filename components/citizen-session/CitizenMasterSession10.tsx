/**
 * Level 8 – The Citizen, Session 10: Citizen Master Challenge (final session)
 * Flow: Intro → Game 1 (Sign Hunt) → Game 2 (Meaning Quiz) → Game 3 (Coin Challenge) → Game 4 (Store Simulation) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Citizen Master Challenge</Text>
            <Text style={styles.introSub}>Use everything you learned about signs and money!</Text>
            <Text style={styles.introSkills}>Sign recognition · Sign meaning · Coin counting · Buying items</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '🔍', title: 'Find the signs', desc: 'EXIT, STOP, TOILET' },
              { icon: '📋', title: 'Choose the meaning', desc: 'STOP, EXIT' },
              { icon: '🪙', title: 'Count the coins', desc: '₹2 + ₹5 + ₹1 = ?' },
              { icon: '🛒', title: 'Buy the item', desc: 'Apple ₹5' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Congratulations! You completed Level 8.</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>🏆</Text>
            <Text style={styles.resultBadgeText}>Citizen Master</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: CITIZEN_COLOR }]}>
        <Text style={[styles.title, { color: '#BE185D' }]}>Citizen Master Challenge</Text>
        <Text style={[styles.subtitle, { color: CITIZEN_COLOR }]}>The Citizen · Session 10</Text>
        <View style={styles.progressRow}>
          <View style={[styles.progressBg, { backgroundColor: `${CITIZEN_COLOR}30` }]}>
            <View style={[styles.progressFill, { width: `${progressPct}%`, backgroundColor: CITIZEN_COLOR }]} />
          </View>
          <Text style={[styles.progressText, { color: '#BE185D' }]}>{step} / {TOTAL_GAMES}</Text>
        </View>
        <View style={styles.starsRow}>
          {Array.from({ length: TOTAL_GAMES }, (_, i) => (
            <Text key={i} style={[styles.star, i < stars && styles.starEarned]}>⭐</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <CitizenMasterNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  progressBg: { flex: 1, height: 14, borderRadius: 7, overflow: 'hidden' },
  ...
  introTitle: { fontSize: 28, fontWeight: '800', color: '#BE185D', textAlign: 'center', marginBottom: 12 },
  introSub: { fontSize: 18, color: '#EC4899', textAlign: 'center', lineHeight: 26, marginBottom: 8 },
  introSkills: { fontSize: 14, color: '#6B7280', textAlign: 'center' },
  ...
  resultTitle: { fontSize: 24, fontWeight: '800', color: '#BE185D', textAlign: 'center', marginBottom: 24 },
  resultBadge: {
    flexDirection: 'row',
    alignItems: 'center',
