/**
 * Game 2 — Place Match: Match sign with place. 🚻 → restroom, 🚪 → exit.
 */
...
const SIGN_STYLE: Record<string, { color: string; glow: string; label: string }> = {
  '🚻': { color: '#1E40AF', glow: '#93C5FD', label: 'RESTROOM' },
  '🚪': { color: '#0F766E', glow: '#5EEAD4', label: 'EXIT' },
};
...
const QUESTIONS: {
  ...
  {
    icon: '🚻',
    correctMeaning: 'restroom',
    options: ['restroom', 'exit', 'entrance'],
    voice: 'What does this sign mean? Tap restroom.',
  },
  {
    icon: '🚪',
    correctMeaning: 'exit',
    options: ['restroom', 'exit', 'entrance'],
    voice: 'What does this sign mean? Tap exit.',
  },
];
...
export function PublicSignMeaningMatch({ onComplete }: { onComplete: () => void }) {
  ...
  const signStyle = SIGN_STYLE[q.icon];
  ...
  const coachLine =
    q.icon === '🚻'
      ? 'The restroom sign points to bathrooms — which word matches?'
      : 'The exit sign shows a way out — find the right meaning!';
  ...
  if (showSuccess) {
    return (
      <SuccessCelebration
        variant="ocean"
        title="Place Match!"
        subtitle="You know the public signs!"
        badgeEmoji="📋"
      />
    );
  }
  ...
  return (
    <CitizenGameShell
      ...
      instruction="What does this sign mean? Tap the correct word."
      mascot="📋"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.matchFrame}>
        <LinearGradient
          colors={[`${signStyle.color}33`, 'transparent', `${PLAZA.amber}22`]}
        ...
        <View style={[styles.signBadge, { borderColor: signStyle.glow }]}>
          <Text style={styles.signEmoji}>{q.icon}</Text>
          <Text style={[styles.signText, { color: signStyle.color }]}>{signStyle.label}</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  signEmoji: { fontSize: 32 },
  signText: { fontSize: 20, fontWeight: '900', letterSpacing: 1.5 },
  prompt: {
    fontSize: 17,
    fontWeight: '800',
    color: CZ.textLight,
