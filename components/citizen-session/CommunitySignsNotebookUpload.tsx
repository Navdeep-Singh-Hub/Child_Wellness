/**
 * Notebook Task — Sign Sketch: Draw a BUS STOP sign. Upload photo, AI verifies.
 */
...
const VOICE = 'Draw a BUS STOP sign in your notebook. Then upload or take a photo.';
const COMMUNITY = { accent: '#10B981', glow: '#6EE7B7', violet: '#8B5CF6', bus: '#166534' } as const;

const STEPS = [
  { icon: '✏️', title: 'Draw', body: 'BUS STOP sign.' },
  { icon: '📓', title: 'Notebook', body: 'Clear text.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
] as const;

function BusStopSignSketch() {
    ...
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <View style={sketch.signPlate}>
          ...
          <Text style={sketch.signText}>BUS STOP</Text>
        </View>
        <Text style={sketch.caption}>green sign · BUS STOP letters · bus symbol</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ BUS STOP text</Text>
...
function CheckRow({ label, ok, pending }: { label: string; ok?: boolean; pending?: boolean }) {
  return (
    <View style={check.row}>
      ...
      >
        {pending ? (
          <ActivityIndicator size="small" color={COMMUNITY.glow} />
        ) : (
...
export function CommunitySignsNotebookUpload({
  onComplete,
}: {
  ...
  const [result, setResult] = useState<{
    correct: boolean;
    feedback: string;
    bus_stop_sign_detected?: boolean;
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-community-signs-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const busStopOk = data.bus_stop_sign_detected ?? true;
      const drawingOk = data.drawing_valid ?? true;
      const correct = data.correct ?? (busStopOk && drawingOk);
      const feedback =
        data.feedback || (correct ? 'Great job!' : "Let's try again! Draw a clearer BUS STOP sign.");

      setResult({
        correct,
        feedback,
        bus_stop_sign_detected: data.bus_stop_sign_detected,
  ...
  const coachLine = uploading
    ? 'Scanning for a BUS STOP sign drawing…'
    : result?.correct
      ? 'Verified! Your BUS STOP sign looks great.'
      : result
        ? 'Write BUS STOP big and clear — then capture again.'
        : 'Draw BUS STOP in bold letters — add a bus icon if you like!';

  return (
    <CitizenGameShell
      studio="SIGN SKETCH · MISSION"
      title="Notebook proof"
      instruction="Draw a BUS STOP sign. Then upload or take a photo."
      mascot="📓"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[COMMUNITY.accent, COMMUNITY.violet]} style={styles.actionGrad}>
            {uploading ? (
      ...
      {(uploading || result) && (
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              <ActivityIndicator color={COMMUNITY.glow} size="large" />
              <Text style={styles.resultTitle}>Sign scan in progress…</Text>
              <CheckRow label="BUS STOP sign detected" pending />
            ...
            <>
              ...
              <CheckRow
                label="BUS STOP sign detected"
                ok={result.bus_stop_sign_detected ?? result.correct}
              />
...
const sketch = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3, color: COMMUNITY.glow, marginBottom: 8 },
  glow: {
    position: 'absolute',
    top: 44,
    width: 150,
  ...
  signText: { fontSize: 20, fontWeight: '900', color: COMMUNITY.glow, letterSpacing: 1.5 },
  caption: { fontSize: 14, fontWeight: '800', color: CZ.textLight, textAlign: 'center' },
  hintRow: { flexDirection: 'row', gap: 12, marginTop: 8 },
  hint: { fontSize: 11, fontWeight: '700', color: COMMUNITY.glow },
});
...
const styles = StyleSheet.create({
  ...
  badgeTxt: { fontSize: 9, fontWeight: '900', letterSpacing: 1, color: COMMUNITY.glow },
  ...
  stepTitle: { fontSize: 11, fontWeight: '800', color: COMMUNITY.glow },
  ...
  actionTxtAlt: { color: COMMUNITY.glow },
  pressed: { opacity: 0.9 },
  ...
  retryBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 12, backgroundColor: COMMUNITY.accent },
