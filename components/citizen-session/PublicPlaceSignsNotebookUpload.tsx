/**
 * Notebook Task — Sign Sketch: Draw a restroom sign 🚻. Upload photo, AI verifies.
 */
...
const VOICE = 'Draw a restroom sign in your notebook. Then upload or take a photo.';
const PLAZA = { accent: '#38BDF8', glow: '#BAE6FD', amber: '#F59E0B' } as const;

const STEPS = [
  { icon: '✏️', title: 'Draw', body: 'Restroom sign.' },
  { icon: '📓', title: 'Notebook', body: '🚻 symbol.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
] as const;

function RestroomSignSketch() {
    ...
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <View style={sketch.signPlate}>
          ...
          <Text style={sketch.signText}>RESTROOM</Text>
        </View>
        <Text style={sketch.caption}>blue sign · clear symbol · easy to read</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ 🚻 symbol</Text>
...
function CheckRow({ label, ok, pending }: { label: string; ok?: boolean; pending?: boolean }) {
  return (
    <View style={check.row}>
      ...
      >
        {pending ? (
          <ActivityIndicator size="small" color={PLAZA.glow} />
        ) : (
...
export function PublicPlaceSignsNotebookUpload({
  onComplete,
}: {
  ...
  const [result, setResult] = useState<{
    correct: boolean;
    feedback: string;
    restroom_sign_detected?: boolean;
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-public-place-signs-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const restroomOk = data.restroom_sign_detected ?? true;
      const drawingOk = data.drawing_valid ?? true;
      const correct = data.correct ?? (restroomOk && drawingOk);
      const feedback =
        data.feedback ||
        (correct ? 'Great job!' : "Let's try again! Draw a clearer restroom sign.");

      setResult({
        correct,
        feedback,
        restroom_sign_detected: data.restroom_sign_detected,
  ...
  const coachLine = uploading
    ? 'Scanning for a restroom sign drawing…'
    : result?.correct
      ? 'Verified! Your restroom sign looks great.'
      : result
        ? 'Make the 🚻 symbol clear and recognizable — then capture again.'
        : 'Draw the restroom sign big — use blue if you can!';

  return (
    <CitizenGameShell
      studio="SIGN SKETCH · MISSION"
      title="Notebook proof"
      instruction="Draw a restroom sign in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.actions}>
        ...
        >
          <LinearGradient colors={[PLAZA.accent, PLAZA.amber]} style={styles.actionGrad}>
            {uploading ? (
      ...
      {(uploading || result) && (
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              <ActivityIndicator color={PLAZA.glow} size="large" />
              <Text style={styles.resultTitle}>Sign scan in progress…</Text>
              <CheckRow label="Restroom sign detected" pending />
            ...
            <>
              ...
              <CheckRow
                label="Restroom sign detected"
                ok={result.restroom_sign_detected ?? result.correct}
              />
...
const sketch = StyleSheet.create({
  ...
  label: { fontSize: 9, fontWeight: '900', letterSpacing: 1.3, color: PLAZA.glow, marginBottom: 8 },
  glow: {
    position: 'absolute',
    top: 44,
    width: 120,
  ...
  signIcon: { fontSize: 40 },
  signText: { fontSize: 16, fontWeight: '900', color: PLAZA.glow, letterSpacing: 1.5 },
  caption: { fontSize: 14, fontWeight: '800', color: CZ.textLight, textAlign: 'center' },
  hintRow: { flexDirection: 'row', gap: 12, marginTop: 8 },
  hint: { fontSize: 11, fontWeight: '700', color: PLAZA.glow },
});
...
const check = StyleSheet.create({
  ...
  iconPending: { backgroundColor: 'rgba(186,230,253,0.15)' },
...
const styles = StyleSheet.create({
  ...
  badgeTxt: { fontSize: 9, fontWeight: '900', letterSpacing: 1, color: PLAZA.glow },
  ...
  stepTitle: { fontSize: 11, fontWeight: '800', color: PLAZA.glow },
  ...
  retryBtn: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 12, backgroundColor: PLAZA.accent },
