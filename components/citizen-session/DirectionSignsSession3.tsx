 ...
 * Flow: Intro → Game 1 (Sign Recognition) → Game 2 (Sign Meaning) → Game 3 (Coin Sorting) → Game 4 (Buy Item) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Direction Signs</Text>
            <Text style={styles.introSub}>Let's learn signs that show directions.</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '⬅️', title: 'Tap the correct sign', desc: 'LEFT, RIGHT, EXIT' },
              { icon: '📋', title: 'Match sign with meaning', desc: 'go left, go right, door to leave' },
              { icon: '🪙', title: 'Sort the coins', desc: '₹1, ₹2, ₹5, ₹10' },
              { icon: '🍎', title: 'Buy the item', desc: 'Choose the right coin' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning direction signs!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Direction Signs Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: CITIZEN_COLOR }]}>
        ...
        <Text style={[styles.title, { color: '#BE185D' }]}>Direction Signs</Text>
        <Text style={[styles.subtitle, { color: CITIZEN_COLOR }]}>The Citizen · Session 3</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <DirectionSignsNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  backButtonText: { fontSize: 17, fontWeight: '700', color: CITIZEN_COLOR },
  header: {
  ...
  introSub: { fontSize: 18, color: '#EC4899', textAlign: 'center', lineHeight: 26 },
