 ...
 * Flow: Intro → Game 1 (Sign Recognition) → Game 2 (Sign–Place Match) → Game 3 (Coin Total) → Game 4 (Buy Bus Ticket) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Community Signs</Text>
            <Text style={styles.introSub}>Let's learn signs we see in the community.</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '🚍', title: 'Tap the correct sign', desc: 'HOSPITAL, POLICE, BUS STOP' },
              { icon: '📋', title: 'Match sign to place', desc: 'bus stop, hospital, police' },
              { icon: '🪙', title: 'Identify coin total', desc: 'Count the coins' },
              { icon: '🎫', title: 'Buy bus ticket', desc: '₹10 required' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning community signs!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Community Signs Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: CITIZEN_COLOR }]}>
        <Text style={[styles.title, { color: '#BE185D' }]}>Community Signs</Text>
        <Text style={[styles.subtitle, { color: CITIZEN_COLOR }]}>The Citizen · Session 9</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <CommunitySignsNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
