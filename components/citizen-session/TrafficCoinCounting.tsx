/**
 * Game 3 — Count the coins. ₹5 + ₹5 + ₹5 = ? Options ₹10, ₹15, ₹20. Correct: ₹15. Session 5: Traffic Signs.
 */
...
export function TrafficCoinCounting({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (amount: string) => {
      if (amount !== CORRECT_TOTAL) {
        speak('Try again. Five plus five plus five is fifteen.');
        triggerShake();
        return;
      }
      speak('Correct! Fifteen rupees!');
      setShowSuccess(true);
  ...
  if (showSuccess) return <SuccessCelebration variant="indigo" title="Great Job!" subtitle="You counted ₹15!" />;
