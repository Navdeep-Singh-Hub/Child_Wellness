/**
 * Game 2 — Meaning Match: FIRE→danger, EXIT→way to leave safely, HELP→ask for assistance.
 */
...
const SIGN_STYLE: Record<string, { color: string; glow: string; emoji: string }> = {
  FIRE: { color: '#991B1B', glow: '#FCA5A5', emoji: '🔥' },
  EXIT: { color: '#166534', glow: '#86EFAC', emoji: '🚪' },
  HELP: { color: '#1E40AF', glow: '#93C5FD', emoji: '🆘' },
};
...
const QUESTIONS: {
  ...
  {
    sign: 'FIRE',
    correctMeaning: 'danger',
    options: ['danger', 'way to leave safely', 'ask for assistance'],
    voice: 'What does FIRE mean? Tap danger.',
  },
  {
    sign: 'EXIT',
    correctMeaning: 'way to leave safely',
    options: ['danger', 'way to leave safely', 'ask for assistance'],
    voice: 'What does EXIT mean? Tap way to leave safely.',
  },
  {
    sign: 'HELP',
    correctMeaning: 'ask for assistance',
    options: ['danger', 'way to leave safely', 'ask for assistance'],
    voice: 'What does HELP mean? Tap ask for assistance.',
  },
];

const EMERGENCY = { accent: '#DC2626', glow: '#FCA5A5', amber: '#F59E0B' } as const;
...
export function EmergencySignMeaningMatch({ onComplete }: { onComplete: () => void }) {
  ...
  const coachLine =
    q.sign === 'FIRE'
      ? 'FIRE warns about something dangerous — which phrase matches?'
      : q.sign === 'EXIT'
        ? 'EXIT points to the safe way out — find the right meaning!'
        : 'HELP means you need someone — pick the matching phrase!';
  ...
  if (showSuccess) {
    return (
      <SuccessCelebration
        variant="sunset"
        title="Meaning Match!"
        subtitle="You know the emergency signs!"
        badgeEmoji="📋"
      />
    );
  }
    ...
    >
      <View style={styles.progressWrap}>
        ...
        <View style={styles.progressBg}>
          <LinearGradient
            colors={[EMERGENCY.accent, EMERGENCY.amber]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
      ...
      <View style={styles.matchFrame}>
        <LinearGradient
          colors={[`${signStyle.color}33`, 'transparent', `${EMERGENCY.amber}22`]}
