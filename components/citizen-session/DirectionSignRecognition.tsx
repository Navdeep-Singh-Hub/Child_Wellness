/**
 * Game 1 — Sign Scan: Tap the correct direction sign. LEFT, RIGHT, EXIT. Answer: LEFT.
 */
...
const SIGNS = [
  { id: 'LEFT', label: 'LEFT', color: '#0D9488', glow: '#5EEAD4', icon: '⬅️' },
  { id: 'RIGHT', label: 'RIGHT', color: '#0891B2', glow: '#67E8F9', icon: '➡️' },
  { id: 'EXIT', label: 'EXIT', color: '#1E40AF', glow: '#93C5FD', icon: '🚪' },
] as const;

type SignId = (typeof SIGNS)[number]['id'];
const CORRECT: SignId = 'LEFT';
const VOICE = 'Tap the LEFT sign.';
const DEPOT = { accent: '#14B8A6', glow: '#5EEAD4' } as const;
...
export function DirectionSignRecognition({ onComplete }: { onComplete: () => void }) {
  ...
  const handleTap = useCallback(
    (id: SignId) => {
      ...
      if (id === CORRECT) {
        ...
        speak('Try again. Tap the LEFT sign.', 0.7);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  const coachLine =
    attempts === 0
      ? 'LEFT points the way to turn — look for the arrow sign!'
      : 'Find the LEFT sign — not RIGHT or EXIT.';
  ...
  return (
    ...
    >
      <View style={styles.depotFrame}>
        <LinearGradient
          colors={[`${DEPOT.accent}33`, 'transparent', `${CZ.amber}22`]}
          style={styles.depotGlow}
        />
        <Text style={styles.depotLabel}>WAYFINDER DEPOT</Text>
        <Text style={styles.prompt}>Which one is LEFT?</Text>
        ...
        <Text style={styles.hint}>⬅️ teal · points you to turn left</Text>
      </View>
    </CitizenGameShell>
  );
}
