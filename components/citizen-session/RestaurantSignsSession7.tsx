 ...
 * Flow: Intro → Game 1 (Sign Recognition) → Game 2 (Sign Meaning) → Game 3 (Coin Value) → Game 4 (Pay Juice) → Notebook → Result
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Restaurant Signs</Text>
            <Text style={styles.introSub}>Let's learn signs we see in restaurants.</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '🍽️', title: 'Tap the correct sign', desc: 'MENU, ORDER, PAY' },
              { icon: '📋', title: 'Match sign with meaning', desc: 'list of food, give money' },
              { icon: '🪙', title: 'Find the correct coin', desc: '₹1, ₹2, ₹5, ₹10' },
              { icon: '🧃', title: 'Pay for the juice', desc: 'Choose the right coin' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning restaurant signs!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Restaurant Signs Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={[styles.header, { borderBottomColor: CITIZEN_COLOR }]}>
        <Text style={[styles.title, { color: '#BE185D' }]}>Restaurant Signs</Text>
        <Text style={[styles.subtitle, { color: CITIZEN_COLOR }]}>The Citizen · Session 7</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <RestaurantSignsNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
