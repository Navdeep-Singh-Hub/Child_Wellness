/**
 * Match the rhyming words — cat→hat, pin→tin, sun→bun. Game 2 for Grouper Session 4.
 */
...
export function RhymePairsMixed({ onComplete }: { onComplete: () => void }) {
  ...
  const choices = [current.rhyme, ...PAIRS.filter((p) => p.rhyme !== current.rhyme).map((p) => p.rhyme)].sort(() => Math.random() - 0.5);

  useEffect(() => {
    if (current) speak(`Match the word that rhymes with ${current.word}.`, 0.75);
  }, [round]);
...
const styles = StyleSheet.create({
  ...
  promptWord: { fontSize: 36, fontWeight: '800', color: '#3730A3' },
  ...
  pressed: { opacity: 0.9 },
  choiceLabel: { fontSize: 24, fontWeight: '800', color: '#1f2937' },
