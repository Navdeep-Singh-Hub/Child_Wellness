/**
 * SuperheroNotebookUpload — Session Task
 * Option 1: Write two three-syllable words from the story (e.g. superhero, butterfly).
 * Option 2: Count and draw 8 hero logos, write the number 8.
 */
...
const INSTRUCTIONS =
  'Option 1: Write two three-syllable words from the story (e.g. superhero, butterfly).\n\nOR\n\nOption 2: Draw 8 hero logos in your notebook and write the number 8.';

export function SuperheroNotebookUpload({
  onComplete,
}: {
  ...
  const [result, setResult] = useState<{
    ...
    syllable_words_present?: boolean;
  } | null>(null);
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-superhero-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const correct =
        data.correct ??
        (data.drawing_present && (data.number_correct || data.syllable_words_present)) ??
        true;
      ...
      setResult({
        ...
        syllable_words_present: data.syllable_words_present,
      });
      if (correct) {
        speak('Great job!');
  ...
  return (
    <GameLayout
      title="Superhero Notebook Activity"
...
const styles = StyleSheet.create({
  ...
  previewWrap: { marginTop: 20, borderRadius: 16, overflow: 'hidden', borderWidth: 3, borderColor: '#EF4444' },
  ...
  resultOk: { backgroundColor: '#FEE2E2', borderColor: '#EF4444' },
  resultFail: { backgroundColor: '#FEE2E2', borderColor: '#DC2626' },
  resultEmoji: { fontSize: 48, marginBottom: 8 },
  resultText: { fontSize: 24, fontWeight: '800', color: '#991B1B' },
