/**
 * SuperheroSession8 — Special Ed Level 2 · Session 8: Superheroes
 * Flow: Intro → Game 1 (Clean It Up /s/) → Game 2 (I-Spy Rhyming) → Game 3 (Clap It Out) → Game 4 (Match Sets) → Notebook → Result
 * Design: Hero Red #EF4444, Cape Blue #3B82F6, Gold #FACC15.
 */
...
import { SuperheroCleanUp } from './SuperheroCleanUp';
import { SuperheroRhymeGame } from './SuperheroRhymeGame';
import { SuperheroSyllableGame } from './SuperheroSyllableGame';
import { SuperheroMatchSets } from './SuperheroMatchSets';
import { SuperheroNotebookUpload } from './SuperheroNotebookUpload';

const TOTAL_GAMES = 5;
const GAME_COUNT = 4;
const SECTION_NUMBER = 2;
const SESSION_NUMBER = 8;
const HERO_RED = '#EF4444';
const CAPE_BLUE = '#3B82F6';
const GOLD = '#FACC15';

interface SuperheroSession8Props {
  onExit?: () => void;
}

export function SuperheroSession8({ onExit }: SuperheroSession8Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🦸', title: 'Clean It Up!', desc: 'Initial sound /s/' },
      { icon: '👀', title: 'I-Spy Rhyming', desc: 'Hero-themed rhymes' },
      { icon: '👏', title: 'Clap It Out!', desc: 'Hero name syllables' },
      { icon: '🛡️', title: 'Matching Sets to Numerals', desc: 'Match groups to numbers' },
    ];
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Superheroes</Text>
            <Text style={styles.introSub}>
              Be a hero! Learn sounds, rhymes, syllables, and counting.
            </Text>
          </View>
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.heroRow}>
            {['🦸', '⭐', '🛡️', '🦸‍♂️', '✨', '⭐'].map((e, i) => (
              <Text key={i} style={styles.heroEmoji}>{e}</Text>
            ))}
          </View>
          <Text style={styles.resultTitle}>You are a learning superhero!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>🦸</Text>
            <Text style={styles.resultBadgeText}>Superhero Badge</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Session 8</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <SuperheroCleanUp onComplete={advance} />}
        {step === 2 && <SuperheroRhymeGame onComplete={advance} />}
        {step === 3 && <SuperheroSyllableGame onComplete={advance} />}
        {step === 4 && <SuperheroMatchSets onComplete={advance} />}
        {step === 5 && <SuperheroNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  backButtonText: { fontSize: 17, fontWeight: '700', color: '#1D4ED8' },
  header: {
    ...
    paddingBottom: 16,
    borderBottomWidth: 3,
    borderBottomColor: HERO_RED,
  },
  title: { fontSize: 22, fontWeight: '800', color: '#991B1B', textAlign: 'center' },
  subtitle: { fontSize: 14, color: '#B91C1C', textAlign: 'center', marginTop: 4 },
  ...
  progressFill: { height: '100%', backgroundColor: HERO_RED, borderRadius: 7 },
  progressText: { fontSize: 16, fontWeight: '700', color: '#991B1B', minWidth: 48 },
  ...
  introTitle: { fontSize: 28, fontWeight: '800', color: '#991B1B', textAlign: 'center', marginBottom: 12 },
  introSub: { fontSize: 18, color: '#B91C1C', textAlign: 'center', lineHeight: 26 },
  ...
  heroRow: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 8, marginBottom: 16 },
  heroEmoji: { fontSize: 32 },
  resultTitle: { fontSize: 24, fontWeight: '800', color: '#991B1B', textAlign: 'center', marginBottom: 24 },
  resultBadge: {
    flexDirection: 'row',
    alignItems: 'center',
