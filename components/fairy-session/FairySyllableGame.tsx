/**
 * FairySyllableGame — Game 3: Clap the Magical Words
 * wizard (2), dragon (2), princess (2), castle (2). Bouncing magical stars, sparkle on correct.
 */
...
const WORDS = [
  { word: 'Wizard', syllables: 2 },
  { word: 'Dragon', syllables: 2 },
  { word: 'Princess', syllables: 2 },
  { word: 'Castle', syllables: 2 },
];

export function FairySyllableGame({ onComplete }: { onComplete: () => void }) {
  const [round, setRound] = useState(0);
  const [showSuccess, setShowSuccess] = useState(false);
  const [iconAnims] = useState(() =>
    [0, 1].map((i) => new Animated.Value(0)).reduce((acc, a, i) => ({ ...acc, [i]: a }), {} as Record<number, Animated.Value>)
  );
...
const styles = StyleSheet.create({
  ...
  iconsRow: { flexDirection: 'row', gap: 20, marginBottom: 24 },
  iconWrap: { alignItems: 'center', justifyContent: 'center' },
  iconEmoji: { fontSize: 36 },
  hint: { fontSize: 18, color: '#4b5563', marginBottom: 20 },
  clapRow: { flexDirection: 'row', gap: 24 },
  clapBtn: {
  ...
  clapEmoji: { fontSize: 32, marginBottom: 8 },
