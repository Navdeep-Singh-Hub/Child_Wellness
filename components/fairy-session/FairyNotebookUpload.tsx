/**
 * FairyNotebookUpload — Fairy Tale Notebook Activity
 * Option 1: Draw 11 mushrooms and write 11. Option 2: Write two rhyming word pairs (e.g. bear–chair, spell–bell).
 */
...
const INSTRUCTIONS =
  'Option 1: Draw 11 mushrooms in your notebook and write the number 11.\n\nOR\n\nOption 2: Write two rhyming word pairs from the story. Example: bear – chair, spell – bell.';

export function FairyNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-fairy-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const correct =
        data.correct ??
        (data.drawing_present && (data.number_correct || data.rhyming_pairs_present)) ??
        true;
      ...
      setResult({
        ...
        rhyming_pairs_present: data.rhyming_pairs_present,
      });
      if (correct) {
        speak('Great job!');
  ...
  return (
    <GameLayout
      title="Fairy Tale Notebook Activity"
...
const styles = StyleSheet.create({
  ...
  previewWrap: { marginTop: 20, borderRadius: 16, overflow: 'hidden', borderWidth: 3, borderColor: '#16A34A' },
  ...
  resultOk: { backgroundColor: '#DCFCE7', borderColor: '#16A34A' },
