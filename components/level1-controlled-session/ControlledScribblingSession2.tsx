/**
 * Level 1 – Session 2: Controlled Scribbling
 * Flow: Intro → Game 1 (Circle) → Game 2 (Triangle) → Game 3 (Big→Small) → Game 4 (Boundary Control) → Task (Upload) → Result
 */
...
export function ControlledScribblingSession2({ onExit }: ControlledScribblingSession2Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '⭕', title: 'Scribble Inside Circle', desc: 'Fill the circle, stay inside', step: 1 },
      { icon: '🔺', title: 'Scribble Inside Triangle', desc: 'Fill the triangle', step: 2 },
      { icon: '📐', title: 'Big → Small Shapes', desc: 'Fill circle, square, triangle', step: 3 },
      { icon: '✋', title: 'Stay Inside!', desc: 'Keep scribbles in the boundary', step: 4 },
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Controlled Scribbling — Stay inside the shapes!</Text>
          </View>
          <Text style={styles.introTitle}>Controlled Scribbling</Text>
          <Text style={styles.introSub}>Session 2 · Explorer</Text>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.resultTitle}>Great control!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeStar}>⭐</Text>
            <Text style={styles.resultBadgeText}>Boundary Star</Text>
          </View>
  ...
  return (
    <View style={styles.safe}>
      {step === 1 && <ScribbleInsideCircleGame currentStep={1} totalSteps={TOTAL_STEPS} onBack={goBack} onComplete={advance} />}
      {step === 2 && <ScribbleInsideTriangleGame currentStep={2} totalSteps={TOTAL_STEPS} onBack={goBack} onComplete={advance} />}
      {step === 3 && <BigSmallShapesFillGame currentStep={3} totalSteps={TOTAL_STEPS} onBack={goBack} onComplete={advance} />}
      {step === 4 && <BoundaryControlGame currentStep={4} totalSteps={TOTAL_STEPS} onBack={goBack} onComplete={advance} />}
      {step === 5 && <ScribbleBoundaryUploadTask currentStep={5} totalSteps={TOTAL_STEPS} onBack={goBack} onComplete={handleTaskComplete} />}
    </View>
  );
}
