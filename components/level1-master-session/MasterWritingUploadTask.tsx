/**
 * AI Task: Write A–Z clearly on paper and upload photo.
 * AI checks recognizability and sequence consistency.
...
export function MasterWritingUploadTask({
  ...
  const upload = async (uri: string) => {
    ...
    try {
      ...
      setResult({ success: ok, message: data.message || (ok ? 'SUCCESS' : 'TRY AGAIN'), feedback: data.feedback || (ok ? 'Perfect writing!' : 'Try again.') });
  ...
  return (
    <GameContainerGrip title="Master Task" currentStep={currentStep} totalSteps={totalSteps} mascot="📷" mascotHint={INSTRUCTION} onBack={onBack}>
...
const styles = StyleSheet.create({
  content: { flex: 1 },
  instruction: { fontSize: 18, fontWeight: '700', color: '#92400E', textAlign: 'center', marginBottom: 20, lineHeight: 26 },
  ...
  btnPri: { backgroundColor: '#92400E' },
  btnSec: { backgroundColor: '#FEF3C7' },
  btnText: { fontSize: 18, fontWeight: '800', color: '#FFF' },
  btnTextSec: { color: '#92400E' },
  pressed: { opacity: 0.9 },
  tryAgain: { marginTop: 12, alignSelf: 'center' },
  tryAgainText: { fontSize: 16, fontWeight: '700', color: '#92400E' },
});
