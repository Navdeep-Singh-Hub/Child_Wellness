/**
 * Level 5 Counter — Session 1, Real World Task: Place THREE objects in a straight line on a table.
 * Upload photo → AI verifies number of objects and alignment. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS = 'Place THREE objects in a straight line on a table.\n\nTake a photo or upload a picture showing three objects in a row.';

export function CounterNotebookUploadThreeInLine({
  onComplete,
}: {
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of three objects in a line on a table.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      if (correct) {
        ...
        speak('Try again. Place three objects in a straight line on a table.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Place three objects in a straight line on a table. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
