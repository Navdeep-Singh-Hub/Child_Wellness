/**
 * Counter Session 5 — Round Hunt Snapshot
 * Photograph something ROUND → AI verifies. Wrong answers retry in-game.
 */
...
export function RoundSnapshotUpload({
  ...
  useEffect(() => {
    speakCounterHint(
      'Round hunt! Find something round or circular, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(
    async (uri: string) => {
      setUploading(true);
      setMsg('Orbit is scanning for something round…');
      setMsgTone('info');
      setActiveStep(3);

      try {
        const formData = new FormData();
        const filename = 'round-object.jpg';
        ...
        const res = await fetch(`${API_BASE_URL}/api/upload-counter-s5-round-task`, {
          method: 'POST',
          headers,
          body: formData,
        });
        ...
        const feedback =
          data.feedback ||
          (correct ? 'SUCCESS! We see something round!' : 'Try again — show something round or circular.');
        ...
        if (correct) {
          ...
          speakCounterHint('Success! We see something round!');
          setCelebrating(true);
          ...
          setMsgTone('error');
          setActiveStep(2);
          speakCounterHint('Try again. Take a photo of something round.');
          try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to photograph something round.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakCounterHint('Find something round, then snap a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          title="Round Found!"
          subtitle="Something round verified!"
          badgeEmoji="⭕"
          variant="ocean"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Find <Text style={styles.highlight}>something round</Text> — a ball, plate, or circle — then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() => speakCounterHint('Find something round and take a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Round examples</Text>
          <Text style={styles.exampleEmoji}>⚽ 🍊 ⭕</Text>
          <Text style={styles.exampleHint}>Anything circular or round!</Text>
        </View>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[...T.doneGradient]} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>I Found Something Round!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
