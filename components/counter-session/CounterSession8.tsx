/**
 * Level 5 Counter — Session 8: Emotion Recognition, Category Match, Word Builder DOG, Shape Puzzle
 * 4 games + 1 real-world task (show something green). Ocean/sky theme.
 */
...
import { EmotionRecognitionCounterGame } from './EmotionRecognitionCounterGame';
import { CategoryMatchCounterGame } from './CategoryMatchCounterGame';
import { WordBuilderDogCounterGame } from './WordBuilderDogCounterGame';
import { ShapePuzzleCounterGame } from './ShapePuzzleCounterGame';
import { CounterNotebookUploadGreen } from './CounterNotebookUploadGreen';
...
interface CounterSession8Props {
  onExit?: () => void;
}

export function CounterSession8({ onExit }: CounterSession8Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '😢', title: 'Emotion Recognition', desc: 'Tap the SAD face' },
      { icon: '🐾', title: 'Category Match', desc: 'Match animals with habitat' },
      { icon: '🐕', title: 'Word Builder', desc: 'Build the word DOG' },
      { icon: '🧩', title: 'Shape Puzzle', desc: 'Fit shapes into holes' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Emotions, animals, words, and shapes!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Emotions, Match & Shapes</Text>
            <Text style={styles.introSub}>Level 5 · The Counter · Session 8</Text>
            <Text style={styles.introDesc}>
              Tap the sad face, match animals to their homes, build DOG, fit shapes into holes, then show something green!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Emotions, Match & Shapes</Text>
        </View>
        <Text style={styles.subtitle}>Level 5 · Counter · Session 8</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <EmotionRecognitionCounterGame onComplete={advance} />}
        {step === 2 && <CategoryMatchCounterGame onComplete={advance} />}
        {step === 3 && <WordBuilderDogCounterGame onComplete={advance} />}
        {step === 4 && <ShapePuzzleCounterGame onComplete={advance} />}
        {step === 5 && <CounterNotebookUploadGreen onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
