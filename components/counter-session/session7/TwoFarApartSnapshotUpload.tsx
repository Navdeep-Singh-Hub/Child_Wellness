/**
 * Counter Session 7 — Far Apart Snapshot
 * TWO objects far apart → photo → AI verifies. Wrong answers retry in-game.
 */
...
export function TwoFarApartSnapshotUpload({
  ...
  useEffect(() => {
    speakCounterHint(
      'Far apart mission! Place two objects with space between them, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(
    async (uri: string) => {
      setUploading(true);
      setMsg('Span is measuring the distance…');
      setMsgTone('info');
      setActiveStep(3);

      try {
        const formData = new FormData();
        const filename = 'two-far-apart.jpg';
        ...
        const res = await fetch(`${API_BASE_URL}/api/upload-counter-s7-two-far-apart-task`, {
          method: 'POST',
          headers,
          body: formData,
        });
        ...
        const feedback =
          data.feedback ||
          (correct ? 'SUCCESS! Two objects far apart!' : 'Try again — place two objects with space between them.');
        ...
        if (correct) {
          ...
          speakCounterHint('Success! We see two objects far apart!');
          setCelebrating(true);
          ...
          setMsgTone('error');
          setActiveStep(2);
          speakCounterHint('Try again. Place two objects with space between them.');
          try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to photograph two objects far apart.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakCounterHint('Place two objects far apart, then snap a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          title="Far Apart!"
          subtitle="Two objects with space verified!"
          badgeEmoji="📏"
          variant="ocean"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Place <Text style={styles.highlight}>two objects</Text> far apart with space between them, then photograph them.
            </Text>
          </View>
          <Pressable
            onPress={() => speakCounterHint('Place two objects far apart and take a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleHint}>Two things with clear space between!</Text>
        </View>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[...T.doneGradient]} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>Objects Are Far Apart!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  exampleEmoji: { fontSize: 22, textAlign: 'center', marginBottom: 4, letterSpacing: 2 },
