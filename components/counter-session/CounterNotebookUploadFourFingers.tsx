/**
 * Level 5 Counter — Session 6, Real World Task: Show FOUR fingers.
 * Upload photo → AI detects hand gesture (4 fingers). Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Show FOUR fingers.\n\nTake a photo or upload a picture showing your hand with four fingers up.';

export function CounterNotebookUploadFourFingers({
  onComplete,
}: {
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo showing four fingers.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see four fingers!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see four fingers!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Show four fingers in the photo.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show FOUR fingers. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
