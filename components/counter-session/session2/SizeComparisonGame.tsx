/**
 * Counter Session 2 — Game 2: Giant's Scale — tap the largest
 */
...
import { GIANT_SCALE_THEME as T, COUNTER_SESSION } from '../counterSessionTheme';
...
const OBJECTS = [
  { id: 'small', size: 'small' as const },
  { id: 'large', size: 'large' as const },
  { id: 'medium', size: 'medium' as const },
];
...
export function SizeComparisonGame({
  ...
  useEffect(() => {
    speakCounterHint('Which one is the largest? Tap the largest object.');
  ...
  const triggerWrong = useCallback(() => {
    ...
    speakCounterHint('Try again. Find the largest one!');
    try {
  ...
  const handleTap = useCallback(
    (id: string) => {
      if (id === CORRECT_ID) {
        speakCounterHint('Correct! That is the largest!');
        try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          title="Largest Found!"
          subtitle="You spotted the giant on the scale!"
          badgeEmoji="📏"
          variant="ocean"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <View style={styles.speechBubble}>
            ...
            <View style={styles.bubbleBody}>
              ...
              <Pressable onPress={() => speakCounterHint('Tap the largest circle.')}>
                <Text style={styles.prompt}>Which is biggest? 🔊</Text>
              </Pressable>
            </View>
          </View>
        </View>
        ...
        <View style={styles.scaleCard}>
          <Text style={styles.scaleLabel}>Compare sizes</Text>
          <Animated.View style={[styles.objectsRow, shakeStyle]}>
            {OBJECTS.map((obj) => (
              <Pressable
                key={obj.id}
...
const styles = StyleSheet.create({
  ...
  scaleLabel: { fontSize: 13, fontWeight: '800', color: T.inkMuted, marginBottom: 24, textTransform: 'uppercase' },
  objectsRow: { flexDirection: 'row', alignItems: 'flex-end', gap: 28, height: 120 },
  objBtn: {
    borderRadius: 999,
    borderWidth: 4,
    borderColor: T.objectBorder,
    backgroundColor: T.objectFill,
    alignItems: 'center',
    justifyContent: 'center',
  },
  small: { width: 52, height: 52 },
  medium: { width: 76, height: 76 },
