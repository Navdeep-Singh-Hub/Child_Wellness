/**
 * Counter Session 2 — Book Stack Snapshot
 * TWO books stacked → photo → AI verifies. Wrong answers retry in-game.
 */
...
import { BOOK_STACK_THEME as T, COUNTER_SESSION } from '../counterSessionTheme';
...
export function TwoBooksStackedUpload({
  onComplete,
  onBack,
  currentStep = 5,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakCounterHint(
      'Book stack mission! Show two books stacked on top of each other, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(
    async (uri: string) => {
      setUploading(true);
      setMsg('Librarian is checking your book stack…');
      setMsgTone('info');
      setActiveStep(3);

      try {
        const formData = new FormData();
        const filename = 'two-books-stacked.jpg';
        ...
        const res = await fetch(`${API_BASE_URL}/api/upload-counter-s2-two-books-stacked-task`, {
          method: 'POST',
          headers,
          body: formData,
        });
        ...
        const feedback =
          data.feedback || (correct ? 'SUCCESS! Two books stacked!' : 'Try again — stack two books.');
        ...
        if (correct) {
          ...
          speakCounterHint('Success! Two books stacked!');
          setCelebrating(true);
          ...
          setMsgTone('error');
          setActiveStep(2);
          speakCounterHint('Try again. Show two books stacked on top of each other.');
          try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of two books stacked.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakCounterHint('Stack two books on top of each other, then snap a photo.');
    try {
    ...
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          ...
          subtitle="Two books stacked perfectly!"
          badgeEmoji="📚"
          variant="ocean"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Stack <Text style={styles.highlight}>two books</Text> on top of each other, then photograph them.
            </Text>
          </View>
          <Pressable
            onPress={() => speakCounterHint('Stack two books and take a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Example stack</Text>
          <Text style={styles.exampleEmoji}>📕{'\n'}📗</Text>
          <Text style={styles.exampleHint}>One book on top of another</Text>
        </View>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[...T.doneGradient]} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>Books Are Stacked!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  exampleEmoji: { fontSize: 32, textAlign: 'center', lineHeight: 36, marginBottom: 4 },
