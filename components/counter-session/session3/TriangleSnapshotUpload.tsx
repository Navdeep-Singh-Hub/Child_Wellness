/**
 * Counter Session 3 — Triangle Hunt Snapshot
 * Photograph a triangle shape → AI verifies. Wrong answers retry in-game.
 */
...
import { TRIANGLE_HUNT_THEME as T, COUNTER_SESSION } from '../counterSessionTheme';
...
export function TriangleSnapshotUpload({
  ...
  useEffect(() => {
    speakCounterHint(
      'Triangle hunt! Find a triangle shape around you, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(
    async (uri: string) => {
      setUploading(true);
      setMsg('Hunter is scanning for a triangle shape…');
      setMsgTone('info');
      setActiveStep(3);

      try {
        const formData = new FormData();
        const filename = 'triangle-shape.jpg';
        ...
        const res = await fetch(`${API_BASE_URL}/api/upload-counter-s3-triangle-task`, {
          method: 'POST',
          headers,
          body: formData,
        });
        ...
        const feedback =
          data.feedback || (correct ? 'SUCCESS! We see a triangle!' : 'Try again — find a triangle shape.');
        ...
        if (correct) {
          ...
          speakCounterHint('Success! We see a triangle!');
          setCelebrating(true);
          ...
          setMsgTone('error');
          setActiveStep(2);
          speakCounterHint('Try again. Show a triangle shape you can find around you.');
          try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of a triangle shape.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakCounterHint('Find something triangular nearby, then snap a photo.');
    try {
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Find a <Text style={styles.highlight}>triangle shape</Text> around you and photograph it.
            </Text>
          </View>
          <Pressable onPress={() => speakCounterHint('Find a triangle and take a photo.')} style={styles.listenBtn}>
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Triangle examples</Text>
          <Text style={styles.exampleEmoji}>🔺  ⛰️  🚸</Text>
          <Text style={styles.exampleHint}>Signs, roofs, and pizza slices count!</Text>
        </View>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[...T.doneGradient]} style={styles.primaryBtn}>
                <Ionicons name="search-outline" size={20} color={T.inkDark} />
                <Text style={styles.primaryBtnTextDark}>I Found a Triangle!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
