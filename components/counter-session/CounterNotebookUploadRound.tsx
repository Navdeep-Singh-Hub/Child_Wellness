/**
 * Level 5 Counter — Session 5, Real World Task: Take a photo of something ROUND.
 * Upload photo → AI verifies circular/round object. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Take a photo of something ROUND.\n\nTake a photo or upload a picture of a circular or round object.';

export function CounterNotebookUploadRound({
  onComplete,
}: {
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of something round.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback = data.feedback || (correct ? 'SUCCESS! We see something round!' : 'TRY AGAIN.');
      setResult({ correct, feedback });
      if (correct) {
        speak('Success! We see something round!');
        setTimeout(() => onComplete(true), 2200);
      } else {
        speak('Try again. Take a photo of something round.');
        onComplete(false);
      }
    } catch (e: any) {
      setResult({
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Take a photo of something ROUND. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
              style={({ pressed }) => [
