/**
 * Counter Session 10 — Game 4: Pattern Peak — triangle, circle, triangle, circle, ?
 */
...
import { PATTERN_PEAK_THEME as T, COUNTER_SESSION } from '../counterSessionTheme';
...
export function LogicalPatternGame({
  onComplete,
  onBack,
  currentStep = 4,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakCounterHint('Complete the pattern. Triangle, circle, triangle, circle. What comes next?');
  ...
  const triggerWrong = useCallback(() => {
    ...
    speakCounterHint('Try again. Triangle, circle, triangle, circle...');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration title="Pattern Complete!" subtitle="Triangle comes next!" badgeEmoji="🔺" variant="ocean" />
      </View>
    );
  }
        ...
        <View style={styles.header}>
          ...
          <View style={styles.speechBubble}>
            ...
            <View style={styles.bubbleBody}>
              ...
              <Pressable
                onPress={() =>
                  speakCounterHint('Triangle, circle, triangle, circle. What shape comes next?')
                }
              >
                <Text style={styles.prompt}>Complete the pattern 🔊</Text>
              </Pressable>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  patternRow: { flexDirection: 'row', gap: 10, alignItems: 'center', marginTop: 24, marginBottom: 28, flexWrap: 'wrap', justifyContent: 'center' },
  patternBox: {
    width: 54,
    height: 54,
    borderRadius: 14,
  ...
  questionText: { fontSize: 26, fontWeight: '800', color: T.inkMuted },
  ...
  optionLabel: { fontSize: 14, fontWeight: '700', color: T.ink },
});
