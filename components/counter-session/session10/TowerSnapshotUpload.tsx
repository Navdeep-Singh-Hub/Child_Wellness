/**
 * Counter Session 10 — Tower Snapshot
 * Stack three objects in a tower → photo → AI verifies. Wrong answers retry in-game.
 */
...
export function TowerSnapshotUpload({
  ...
  useEffect(() => {
    speakCounterHint('Tower snapshot! Stack three objects in a tower, then take a photo.');
  ...
  const uploadImage = useCallback(
    async (uri: string) => {
      setUploading(true);
      setMsg('Stack is checking your tower…');
      setMsgTone('info');
      setActiveStep(3);

      try {
        const formData = new FormData();
        const filename = 'tower-3-objects.jpg';
        ...
        const res = await fetch(`${API_BASE_URL}/api/upload-counter-s10-tower-3-task`, {
          method: 'POST',
          headers,
          body: formData,
        });
        ...
        const feedback =
          data.feedback ||
          (correct ? 'SUCCESS! We see a 3-object tower!' : 'Try again — stack three objects in a tower.');
        ...
        if (correct) {
          ...
          speakCounterHint('Success! Great tower!');
          setCelebrating(true);
          ...
          setMsgTone('error');
          setActiveStep(2);
          speakCounterHint('Try again. Stack three objects into a small tower.');
          try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to photograph a 3-object tower.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakCounterHint('Stack three objects in a tower, then snap a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          title="Tower Built!"
          subtitle="Three-object tower verified!"
          badgeEmoji="🗼"
          variant="ocean"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Stack <Text style={styles.highlight}>three objects</Text> on top of each other, then photograph your tower.
            </Text>
          </View>
          <Pressable
            onPress={() => speakCounterHint('Stack three objects in a tower and take a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Tower example</Text>
          <Text style={styles.exampleEmoji}>📦{'\n'}📦{'\n'}📦</Text>
          <Text style={styles.exampleHint}>One on top of the other!</Text>
        </View>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[...T.doneGradient]} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>I Built a Tower!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  exampleEmoji: { fontSize: 22, textAlign: 'center', marginBottom: 4, lineHeight: 28 },
