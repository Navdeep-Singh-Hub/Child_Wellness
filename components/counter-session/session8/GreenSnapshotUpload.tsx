/**
 * Counter Session 8 — Green Hunt Snapshot
 * Show something GREEN → photo → AI verifies. Wrong answers retry in-game.
 */
...
export function GreenSnapshotUpload({
  ...
  useEffect(() => {
    speakCounterHint('Green hunt! Find something green, then take or upload a photo.');
  ...
  const uploadImage = useCallback(
    async (uri: string) => {
      setUploading(true);
      setMsg('Leaf is checking for green…');
      setMsgTone('info');
      setActiveStep(3);

      try {
        const formData = new FormData();
        const filename = 'green-object.jpg';
        ...
        const res = await fetch(`${API_BASE_URL}/api/upload-counter-s8-green-task`, {
          method: 'POST',
          headers,
          body: formData,
        });
        ...
        const feedback =
          data.feedback ||
          (correct ? 'SUCCESS! We see something green!' : 'Try again — show something green in the photo.');
        ...
        if (correct) {
          ...
          speakCounterHint('Success! We see something green!');
          setCelebrating(true);
          ...
          setMsgTone('error');
          setActiveStep(2);
          speakCounterHint('Try again. Show something green in the photo.');
          try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to photograph something green.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakCounterHint('Find something green, then snap a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          title="Green Found!"
          subtitle="Something green verified!"
          badgeEmoji="🟢"
          variant="ocean"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Find <Text style={styles.highlight}>something green</Text> — a leaf, toy, or cup — then photograph it.
            </Text>
          </View>
          <Pressable
            onPress={() => speakCounterHint('Find something green and take a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Green examples</Text>
          <Text style={styles.exampleEmoji}>🍃 🥒 🟢</Text>
          <Text style={styles.exampleHint}>Anything that is green!</Text>
        </View>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[...T.doneGradient]} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>I Found Something Green!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
