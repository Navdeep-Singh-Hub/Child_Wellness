/**
 * Level 5 Counter — Session 3: Shape Rotation, Sound Recognition, Memory (8 cards), Drag Parts House
 * 4 games + 1 real-world task (show a triangle shape). Ocean/sky theme.
 */
...
import { ShapeRotationCounterGame } from './ShapeRotationCounterGame';
import { SoundRecognitionCounterGame } from './SoundRecognitionCounterGame';
import { Memory8CardsCounterGame } from './Memory8CardsCounterGame';
import { DragPartsHouseCounterGame } from './DragPartsHouseCounterGame';
import { CounterNotebookUploadTriangle } from './CounterNotebookUploadTriangle';
...
interface CounterSession3Props {
  onExit?: () => void;
}

export function CounterSession3({ onExit }: CounterSession3Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔺', title: 'Shape Rotation', desc: 'Select the rotated triangle' },
      { icon: '🔊', title: 'Sound Recognition', desc: 'Match animal with sound' },
      { icon: '🎴', title: 'Memory Game', desc: 'Match 8 cards' },
      { icon: '🏠', title: 'Assemble House', desc: 'Roof, walls, door' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Shapes, sounds, memory, and build a house!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Shapes & Sounds</Text>
            <Text style={styles.introSub}>Level 5 · The Counter · Session 3</Text>
            <Text style={styles.introDesc}>Find the rotated triangle, match animal sounds, play memory with 8 cards, assemble a house, then show a triangle shape!</Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Session 3 Complete</Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Shapes & Sounds</Text>
        </View>
        <Text style={styles.subtitle}>Level 5 · Counter · Session 3</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <ShapeRotationCounterGame onComplete={advance} />}
        {step === 2 && <SoundRecognitionCounterGame onComplete={advance} />}
        {step === 3 && <Memory8CardsCounterGame onComplete={advance} />}
        {step === 4 && <DragPartsHouseCounterGame onComplete={advance} />}
        {step === 5 && <CounterNotebookUploadTriangle onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
