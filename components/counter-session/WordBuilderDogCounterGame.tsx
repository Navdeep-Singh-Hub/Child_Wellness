/**
 * Level 5 Counter — Session 8, Game 3: Word Builder
 * Letters D, O, G — user taps letters in order to build "DOG".
 */
...
export interface WordBuilderDogCounterGameProps {
  onComplete: () => void;
}

export function WordBuilderDogCounterGame({ onComplete }: WordBuilderDogCounterGameProps) {
  const [nextIndex, setNextIndex] = useState(0);
  const [showSuccess, setShowSuccess] = useState(false);
  const [wrongShake] = useState(() => new Animated.Value(0));

  useEffect(() => {
    speak('Build the word DOG. Tap the letters in order: D, O, G.', 0.75);
  }, []);
  ...
  const triggerWrong = useCallback(() => {
    ...
    speak('Try again. Tap D, then O, then G.', 0.7);
  }, [wrongShake]);
  ...
  return (
    <GameLayout
      title="Word Builder"
      instruction="Tap the letters in order to spell DOG."
      icon="🐕"
      backgroundVariant="ocean"
    >
      <View style={styles.container}>
        <Text style={styles.wordLabel}>Build: DOG</Text>
...
const styles = StyleSheet.create({
  ...
  slotLetter: { fontSize: 28, fontWeight: '800', color: '#0C4A6E' },
  ...
  letterText: { fontSize: 32, fontWeight: '800', color: '#0C4A6E' },
