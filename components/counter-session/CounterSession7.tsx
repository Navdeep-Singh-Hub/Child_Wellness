/**
 * Level 5 Counter — Session 7: Pattern Memory, Bigger or Smaller, Letter Match, Build Car
 * 4 games + 1 real-world task (place two objects far apart). Ocean/sky theme.
 */
...
import { PatternMemoryCounterGame } from './PatternMemoryCounterGame';
import { BiggerOrSmallerCounterGame } from './BiggerOrSmallerCounterGame';
import { LetterMatchCounterGame } from './LetterMatchCounterGame';
import { BuildCarCounterGame } from './BuildCarCounterGame';
import { CounterNotebookUploadTwoFarApart } from './CounterNotebookUploadTwoFarApart';
...
interface CounterSession7Props {
  onExit?: () => void;
}

export function CounterSession7({ onExit }: CounterSession7Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧠', title: 'Pattern Memory', desc: 'Remember then repeat' },
      { icon: '⚖️', title: 'Bigger or Smaller', desc: 'Tap the smaller one' },
      { icon: '🔤', title: 'Letter Match', desc: 'Match A with a' },
      { icon: '🚗', title: 'Build the Car', desc: 'Drag parts to build' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Memory, size, letters, and build a car!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Pattern, Match & Build</Text>
            <Text style={styles.introSub}>Level 5 · The Counter · Session 7</Text>
            <Text style={styles.introDesc}>
              Remember the pattern, tap the smaller object, match uppercase and lowercase, build the car, then place two objects far apart!
            </Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Session 7 Complete</Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Pattern, Match & Build</Text>
        </View>
        <Text style={styles.subtitle}>Level 5 · Counter · Session 7</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <PatternMemoryCounterGame onComplete={advance} />}
        {step === 2 && <BiggerOrSmallerCounterGame onComplete={advance} />}
        {step === 3 && <LetterMatchCounterGame onComplete={advance} />}
        {step === 4 && <BuildCarCounterGame onComplete={advance} />}
        {step === 5 && <CounterNotebookUploadTwoFarApart onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
