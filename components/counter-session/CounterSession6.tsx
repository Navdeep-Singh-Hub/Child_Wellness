/**
 * Level 5 Counter — Session 6: Spot the Difference, Counting Objects, Drag Puzzle, Shape Identification
 * 4 games + 1 real-world task (show four fingers). Ocean/sky theme.
 */
...
import { SpotTheDifferenceCounterGame } from './SpotTheDifferenceCounterGame';
import { CountingObjectsCounterGame } from './CountingObjectsCounterGame';
import { DragPuzzleCounterGame } from './DragPuzzleCounterGame';
import { ShapeIdentificationCounterGame } from './ShapeIdentificationCounterGame';
import { CounterNotebookUploadFourFingers } from './CounterNotebookUploadFourFingers';
...
interface CounterSession6Props {
  onExit?: () => void;
}

export function CounterSession6({ onExit }: CounterSession6Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔍', title: 'Spot the Difference', desc: 'Find 3 differences' },
      { icon: '🍎', title: 'Counting Objects', desc: 'Count 9 apples' },
      { icon: '🐕', title: 'Animal Puzzle', desc: 'Complete the dog puzzle' },
      { icon: '▭', title: 'Shape Identification', desc: 'Tap the rectangle' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Differences, counting, puzzle, and shapes!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Spot, Count & Shapes</Text>
            <Text style={styles.introSub}>Level 5 · The Counter · Session 6</Text>
            <Text style={styles.introDesc}>
              Find 3 differences, count 9 apples, complete the animal puzzle, tap the rectangle, then show four fingers!
            </Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Session 6 Complete</Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Spot, Count & Shapes</Text>
        </View>
        <Text style={styles.subtitle}>Level 5 · Counter · Session 6</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <SpotTheDifferenceCounterGame onComplete={advance} />}
        {step === 2 && <CountingObjectsCounterGame onComplete={advance} />}
        {step === 3 && <DragPuzzleCounterGame onComplete={advance} />}
        {step === 4 && <ShapeIdentificationCounterGame onComplete={advance} />}
        {step === 5 && <CounterNotebookUploadFourFingers onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
