/**
 * Level 5 Counter — Session 2: Counting Challenge, Size Comparison, Drag Sequence, Picture Logic
 * 4 games + 1 real-world task (two books stacked). Ocean/sky theme.
 */
...
import { CountingChallengeCounterGame } from './CountingChallengeCounterGame';
import { SizeComparisonCounterGame } from './SizeComparisonCounterGame';
import { DragSequenceCounterGame } from './DragSequenceCounterGame';
import { PictureLogicCounterGame } from './PictureLogicCounterGame';
import { CounterNotebookUploadTwoBooksStacked } from './CounterNotebookUploadTwoBooksStacked';
...
export function CounterSession2({ onExit }: CounterSession2Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '⭐', title: 'Counting Challenge', desc: 'Count 7 stars' },
      { icon: '📏', title: 'Size Comparison', desc: 'Select the largest' },
      { icon: '🔢', title: 'Number Sequence', desc: 'Arrange 4, 5, 6, 7' },
      { icon: '🧠', title: 'Picture Logic', desc: 'Which does NOT belong?' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Count, compare, order, and find the odd one out!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Count & Compare</Text>
            <Text style={styles.introSub}>Level 5 · The Counter · Session 2</Text>
            <Text style={styles.introDesc}>Count stars, find the largest, order numbers, pick the one that doesn't belong, then show two books stacked!</Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Count & Compare</Text>
        </View>
        <Text style={styles.subtitle}>Level 5 · Counter · Session 2</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <CountingChallengeCounterGame onComplete={advance} />}
        {step === 2 && <SizeComparisonCounterGame onComplete={advance} />}
        {step === 3 && <DragSequenceCounterGame onComplete={advance} />}
        {step === 4 && <PictureLogicCounterGame onComplete={advance} />}
        {step === 5 && <CounterNotebookUploadTwoBooksStacked onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
