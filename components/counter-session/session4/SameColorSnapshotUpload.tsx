/**
 * Counter Session 4 — Same Color Snapshot
 * THREE objects of the SAME color → photo → AI verifies. Wrong answers retry in-game.
 */
...
export function SameColorSnapshotUpload({
  ...
  useEffect(() => {
    speakCounterHint(
      'Same color mission! Find three objects that are the same color, then take or upload a photo.'
    );
  ...
  const uploadImage = useCallback(
    async (uri: string) => {
      setUploading(true);
      setMsg('Chroma is checking your colors…');
      setMsgTone('info');
      setActiveStep(3);

      try {
        const formData = new FormData();
        const filename = 'same-color-objects.jpg';
        ...
        const res = await fetch(`${API_BASE_URL}/api/upload-counter-s4-same-color-task`, {
          method: 'POST',
          headers,
          body: formData,
        });
        ...
        const feedback =
          data.feedback ||
          (correct ? 'SUCCESS! Three same-color objects!' : 'Try again — show three objects of the same color.');
        ...
        if (correct) {
          ...
          speakCounterHint('Success! Three objects of the same color!');
          setCelebrating(true);
          ...
          setMsgTone('error');
          setActiveStep(2);
          speakCounterHint('Try again. Find three objects that are the same color.');
          try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to photograph three same-color objects.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakCounterHint('Gather three objects of the same color, then snap a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        ...
        <SuccessCelebration
          title="Color Match!"
          subtitle="Three same-color objects verified!"
          badgeEmoji="🎨"
          variant="ocean"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Find <Text style={styles.highlight}>three objects</Text> that are the same color, then photograph them.
            </Text>
          </View>
          <Pressable
            onPress={() => speakCounterHint('Find three same-color objects and take a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Example set</Text>
          <Text style={styles.exampleEmoji}>🔴 🔴 🔴</Text>
          <Text style={styles.exampleHint}>Three red things — or any matching color!</Text>
        </View>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[...T.doneGradient]} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>Objects Are Ready!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  exampleEmoji: { fontSize: 28, textAlign: 'center', marginBottom: 4, letterSpacing: 8 },
