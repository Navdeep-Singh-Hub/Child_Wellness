/**
 * Counter Session 4 — Game 3: Number Nest — tap 8, 9, 10 in order
 */
...
import { NUMBER_NEST_THEME as T, COUNTER_SESSION } from '../counterSessionTheme';
...
export function NumberPuzzleGame({
  ...
  useEffect(() => {
    speakCounterHint('Tap the numbers in order: 8, 9, 10.');
  ...
  const triggerWrong = useCallback(() => {
    ...
    speakCounterHint('Try again. Tap 8, then 9, then 10.');
    try {
    ...
    (num: number) => {
      ...
      if (num === expected) {
        ...
        if (newIndex >= SEQUENCE.length) {
          speakCounterHint('Correct! 8, 9, 10!');
          try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration title="Nest Complete!" subtitle="You ordered 8, 9, 10!" badgeEmoji="🔢" variant="ocean" />
      </View>
    );
  }
        ...
        <View style={styles.header}>
          ...
          <View style={styles.speechBubble}>
            ...
            <View style={styles.bubbleBody}>
              ...
              <Pressable onPress={() => speakCounterHint('Tap eight, nine, ten in order.')}>
                <Text style={styles.prompt}>Fill the nest 🔊</Text>
              </Pressable>
            </View>
          </View>
        </View>
        <View style={styles.nestCard}>
          <Text style={styles.nestLabel}>Order: 8 → 9 → 10</Text>
...
const styles = StyleSheet.create({
  ...
  nestLabel: { fontSize: 14, fontWeight: '800', color: T.inkMuted, marginBottom: 16 },
  ...
  numbersRow: { flexDirection: 'row', justifyContent: 'center', gap: 16, paddingHorizontal: 20 },
  numBtn: {
    width: 64,
    height: 64,
    borderRadius: 18,
