/**
 * Level 5 Counter — Session 5, Game 3: Word Builder
 * Letters S, U, N — user taps letters in order to build "SUN".
 */
...
export interface WordBuilderSunCounterGameProps {
  onComplete: () => void;
}

export function WordBuilderSunCounterGame({ onComplete }: WordBuilderSunCounterGameProps) {
  const [nextIndex, setNextIndex] = useState(0);
  const [showSuccess, setShowSuccess] = useState(false);
  const [wrongShake] = useState(() => new Animated.Value(0));

  useEffect(() => {
    speak('Build the word SUN. Tap the letters in order: S, U, N.', 0.75);
  }, []);
  ...
  const triggerWrong = useCallback(() => {
    ...
    speak('Try again. Tap S, then U, then N.', 0.7);
  }, [wrongShake]);
