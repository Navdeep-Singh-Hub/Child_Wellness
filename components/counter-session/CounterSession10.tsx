/**
 * Level 5 Counter — Session 10 (Final): Mixed Quiz, Advanced Memory, Word Builder HOUSE, Logical Pattern
 * 4 games + 1 real-world task (build a 3-object tower). Ocean/sky theme.
 */
...
import { MixedQuizCounterFinalGame } from './MixedQuizCounterFinalGame';
import { AdvancedMemory12CounterGame } from './AdvancedMemory12CounterGame';
import { WordBuilderHouseCounterGame } from './WordBuilderHouseCounterGame';
import { LogicalPatternCounterFinalGame } from './LogicalPatternCounterFinalGame';
import { CounterNotebookUploadTower3 } from './CounterNotebookUploadTower3';
...
interface CounterSession10Props {
  onExit?: () => void;
}

export function CounterSession10({ onExit }: CounterSession10Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '❓', title: 'Mixed Quiz', desc: 'Shapes and colors' },
      { icon: '🎴', title: 'Advanced Memory', desc: 'Match 12 cards' },
      { icon: '🏠', title: 'Word Builder', desc: 'Build HOUSE' },
      { icon: '🧩', title: 'Logical Pattern', desc: 'Triangle, circle, ...' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Final challenge! Quiz, memory, words, and patterns.</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Counter Master (Final)</Text>
            <Text style={styles.introSub}>Level 5 · The Counter · Session 10</Text>
            <Text style={styles.introDesc}>
              Identify shapes and colors, match 12 cards, build HOUSE, complete the pattern, then build a 3-object tower!
            </Text>
          </View>
    ...
    if (taskCorrect === true) {
      return (
        <SuccessCelebration
          ...
          subtitle="You completed all Counter sessions! Next level unlocked!"
          badgeEmoji="🏆"
        />
      );
    }
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Counter Master</Text>
        </View>
        <Text style={styles.subtitle}>Level 5 · Counter · Session 10</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <MixedQuizCounterFinalGame onComplete={advance} />}
        {step === 2 && <AdvancedMemory12CounterGame onComplete={advance} />}
        {step === 3 && <WordBuilderHouseCounterGame onComplete={advance} />}
        {step === 4 && <LogicalPatternCounterFinalGame onComplete={advance} />}
        {step === 5 && <CounterNotebookUploadTower3 onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
