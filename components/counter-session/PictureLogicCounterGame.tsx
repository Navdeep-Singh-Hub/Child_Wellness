/**
 * Level 5 Counter — Session 2, Game 4: Picture Logic
 * Select the object that does NOT belong: apple, banana, car. Correct: car (not a fruit).
 */
...
const CORRECT_ID = 'car'; // does not belong (not a fruit)

export interface PictureLogicCounterGameProps {
  onComplete: () => void;
}

export function PictureLogicCounterGame({ onComplete }: PictureLogicCounterGameProps) {
  const [showSuccess, setShowSuccess] = useState(false);
  const [wrongShake] = useState(() => new Animated.Value(0));

  useEffect(() => {
    speak('Which one does NOT belong? Apple, banana, car. Tap the one that is different.', 0.75);
  }, []);
