/**
 * Counter Session 9 — Triangle Snapshot
 * Arrange three objects in a triangle → photo → AI verifies. Wrong answers retry in-game.
 */
...
export function TriangleSnapshotUpload({
  ...
  useEffect(() => {
    speakCounterHint('Triangle snapshot! Arrange three objects in a triangle, then take a photo.');
  ...
  const uploadImage = useCallback(
    async (uri: string) => {
      setUploading(true);
      setMsg('Vertex is checking your triangle…');
      setMsgTone('info');
      setActiveStep(3);

      try {
        const formData = new FormData();
        const filename = 'triangle-layout.jpg';
        ...
        const res = await fetch(`${API_BASE_URL}/api/upload-counter-s9-triangle-layout-task`, {
          method: 'POST',
          headers,
          body: formData,
        });
        ...
        const feedback =
          data.feedback ||
          (correct ? 'SUCCESS! We see three objects in a triangle!' : 'Try again — arrange three objects in a triangle.');
        ...
        if (correct) {
          ...
          speakCounterHint('Success! We see three objects in a triangle!');
          setCelebrating(true);
          ...
          setMsgTone('error');
          setActiveStep(2);
          speakCounterHint('Try again. Arrange three objects in a triangle shape.');
          try {
  ...
  const takePhoto = useCallback(async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to photograph three objects in a triangle.');
      return;
    }
  ...
  const handleReady = () => {
    setActiveStep(1);
    speakCounterHint('Arrange three objects in a triangle, then snap a photo.');
    try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration
          title="Triangle Found!"
          subtitle="Three objects in a triangle verified!"
          badgeEmoji="🔺"
          variant="ocean"
        />
      </View>
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.missionCard}>
          ...
          <View style={styles.missionBody}>
            ...
            <Text style={styles.missionText}>
              Arrange <Text style={styles.highlight}>three objects</Text> in a triangle shape, then photograph them.
            </Text>
          </View>
          <Pressable
            onPress={() => speakCounterHint('Arrange three objects in a triangle and take a photo.')}
        ...
        <View style={styles.exampleCard}>
          ...
          <Text style={styles.exampleTitle}>Triangle layout</Text>
          <Text style={styles.exampleEmoji}>🔺</Text>
          <Text style={styles.exampleHint}>One object at each corner!</Text>
        </View>
          ...
          {activeStep === 0 ? (
            <Pressable onPress={handleReady} style={({ pressed }) => [pressed && styles.pressed]}>
              <LinearGradient colors={[...T.doneGradient]} style={styles.primaryBtn}>
                ...
                <Text style={styles.primaryBtnTextDark}>I Arranged a Triangle!</Text>
              </LinearGradient>
            </Pressable>
          ) : (
            <>
...
const styles = StyleSheet.create({
  ...
  exampleEmoji: { fontSize: 36, textAlign: 'center', marginBottom: 4 },
