/**
 * Counter Session 9 — Game 4: Pentagon Pick — tap the pentagon
 */
...
import { PENTAGON_PICK_THEME as T, COUNTER_SESSION } from '../counterSessionTheme';
...
export function IdentifyShapeGame({
  onComplete,
  onBack,
  currentStep = 4,
  totalSteps = 5,
  ...
  useEffect(() => {
    speakCounterHint('Tap the pentagon. Find the shape with five sides.');
  ...
  const triggerWrong = useCallback(() => {
    ...
    speakCounterHint('Try again. A pentagon has five sides.');
    try {
  ...
  const handleTap = useCallback(
    (id: string) => {
      if (id === CORRECT_ID) {
        speakCounterHint('Correct! That is the pentagon!');
        try {
  ...
  if (celebrating) {
    return (
      <View style={styles.root}>
        <ConfettiEffect />
        <SuccessCelebration title="Pentagon Found!" subtitle="You found the five-sided shape!" badgeEmoji="⬠" variant="ocean" />
      </View>
    );
  }
        ...
        <View style={styles.header}>
          ...
          <View style={styles.speechBubble}>
            ...
            <View style={styles.bubbleBody}>
              ...
              <Pressable onPress={() => speakCounterHint('Tap the pentagon with five sides.')}>
...
const styles = StyleSheet.create({
  ...
  shapesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 14, justifyContent: 'center', marginTop: 24 },
  shapeBtn: {
    minWidth: 88,
    paddingVertical: 20,
  ...
  shapeLabel: { fontSize: 14, fontWeight: '700', color: T.ink },
});
