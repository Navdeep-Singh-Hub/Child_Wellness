/**
 * Level 5 Counter — Session 5: Direction Game, Memory Advanced, Word Builder SUN, Object Classification
 * 4 games + 1 real-world task (photo of something round). Ocean/sky theme.
 */
...
import { DirectionGameCounterGame } from './DirectionGameCounterGame';
import { MemoryAdvancedCounterGame } from './MemoryAdvancedCounterGame';
import { WordBuilderSunCounterGame } from './WordBuilderSunCounterGame';
import { ObjectClassificationCounterGame } from './ObjectClassificationCounterGame';
import { CounterNotebookUploadRound } from './CounterNotebookUploadRound';
...
interface CounterSession5Props {
  onExit?: () => void;
}

export function CounterSession5({ onExit }: CounterSession5Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧭', title: 'Direction Game', desc: 'Move LEFT, RIGHT, UP, DOWN' },
      { icon: '🎴', title: 'Memory Advanced', desc: 'Match 8 cards' },
      { icon: '☀️', title: 'Word Builder', desc: 'Build the word SUN' },
      { icon: '🏠', title: 'Object Classification', desc: 'Kitchen / Bedroom' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Directions, memory, words, and sorting!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Directions & Round</Text>
            <Text style={styles.introSub}>Level 5 · The Counter · Session 5</Text>
            <Text style={styles.introDesc}>
              Move the character with arrows, match 8 cards, build SUN, sort Kitchen and Bedroom, then take a photo of something round!
            </Text>
          </View>
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.resultBadge, { borderColor: DESIGN.primary, backgroundColor: DESIGN.primaryLight }]}>
            ...
            <Text style={styles.resultBadgeText}>Session 5 Complete</Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Directions & Round</Text>
        </View>
        <Text style={styles.subtitle}>Level 5 · Counter · Session 5</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <DirectionGameCounterGame onComplete={advance} />}
        {step === 2 && <MemoryAdvancedCounterGame onComplete={advance} />}
        {step === 3 && <WordBuilderSunCounterGame onComplete={advance} />}
        {step === 4 && <ObjectClassificationCounterGame onComplete={advance} />}
        {step === 5 && <CounterNotebookUploadRound onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
