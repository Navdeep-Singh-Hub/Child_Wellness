/**
 * Rhyme Match — Game 2: Find the rhyme (cat → bat)
 * AAC-friendly: large buttons, clear prompt.
 */
...
const PROMPT_WORD = 'cat';
const CHOICES = [
  { id: 'bat', label: 'bat', correct: true, image: 'https://placehold.co/120x120/E5E7EB/374151?text=bat' },
  { id: 'sun', label: 'sun', correct: false, image: 'https://placehold.co/120x120/FEF9C3/854D0E?text=sun' },
  { id: 'pen', label: 'pen', correct: false, image: 'https://placehold.co/120x120/DBEAFE/1E40AF?text=pen' },
].sort(() => Math.random() - 0.5);

export function RhymeMatch({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    speak(`Find a word that rhymes with ${PROMPT_WORD}.`, 0.75);
  }, []);
  ...
  return (
    <GameLayout
      ...
      instruction={`Find a word that rhymes with ${PROMPT_WORD}.`}
    >
