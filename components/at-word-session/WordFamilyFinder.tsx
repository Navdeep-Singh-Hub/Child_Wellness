/**
 * Word Family Finder — Game 1: Find the -AT words
 * Tap cat, bat, hat, rat (correct); dog, pen (wrong). AAC-friendly, large targets.
 */
...
const CORRECT = [
  ...
  { id: 'rat', label: 'rat', image: 'https://placehold.co/140x140/E5E7EB/4B5563?text=rat' },
];
const WRONG = [
  { id: 'dog', label: 'dog', image: 'https://placehold.co/140x140/FDE68A/92400E?text=dog' },
  { id: 'pen', label: 'pen', image: 'https://placehold.co/140x140/DBEAFE/1E40AF?text=pen' },
];
const ALL = [...CORRECT, ...WRONG].sort(() => Math.random() - 0.5);

export function WordFamilyFinder({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    speak('Tap the words that belong to the -AT family.', 0.75);
  }, []);
  ...
  return (
    <GameLayout
      title="Find the -AT words"
      instruction="Tap the words that belong to the -AT family."
    >
