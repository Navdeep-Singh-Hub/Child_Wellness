/**
 * AT Word Session 1 — Level 4 (Grouper) Session 1: The -AT Word Family
...
import { WordFamilyFinder } from '@/components/WordFamilyFinderAT';
import { RhymeMatch } from '@/components/RhymeMatchAT';
import { ClapWord } from '@/components/ClapWordAT';
import { ObjectSorting } from '@/components/ObjectSortingAT';
import { ATWordNotebookUpload } from '@/components/ATWordNotebookUpload';
...
interface ATWordSession1Props {
  onExit?: () => void;
}

export function ATWordSession1({ onExit }: ATWordSession1Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔍', title: 'Find the -AT words', desc: 'Word family' },
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>The -AT Family</Text>
            <Text style={styles.introSub}>
              Let's learn words that sound the same at the end.
            </Text>
          </View>
          <View style={styles.exampleBox}>
            <Text style={styles.exampleTitle}>Example words:</Text>
            <View style={styles.exampleWordsRow}>
              <Text style={styles.exampleWord}>cat</Text>
              <Text style={styles.exampleWord}>bat</Text>
              <Text style={styles.exampleWord}>hat</Text>
              <Text style={styles.exampleWord}>rat</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning the -AT family!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>AT Family Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        <Text style={styles.title}>The -AT Family</Text>
        <Text style={styles.subtitle}>Session 1</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <WordFamilyFinder onComplete={advance} />}
        {step === 2 && <RhymeMatch onComplete={advance} />}
        {step === 3 && <ClapWord onComplete={advance} />}
        {step === 4 && <ObjectSorting onComplete={advance} />}
        {step === 5 && <ATWordNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
