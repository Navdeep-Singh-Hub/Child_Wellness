/**
 * Notebook Task — Write 3 lines of dialogue. Upload photo; AI check: writing_detected, three_lines_detected, dialogue_detected. Session 8: Dialogue Builder.
 */
...
const VOICE =
  'Write 3 lines of dialogue in your notebook. Example: Hello, How are you?, I am fine. Then upload or take a photo.';
const PALETTE = { accent: '#2563EB', glow: '#93C5FD', secondary: '#3B82F6' } as const;

const EXAMPLE_LINES = [
  { num: '1', text: 'Hello' },
  { num: '2', text: 'How are you?' },
  { num: '3', text: 'I am fine.' },
] as const;
...
function DialogueSketch() {
  ...
  return (
    <View style={sketch.wrap}>
      <Text style={sketch.label}>TARGET DIALOGUE</Text>
      <Animated.View style={[sketch.glow, glow]} />
      <View style={sketch.frame}>
        <View style={sketch.sceneRow}>
          <Text style={sketch.sceneEmoji}>💬</Text>
          <Text style={sketch.sceneHint}>hello conversation</Text>
        </View>
        <View style={sketch.dialoguePlate}>
          {EXAMPLE_LINES.map((line, i) => (
            <View key={line.num}>
              {i > 0 && <View style={sketch.divider} />}
              <View style={sketch.lineRow}>
                <Text style={sketch.lineNum}>{line.num}</Text>
                <Text style={sketch.lineText}>{line.text}</Text>
              </View>
            </View>
          ))}
        </View>
        <Text style={sketch.caption}>three lines · greeting and reply</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ three lines</Text>
...
export function DialogueBuilderNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-graduate-dialogue-builder-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const threeLinesOk = data.three_lines_detected ?? true;
      const dialogueOk = data.dialogue_detected ?? true;
      const correct = data.correct ?? (writingOk && threeLinesOk && dialogueOk);
      const feedback =
        data.feedback ||
        (correct ? 'Great job!' : "Let's try again! Write three clear dialogue lines.");

      setResult({
        correct,
        feedback,
        writing_detected: data.writing_detected,
        three_lines_detected: data.three_lines_detected,
        dialogue_detected: data.dialogue_detected,
      });
  ...
  const coachLine = uploading
    ? 'Scanning your notebook for three dialogue lines…'
    : result?.correct
      ? 'Verified! Your dialogue looks great.'
      : result
        ? 'Write three clear lines — Hello, How are you?, I am fine.'
        : 'Write Hello, then How are you?, then I am fine.';

  return (
    <GraduateGameShell
      studio="DIALOGUE LEDGER · MISSION"
      title="Notebook proof"
      instruction="Write 3 lines of dialogue in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
      onReplayVoice={playVoice}
    >
      <View style={styles.badge}>
        <Text style={styles.badgeTxt}>SESSION 8 · NOTEBOOK TASK</Text>
      </View>
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <Text style={styles.resultTitle}>Dialogue scan in progress…</Text>
              <CheckRow label="Writing detected" pending />
              <CheckRow label="Three lines detected" pending />
              <CheckRow label="Dialogue detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Dialogue verified!' : 'Adjust your writing'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Writing detected" ok={result.writing_detected ?? result.correct} />
              <CheckRow label="Three lines detected" ok={result.three_lines_detected ?? result.correct} />
              <CheckRow label="Dialogue detected" ok={result.dialogue_detected ?? result.correct} />
              {!result.correct && (
...
const sketch = StyleSheet.create({
  ...
  lineNum: { fontSize: 14, fontWeight: '900', color: PALETTE.glow, width: 20 },
  lineText: { fontSize: 16, fontWeight: '800', color: GR.textLight, flex: 1 },
  divider: { height: 1, backgroundColor: `${PALETTE.glow}44`, marginVertical: 4 },
...
const check = StyleSheet.create({
  ...
  iconPending: { backgroundColor: 'rgba(147,197,253,0.15)' },
