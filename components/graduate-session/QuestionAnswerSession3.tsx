 ...
 * Flow: Intro → Game 1 (Question Choice) → Game 2 (Dialogue Complete) → Game 3 (Word Problem) → Game 4 (Logic Sorting) → Notebook → Result
...
export function QuestionAnswerSession3({ onExit }: QuestionAnswerSession3Props = {}) {
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Question & Answer</Text>
            <Text style={styles.introSub}>Let's answer questions and solve simple problems.</Text>
            <Text style={styles.introSkills}>Questions · Dialogue · Subtraction · Sorting</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '❓', title: 'Choose the correct answer', desc: 'What do we drink?' },
              { icon: '💬', title: 'Finish the conversation', desc: 'Mom: Eat your food' },
              { icon: '🍬', title: 'Solve the story', desc: '5 candies, eat 2. How many left?' },
              { icon: '📂', title: 'Sort the pictures', desc: 'Food or Animals?' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job answering questions!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Question Solver</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {step === 5 && <QuestionAnswerNotebookUpload onComplete={handleNotebookComplete} />}
    </SafeAreaView>
  );
}
