/**
 * Final Notebook Task — Write a small story with numbers. Upload photo; AI check:
 * writing_detected, sentence_present, numbers_present. Session 10: Graduate Master Challenge (finale).
 */
...
const VOICE =
  'Write a small story with numbers in your notebook. Example: I had 2 apples. I got 1 more. Then upload or take a photo.';

const PALETTE = { accent: '#4F46E5', glow: '#FCD34D', secondary: '#818CF8' } as const;

const EXAMPLE_LINES = ['I had 2 apples.', 'I got 1 more.'] as const;

const STEPS = [
  { icon: '✏️', title: 'Write', body: 'Story + numbers.' },
    ...
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <View style={sketch.sceneRow}>
          ...
          <Text style={sketch.sceneHint}>number story</Text>
        </View>
        ...
        <Text style={sketch.caption}>two lines · a sentence and numbers</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ sentences</Text>
          <Text style={sketch.hint}>✓ numbers</Text>
        </View>
      </View>
    </View>
  );
...
export function GraduateMasterNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-graduate-master-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const sentenceOk = data.sentence_present ?? true;
      const numbersOk = data.numbers_present ?? true;
      const correct = data.correct ?? (writingOk && sentenceOk && numbersOk);
      const feedback =
        data.feedback ||
        (correct ? 'Great job!' : "Let's try again! Write a clear story with numbers.");

      setResult({
        correct,
        feedback,
        writing_detected: data.writing_detected,
        sentence_present: data.sentence_present,
        numbers_present: data.numbers_present,
      });
  ...
  const coachLine = uploading
    ? 'Scanning your notebook for writing, sentences, and numbers…'
    : result?.correct
      ? 'Verified! Your number story looks great — graduate master!'
      : result
        ? 'Write two clear lines with at least one number in your story.'
        : 'Try: I had 2 apples. I got 1 more.';

  return (
    <GraduateGameShell
      studio="MASTER LEDGER · FINALE MISSION"
      title="Final notebook proof"
      instruction="Write a small story with numbers, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <CheckRow label="Sentence present" pending />
              <CheckRow label="Numbers present" pending />
            </>
          ) : result ? (
            <>
              ...
              <CheckRow label="Sentence present" ok={result.sentence_present ?? result.correct} />
              <CheckRow label="Numbers present" ok={result.numbers_present ?? result.correct} />
              {!result.correct && (
