/**
 * Game 3 — Word problem. 4 balloons, 1 pops. How many left? Options 2, 3, 4. Correct: 3. Session 4: Daily Stories.
 */
...
export function WordProblemBalloons({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      if (num !== CORRECT) {
        speak('Try again. Four minus one is three.');
        triggerShake();
        return;
      }
      speak('Correct! Three balloons left!');
      setShowSuccess(true);
      ...
      <View style={styles.content}>
        <View style={styles.storyBox}>
          <Text style={styles.storyLine}>
            You have <Text style={styles.bold}>4 balloons</Text>.
          </Text>
          <Text style={styles.storyLine}>
            <Text style={styles.bold}>1 balloon pops</Text>.
          </Text>
