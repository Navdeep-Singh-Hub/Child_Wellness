/**
 * Game 3 — Word problem. 4 balloons, 1 pops. How many left? Options 2, 3, 4. Correct: 3.
 */
...
export function WordProblemBalloons({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      ...
      if (num === CORRECT) {
        ...
        speak('Correct! Three balloons left!', 0.7);
        setShowSuccess(true);
        setTimeout(() => onComplete(), 2200);
      } else {
        setFeedback('wrong');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Four minus one is three.', 0.65);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  return (
    <GraduateGameShell
      ...
      mascot="🎈"
      coachLine="Four balloons, one pops — count how many stay in the air!"
    ...
    >
      <View style={styles.storyFrame}>
        ...
        <Text style={styles.storyLine}>
          You have <Text style={styles.highlight}>4 balloons</Text>.
        </Text>
        <Text style={styles.storyLine}>
          <Text style={styles.highlight}>1 balloon pops</Text>.
        </Text>

        <View style={styles.visualRow}>
          <View style={styles.balloonGroup}>
            {Array.from({ length: 4 }, (_, i) => (
              <Text key={i} style={styles.balloonEmoji}>
                🎈
              </Text>
            ))}
          </View>
        ...
        <Text style={styles.question}>How many balloons are left?</Text>
...
const styles = StyleSheet.create({
  ...
  balloonGroup: { flexDirection: 'row', flexWrap: 'wrap', gap: 2, justifyContent: 'center' },
  balloonEmoji: { fontSize: 22 },
  minusSign: { fontSize: 22, fontWeight: '900', color: GR.warn },
  popEmoji: { fontSize: 24 },
