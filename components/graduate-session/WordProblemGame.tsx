/**
 * Game 3 — Word problem. Riya has 2 apples, gets 1 more. How many now? Options 2, 3, 4. Correct: 3.
 */
...
export function WordProblemGame({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      ...
      if (num === CORRECT) {
        ...
        speak('Correct! Three apples!', 0.7);
        setShowSuccess(true);
        setTimeout(() => onComplete(), 2200);
      } else {
        setFeedback('wrong');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Two plus one is three.', 0.65);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  return (
    <GraduateGameShell
      ...
      mascot="📖"
      coachLine="Start with 2 apples, then add 1 more — count them all!"
    ...
    >
      <View style={styles.storyFrame}>
        ...
        <View style={styles.heroRow}>
          ...
          <Text style={styles.heroName}>Riya</Text>
        </View>

        <Text style={styles.storyLine}>
          Riya has <Text style={styles.highlight}>2 apples</Text>.
        </Text>
        <Text style={styles.storyLine}>
          She gets <Text style={styles.highlight}>1 more apple</Text>.
        </Text>
...
const styles = StyleSheet.create({
  ...
  heroName: { fontSize: 18, fontWeight: '900', color: PALETTE.glow },
  storyLine: {
    fontSize: 17,
    fontWeight: '600',
    color: GR.textLight,
  ...
  appleGroup: { flexDirection: 'row', gap: 4 },
  appleEmoji: { fontSize: 26 },
