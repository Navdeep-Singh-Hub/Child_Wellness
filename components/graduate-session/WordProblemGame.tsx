/**
 * Game 3 — Word problem. Riya has 2 apples, gets 1 more. How many now? Options 2, 3, 4. Correct: 3. Session 1: Simple Conversations.
 */
...
export function WordProblemGame({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      if (num !== CORRECT) {
        speak('Try again. Two plus one is three.');
        triggerShake();
        return;
      }
      speak('Correct! Three apples!');
      setShowSuccess(true);
    ...
    >
      <View style={styles.content}>
        <View style={styles.storyBox}>
          <Text style={styles.storyLine}>Riya has <Text style={styles.bold}>2 apples</Text>.</Text>
          <Text style={styles.storyLine}>She gets <Text style={styles.bold}>1 more apple</Text>.</Text>
