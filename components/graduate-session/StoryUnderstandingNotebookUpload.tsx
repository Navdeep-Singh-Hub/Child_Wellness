/**
 * Notebook Task — Write one short story sentence. Upload photo; AI check: writing_detected, sentence_detected. Session 6: Story Understanding.
 */
...
const VOICE =
  'Write one short story sentence in your notebook. Example: The boy plays with a ball. Then upload or take a photo.';
const PALETTE = { accent: '#EC4899', glow: '#F9A8D4', secondary: '#F472B6' } as const;
const EXAMPLE = 'The boy plays with a ball.';

const STEPS = [
  { icon: '✏️', title: 'Write', body: 'One sentence.' },
  { icon: '📓', title: 'Notebook', body: 'Clear text.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
] as const;

function StorySentenceSketch() {
  ...
  return (
    <View style={sketch.wrap}>
      <Text style={sketch.label}>TARGET SENTENCE</Text>
      <Animated.View style={[sketch.glow, glow]} />
      <View style={sketch.frame}>
        <View style={sketch.sceneRow}>
          <Text style={sketch.sceneEmoji}>👦⚽</Text>
          <Text style={sketch.sceneHint}>story picture</Text>
        </View>
        <View style={sketch.sentencePlate}>
          <Text style={sketch.sentenceText}>{EXAMPLE}</Text>
        </View>
        <Text style={sketch.caption}>one short story sentence · who + action</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ one sentence</Text>
...
export function StoryUnderstandingNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-graduate-story-understanding-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const sentenceOk = data.sentence_detected ?? true;
      const correct = data.correct ?? (writingOk && sentenceOk);
      const feedback =
        data.feedback ||
        (correct ? 'Great job!' : "Let's try again! Write one clear story sentence.");

      setResult({
        correct,
        feedback,
        writing_detected: data.writing_detected,
        sentence_detected: data.sentence_detected,
      });
  ...
  const coachLine = uploading
    ? 'Scanning your notebook for a story sentence…'
    : result?.correct
      ? 'Verified! Your story sentence looks great.'
      : result
        ? 'Write one clear sentence — who is in the story and what they do.'
        : 'Try: The boy plays with a ball.';

  return (
    <GraduateGameShell
      studio="STORY LEDGER · MISSION"
      title="Notebook proof"
      instruction="Write one short story sentence in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <CheckRow label="Sentence detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Story sentence verified!' : 'Adjust your writing'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Writing detected" ok={result.writing_detected ?? result.correct} />
              <CheckRow label="Sentence detected" ok={result.sentence_detected ?? result.correct} />
              {!result.correct && (
...
const check = StyleSheet.create({
  ...
  iconPending: { backgroundColor: 'rgba(249,168,212,0.15)' },
