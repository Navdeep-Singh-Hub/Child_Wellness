/**
 * Notebook Task — Draw a small story scene. Upload photo; AI check: drawing_detected, story_scene_detected. Session 4: Daily Stories.
 */
...
const INSTRUCTIONS = 'Draw a small story scene in your notebook. For example: a child playing with a ball, or a child going to school. Then upload or take a photo.';

export function DailyStoriesNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-graduate-daily-stories-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const storyOk = data.story_scene_detected ?? true;
      const correct = data.correct ?? (drawingOk && storyOk);
      const feedback = data.feedback || (correct ? 'Great job!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        drawing_detected: data.drawing_detected,
        story_scene_detected: data.story_scene_detected,
      });
      if (correct) {
        speak('Great job!');
  ...
  return (
    <GameLayout
      ...
      instruction="Draw a small story scene. Then upload or take a photo."
      icon="📓"
    ...
    >
      <View style={styles.content}>
        ...
        <View style={styles.exampleRow}>
          ...
          <Text style={styles.exampleLine}>A child playing with a ball</Text>
          <Text style={styles.exampleLine}>A child going to school</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  exampleLine: { fontSize: 16, fontWeight: '700', color: '#5B21B6', marginBottom: 4 },
