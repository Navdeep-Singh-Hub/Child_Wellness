 ...
 * Flow: Intro → Game 1 (Greeting Choice) → Game 2 (Dialogue Match) → Game 3 (Word Problem) → Game 4 (Pattern) → Notebook → Result
...
export function SocialDialogueSession5({ onExit }: SocialDialogueSession5Props = {}) {
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Social Dialogue</Text>
            <Text style={styles.introSub}>Let's practice greetings and polite conversations.</Text>
            <Text style={styles.introSkills}>Greetings · Polite replies · Addition · Patterns</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '👋', title: 'Choose the correct greeting', desc: 'Good morning or Good night?' },
              { icon: '💬', title: 'Match the polite response', desc: 'Thank you → Welcome' },
              { icon: '🐕', title: 'Solve the story', desc: '2 dogs + 2 more dogs' },
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job practicing social dialogue!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Social Communicator</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {step === 5 && <SocialDialogueNotebookUpload onComplete={handleNotebookComplete} />}
    </SafeAreaView>
  );
}
