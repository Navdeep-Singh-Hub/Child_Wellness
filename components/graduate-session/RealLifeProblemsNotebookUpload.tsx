...
const VOICE =
  'Write a small problem story in your notebook. Example: I had 3 pencils. My friend gave me 2 more. Then upload or take a photo.';
const PALETTE = { accent: '#14B8A6', glow: '#5EEAD4', secondary: '#2DD4BF' } as const;

const EXAMPLE_LINES = ['I had 3 pencils.', 'My friend gave me 2 more.'] as const;

const STEPS = [
  { icon: '✏️', title: 'Write', body: 'Problem story.' },
  { icon: '📓', title: 'Notebook', body: 'Clear text.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
] as const;

function ProblemStorySketch() {
  ...
  return (
    <View style={sketch.wrap}>
      <Text style={sketch.label}>TARGET PROBLEM STORY</Text>
      <Animated.View style={[sketch.glow, glow]} />
      <View style={sketch.frame}>
        <View style={sketch.sceneRow}>
          <Text style={sketch.sceneEmoji}>✏️➕</Text>
          <Text style={sketch.sceneHint}>real-life math story</Text>
        </View>
        ...
        <Text style={sketch.caption}>start amount · then something changes</Text>
...
export function RealLifeProblemsNotebookUpload({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'Great job!' : "Let's try again! Write a clear problem story.");
  ...
  const coachLine = uploading
    ? 'Scanning your notebook for a problem story…'
    : result?.correct
      ? 'Verified! Your problem story looks great.'
      : result
        ? 'Write how many you had, then what changed — like pencils given or taken away.'
        : 'Try: I had 3 pencils. My friend gave me 2 more.';

  return (
    <GraduateGameShell
      studio="PROBLEM LEDGER · MISSION"
      title="Notebook proof"
      instruction="Write a small problem story in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <Text style={styles.resultTitle}>Problem story scan in progress…</Text>
              <CheckRow label="Writing detected" pending />
              <CheckRow label="Story problem detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Problem story verified!' : 'Adjust your writing'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Writing detected" ok={result.writing_detected ?? result.correct} />
              <CheckRow
                label="Story problem detected"
                ok={result.story_problem_detected ?? result.correct}
              />
              {!result.correct && (
