 ...
 * Flow: Intro → Game 1 (Dialogue Arrange) → Game 2 (Reply Choice) → Game 3 (Word Problem) → Game 4 (Logic Sequence) → Notebook → Result
...
export function DialogueBuilderSession8({ onExit }: DialogueBuilderSession8Props = {}) {
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Dialogue Builder</Text>
            <Text style={styles.introSub}>Let's build conversations and solve problems.</Text>
            <Text style={styles.introSkills}>Arrange dialogue · Reply · Addition · Sequence</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '💬', title: 'Put the dialogue in order', desc: 'Hello, How are you?, I am fine.' },
              { icon: '💬', title: 'Choose the best response', desc: 'Do you want to play?' },
              { icon: '🍎', title: 'Solve the story', desc: '5 apples + 5 more' },
              { icon: '🔢', title: 'Complete the sequence', desc: '⭐ 🔵 ⭐ 🔵 __' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job building conversations!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Dialogue Builder</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {step === 5 && <DialogueBuilderNotebookUpload onComplete={handleNotebookComplete} />}
    </SafeAreaView>
  );
}
