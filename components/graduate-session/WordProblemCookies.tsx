...
export function WordProblemCookies({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      if (num !== CORRECT) {
        speak('Try again. Ten minus three is seven.');
        triggerShake();
        return;
      }
      speak('Correct! Seven cookies left!');
      setShowSuccess(true);
      ...
      <View style={styles.content}>
        <View style={styles.storyBox}>
          <Text style={styles.storyLine}>
            You have <Text style={styles.bold}>10 cookies</Text>.
          </Text>
          <Text style={styles.storyLine}>
            You give <Text style={styles.bold}>3 cookies</Text> away.
          </Text>
