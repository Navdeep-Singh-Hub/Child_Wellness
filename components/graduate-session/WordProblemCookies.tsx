/**
 * Game 3 — Word problem. 10 cookies, give 3 away. How many left? Options 6, 7, 8. Correct: 7. Session 6: Story Understanding.
 */
...
export function WordProblemCookies({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      ...
      if (num === CORRECT) {
        ...
        speak('Correct! Seven cookies left!', 0.7);
        setShowSuccess(true);
        setTimeout(() => onComplete(), 2200);
      } else {
        setFeedback('wrong');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Ten minus three is seven.', 0.65);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  return (
    <GraduateGameShell
      ...
      mascot="🍪"
      coachLine="Ten cookies on the plate — give three away and count what's left!"
    ...
    >
      <View style={styles.storyFrame}>
        ...
        <Text style={styles.storyLine}>
          You have <Text style={styles.highlight}>10 cookies</Text>.
        </Text>
        <Text style={styles.storyLine}>
          You give <Text style={styles.highlight}>3 cookies</Text> away.
        </Text>

        <View style={styles.visualRow}>
          <View style={styles.cookieGroup}>
            {Array.from({ length: 10 }, (_, i) => (
              <Text key={i} style={styles.cookieEmoji}>
                🍪
              </Text>
            ))}
          </View>
          <Text style={styles.minusSign}>−</Text>
          <View style={styles.giveGroup}>
            <Text style={styles.cookieEmoji}>🍪</Text>
            <Text style={styles.cookieEmoji}>🍪</Text>
            <Text style={styles.cookieEmoji}>🍪</Text>
            <Text style={styles.giveArrow}>🎁</Text>
          </View>
          <Text style={styles.equalsSign}>=</Text>
          <Text style={styles.questionMark}>?</Text>
        </View>

        <Text style={styles.question}>How many cookies are left?</Text>
...
const styles = StyleSheet.create({
  ...
  cookieEmoji: { fontSize: 20 },
  giveArrow: { fontSize: 18 },
