 ...
 * Flow: Intro → Game 1 (Sentence Arrange) → Game 2 (Dialogue Choice) → Game 3 (Word Problem) → Game 4 (Pattern) → Notebook → Result
...
export function StorySentencesSession2({ onExit }: StorySentencesSession2Props = {}) {
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Story Sentences</Text>
            <Text style={styles.introSub}>Let's arrange sentences and solve story problems.</Text>
            <Text style={styles.introSkills}>Sequence · Replies · Word problems · Patterns</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '📖', title: 'Put the actions in order', desc: 'I wake up, brush teeth, go to school' },
              { icon: '💬', title: 'Pick the correct reply', desc: 'Teacher: Sit down.' },
              { icon: '🐦', title: 'Solve the story', desc: '3 birds + 2 more birds' },
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job arranging story sentences!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Story Builder</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {step === 5 && <StorySentencesNotebookUpload onComplete={handleNotebookComplete} />}
    </SafeAreaView>
  );
}
