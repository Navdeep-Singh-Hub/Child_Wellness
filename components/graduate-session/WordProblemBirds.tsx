/**
 * Game 3 — Word problem. 3 birds, 2 more come. How many? Options 4, 5, 6. Correct: 5. Session 2: Story Sentences.
 */
...
export function WordProblemBirds({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      if (num !== CORRECT) {
        speak('Try again. Three plus two is five.');
        triggerShake();
        return;
      }
      speak('Correct! Five birds!');
      setShowSuccess(true);
      ...
      <View style={styles.content}>
        <View style={styles.storyBox}>
          <Text style={styles.storyLine}>
            <Text style={styles.bold}>3 birds</Text> sit on a tree.
          </Text>
          <Text style={styles.storyLine}>
            <Text style={styles.bold}>2 more birds</Text> come.
          </Text>
