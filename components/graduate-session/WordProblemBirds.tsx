/**
 * Game 3 — Word problem. 3 birds, 2 more come. How many? Options 4, 5, 6. Correct: 5.
 */
...
export function WordProblemBirds({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      ...
      if (num === CORRECT) {
        ...
        speak('Correct! Five birds!', 0.7);
        setShowSuccess(true);
        setTimeout(() => onComplete(), 2200);
      } else {
        setFeedback('wrong');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Three plus two is five.', 0.65);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  return (
    <GraduateGameShell
      ...
      mascot="🐦"
      coachLine="Three birds on the tree, then two more arrive — add them up!"
    ...
    >
      <View style={styles.storyFrame}>
        ...
        <View style={styles.sceneRow}>
          ...
          <Text style={styles.sceneLabel}>On the tree</Text>
        </View>

        <Text style={styles.storyLine}>
          <Text style={styles.highlight}>3 birds</Text> sit on a tree.
        </Text>
        <Text style={styles.storyLine}>
          <Text style={styles.highlight}>2 more birds</Text> come.
        </Text>

        <View style={styles.visualRow}>
          <View style={styles.birdGroup}>
        ...
        <Text style={styles.question}>How many birds are there now?</Text>
...
const styles = StyleSheet.create({
  ...
  sceneLabel: { fontSize: 14, fontWeight: '800', color: PALETTE.glow },
  storyLine: {
    fontSize: 17,
    fontWeight: '600',
    color: GR.textLight,
  ...
  birdGroup: { flexDirection: 'row', gap: 2 },
