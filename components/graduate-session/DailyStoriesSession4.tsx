 ...
 * Flow: Intro → Game 1 (Picture Sentence) → Game 2 (Dialogue) → Game 3 (Word Problem) → Game 4 (Logic Sequence) → Notebook → Result
...
export function DailyStoriesSession4({ onExit }: DailyStoriesSession4Props = {}) {
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Daily Stories</Text>
            <Text style={styles.introSub}>Let's understand stories and solve problems.</Text>
            <Text style={styles.introSkills}>Picture story · Dialogue · Subtraction · Sequence</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '📖', title: 'Choose the correct sentence', desc: 'Look at the picture' },
              { icon: '💬', title: 'Choose the correct reply', desc: "Friend: Let's play" },
              { icon: '🎈', title: 'Solve the story', desc: '4 balloons, 1 pops' },
              { icon: '🔢', title: 'Complete the sequence', desc: '☀️ 🌙 ☀️ 🌙 __' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job understanding daily stories!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Story Explorer</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {step === 5 && <DailyStoriesNotebookUpload onComplete={handleNotebookComplete} />}
    </SafeAreaView>
  );
}
