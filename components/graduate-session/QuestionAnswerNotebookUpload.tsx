/**
 * Notebook Task — Write one question and one answer. Upload photo; AI check: writing_detected, question_detected, answer_detected.
 */
...
function QuestionAnswerSketch() {
  ...
  return (
    <View style={sketch.wrap}>
      <Text style={sketch.label}>TARGET Q & A</Text>
      <Animated.View style={[sketch.glow, glow]} />
      <View style={sketch.frame}>
        <View style={sketch.qaPlate}>
          <View style={sketch.row}>
            <Text style={sketch.badge}>Q</Text>
            <Text style={sketch.lineText}>What do we drink?</Text>
          </View>
          <View style={sketch.divider} />
          <View style={sketch.row}>
            <Text style={[sketch.badge, sketch.answerBadge]}>A</Text>
            <Text style={sketch.lineText}>Water</Text>
          </View>
        </View>
        <Text style={sketch.caption}>one question · one answer</Text>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ question mark</Text>
          <Text style={sketch.hint}>✓ answer line</Text>
        </View>
      </View>
    </View>
  );
...
export function QuestionAnswerNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-graduate-question-answer-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const questionOk = data.question_detected ?? true;
      const answerOk = data.answer_detected ?? true;
      const correct = data.correct ?? (writingOk && questionOk && answerOk);
      const feedback =
        data.feedback ||
        (correct ? 'Great job!' : "Let's try again! Write a clear question and answer.");

      setResult({
        correct,
        feedback,
        writing_detected: data.writing_detected,
        question_detected: data.question_detected,
        answer_detected: data.answer_detected,
      });
  ...
  const coachLine = uploading
    ? 'Scanning your notebook for a question and answer…'
    : result?.correct
      ? 'Verified! Your question and answer look great.'
      : result
        ? 'Label Question and Answer — make both lines easy to read!'
        : 'Write a question, then the answer on the next line!';

  return (
    <GraduateGameShell
      studio="ANSWER LEDGER · MISSION"
      title="Notebook proof"
      instruction="Write one question and one answer in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <Text style={styles.resultTitle}>Q & A scan in progress…</Text>
              <CheckRow label="Writing detected" pending />
              <CheckRow label="Question detected" pending />
              <CheckRow label="Answer detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Question & answer verified!' : 'Adjust your writing'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Writing detected" ok={result.writing_detected ?? result.correct} />
              <CheckRow label="Question detected" ok={result.question_detected ?? result.correct} />
              <CheckRow label="Answer detected" ok={result.answer_detected ?? result.correct} />
              {!result.correct && (
...
const sketch = StyleSheet.create({
  ...
  answerBadge: { backgroundColor: PALETTE.secondary },
...
const check = StyleSheet.create({
  ...
  iconPending: { backgroundColor: 'rgba(125,211,252,0.15)' },
