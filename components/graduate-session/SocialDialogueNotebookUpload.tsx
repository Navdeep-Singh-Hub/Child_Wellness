/**
 * Notebook Task — Write two greetings. Upload photo; AI check: writing_detected, greetings_detected. Session 5: Social Dialogue.
 */
...
const VOICE = 'Write two greetings in your notebook. Example: Good morning, Good night. Then upload or take a photo.';
const PALETTE = { accent: '#10B981', glow: '#6EE7B7', secondary: '#34D399' } as const;

const STEPS = [
  { icon: '✏️', title: 'Write', body: 'Two greetings.' },
  { icon: '📓', title: 'Notebook', body: 'Clear text.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
] as const;

function GreetingsSketch() {
  ...
  return (
    <View style={sketch.wrap}>
      <Text style={sketch.label}>TARGET GREETINGS</Text>
      <Animated.View style={[sketch.glow, glow]} />
      <View style={sketch.frame}>
        <View style={sketch.greetingPlate}>
          <View style={sketch.lineRow}>
            <Text style={sketch.emoji}>☀️</Text>
            <Text style={sketch.lineText}>Good morning</Text>
          </View>
          <View style={sketch.divider} />
          <View style={sketch.lineRow}>
            <Text style={sketch.emoji}>🌙</Text>
            <Text style={sketch.lineText}>Good night</Text>
          </View>
        </View>
        <Text style={sketch.caption}>two greetings · morning and night</Text>
...
export function SocialDialogueNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-graduate-social-dialogue-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const greetingsOk = data.greetings_detected ?? true;
      const correct = data.correct ?? (writingOk && greetingsOk);
      const feedback =
        data.feedback ||
        (correct ? 'Great job!' : "Let's try again! Write two clear greetings.");

      setResult({
        correct,
        feedback,
        writing_detected: data.writing_detected,
        greetings_detected: data.greetings_detected,
      });
  ...
  const coachLine = uploading
    ? 'Scanning your notebook for two greetings…'
    : result?.correct
      ? 'Verified! Your greetings look great.'
      : result
        ? 'Write two clear greetings — one for morning, one for night.'
        : 'Write Good morning on one line and Good night on the next!';

  return (
    <GraduateGameShell
      studio="SOCIAL LEDGER · MISSION"
      title="Notebook proof"
      instruction="Write two greetings in your notebook, then upload or photograph it."
      mascot="📓"
      coachLine={coachLine}
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <Text style={styles.resultTitle}>Greeting scan in progress…</Text>
              <CheckRow label="Writing detected" pending />
              <CheckRow label="Greetings detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Greetings verified!' : 'Adjust your writing'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Writing detected" ok={result.writing_detected ?? result.correct} />
              <CheckRow label="Greetings detected" ok={result.greetings_detected ?? result.correct} />
              {!result.correct && (
