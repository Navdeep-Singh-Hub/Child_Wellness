/**
 * Game 3 — Word problem. Riya has 4 apples, buys 3 more. How many now? Options 6, 7, 8. Correct: 7.
...
export function WordProblemRiyaApples({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      ...
      if (num === CORRECT) {
        ...
        speak('Correct! Seven apples!', 0.7);
        setShowSuccess(true);
        setTimeout(() => onComplete(), 2200);
      } else {
        setFeedback('wrong');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Four plus three is seven.', 0.65);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  return (
    <GraduateGameShell
      studio="STORY PROBLEM · FINALE GAME 3"
      title="Solve the story problem"
      instruction="Read the story and choose the correct number."
      mascot="🍎"
      coachLine="Riya starts with four apples and buys three more — add them together!"
    ...
    >
      <View style={styles.storyFrame}>
        ...
        />
        <Text style={styles.frameLabel}>STORY CARD · RIYA'S APPLES</Text>

        <View style={styles.characterRow}>
          <Text style={styles.characterEmoji}>👧</Text>
          <Text style={styles.characterName}>Riya</Text>
        </View>

        <Text style={styles.storyLine}>
          Riya has <Text style={styles.highlight}>4 apples</Text>.
        </Text>
        <Text style={styles.storyLine}>
          She buys <Text style={styles.highlight}>3 more apples</Text>.
        </Text>

        <View style={styles.visualRow}>
          <View style={styles.appleGroup}>
            {Array.from({ length: 4 }, (_, i) => (
              <Text key={i} style={styles.appleEmoji}>
                🍎
              </Text>
            ))}
          </View>
        ...
        <Text style={styles.question}>How many apples does she have now?</Text>
...
const styles = StyleSheet.create({
  ...
  characterEmoji: { fontSize: 22 },
  characterName: { fontSize: 15, fontWeight: '800', color: GR.textLight },
  storyLine: {
    fontSize: 17,
    fontWeight: '600',
    color: GR.textLight,
  ...
  appleEmoji: { fontSize: 20 },
  bagEmoji: { fontSize: 18 },
  plusSign: { fontSize: 22, fontWeight: '900', color: GR.good },
