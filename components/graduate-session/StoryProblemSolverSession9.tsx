 ...
 * Flow: Intro → Game 1 (Story Reading) → Game 2 (Dialogue) → Game 3 (Word Problem) → Game 4 (Logic Puzzle) → Notebook → Result
...
export function StoryProblemSolverSession9({ onExit }: StoryProblemSolverSession9Props = {}) {
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Story Problem Solver</Text>
            <Text style={styles.introSub}>Let's read stories and solve problems.</Text>
            <Text style={styles.introSkills}>Story · Dialogue · Subtraction · Logic</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '📖', title: 'Answer the question', desc: 'What does Riya have?' },
              { icon: '💬', title: 'Finish the conversation', desc: 'Can you help me?' },
              { icon: '🍬', title: 'Solve the story', desc: '8 candies, eat 3' },
              { icon: '🔢', title: 'Find the missing item', desc: '🍎 🍎 🍌 🍎 🍎 🍌 __' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job solving story problems!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Story Problem Solver</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {step === 5 && <StoryProblemSolverNotebookUpload onComplete={handleNotebookComplete} />}
    </SafeAreaView>
  );
}
