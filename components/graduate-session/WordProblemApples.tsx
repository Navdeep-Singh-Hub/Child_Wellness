...
export function WordProblemApples({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      ...
      if (num === CORRECT) {
        ...
        speak('Correct! Ten apples!', 0.7);
        setShowSuccess(true);
        setTimeout(() => onComplete(), 2200);
      } else {
        setFeedback('wrong');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Five plus five is ten.', 0.65);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  return (
    <GraduateGameShell
      ...
      mascot="🍎"
      coachLine="Five apples on the table — five more arrive, add them up!"
    ...
    >
      <View style={styles.storyFrame}>
        ...
        <Text style={styles.storyLine}>
          You have <Text style={styles.highlight}>5 apples</Text>.
        </Text>
        <Text style={styles.storyLine}>
          You get <Text style={styles.highlight}>5 more apples</Text>.
        </Text>
