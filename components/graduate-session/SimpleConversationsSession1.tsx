 ...
 * Flow: Intro → Game 1 (Complete Dialogue) → Game 2 (Reply Choice) → Game 3 (Word Problem) → Game 4 (Pattern Puzzle) → Notebook → Result
...
export function SimpleConversationsSession1({ onExit }: SimpleConversationsSession1Props = {}) {
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Simple Conversations</Text>
            <Text style={styles.introSub}>Let's practice talking and solving problems.</Text>
            <Text style={styles.introSkills}>Conversation · Replies · Word problems · Patterns</Text>
          </View>
          <View style={styles.cardsWrap}>
            {[
              { icon: '💬', title: 'Finish the conversation', desc: 'Choose the correct reply' },
              { icon: '💬', title: 'Pick the correct answer', desc: 'How are you?' },
              { icon: '📖', title: 'Solve the story', desc: 'Riya has 2 apples, gets 1 more' },
              { icon: '🔢', title: 'Find the pattern', desc: '🍎 🍌 🍎 🍌 __' },
            ].map((card, i) => {
              const gameNum = i + 1;
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job starting a conversation!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>Conversation Starter</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {step === 5 && <GraduateConversationNotebookUpload onComplete={handleNotebookComplete} />}
    </SafeAreaView>
  );
}
