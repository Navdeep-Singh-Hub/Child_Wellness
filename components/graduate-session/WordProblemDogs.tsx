/**
 * Game 3 — Word problem. 2 dogs, 2 more come. How many now? Options 3, 4, 5. Correct: 4.
 */
...
export function WordProblemDogs({ onComplete }: { onComplete: () => void }) {
  ...
  const handleChoice = useCallback(
    (num: string) => {
      ...
      if (num === CORRECT) {
        ...
        speak('Correct! Four dogs!', 0.7);
        setShowSuccess(true);
        setTimeout(() => onComplete(), 2200);
      } else {
        setFeedback('wrong');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Two plus two is four.', 0.65);
        setTimeout(() => {
          setFeedback('idle');
          setSelected(null);
        }, 900);
  ...
  return (
    <GraduateGameShell
      ...
      mascot="🐕"
      coachLine="Two dogs here, two more arrive — add them together!"
    ...
    >
      <View style={styles.storyFrame}>
        ...
        <Text style={styles.storyLine}>
          There are <Text style={styles.highlight}>2 dogs</Text>.
        </Text>
        <Text style={styles.storyLine}>
          <Text style={styles.highlight}>2 more dogs</Text> come.
        </Text>

        <View style={styles.visualRow}>
          <View style={styles.dogGroup}>
...
const styles = StyleSheet.create({
  ...
  dogGroup: { flexDirection: 'row', gap: 4 },
