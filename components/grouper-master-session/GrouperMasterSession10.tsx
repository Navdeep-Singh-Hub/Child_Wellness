/**
 * Level 4 Grouper — Session 10: The Grouper Master
 * Hub → 4 games + master journal task → completion
 */
import { ConfettiEffect } from '@/components/games/Level1/ConfettiEffect';
import { SuccessCelebration } from '@/components/ui/SuccessCelebration';
import { markSessionGameComplete } from '@/components/special-education/shared/useSessionIntroProgress';
import { FamilyHuntGame } from '@/components/grouper-session/session10/FamilyHuntGame';
import { RhymeQuizGame } from '@/components/grouper-session/session10/RhymeQuizGame';
import { SyllableClapGame } from '@/components/grouper-session/session10/SyllableClapGame';
import { BigSortingGame } from '@/components/grouper-session/session10/BigSortingGame';
import { GrouperMasterNotebookUpload } from '@/components/grouper-session/session10/GrouperMasterNotebookUpload';
import {
  GROUPER_S10_HUB_THEME as H,
  GROUPER_S10_QUESTS,
  GROUPER_SESSION,
} from '@/components/grouper-session/grouperSessionTheme';
import { SummitSandsBackground } from '@/components/grouper-session/SummitSandsBackground';
...
const PLAY_STEPS = 5;
const SESSION_TITLE = 'Session 10 · Grouper Master';
        ...
        <SafeAreaView style={styles.hubSafe}>
          ...
          <ScrollView contentContainerStyle={styles.hubScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.hubHeader}>
              <Text style={styles.hubEyebrow}>Grouper · Session 10 · Summit Sands</Text>
              <Text style={styles.hubTitle}>{H.name}</Text>
              <Text style={styles.hubSession}>The Grouper Master</Text>

              <View style={styles.exampleChip}>
                <Text style={styles.exampleText}>Families · Rhymes · Syllables · Sorting</Text>
              </View>
              ...
              <View style={styles.speechBubble}>
                ...
                <View style={styles.bubbleBody}>
                  ...
                  <Text style={styles.hubPrompt}>
                    You made it to the summit! Use everything you learned — hunt families, match rhymes, clap syllables, sort big puzzles, then earn your master journal!
                  </Text>
                </View>
              </View>
            </View>

            {GROUPER_S10_QUESTS.map((quest) => {
  ...
  if (step === 6) {
    return (
      <View style={styles.root}>
        ...
        <ConfettiEffect />
        <SuccessCelebration
          title="Congratulations! You completed Level 4!"
          subtitle={`Grouper Master 🏆\n${SESSION_TITLE}\nCompleted ${doneCount}/${PLAY_STEPS} builds${
            taskCorrect === false ? '\nMaster Journal: try again next time!' : ''
          }`}
          badgeEmoji="🏆"
          variant="sunset"
        />
...
const styles = StyleSheet.create({
  ...
  exampleText: { fontSize: 16, fontWeight: '900', color: H.accentDeep, letterSpacing: 0.5, textAlign: 'center' },
