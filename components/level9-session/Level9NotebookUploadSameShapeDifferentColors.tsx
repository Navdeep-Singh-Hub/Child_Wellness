/**
 * Level 9 (Clockwise) — Session 2, Real World Task: Show TWO objects that are the same shape but different colors.
 * Upload photo → AI checks shape similarity and color difference.
 */
...
const VOICE =
  'Show two objects that are the same shape but different colors. For example, two balls or two books. Then upload or take a photo.';

const PALETTE = { accent: '#0891B2', glow: '#67E8F9', secondary: '#22D3EE' } as const;

const STEPS = [
  { icon: '🔍', title: 'Find', body: 'Two objects.' },
  { icon: '⬜', title: 'Same shape', body: 'Match form.' },
  { icon: '🎨', title: 'Diff colors', body: 'Not the same.' },
  ...
  return (
    <View style={sketch.wrap}>
      ...
      <View style={sketch.frame}>
        <Text style={sketch.caption}>Same shape · different colors</Text>
        <View style={sketch.pairsRow}>
          <View style={sketch.pairCard}>
            <Text style={sketch.pairLabel}>Example A</Text>
            <View style={sketch.pairObjects}>
              <Text style={sketch.object}>🔴</Text>
              <Text style={sketch.object}>🔵</Text>
            </View>
            <Text style={sketch.pairHint}>two round balls</Text>
          </View>
          <View style={sketch.pairCard}>
            <Text style={sketch.pairLabel}>Example B</Text>
            <View style={sketch.pairObjects}>
              <Text style={sketch.object}>🟢</Text>
              <Text style={sketch.object}>🟡</Text>
            </View>
            <Text style={sketch.pairHint}>two rectangle books</Text>
          </View>
        </View>
        <View style={sketch.hintRow}>
          <Text style={sketch.hint}>✓ 2 objects</Text>
          <Text style={sketch.hint}>✓ same shape</Text>
          <Text style={sketch.hint}>✓ different colors</Text>
        </View>
      </View>
    </View>
  );
...
export function Level9NotebookUploadSameShapeDifferentColors({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const filename = 'level9-s2-same-shape-different-colors.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-level9-s2-same-shape-different-colors-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      if (correct) {
        ...
        speak('Success! Same shape, different colors!', 0.75);
        setTimeout(() => onComplete(true), 2600);
      } else {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Show two objects that are the same shape but different colors.', 0.75);
      }
    } catch (e: unknown) {
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your two objects.');
      return;
    }
  ...
  const coachLine = uploading
    ? 'Scanning your photo for matching shapes and different colors…'
    : result?.correct
      ? 'Verified! Same shape, different colors — great spotting!'
      : result
        ? 'Pick two objects with the same form but different colors.'
        : 'Try balls, cups, books, or blocks — shape must match, colors must differ!';

  return (
    <ClockwiseGameShell
      studio="SHAPE & COLOR · REAL-WORLD MISSION"
      title="Same shape, different colors"
      instruction="Show two objects that share the same shape but have different colors, then photograph them."
      mascot="🎨"
      coachLine={coachLine}
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <CheckRow label="Two objects visible" pending />
              <CheckRow label="Same shape detected" pending />
              <CheckRow label="Different colors" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Pair verified!' : 'Adjust your pair'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Two objects visible" ok={result.correct} />
              <CheckRow label="Same shape detected" ok={result.correct} />
              <CheckRow label="Different colors" ok={result.correct} />
              {!result.correct && (
...
const sketch = StyleSheet.create({
  ...
  pairsRow: { flexDirection: 'row', gap: 12, marginBottom: 12, flexWrap: 'wrap', justifyContent: 'center' },
  pairCard: {
    alignItems: 'center',
    paddingVertical: 10,
  ...
  pairLabel: { fontSize: 10, fontWeight: '900', letterSpacing: 0.8, color: PALETTE.glow, marginBottom: 6 },
  pairObjects: { flexDirection: 'row', gap: 10, marginBottom: 4 },
  object: { fontSize: 28 },
  pairHint: { fontSize: 10, fontWeight: '700', color: CW.textMuted, textAlign: 'center' },
