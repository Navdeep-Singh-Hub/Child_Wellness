 ...
 * Upload photo → AI checks shape similarity and color difference. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Show TWO objects that are the same shape but different colors.\n\nFor example: two balls (round) in different colors, or two books (rectangle) in different colors. Take a photo or upload a picture.';
...
export function Level9NotebookUploadSameShapeDifferentColors({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see two objects with the same shape and different colors!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show TWO objects that are the same shape but different colors. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
