/**
 * Level 9 (Clockwise) — Session 9, Game 4: Sequence Forge
 * square, triangle, circle, square, triangle, ? → circle (repeating: square, triangle, circle).
 */
...
const PATTERN = ['square', 'triangle', 'circle', 'square', 'triangle'] as const;
...
const VOICE =
  'Complete the pattern. Square, triangle, circle, square, triangle. What comes next?';
const FORGE = { accent: '#7C3AED', glow: '#C4B5FD', teal: '#2DD4BF' } as const;
...
function SequenceCell({ shape, index }: { shape: ShapeId; index: number }) {
...
function VoidSlot() {
  ...
  const anim = useAnimatedStyle(() => ({
    borderColor: `rgba(124,58,237,${0.4 + pulse.value * 0.5})`,
...
export interface PatternLogicSquareTriangleCircleLevel9Session9GameProps {
  onComplete: () => void;
}

export function PatternLogicSquareTriangleCircleLevel9Session9Game({
  onComplete,
}: PatternLogicSquareTriangleCircleLevel9Session9GameProps) {
  ...
  const handleTap = useCallback(
    (id: ShapeId) => {
      ...
      if (id === CORRECT_ID) {
        ...
        speak(
          id === 'triangle'
            ? 'Triangle was just before. After triangle comes circle in this pattern!'
            : 'Square starts the cycle — look at what comes after triangle!',
          0.7,
        );
        setTimeout(() => {
          setFeedback('idle');
  ...
  const coachLine =
    attempts === 0
      ? 'Square, triangle, circle repeat — what follows triangle?'
      : 'Say it aloud: ⬜ 🔺 ⭕ ⬜ 🔺 — ?';
  ...
  if (showSuccess) {
    return (
      <SuccessCelebration
        variant="indigo"
        title="Sequence Forge!"
        subtitle="Circle completes the shape sequence!"
        badgeEmoji="⭕"
      />
    );
  }
  ...
  return (
    <ClockwiseGameShell
      studio="SEQUENCE FORGE · GAME 4"
      title="Complete the pattern"
      instruction="Square, triangle, circle, square, triangle — what comes next?"
      mascot="⭕"
      coachLine={coachLine}
    ...
    >
      <View style={styles.railWrap}>
        <LinearGradient
          colors={[`${FORGE.accent}44`, 'transparent', `${FORGE.teal}33`]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 0 }}
        ...
        <View style={styles.cycleRow}>
          <Text style={styles.cycleTag}>⬜</Text>
      ...
      <View style={styles.legend}>
        <Text style={styles.legendTxt}>Pattern rule: ⬜ 🔺 ⭕ repeats in a three-shape cycle</Text>
      </View>
    </ClockwiseGameShell>
  );
}
...
const styles = StyleSheet.create({
  ...
  cycleTag: { fontSize: 12, fontWeight: '900', color: FORGE.glow },
  cycleArrow: { fontSize: 11, fontWeight: '900', color: FORGE.teal, opacity: 0.9 },
  ...
  orbLabel: { fontSize: 13, fontWeight: '800', color: CW.textMuted, marginTop: 6 },
