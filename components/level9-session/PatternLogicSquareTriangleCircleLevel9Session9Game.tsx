/**
 * Level 9 (Clockwise) — Session 9, Game 4: Pattern Logic
...
const PATTERN = ['square', 'triangle', 'circle', 'square', 'triangle'];
const OPTIONS = [
...
export function PatternLogicSquareTriangleCircleLevel9Session9Game({ onComplete }: PatternLogicSquareTriangleCircleLevel9Session9GameProps) {
  const [showSuccess, setShowSuccess] = useState(false);
  const [wrongShake] = useState(() => new Animated.Value(0));

  useEffect(() => {
    speak('Complete the pattern. Square, triangle, circle, square, triangle. What comes next?', 0.75);
  }, []);
  ...
  const triggerWrong = useCallback(() => {
    ...
    speak('Try again. The pattern repeats: square, triangle, circle.', 0.7);
  }, [wrongShake]);
