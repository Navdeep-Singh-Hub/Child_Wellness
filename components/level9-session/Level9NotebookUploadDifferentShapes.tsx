 ...
 * Upload photo → AI verifies shape differences. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Show THREE objects of different shapes.\n\nTake a photo (or upload a picture) showing three objects that have different shapes (e.g. round, square, triangular).';
...
export function Level9NotebookUploadDifferentShapes({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of three objects with different shapes.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see three objects of different shapes!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show THREE objects of different shapes. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
