/**
 * Level 9 (Clockwise) — Session 8, Game 3: Shape Puzzle
...
export function ShapePuzzleBoardLevel9Session8Game({ onComplete }: ShapePuzzleBoardLevel9Session8GameProps) {
  const [slots, setSlots] = useState<Record<ShapeId, boolean>>({ circle: false, square: false, triangle: false, star: false });
...
const styles = StyleSheet.create({
  ...
  shapesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, justifyContent: 'center', marginBottom: 20 },
  shapeBtn: {
    paddingVertical: 12,
  ...
  placed: { opacity: 0.6 },
  shapeSymbol: { fontSize: 32, marginBottom: 4 },
  shapeLabel: { fontSize: 12, fontWeight: '700', color: '#4338CA' },
  board: {
  ...
  holesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 14, justifyContent: 'center' },
  hole: {
    width: 58,
    height: 58,
    borderRadius: 14,
