/**
 * Level 9 (Clockwise) — Session 9: Logical Reasoning, Memory 18, Word HOSPITAL, Pattern Logic
 * 4 games + 1 real-world task (two objects same size). Galaxy/indigo theme.
 */
...
export function Level9Session9({ onExit }: Level9Session9Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧠', title: 'Logical Reasoning', desc: "Which doesn't belong? spoon, fork, knife, chair" },
      { icon: '🎴', title: 'Memory Grid', desc: 'Match 18 cards (9 pairs)' },
      { icon: '🏥', title: 'Word Builder', desc: 'Build HOSPITAL' },
      { icon: '⬜', title: 'Pattern Logic', desc: 'Square, triangle, circle...' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Find what doesn't belong, match 18 cards, build HOSPITAL, complete the pattern, then show two objects the same size!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Level 9 · Session 9</Text>
            <Text style={styles.introSub}>Clockwise · Galaxy · Session 9</Text>
            <Text style={styles.introDesc}>
              Logical reasoning, memory grid, word builder, pattern logic, then a real-world photo task!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Level 9 · Session 9</Text>
        </View>
        <Text style={styles.subtitle}>Clockwise · Galaxy · Session 9</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <Level9NotebookUploadSameSize onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
