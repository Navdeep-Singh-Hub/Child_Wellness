 ...
 * Upload photo → AI verifies both shapes. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Show something ROUND and something RECTANGULAR together.\n\nTake a photo (or upload a picture) showing one round object and one rectangular object in the same image.';
...
export function Level9NotebookUploadRoundRectangular({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of something round and something rectangular.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see something round and something rectangular!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show something ROUND and something RECTANGULAR together. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
