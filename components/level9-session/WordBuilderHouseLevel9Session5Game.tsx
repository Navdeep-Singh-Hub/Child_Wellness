/**
 * Level 9 (Clockwise) — Session 5, Game 3: Word Dock
...
const TARGET = ['H', 'O', 'U', 'S', 'E'] as const;
const LETTERS = ['E', 'H', 'O', 'S', 'U'] as const;
const WORD = 'HOUSE';

const VOICE = 'Build the word HOUSE. Tap the letters in order: H, O, U, S, E.';
const S5 = { accent: '#EC4899', glow: '#FDA4AF', cyan: '#F472B6', home: '#FBBF24' } as const;
...
function DockSlot({
  ...
  const anim = useAnimatedStyle(() => ({
    borderColor:
      active && !filled
        ? `rgba(236,72,153,${0.5 + pulse.value * 0.45})`
        : filled
          ? `${CW.good}99`
          : CW.glassBorder,
...
function CargoLetter({
  ...
  return (
    <Animated.View style={anim}>
      ...
      >
        <LinearGradient
          colors={[`${S5.accent}88`, `${S5.cyan}44`]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
...
export interface WordBuilderHouseLevel9Session5GameProps {
  onComplete: () => void;
}

export function WordBuilderHouseLevel9Session5Game({ onComplete }: WordBuilderHouseLevel9Session5GameProps) {
  ...
  const coachLine =
    nextIndex === 0
      ? 'Spell HOUSE: start with H, then O, U, S, and E!'
  ...
  return (
    <ClockwiseGameShell
      ...
      title="Build HOUSE"
      instruction="Tap the letter crates in order to spell HOUSE."
      mascot="🏠"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.dockFrame}>
        <LinearGradient
          colors={[`${S5.accent}33`, 'transparent', `${S5.cyan}22`]}
      ...
      <View style={styles.legend}>
        <Text style={styles.legendTxt}>
          Order: H → O → U → S → E{attempts > 0 ? ` · Taps: ${attempts}` : ''}
        </Text>
      </View>
    </ClockwiseGameShell>
  );
...
const styles = StyleSheet.create({
  ...
  targetWord: { fontSize: 26, fontWeight: '900', color: CW.textLight, letterSpacing: 3 },
  ...
  slotQ: { fontSize: 20, fontWeight: '900', color: S5.glow },
  prompt: {
    fontSize: 16,
    fontWeight: '800',
    color: CW.textLight,
  ...
  crateLetter: { fontSize: 28, fontWeight: '900', color: CW.textLight },
  crateMark: {
    position: 'absolute',
    bottom: 4,
    right: 6,
