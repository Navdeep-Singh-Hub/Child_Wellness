 ...
 * Upload photo → AI verifies circular arrangement. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Arrange FIVE objects in a circle.\n\nTake a photo (or upload a picture) showing five objects placed in a circular arrangement.';
...
export function Level9NotebookUploadFiveCircle({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see five objects in a circle!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Arrange FIVE objects in a circle. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
