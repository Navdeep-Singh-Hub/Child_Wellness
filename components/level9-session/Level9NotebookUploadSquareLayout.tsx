/**
 * Level 9 (Clockwise) — Session 3, Real World Task: Arrange FOUR objects in a square layout.
 * Upload photo → AI verifies square arrangement.
 */
...
const VOICE =
  'Arrange four objects in a square layout. Then take a photo or upload a picture showing them.';

const PALETTE = { accent: '#7C3AED', glow: '#C4B5FD', secondary: '#A78BFA' } as const;

const STEPS = [
  { icon: '⬜', title: 'Arrange', body: 'Four corners.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
  { icon: '🔭', title: 'Verify', body: 'Orbit scan!' },
] as const;
...
export function Level9NotebookUploadSquareLayout({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const filename = 'level9-s3-square-layout.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-level9-s3-square-layout-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const feedback =
        ...
        (correct
          ...
          : 'TRY AGAIN. Arrange four objects in a square layout and take a photo.');

      setResult({ correct, feedback });

      if (correct) {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success).catch(() => {});
        speak('Success! Great square layout!', 0.75);
        setTimeout(() => onComplete(true), 2600);
      } else {
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning).catch(() => {});
        speak('Try again. Arrange four objects in a square layout.', 0.75);
      }
    } catch (e: unknown) {
  ...
  return (
    <ClockwiseGameShell
      studio="SQUARE GRID · REAL-WORLD MISSION"
      title="Four objects in a square"
      instruction="Arrange four objects in a square layout, then upload or photograph them."
      mascot="⬜"
      coachLine={coachLine}
          ...
          {uploading ? (
            ...
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Square grid verified!' : 'Adjust your layout'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Four objects detected" ok={result.correct} />
              <CheckRow label="Square layout verified" ok={result.correct} />
              {!result.correct && (
...
const sketch = StyleSheet.create({
  ...
  caption: { fontSize: 14, fontWeight: '800', color: CW.textLight, textAlign: 'center', marginTop: 12 },
