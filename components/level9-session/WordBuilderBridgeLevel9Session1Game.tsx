...
const TARGET = ['B', 'R', 'I', 'D', 'G', 'E'] as const;
const LETTERS = ['E', 'B', 'R', 'I', 'D', 'G'] as const;
const WORD = 'BRIDGE';

const VOICE = 'Build the word BRIDGE. Tap the letters in order: B, R, I, D, G, E.';
...
function OrbitSlot({
  ...
  const anim = useAnimatedStyle(() => ({
    borderColor:
      active && !filled
        ? `rgba(165,180,252,${0.5 + pulse.value * 0.45})`
        : filled
          ? `${CW.good}99`
          : CW.glassBorder,
...
export function WordBuilderBridgeLevel9Session1Game({ onComplete }: WordBuilderBridgeLevel9Session1GameProps) {
  ...
  const coachLine =
    nextIndex === 0
      ? 'Spell BRIDGE: start with B, then R, I, D, G, and E!'
  ...
  return (
    <ClockwiseGameShell
      ...
      title="Build BRIDGE"
      instruction="Tap the letter comets in order to spell BRIDGE."
      mascot="🌉"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.legend}>
        <Text style={styles.legendTxt}>Order: B → R → I → D → G → E</Text>
      </View>
    </ClockwiseGameShell>
  );
}
