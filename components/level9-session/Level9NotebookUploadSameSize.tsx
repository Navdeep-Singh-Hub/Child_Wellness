 ...
 * Upload photo → AI verifies size similarity. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Show TWO objects that are the same size.\n\nTake a photo (or upload a picture) showing two objects that look about the same size.';
...
export function Level9NotebookUploadSameSize({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of two objects the same size.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see two objects the same size!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Show TWO objects that are the same size. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
