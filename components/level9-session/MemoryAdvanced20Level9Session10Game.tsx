/**
 * Level 9 (Clockwise) — Session 10, Game 2: Apex Matrix
...
const VOICE = 'Match 10 pairs. Flip two cards at a time. 20 cards.';
...
export function MemoryAdvanced20Level9Session10Game({ onComplete }: MemoryAdvanced20Level9Session10GameProps) {
  ...
  return (
    <ClockwiseGameShell
      ...
      instruction="Tap two cards to find matching pictures. 20 cards, 10 pairs."
      mascot="🎴"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.gridFrame}>
        ...
        />
        <Text style={styles.gridLabel}>APEX MATRIX · 20 CARDS</Text>
      ...
      <View style={styles.legend}>
        <Text style={styles.legendTxt}>
          {attempts === 0 ? 'Tap any two cards to begin' : `Scans: ${attempts} · Match all 10 pairs`}
        </Text>
      </View>
    </ClockwiseGameShell>
  );
...
const styles = StyleSheet.create({
  ...
  dotRow: { flexDirection: 'row', justifyContent: 'center', gap: 4, marginTop: 10, flexWrap: 'wrap' },
  dot: {
    width: 8,
    height: 8,
    borderRadius: 4,
  ...
  backGlyph: { fontSize: 15, color: PALETTE.glow, opacity: 0.9 },
  backNum: {
    position: 'absolute',
    bottom: 2,
    right: 4,
