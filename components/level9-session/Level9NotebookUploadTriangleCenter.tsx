/**
 * Level 9 (Clockwise) — Session 6, Real World Task: Place THREE objects in a triangle and ONE object in the center.
 * Upload photo → AI verifies spatial arrangement. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Place THREE objects in a triangle and ONE object in the center.\n\nTake a photo (or upload a picture) showing three objects arranged in a triangle shape, with one object in the middle.';
...
export function Level9NotebookUploadTriangleCenter({
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your arrangement.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see three objects in a triangle and one in the center!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Place THREE objects in a triangle and ONE in the center. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
