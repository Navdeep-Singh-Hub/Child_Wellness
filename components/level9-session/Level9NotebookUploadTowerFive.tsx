 ...
 * Upload photo → AI verifies stacked vertical objects.
 */
...
const VOICE =
  'Build a tower using five objects. Stack them on top of each other, then upload or take a photo.';

const PALETTE = { accent: '#EAB308', glow: '#FDE047', secondary: '#A855F7' } as const;

const STEPS = [
  { icon: '📦', title: 'Gather', body: 'Find 5 objects.' },
  { icon: '🏗️', title: 'Stack', body: 'Build a tower.' },
  { icon: '📷', title: 'Capture', body: 'Photograph it.' },
] as const;

const TOWER_LAYERS = ['🟫', '📚', '☕', '🧱', '⭐'] as const;
...
function TowerSketch() {
  ...
  return (
    <View style={sketch.wrap}>
      <Text style={sketch.label}>TARGET TOWER</Text>
      <Animated.View style={[sketch.glow, glow]} />
      <View style={sketch.frame}>
        <Text style={sketch.caption}>five objects · stacked vertically · one photo</Text>
        <View style={sketch.towerCol}>
          {[...TOWER_LAYERS].reverse().map((emoji, i) => (
            <View key={i} style={sketch.layer}>
              <Text style={sketch.layerEmoji}>{emoji}</Text>
            </View>
          ))}
          <View style={sketch.baseLine} />
        </View>
...
export function Level9NotebookUploadTowerFive({
  ...
  const coachLine = uploading
    ? 'Scanning your photo for a five-object tower…'
    : result?.correct
      ? 'Verified! Five objects stacked in a tower!'
      : result
        ? 'Stack blocks, books, or cups — tallest on top, all five visible!'
        : 'Build your tallest tower with five items, then photograph it!';

  return (
    <ClockwiseGameShell
      studio="TOWER FORGE · FINALE MISSION"
      title="Build a five-object tower"
      instruction="Stack five objects on top of each other, then photograph your tower."
      mascot="🏗️"
      coachLine={coachLine}
      onReplayVoice={playVoice}
    >
      <View style={styles.badge}>
        <Text style={styles.badgeTxt}>SESSION 10 · FINALE MISSION</Text>
      </View>
        ...
        <View style={[styles.resultCard, result?.correct ? styles.resultOk : styles.resultPending]}>
          {uploading ? (
            <>
              ...
              <CheckRow label="Vertical stack detected" pending />
            </>
          ) : result ? (
            <>
              <Text style={styles.resultTitle}>
                {result.correct ? 'Tower verified!' : 'Adjust your stack'}
              </Text>
              <Text style={styles.resultFeedback}>{result.feedback}</Text>
              <CheckRow label="Five objects detected" ok={result.correct} />
              <CheckRow label="Vertical stack detected" ok={result.correct} />
              {!result.correct && (
...
const sketch = StyleSheet.create({
  ...
  towerCol: { alignItems: 'center', marginBottom: 12 },
  layer: {
    width: 72,
    paddingVertical: 6,
    borderRadius: 10,
