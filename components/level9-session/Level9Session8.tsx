/**
 * Level 9 (Clockwise) — Session 8: Pattern Sequence, Number Pattern, Shape Puzzle, Robot Assembly
 * 4 games + 1 real-world task (five objects in a circle). Galaxy/indigo theme.
 */
...
export function Level9Session8({ onExit }: Level9Session8Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔴', title: 'Pattern Sequence', desc: 'Red, blue, green, red, blue, ?' },
      { icon: '🔢', title: 'Number Pattern', desc: '3, 9, 27, ?' },
      { icon: '🧩', title: 'Shape Puzzle', desc: 'Fit shapes into the board' },
      { icon: '🤖', title: 'Assembly Game', desc: 'Build a robot' },
    ];
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.mascotRow}>
            ...
            <Text style={styles.mascotHint}>Spot the color pattern, find the number, fit the shapes, build the robot, then arrange five objects in a circle!</Text>
          </View>
          <View style={styles.introHeader}>
            <Text style={[styles.introTitle, { color: DESIGN.primary }]}>Level 9 · Session 8</Text>
            <Text style={styles.introSub}>Clockwise · Galaxy · Session 8</Text>
            <Text style={styles.introDesc}>
              Pattern sequence, number pattern, shape puzzle, robot assembly, then a real-world photo task!
            </Text>
          </View>
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor: DESIGN.background }]}>
      <View style={[styles.header, { borderBottomColor: DESIGN.primary }]}>
        ...
        <View style={styles.titleRow}>
          ...
          <Text style={[styles.title, { color: DESIGN.primary }]}>Level 9 · Session 8</Text>
        </View>
        <Text style={styles.subtitle}>Clockwise · Galaxy · Session 8</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {step === 5 && <Level9NotebookUploadFiveCircle onComplete={handleTaskComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
