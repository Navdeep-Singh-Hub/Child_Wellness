 ...
 * Upload photo → AI verifies object count and alignment. Return SUCCESS or TRY AGAIN.
 */
...
const INSTRUCTIONS =
  'Place FOUR objects in a straight line with equal spacing.\n\nTake a photo (or upload a picture) showing four objects arranged in a row with similar gaps between them.';
...
export function Level9NotebookUploadFourLineEqual({
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const feedback =
        data.feedback ||
        (correct ? 'SUCCESS! We see four objects in a straight line with equal spacing!' : 'TRY AGAIN.');
  ...
  return (
    ...
    >
      <View style={styles.content}>
        {!result ? (
          <>
            <Text style={styles.taskLabel}>Place FOUR objects in a straight line with equal spacing. Then:</Text>
            <Pressable
              onPress={pickImage}
              disabled={uploading}
