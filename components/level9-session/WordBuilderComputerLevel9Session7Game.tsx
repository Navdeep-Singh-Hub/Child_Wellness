...
const TARGET = ['C', 'O', 'M', 'P', 'U', 'T', 'E', 'R'] as const;
const LETTERS = ['R', 'E', 'C', 'O', 'M', 'P', 'U', 'T'] as const;
const WORD = 'COMPUTER';

const VOICE = 'Build the word COMPUTER. Tap the letters in order.';
const PALETTE = { accent: '#F43F5E', glow: '#FDA4AF', rose: '#FB7185', tech: '#38BDF8' } as const;
...
function CargoLetter({
  ...
  return (
    <Animated.View style={anim}>
      ...
      >
        <LinearGradient
          colors={[`${PALETTE.accent}88`, `${PALETTE.rose}44`]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
...
export function WordBuilderComputerLevel9Session7Game({
  onComplete,
}: WordBuilderComputerLevel9Session7GameProps) {
  ...
  const coachLine =
    nextIndex === 0
      ? 'Spell COMPUTER: start with C, then O, M, P, U, T, E, R!'
  ...
  return (
    <ClockwiseGameShell
      ...
      instruction="Tap the letter crates in order to spell COMPUTER."
      mascot="💻"
      coachLine={coachLine}
    ...
    >
      ...
      <View style={styles.progressWrap}>
        ...
        <View style={styles.progressBg}>
          <LinearGradient
            colors={[PALETTE.accent, PALETTE.tech]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
      ...
      <View style={styles.legend}>
        <Text style={styles.legendTxt}>
          Order: C → O → M → P → U → T → E → R{attempts > 0 ? ` · Taps: ${attempts}` : ''}
        </Text>
      </View>
    </ClockwiseGameShell>
  );
...
const styles = StyleSheet.create({
  ...
  targetWord: { fontSize: 22, fontWeight: '900', color: CW.textLight, letterSpacing: 2 },
