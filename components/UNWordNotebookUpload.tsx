/**
 * UN Word Notebook — Draw 3 -UN words. Photo upload + AI check. Grouper Session 3.
 * AI checks: un_objects_present, three_drawings_present.
 */
...
const INSTRUCTIONS = 'Draw 3 -UN words. Examples: sun, bun, run.';
...
export function UNWordNotebookUpload({
  ...
  const [result, setResult] = useState<{
    correct: boolean;
    feedback: string;
    un_objects_present?: boolean;
    three_drawings_present?: boolean;
  } | null>(null);
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const unObjects = data.un_objects_present ?? true;
      const threeDrawings = data.three_drawings_present ?? true;
      const correct = data.correct ?? (unObjects && threeDrawings);
      const feedback = data.feedback || (correct ? 'Great job!' : "Let's try again!");
      setResult({
        correct,
        feedback,
        un_objects_present: data.un_objects_present,
        three_drawings_present: data.three_drawings_present,
      });
      if (correct) {
        speak('Great job!');
  ...
  return (
    <GameLayout
      ...
      instruction="Draw 3 -UN words. Then upload or take a photo."
    >
      <View style={styles.content}>
        <Text style={styles.instructions}>{INSTRUCTIONS}</Text>
        <View style={styles.exampleRow}>
          <Text style={styles.exampleLabel}>Examples: </Text>
          <Text style={styles.exampleWords}>sun · bun · run</Text>
        </View>
