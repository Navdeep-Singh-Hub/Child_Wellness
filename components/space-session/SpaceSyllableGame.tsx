/**
 * SpaceSyllableGame — Game 3: Clap Space Words
 * planet (2), astronaut (3), rocket (2), comet (2). Bouncing planets, star on correct.
 */
...
const WORDS = [
  ...
  { word: 'Astronaut', syllables: 3 },
  { word: 'Rocket', syllables: 2 },
  { word: 'Comet', syllables: 2 },
];

export function SpaceSyllableGame({ onComplete }: { onComplete: () => void }) {
  const [round, setRound] = useState(0);
  const [showSuccess, setShowSuccess] = useState(false);
  const [planetAnims] = useState(() =>
  ...
  const triggerPlanets = useCallback((n: number) => {
    for (let i = 0; i < 3; i++) {
      const a = planetAnims[i];
      if (!a) continue;
      a.setValue(0);
      if (i < n) {
        Animated.sequence([
  ...
  const handleClapChoice = useCallback(
    ...
    [current, round, onComplete, triggerPlanets, triggerStar]
  );
    ...
    >
      <View style={styles.content}>
        ...
        <View style={styles.planetsRow}>
          {[0, 1, 2].map((i) => {
            const opacity = planetAnims[i]?.interpolate({ inputRange: [0, 1], outputRange: [0.4, 1] });
            return (
              <Animated.View key={i} style={[styles.planet, opacity != null ? { opacity } : { opacity: 0.4 }]}>
...
const styles = StyleSheet.create({
  ...
  wordText: { fontSize: 28, fontWeight: '800', color: '#1E3A8A' },
  planetsRow: { flexDirection: 'row', gap: 16, marginBottom: 24 },
