/**
 * SpaceNotebookUpload — Space Notebook Activity
 * Option 1: Draw 4 rockets + write 4. Option 2: Write two words with 2–3 syllables (e.g. rocket, planet, astronaut) + draw.
 */
...
const INSTRUCTIONS =
  'Option 1: Draw 4 rockets in your notebook and write the number 4.\n\nOR\n\nOption 2: Write two words that have 2 or 3 syllables (e.g. rocket, planet, astronaut) and draw the objects.';

export function SpaceNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-space-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
  ...
  return (
    <GameLayout title="Space Notebook Activity" instruction={INSTRUCTIONS}>
...
const styles = StyleSheet.create({
  ...
  resultTitle: { fontSize: 28, fontWeight: '800', color: '#1E3A8A', marginBottom: 8 },
