/**
 * SpaceSession4 — Special Ed Level 2 · Session 4: Space Journey
 * Flow: Intro → Game 1 (Space Clean-Up) → Game 2 (I Spy Rhymes) → Game 3 (Clap Space Words) → Game 4 (Count the Stars) → Notebook → Result
 * Design: Space Blue #1E3A8A, Galaxy Purple #8B5CF6, Star Yellow #FACC15.
...
import { SpaceCleanUp } from './SpaceCleanUp';
import { SpaceRhymeGame } from './SpaceRhymeGame';
import { SpaceSyllableGame } from './SpaceSyllableGame';
import { SpaceDragToCount } from './SpaceDragToCount';
import { SpaceNotebookUpload } from './SpaceNotebookUpload';

const TOTAL_GAMES = 5;
const GAME_COUNT = 4;
const SECTION_NUMBER = 2;
const SESSION_NUMBER = 4;
const SPACE_BLUE = '#1E3A8A';
const GALAXY_PURPLE = '#8B5CF6';
const STAR_YELLOW = '#FACC15';

interface SpaceSession4Props {
  onExit?: () => void;
}

export function SpaceSession4({ onExit }: SpaceSession4Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🧹', title: 'Space Clean-Up', desc: 'Find the sound' },
      { icon: '👀', title: 'I Spy Space Rhymes', desc: 'Rhyming hunt' },
      { icon: '👏', title: 'Clap Space Words', desc: 'Syllables' },
      { icon: '⭐', title: 'Count the Stars', desc: 'Drag to count' },
    ];
      ...
      <SafeAreaView style={styles.safe}>
        ...
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>Space Journey</Text>
            <Text style={styles.introSub}>
              Blast off into space and learn sounds, rhymes, and counting!
            </Text>
          </View>
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.starsRow}>
            {['⭐', '🚀', '✨', '🌟', '⭐', '🪐'].map((e, i) => (
              <Text key={i} style={styles.sparkleEmoji}>{e}</Text>
            ))}
          </View>
          <Text style={styles.resultTitle}>You did a great job exploring space!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeStar}>⭐</Text>
            <Text style={styles.resultBadgeText}>Space Explorer Badge</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Space Journey</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <SpaceCleanUp onComplete={advance} />}
        {step === 2 && <SpaceRhymeGame onComplete={advance} />}
        {step === 3 && <SpaceSyllableGame onComplete={advance} />}
        {step === 4 && <SpaceDragToCount onComplete={advance} />}
        {step === 5 && <SpaceNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  backButtonText: { fontSize: 17, fontWeight: '700', color: '#1E3A8A' },
  header: {
    ...
    paddingBottom: 16,
    borderBottomWidth: 3,
    borderBottomColor: SPACE_BLUE,
  },
  title: { fontSize: 22, fontWeight: '800', color: '#1E3A8A', textAlign: 'center' },
  subtitle: { fontSize: 14, color: '#3730A3', textAlign: 'center', marginTop: 4 },
  ...
  progressFill: { height: '100%', backgroundColor: SPACE_BLUE, borderRadius: 7 },
  progressText: { fontSize: 16, fontWeight: '700', color: '#1E3A8A', minWidth: 48 },
  ...
  introTitle: { fontSize: 28, fontWeight: '800', color: '#1E3A8A', textAlign: 'center', marginBottom: 12 },
  introSub: { fontSize: 18, color: '#3730A3', textAlign: 'center', lineHeight: 26 },
  ...
  sparkleEmoji: { fontSize: 32 },
  resultTitle: { fontSize: 24, fontWeight: '800', color: '#1E3A8A', textAlign: 'center', marginBottom: 24 },
  resultBadge: {
    flexDirection: 'row',
    alignItems: 'center',
