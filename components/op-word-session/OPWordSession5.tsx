/**
 * Level 4 Grouper — Session 5: The -OP Word Family
 * Hub → 4 games + sandstone journal task → completion
 */
import { ConfettiEffect } from '@/components/games/Level1/ConfettiEffect';
import { SuccessCelebration } from '@/components/ui/SuccessCelebration';
import { markSessionGameComplete } from '@/components/special-education/shared/useSessionIntroProgress';
import { WordFamilyFinderGame } from '@/components/grouper-session/session5/WordFamilyFinderGame';
import { RhymeMatchGame } from '@/components/grouper-session/session5/RhymeMatchGame';
import { ClapWordGame } from '@/components/grouper-session/session5/ClapWordGame';
import { ObjectSortingGame } from '@/components/grouper-session/session5/ObjectSortingGame';
import { OPWordNotebookUpload } from '@/components/grouper-session/session5/OPWordNotebookUpload';
import {
  GROUPER_S5_HUB_THEME as H,
  GROUPER_S5_QUESTS,
  GROUPER_SESSION,
} from '@/components/grouper-session/grouperSessionTheme';
import { SandstoneShelfBackground } from '@/components/grouper-session/SandstoneShelfBackground';
...
const PLAY_STEPS = 5;
const SESSION_TITLE = 'Session 5 · The -OP Family';
const SECTION_NUMBER = 4;
const SESSION_NUMBER = 5;

interface OPWordSession5Props {
  onExit?: () => void;
}

export function OPWordSession5({ onExit }: OPWordSession5Props = {}) {
        ...
        <SafeAreaView style={styles.hubSafe}>
          ...
          <ScrollView contentContainerStyle={styles.hubScroll} showsVerticalScrollIndicator={false}>
            <View style={styles.hubHeader}>
              <Text style={styles.hubEyebrow}>Grouper · Session 5 · Sandstone Shelf</Text>
              <Text style={styles.hubTitle}>{H.name}</Text>
              <Text style={styles.hubSession}>The -OP Word Family</Text>

              <View style={styles.exampleChip}>
                <Text style={styles.exampleText}>top · hop · mop · pop</Text>
              </View>
              ...
              <View style={styles.speechBubble}>
                ...
                <View style={styles.bubbleBody}>
                  ...
                  <Text style={styles.hubPrompt}>
                    On the sandstone shelf, -OP words stack together — scout them, echo rhymes, clap, sort, then sketch your journal!
                  </Text>
                </View>
              </View>
            </View>

            {GROUPER_S5_QUESTS.map((quest) => {
  ...
  if (step === 6) {
    return (
      <View style={styles.root}>
        ...
        <ConfettiEffect />
        <SuccessCelebration
          title="Great job learning the -OP family!"
          subtitle={`OP Family Star 🪣\n${SESSION_TITLE}\nCompleted ${doneCount}/${PLAY_STEPS} builds${
            taskCorrect === false ? '\nSandstone Journal: try again next time!' : ''
          }`}
          badgeEmoji="⭐"
          variant="sunset"
        />
