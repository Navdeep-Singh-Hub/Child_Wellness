/**
 * OP Word Session 5 — Level 4 (Grouper) Session 5: The -OP Word Family
...
import { WordFamilyFinderOP } from '@/components/WordFamilyFinderOP';
import { RhymeMatchOP } from '@/components/RhymeMatchOP';
import { ClapWordOP } from '@/components/ClapWordOP';
import { ObjectSortingOP } from '@/components/ObjectSortingOP';
import { OPWordNotebookUpload } from '@/components/OPWordNotebookUpload';
...
export function OPWordSession5({ onExit }: OPWordSession5Props = {}) {
  ...
  if (step === 0) {
    const cards = [
      { icon: '🔍', title: 'Find the -OP words', desc: 'Word family' },
      ...
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.introScroll} showsVerticalScrollIndicator={false}>
          <View style={styles.introHeader}>
            <Text style={styles.introTitle}>The -OP Family</Text>
            <Text style={styles.introSub}>
              Let's learn words that end with -op.
            </Text>
          </View>
          <View style={styles.exampleBox}>
            <Text style={styles.exampleTitle}>Example words:</Text>
            <View style={styles.exampleWordsRow}>
              <Text style={styles.exampleWord}>top</Text>
              <Text style={styles.exampleWord}>hop</Text>
              <Text style={styles.exampleWord}>mop</Text>
              <Text style={styles.exampleWord}>pop</Text>
            </View>
          </View>
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <ScrollView contentContainerStyle={styles.resultScroll} showsVerticalScrollIndicator={false}>
          <Text style={styles.resultTitle}>Great job learning the -OP family!</Text>
          <View style={styles.resultBadge}>
            <Text style={styles.resultBadgeEmoji}>⭐</Text>
            <Text style={styles.resultBadgeText}>OP Family Star</Text>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <View style={styles.header}>
        <Text style={styles.title}>The -OP Family</Text>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        {step === 1 && <WordFamilyFinderOP onComplete={advance} />}
        {step === 2 && <RhymeMatchOP onComplete={advance} />}
        {step === 3 && <ClapWordOP onComplete={advance} />}
        {step === 4 && <ObjectSortingOP onComplete={advance} />}
        {step === 5 && <OPWordNotebookUpload onComplete={handleNotebookComplete} />}
      </ScrollView>
    </SafeAreaView>
  );
}
