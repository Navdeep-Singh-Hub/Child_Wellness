/**
 * OceanNotebookUpload — Ocean Notebook Activity
 * Option 1: Draw 5 fish + write 5. Option 2: Write 3 rhyming words (fish, dish, wish) + drawings.
 */
...
const INSTRUCTIONS =
  'Option 1: Draw 5 fish in your notebook and write the number 5.\n\nOR\n\nOption 2: Write 3 rhyming ocean words (e.g. fish, dish, wish) and draw pictures of them.';

export function OceanNotebookUpload({
  onComplete,
}: {
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/upload-ocean-task`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      const correct =
        data.correct ??
        (data.objects_detected && data.correct_count && data.rhyming_words) ??
        true;
      ...
      setResult({
        ...
        rhyming_words: data.rhyming_words,
      });
      if (correct) {
        speak('Great job!');
  ...
  return (
    <GameLayout title="Ocean Notebook Activity" instruction={INSTRUCTIONS}>
...
const styles = StyleSheet.create({
  ...
  resultTitle: { fontSize: 28, fontWeight: '800', color: '#0EA5E9', marginBottom: 8 },
