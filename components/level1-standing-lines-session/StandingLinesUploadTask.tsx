/**
 * AI Task: Draw standing (vertical) lines on paper and upload photo. POST /api/verify-standing-lines
 */
...
const INSTRUCTION = 'Draw standing (vertical) lines on paper and upload a photo.';

export function StandingLinesUploadTask({
  currentStep,
  totalSteps,
  onBack,
  onComplete,
  ...
  const takePhoto = async () => {
    ...
    if (permission.status !== 'granted') {
      Alert.alert('Permission needed', 'Allow camera to take a photo of your lines.');
      return;
    }
  ...
  const uploadImage = async (uri: string) => {
    ...
    try {
      ...
      const filename = 'standing-lines.jpg';
      ...
      const res = await fetch(`${API_BASE_URL}/api/verify-standing-lines`, {
        method: 'POST',
        headers,
        body: formData,
      });
      ...
      setResult({
        ...
        feedback: data.feedback || (success ? 'Great straight lines!' : 'Try again. Draw vertical lines on paper.'),
      });
