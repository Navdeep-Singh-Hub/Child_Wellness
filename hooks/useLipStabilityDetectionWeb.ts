/**
 * Web lip stability — MediaPipe FaceMesh, four lip landmarks only.
 */
...
import {
  ...
  UPPER_LIP_INDEX,
} from '@/components/game/speech/lip-closure/modules/LipStabilityEngine';
import type { LipStabilitySnapshot } from '@/components/game/speech/lip-closure/modules/lipHoldTypes';
import { useEffect, useRef, useState } from 'react';

export interface LipStabilityResult extends LipStabilitySnapshot {
  isDetecting: boolean;
  hasCamera: boolean;
  error?: string;
...
async function initMediaPipe() {
  ...
  try {
    ...
    console.warn('[lip stability web] init failed', e);
    return false;
  }
}

export function useLipStabilityDetectionWeb(enabled: boolean): LipStabilityResult {
  const [result, setResult] = useState<LipStabilityResult>(IDLE);
  const engineRef = useRef(new LipStabilityEngine());
  const fpsRef = useRef(new FPSOptimizer(25));
  const videoRef = useRef<HTMLVideoElement | null>(null);
  const rafRef = useRef<number | null>(null);
  const containerId = 'lip-stability-preview';
  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      if (video && faceLandmarker && video.readyState >= 2 && fpsRef.current.shouldProcess()) {
        try {
          ...
          if (lm) {
            const geo = geometryFromLandmarks(
              lm[UPPER_LIP_INDEX],
              lm[LOWER_LIP_INDEX],
              lm[LEFT_LIP_CORNER_INDEX],
              lm[RIGHT_LIP_CORNER_INDEX],
            );
            const snap = engineRef.current.processGeometry(geo);
            setResult({
              ...snap,
              isDetecting: true,
              hasCamera: true,
    ...
    (async () => {
      ...
      if (!ok || cancelled) {
        setResult({ ...IDLE, error: 'Camera setup failed. Use Steady / Moving buttons.' });
        return;
      }
      try {
        ...
        setResult({ ...IDLE, error: 'Allow camera access, or use Steady / Moving buttons.' });
      }
    })();
