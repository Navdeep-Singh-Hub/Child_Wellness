/**
 * Web lip resistance — MediaPipe FaceMesh, four lip landmarks only.
 */
...
import {
  ...
  UPPER_LIP_INDEX,
} from '@/components/game/speech/lip-closure/modules/LipResistanceEngine';
import type { LipResistanceSnapshot } from '@/components/game/speech/lip-closure/modules/lipResistanceTypes';
import { useEffect, useRef, useState } from 'react';

export interface LipResistanceResult extends LipResistanceSnapshot {
  isDetecting: boolean;
  hasCamera: boolean;
  error?: string;
...
async function initMediaPipe() {
  ...
  try {
    ...
    console.warn('[lip resistance web] init failed', e);
    return false;
  }
}

export function useLipResistanceDetectionWeb(enabled: boolean): LipResistanceResult {
  const [result, setResult] = useState<LipResistanceResult>(IDLE);
  const engineRef = useRef(new LipResistanceEngine());
  const fpsRef = useRef(new FPSOptimizer(25));
  const videoRef = useRef<HTMLVideoElement | null>(null);
  const rafRef = useRef<number | null>(null);
  const containerId = 'lip-resistance-preview';
  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      if (video && faceLandmarker && video.readyState >= 2 && fpsRef.current.shouldProcess()) {
        try {
          ...
          if (lm) {
            const m = metricsFromLandmarks(
              lm[UPPER_LIP_INDEX],
              lm[LOWER_LIP_INDEX],
              lm[LEFT_LIP_CORNER_INDEX],
              lm[RIGHT_LIP_CORNER_INDEX],
            );
            const snap = engineRef.current.processMetrics(
              m?.lipGap ?? null,
              m?.roundness ?? null,
              m?.spread ?? null,
              m?.mouthWidth ?? null,
    ...
    (async () => {
      ...
      if (!ok || cancelled) {
        setResult({ ...IDLE, error: 'Camera setup failed. Use Strong / Shaky buttons.' });
        return;
      }
      try {
        ...
        setResult({ ...IDLE, error: 'Allow camera access, or use Strong / Shaky buttons.' });
      }
    })();
